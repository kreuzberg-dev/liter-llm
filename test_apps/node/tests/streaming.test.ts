...
// alef:hash:7caadc7d4130fa0051189f8c3fa777ea336799ecd955362226a61ad6f485c5e4
...
describe("streaming", () => {
	...
	it("edge_stream_function_call: Streaming chat completion with tool/function call chunks", async () => {
		...
		const stream = await client.chatStream({ messages: [{ content: "What's the weather?", role: "user" }], model: "gpt-4", tools: [{ function: { name: "get_weather", parameters: { properties: { city: { type: "string" } }, type: "object" } }, toolType: "function" }] });
		const chunks: any[] = [];
    for await (const _chunk of stream) { chunks.push(_chunk); }
    expect(chunks.length).toBeGreaterThanOrEqual(2);
