...
// alef:hash:2dc587317b4986ef41b6b6e9f619f48f6885f99f7567ed0d034ae5c72fe7bffa
// To regenerate: alef generate
// To verify freshness: alef verify

import { describe, expect, it } from "vitest";
import { createClient, WasmChatCompletionRequest, WasmCreateBatchRequest, WasmChatCompletionTool, WasmFunctionDefinition, WasmStreamOptions, WasmBatchStatus } from "@xberg-io/liter-llm-wasm";
...
describe("batches", () => {
	...
	it("error_batch_auth_401: 401 Unauthorized when creating a batch with invalid API key", async () => {
		...
		await expect(async () => {
			await client.createBatch((() => { const _u0 = WasmCreateBatchRequest.default(); _u0.completionWindow = "24h"; _u0.endpoint = "/v1/chat/completions"; _u0.inputFileId = "file-abc123"; return _u0; })());
		}).rejects.toSatisfy((error) => {
				const _message = error instanceof Error ? error.message : String(error);
				const _name = error instanceof Error ? error.name : "";
	...
	it("error_batch_invalid_file: 400 Bad Request when creating a batch with invalid input file", async () => {
		...
		await expect(async () => {
			await client.createBatch((() => { const _u0 = WasmCreateBatchRequest.default(); _u0.completionWindow = "24h"; _u0.endpoint = "/v1/chat/completions"; _u0.inputFileId = "file-wrong-purpose"; return _u0; })());
		}).rejects.toSatisfy((error) => {
				const _message = error instanceof Error ? error.message : String(error);
				const _name = error instanceof Error ? error.name : "";
				return /BadRequest/.test(_message) || /BadRequest/.test(_name);
	...
	it("error_batch_not_found: 404 Not Found when retrieving a nonexistent batch", async () => {
		...
		await expect(async () => {
			await client.retrieveBatch("batch-nonexistent");
				...
				const _message = error instanceof Error ? error.message : String(error);
				const _name = error instanceof Error ? error.name : "";
				return /NotFound/.test(_message) || /NotFound/.test(_name);
			});
		// skipped: assertion type 'equals' has no accessor for error field error.status_code in this backend
	}, 30000);
	...
	it("smoke_create_batch: Create a new batch processing job", async () => {
		...
		const result = await client.createBatch((() => { const _u0 = WasmCreateBatchRequest.default(); _u0.completionWindow = "24h"; _u0.endpoint = "/v1/chat/completions"; _u0.inputFileId = "file-abc123"; return _u0; })());
    {
        const _v = result.id;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
