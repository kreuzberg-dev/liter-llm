...
// alef:hash:224d0f64c5efdfded12879a9400f2b71eef52a4d9cec4f39aaa2b502ff99330a
// To regenerate: alef generate
// To verify freshness: alef verify

import { describe, expect, it } from "vitest";
import { createClient, WasmChatCompletionRequest, WasmOcrRequest, WasmChatCompletionTool, WasmFunctionDefinition, WasmStreamOptions } from "@xberg-io/liter-llm-wasm";
...
describe("ocr", () => {
	...
	it("ocr_error_400: 400 Bad Request error when OCR input has an invalid image format", async () => {
		...
		await expect(async () => {
			await client.ocr((() => { const _u0 = WasmOcrRequest.default(); _u0.document = { type: "document_url", url: "invalid://url" }; _u0.model = "mistral/mistral-ocr-latest"; return _u0; })());
	...
	it("ocr_error_401: 401 Unauthorized error on OCR request due to invalid API credentials", async () => {
		...
		await expect(async () => {
			await client.ocr((() => { const _u0 = WasmOcrRequest.default(); _u0.document = { type: "document_url", url: "https://example.com/doc.pdf" }; _u0.model = "mistral/mistral-ocr-latest"; return _u0; })());
		}).rejects.toSatisfy((error) => {
				const _message = error instanceof Error ? error.message : String(error);
				const _name = error instanceof Error ? error.name : "";
	...
	it("ocr_multi_page: OCR request returning multiple pages of document content", async () => {
		...
		const result = await client.ocr((() => { const _u0 = WasmOcrRequest.default(); _u0.document = { type: "document_url", url: "https://example.com/multipage.pdf" }; _u0.model = "mistral/mistral-ocr-latest"; return _u0; })());
	...
	it("ocr_url_document: OCR request with a document URL input", async () => {
		...
		const result = await client.ocr((() => { const _u0 = WasmOcrRequest.default(); _u0.document = { type: "document_url", url: "https://example.com/doc.pdf" }; _u0.model = "mistral/mistral-ocr-latest"; return _u0; })());
