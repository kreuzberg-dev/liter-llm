...
// alef:hash:ddcc195fb1c2591068dea0cbdf26304a74c0138ccf7316a9c58362c773c2a601
...
final class ModerateTest extends TestCase
{
    ...
    public function test_edge_moderate_all_categories(): void
    {
        ...
        $result = $client->moderateAsync(json_encode("Extremely harmful content targeting multiple categories"));
    ...
    {
        $client = \Liter\Llm\LiterLlm::createClient('test-key');
        $result = $client->moderateAsync(json_encode(""));
        $this->assertCount(1, $result->results);
    ...
    public function test_error_moderate_auth_401(): void
    {
        ...
        $client->moderateAsync(json_encode("Hello"));
    }
    ...
    {
        $client = \Liter\Llm\LiterLlm::createClient('test-key');
        $result = $client->moderateAsync(json_encode(["Hello world", "Nice weather today"]));
    ...
    {
        $client = \Liter\Llm\LiterLlm::createClient('test-key');
        $result = $client->moderateAsync(json_encode("I want to hurt someone very badly"));
        $this->assertCount(1, $result->results);
        $this->assertEquals(true, $result->results["0"]->flagged);
    }
    ...
    {
        $client = \Liter\Llm\LiterLlm::createClient('test-key');
        $result = $client->moderateAsync(json_encode("The weather is nice today."));
        $this->assertCount(1, $result->results);
        $this->assertEquals(false, $result->results["0"]->flagged);
    }
}
