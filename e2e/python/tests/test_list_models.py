...
# alef:hash:17d3d01a99b498ecffe5174e695f80390f13e1082939ea20f8b68cb61daac60c
...
async def test_empty_model_list() -> None:
    ...
    client = create_client(
        api_key="test-key",
        base_url=os.environ["MOCK_SERVER_URL"] + "/fixtures/empty_model_list",
    )
...
async def test_list_models_error_401() -> None:
    ...
    client = create_client(
        api_key="test-key",
        base_url=os.environ["MOCK_SERVER_URL"] + "/fixtures/list_models_error_401",
    )
...
async def test_list_models_error_500() -> None:
    ...
    client = create_client(
        api_key="test-key",
        base_url=os.environ["MOCK_SERVER_URL"] + "/fixtures/list_models_error_500",
    )
    with pytest.raises(Exception) as exc_info:  # noqa: B017
        await client.list_models()
...
async def test_list_models_filtered() -> None:
    ...
    client = create_client(
        api_key="test-key",
        base_url=os.environ["MOCK_SERVER_URL"] + "/fixtures/list_models_filtered",
    )
    result = await client.list_models()
