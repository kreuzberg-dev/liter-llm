...
# alef:hash:a94c0677a68ae95b7dafd8777a7a552dab3766b5f56453e9933fa80eada54a51
...
async def test_edge_file_large_upload() -> None:
    ...
    result = await client.create_file(request=request)
...
async def test_error_file_bad_purpose() -> None:
    ...
    request = {"file": "data.jsonl", "purpose": "invalid-purpose"}
    client = create_client(
        api_key="test-key",
        base_url=os.environ["MOCK_SERVER_URL"] + "/fixtures/error_file_bad_purpose",
    )
    with pytest.raises(Exception) as exc_info:  # noqa: B017
        await client.create_file(request=request)
...
async def test_error_file_not_found() -> None:
    ...
    with pytest.raises(Exception) as exc_info:  # noqa: B017
        await client.retrieve_file(file_id=file_id)
...
async def test_smoke_delete_file() -> None:
    ...
    result = await client.delete_file(file_id=file_id)
    assert result.id  # noqa: S101
...
async def test_smoke_file_content() -> None:
    ...
    result = await client.file_content(file_id=file_id)
...
async def test_smoke_retrieve_file() -> None:
    ...
    result = await client.retrieve_file(file_id=file_id)
    assert result.id  # noqa: S101
