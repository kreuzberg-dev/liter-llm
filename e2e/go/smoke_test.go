...
// alef:hash:c4ba93c6f4c090b423ca705db41adf58243d222b0dc90e3417cc719bbc45fb9b
...
func Test_LocalListModelsOllama(t *testing.T) {
	...
	result, err := pkg.ListModels(`{"model":"ollama/any"}`)
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	assert.GreaterOrEqual(t, len(result.Data), 1, "expected at least 1 elements")
...
func Test_SmokeListModelsOpenai(t *testing.T) {
	...
	result, err := pkg.ListModels(`{"model":"openai/gpt-4o-mini"}`)
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	assert.GreaterOrEqual(t, len(result.Data), 1, "expected at least 1 elements")
