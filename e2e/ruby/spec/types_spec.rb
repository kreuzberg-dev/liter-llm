...
RSpec.describe 'types' do
  it 'all_message_types' do
    ...
    expect(body.dig('choices', 0, 'message',
                    'content')).to eq('The weather in Paris is currently 18°C and partly cloudy.')
  ...
  it 'multimodal_content' do
    ...
    expect(body['choices'].size).to eq(1)
    expect(body.dig('choices', 0, 'message',
                    'content')).to eq('The image shows a transparent PNG demonstration with colored dice.')
    expect(body.dig('choices', 0, 'finish_reason')).to eq('stop')
