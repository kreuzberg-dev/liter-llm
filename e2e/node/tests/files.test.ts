...
// alef:hash:c4d4ef15f92bac8c9effd3b80c63e2e49549c4a47ac3497e70098164ce29d58c
...
describe('files', () => {
  ...
  it('edge_file_large_upload: Upload a large file successfully', async () => {
    ...
    const result = await client.createFile({ file: "large_training_data.jsonl", purpose: "fine-tune" });
  ...
  it('smoke_create_file: Upload a file for use with the API', async () => {
    ...
    const result = await client.createFile({ file: "training_data.jsonl", purpose: "fine-tune" });
    expect(result.id != null).toBe(true);
  });
