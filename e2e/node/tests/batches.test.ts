...
// alef:hash:ea6104bbae575ab29891f6c2c1bfcd8f3d99130881ab4ea0d7a22a3d1fd61176
...
describe('batches', () => {
  it('edge_batch_already_cancelled: Attempt to cancel an already-cancelled batch', async () => {
    ...
    await expect(async () => {
      await client.cancel_batch("batch-cancelled001");
    }).rejects.toThrow();
  });
  ...
  it('error_batch_auth_401: 401 Unauthorized when creating a batch with invalid API key', async () => {
    ...
    await expect(async () => {
      await client.create_batch({ completion_window: "24h", endpoint: "/v1/chat/completions", input_file_id: "file-abc123" });
    }).rejects.toThrow();
  });
  ...
  it('error_batch_invalid_file: 400 Bad Request when creating a batch with invalid input file', async () => {
    ...
    await expect(async () => {
      await client.create_batch({ completion_window: "24h", endpoint: "/v1/chat/completions", input_file_id: "file-wrong-purpose" });
    }).rejects.toThrow();
  });
  ...
  it('error_batch_not_found: 404 Not Found when retrieving a nonexistent batch', async () => {
    ...
    await expect(async () => {
      await client.retrieve_batch("batch-nonexistent");
    }).rejects.toThrow();
  });
  ...
  it('smoke_create_batch: Create a new batch processing job', async () => {
    ...
    const result = await client.create_batch({ completion_window: "24h", endpoint: "/v1/chat/completions", input_file_id: "file-abc123" });
    expect(result.id.length).toBeGreaterThan(0);
