...
// alef:hash:6928a90909234a3d3373e091d072ea959d512370384ea84c254e96e186bb0868
...
describe("responses", () => {
	it("edge_response_empty_output: Response completes with empty output items", async () => {
		...
		const result = await client.create_response({ input: "", model: "gpt-4o" });
	...
	it("edge_response_large_input: Response created with a very large input text", async () => {
		...
		const result = await client.create_response({
			input:
	...
	it("error_response_auth_401: 401 Unauthorized when creating a response with invalid API key", async () => {
		...
		await expect(async () => {
			await client.create_response({ input: "Hello", model: "gpt-4o" });
		}).rejects.toThrow();
	});
	...
	it("error_response_not_found: 404 Not Found when retrieving a nonexistent response", async () => {
		...
		await expect(async () => {
			await client.retrieve_response("");
		}).rejects.toThrow();
	});
