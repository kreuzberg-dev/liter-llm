...
// alef:hash:e25cd905735b00d8febe1473fd05d1890068fdfba0fba22af5b1a82b506fa1b8
...
describe("streaming", () => {
	...
	it("stream_content_policy_error: 400 Bad Request error on stream due to content policy violation", async () => {
		...
		options.stream = true;
		await expect(
			async () => await client.chat_stream(options),
		).rejects.toThrow();
	});
	...
	it("stream_with_tool_calls: Streaming chat completion where the assistant responds with a tool call across multiple chunks", async () => {
		...
		const options = new WasmChatCompletionRequest();
		options.messages = [
			{ content: "What is the weather in NYC?", role: "user" },
		];
		options.model = "gpt-4";
		options.stream = true;
		options.tools = [
