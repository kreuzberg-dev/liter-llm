...
// alef:hash:61e314a4520a02fbd0dd2e1418efe46a4d25868ea6a4c3b3388dcdb05d63de69
...
describe("chat", () => {
  ...
  it("edge_chat_max_tokens: Chat request with max_tokens=1 terminates with length finish_reason", async () => {
    ...
    expect(
      (() => {
        const v = result.choices["0"].message.content as unknown;
  ...
  it("seed_parameter: Chat request with seed parameter for deterministic output; response includes system_fingerprint", async () => {
    ...
    expect(
      (() => {
        const v = result.systemFingerprint as unknown;
        return typeof v === "string" || Array.isArray(v)
          ? (v as { length: number }).length
          : Object.keys(v as object).length;
      })(),
