...
// alef:hash:2e1e37e2a5d33d7385f8e43fa00bf3fbb23243f7932476df63f1f05ac4be5a7a
...
describe("smoke", () => {
  ...
  it("local_chat_ollama: Chat completion against local Ollama with qwen2:0.5b model", async () => {
    ...
    expect(
      (() => {
        const v = result.choices as unknown;
  ...
  it("smoke_cache_memory: Test in-memory caching by sending identical requests twice", async () => {
    ...
    const result = await client.chat({
      maxTokens: 5,
      messages: [{ content: "What is 2+2? Answer with just the number.", role: "user" }],
  ...
  it("smoke_chat_anthropic: Basic chat completion against real Anthropic API", async () => {
    ...
    expect(
      (() => {
        const v = result.usage as unknown;
        return typeof v === "string" || Array.isArray(v)
          ? (v as { length: number }).length
          : Object.keys(v as object).length;
      })(),
  ...
  it("smoke_embed_openai: Embeddings request against real OpenAI API", async () => {
    ...
    expect(
      (() => {
        const v = result.data as unknown;
        return typeof v === "string" || Array.isArray(v)
          ? (v as { length: number }).length
          : Object.keys(v as object).length;
      })(),
  ...
  it("smoke_list_models_openai: List models against real OpenAI API", async () => {
    ...
    const result = await client.listModels({ model: "openai/gpt-4o-mini" });
    expect(result.data.length).toBeGreaterThanOrEqual(1);
  });
