...
// alef:hash:73a7469c62a2ecd7c6223f3edcaaf6c824cadcafa323ed732d1cd355cda860fd
...
describe("configuration", () => {
    ...
    expect(result.choices.length).toBe(1);
    expect((result.choices[0].message.content ?? "")).toBe("Hi there! I'm running locally.");
    expect(result.choices[0].finishReason).toBe("stop");
    ...
    expect(result.choices.length).toBe(1);
    expect((result.choices[0].message.content ?? "")).toBe("Hello! How may I assist you?");
    expect(result.choices[0].finishReason).toBe("stop");
    expect(result.model).toBe("meta-llama/Llama-3.2-1B");
