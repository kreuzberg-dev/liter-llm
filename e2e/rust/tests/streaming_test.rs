...
// alef:hash:8e9241136e159051251ea613c76f270296be3f303668688178ec1c954f03b017
...
async fn test_anthropic_stream() {
    ...
    let result = client.chat_stream(&request).await.expect("should succeed");
    assert!(
        result.chunks.len() >= 3,
...
async fn test_stream_content_policy_error() {
    ...
    assert!(
        result.as_ref().unwrap_err().to_string().contains("ContentPolicy"),
...
async fn test_stream_multiple_choices() {
    ...
    let client =
        liter_llm::create_client("test-key".to_string(), Some(mock_server.url.clone()), None, None, None).unwrap();
    let _ = client.chat_stream(&request).await;
}
...
async fn test_stream_with_tool_calls() {
    ...
    assert_eq!(
        result.tool_calls.get("0").map(|s| s.as_str()).function.name.trim(),
        r#"get_weather"#,
        "equals assertion failed"
    );
}
