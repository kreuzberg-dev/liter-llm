...
// alef:hash:6de5d9ef0fbff9e965cf85421494149e7d8e502556ac6d627a8ed187e34d311e
...
async fn test_anthropic_stream() {
    ...
    let chunks: Vec<_> = tokio_stream::StreamExt::collect::<Vec<_>>(stream)
        ...
        .collect();
    assert!(chunks.len() >= 3 as usize, "expected >= 3 chunks");
    assert_eq!(
        chunks
            .iter()
            .map(|c| c
...
async fn test_empty_stream() {
    ...
    let chunks: Vec<_> = tokio_stream::StreamExt::collect::<Vec<_>>(stream)
        ...
        .map(|r| r.expect("stream item failed"))
        .collect();
    assert!(chunks.len() >= 0 as usize, "expected >= 0 chunks");
    assert_eq!(
        chunks
            .iter()
            .map(|c| c
...
async fn test_stream_content_policy_error() {
    ...
    let mock_server = MockServer::start(vec![mock_route]).await;
    let req_json: serde_json::Value = serde_json::from_str(
        r#"{"messages":[{"content":"Generate harmful content","role":"user"}],"model":"gpt-4o","stream":true}"#,
    )
    .unwrap();
    let req = serde_json::from_value(req_json).unwrap();
    let client =
...
async fn test_stream_multiple_choices() {
    ...
    let mock_server = MockServer::start(vec![mock_route]).await;
    let req_json: serde_json::Value = serde_json::from_str(
        r#"{"messages":[{"content":"Hello","role":"user"}],"model":"gpt-4o","n":2,"stream":true}"#,
    )
    .unwrap();
    let req = serde_json::from_value(req_json).unwrap();
    let client =
...
async fn test_stream_with_tool_calls() {
    ...
    let chunks: Vec<_> = tokio_stream::StreamExt::collect::<Vec<_>>(stream)
        ...
        .map(|r| r.expect("stream item failed"))
        .collect();
    assert!(chunks.len() >= 4 as usize, "expected >= 4 chunks");
    assert_eq!(
        chunks
            .last()
            .and_then(|c| c.choices.first())
    ...
    assert!(
        chunks
            .iter()
            .flat_map(|c| c.choices.iter().flat_map(|ch| ch.delta.tool_calls.iter().flatten()))
            .collect::<Vec<_>>()
            .as_ref()
