...
// alef:hash:dc2f07bb5efdc920c1a04738203973c70d31f9f4d3533be3490609cc4338427b
...
async fn test_content_policy_violation() {
    ...
    assert!(result.is_err(), "expected call to fail");
    assert!(
        result.as_ref().err().unwrap().to_string().contains("ContentPolicy"),
        "error message mismatch"
    );
    // skipped: field 'error.status_code' not available on result type
}
...
async fn test_server_error_500() {
    ...
    assert!(result.is_err(), "expected call to fail");
    assert!(
        result.as_ref().err().unwrap().to_string().contains("ServerError"),
        "error message mismatch"
    );
    // skipped: field 'error.status_code' not available on result type
}
...
async fn test_timeout_error() {
    ...
    assert!(result.is_err(), "expected call to fail");
    assert!(
        result.as_ref().err().unwrap().to_string().contains("Timeout"),
        "error message mismatch"
    );
    // skipped: field 'error.status_code' not available on result type
}
