...
// alef:hash:e18c8d2508d8306587a4c8cca6d69041a8b6c40bd93b289cac3e5c93b0941419
...
fn export_generated_headers() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut header = String::from_utf8(bytes).map_err(|error| {
        io::Error::new(
            ...
            format!("cbindgen generated invalid UTF-8 for include/liter_llm.h: {error}"),
        )
    })?;

    stamp_enabled_features(&mut header);
    let destinations = [
        Path::new("include/liter_llm.h"),
        Path::new("../../packages/go/include/liter_llm.h"),
    ];
...
fn stamp_enabled_features(header: &mut String) {
    ...
    if header.contains(marker) || !header.contains("LITERLLM_FEATURE_") {
        return;
    }
    let mut defines: Vec<String> = std::env::vars_os()
        .filter_map(|(key, _)| {
            ...
            Some(format!("#define LITERLLM_FEATURE_{name} 1"))
        })
        .collect();
    defines.sort();
    defines.dedup();
    let block = format!("\n{marker}\n{}\n", defines.join("\n"));
    let anchor = "#define LITERLLM_H\n";
