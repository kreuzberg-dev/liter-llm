// This file is auto-generated by alef. DO NOT EDIT.
// alef:hash:7950f3b3d3451505eb720eabdb7746f3b0965dbc486743ec03fe24eca1ef9f9b
// Re-generate with: alef generate
#![allow(dead_code, unused_imports, unused_variables, missing_docs)]
#![allow(unsafe_code)]
#![allow(non_snake_case)]
#![allow(
    clippy::too_many_arguments,
    clippy::let_unit_value,
    clippy::needless_borrow,
    clippy::map_identity,
    clippy::just_underscores_and_digits,
    clippy::unnecessary_cast,
    clippy::unused_unit,
    clippy::unwrap_or_default,
    clippy::derivable_impls,
    clippy::redundant_field_names,
    clippy::needless_borrows_for_generic_args,
    clippy::unnecessary_fallible_conversions,
    clippy::arc_with_non_send_sync,
    clippy::collapsible_if,
    clippy::clone_on_copy,
    clippy::should_implement_trait,
    clippy::useless_conversion
)]
#![allow(clippy::single_match, clippy::collapsible_match)]
#![cfg_attr(windows, feature(abi_vectorcall))]
#![cfg_attr(windows, allow(unused_features))]

use ext_php_rs::prelude::*;
use liter_llm::client::BatchClient;
use liter_llm::client::BatchRetriever;
use liter_llm::client::FileClient;
use liter_llm::client::LlmClient;
use liter_llm::client::ResponseClient;
use std::collections::HashMap;
use std::sync::Arc;

#[derive(Debug, Clone, Default)]
pub struct PhpBytes(pub Vec<u8>);

impl<'a> ext_php_rs::convert::FromZval<'a> for PhpBytes {
    const TYPE: ext_php_rs::flags::DataType = ext_php_rs::flags::DataType::String;
    fn from_zval(zval: &'a ext_php_rs::types::Zval) -> Option<Self> {
        zval.zend_str().map(|zs| PhpBytes(zs.as_bytes().to_vec()))
    }
}

impl From<PhpBytes> for Vec<u8> {
    fn from(b: PhpBytes) -> Self {
        b.0
    }
}

impl From<Vec<u8>> for PhpBytes {
    fn from(v: Vec<u8>) -> Self {
        PhpBytes(v)
    }
}

static WORKER_RUNTIME: std::sync::LazyLock<tokio::runtime::Runtime> = std::sync::LazyLock::new(|| {
    tokio::runtime::Builder::new_multi_thread()
        .enable_all()
        .build()
        .expect("Failed to create Tokio runtime")
});

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\AssistantMessage")]
#[serde(default)]
pub struct AssistantMessage {
    /// The assistant's response: plain text, structured parts, or absent.
    ///
    /// `None` is valid when the model replies with tool calls only.
    pub content: Option<serde_json::Value>,
    /// Optional name for the assistant.
    #[php(prop, name = "name")]
    pub name: Option<String>,
    /// Tool calls the model wants to execute, if any.
    #[serde(alias = "toolCalls")]
    pub tool_calls: Option<Vec<ToolCall>>,
    /// Refusal reason, if the model declined to respond per safety policies.
    ///
    /// OpenAI's response schema requires this key to be present even when null,
    /// so it is deliberately not `skip_serializing_if`.
    #[php(prop, name = "refusal")]
    pub refusal: Option<String>,
    /// Deprecated legacy function_call field; retained for API compatibility.
    #[serde(alias = "functionCall")]
    pub function_call: Option<FunctionCall>,
    /// Reasoning/thinking tokens returned by the provider, if any (e.g. DeepSeek R1, Qwen
    /// `reasoning_content`, or Anthropic extended thinking).
    #[php(prop, name = "reasoningContent")]
    #[serde(alias = "reasoningContent")]
    pub reasoning_content: Option<String>,
}

impl Default for AssistantMessage {
    fn default() -> Self {
        <liter_llm::types::AssistantMessage as Default>::default().into()
    }
}

#[php_impl]
impl AssistantMessage {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    pub fn get_content(&self) -> Option<String> {
        self.content.as_ref().and_then(|v| serde_json::to_string(v).ok())
    }

    pub fn get_name(&self) -> Option<String> {
        self.name.clone()
    }

    pub fn get_tool_calls(&self) -> Option<Vec<ToolCall>> {
        self.tool_calls.clone()
    }

    pub fn get_refusal(&self) -> Option<String> {
        self.refusal.clone()
    }

    pub fn get_function_call(&self) -> Option<FunctionCall> {
        self.function_call.clone().map(Into::into)
    }

    pub fn set_function_call(&mut self, value: Option<&FunctionCall>) {
        self.function_call = value.map(|value| value.clone().into());
    }

    pub fn get_reasoning_content(&self) -> Option<String> {
        self.reasoning_content.clone()
    }

    pub fn text(&self) -> Option<String> {
        #[allow(clippy::needless_update)]
        let core_self = liter_llm::types::AssistantMessage {
            content: self.content.clone().and_then(|v| serde_json::from_value(v).ok()),
            name: self.name.clone(),
            tool_calls: self.tool_calls.clone().map(|v| v.into_iter().map(Into::into).collect()),
            refusal: self.refusal.clone(),
            function_call: self.function_call.clone().map(Into::into),
            reasoning_content: self.reasoning_content.clone(),
            ..Default::default()
        };
        core_self.text()
    }

    pub fn refusal_text(&self) -> Option<String> {
        #[allow(clippy::needless_update)]
        let core_self = liter_llm::types::AssistantMessage {
            content: self.content.clone().and_then(|v| serde_json::from_value(v).ok()),
            name: self.name.clone(),
            tool_calls: self.tool_calls.clone().map(|v| v.into_iter().map(Into::into).collect()),
            refusal: self.refusal.clone(),
            function_call: self.function_call.clone().map(Into::into),
            reasoning_content: self.reasoning_content.clone(),
            ..Default::default()
        };
        core_self.refusal_text().map(Into::into)
    }

    pub fn reasoning_text(&self) -> Option<String> {
        #[allow(clippy::needless_update)]
        let core_self = liter_llm::types::AssistantMessage {
            content: self.content.clone().and_then(|v| serde_json::from_value(v).ok()),
            name: self.name.clone(),
            tool_calls: self.tool_calls.clone().map(|v| v.into_iter().map(Into::into).collect()),
            refusal: self.refusal.clone(),
            function_call: self.function_call.clone().map(Into::into),
            reasoning_content: self.reasoning_content.clone(),
            ..Default::default()
        };
        core_self.reasoning_text().map(Into::into)
    }

    pub fn output_images(&self) -> Vec<ImageUrl> {
        #[allow(clippy::needless_update)]
        let core_self = liter_llm::types::AssistantMessage {
            content: self.content.clone().and_then(|v| serde_json::from_value(v).ok()),
            name: self.name.clone(),
            tool_calls: self.tool_calls.clone().map(|v| v.into_iter().map(Into::into).collect()),
            refusal: self.refusal.clone(),
            function_call: self.function_call.clone().map(Into::into),
            reasoning_content: self.reasoning_content.clone(),
            ..Default::default()
        };
        core_self.output_images().into_iter().map(Into::into).collect()
    }

    pub fn output_audio(&self) -> Vec<AudioContent> {
        #[allow(clippy::needless_update)]
        let core_self = liter_llm::types::AssistantMessage {
            content: self.content.clone().and_then(|v| serde_json::from_value(v).ok()),
            name: self.name.clone(),
            tool_calls: self.tool_calls.clone().map(|v| v.into_iter().map(Into::into).collect()),
            refusal: self.refusal.clone(),
            function_call: self.function_call.clone().map(Into::into),
            reasoning_content: self.reasoning_content.clone(),
            ..Default::default()
        };
        core_self.output_audio().into_iter().map(Into::into).collect()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\AudioContent")]
#[serde(default)]
pub struct AudioContent {
    /// Base64-encoded audio data.
    #[php(prop, name = "data")]
    pub data: String,
    /// Audio format (e.g., "wav", "mp3", "ogg").
    #[php(prop, name = "format")]
    pub format: String,
}

impl Default for AudioContent {
    fn default() -> Self {
        <liter_llm::types::AudioContent as Default>::default().into()
    }
}

#[php_impl]
impl AudioContent {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(data: String, format: String) -> Self {
        Self { data, format }
    }

    pub fn get_data(&self) -> String {
        self.data.clone()
    }

    pub fn get_format(&self) -> String {
        self.format.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize, Default)]
#[php_class]
#[php(name = "Liter\\Llm\\AuthConfig")]
#[serde(default)]
pub struct AuthConfig {
    /// Auth scheme classification.
    #[php(prop, name = "authType")]
    #[serde(skip_serializing_if = "String::is_empty")]
    #[serde(alias = "authType")]
    pub auth_type: String,
    /// Name of the environment variable that holds the API key (e.g. `"OPENAI_API_KEY"`).
    /// Holds the variable name, never the secret value.
    #[php(prop, name = "envVar")]
    #[serde(alias = "envVar")]
    pub env_var: Option<String>,
}

#[php_impl]
impl AuthConfig {
    #[php(constructor)]
    pub fn new(authType: String, envVar: Option<String>) -> Self {
        Self {
            auth_type: authType,
            env_var: envVar,
        }
    }

    pub fn get_auth_type(&self) -> String {
        self.auth_type.clone()
    }

    pub fn get_env_var(&self) -> Option<String> {
        self.env_var.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\BatchListQuery")]
#[serde(default)]
pub struct BatchListQuery {
    /// Maximum number of results to return. Defaults to 20.
    #[php(prop, name = "limit")]
    pub limit: Option<u32>,
    /// Pagination cursor: return results after this batch ID.
    #[php(prop, name = "after")]
    pub after: Option<String>,
}

impl Default for BatchListQuery {
    fn default() -> Self {
        <liter_llm::types::BatchListQuery as Default>::default().into()
    }
}

#[php_impl]
impl BatchListQuery {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    pub fn get_limit(&self) -> Option<u32> {
        self.limit.clone()
    }

    pub fn get_after(&self) -> Option<String> {
        self.after.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\BatchListResponse")]
#[serde(default)]
pub struct BatchListResponse {
    /// Object type (always `"list"`).
    #[php(prop, name = "object")]
    pub object: String,
    /// List of batch objects.
    pub data: Vec<BatchObject>,
    /// Whether more results are available.
    #[php(prop, name = "hasMore")]
    #[serde(alias = "hasMore")]
    pub has_more: Option<bool>,
    /// First batch ID in the result set (for pagination).
    #[php(prop, name = "firstId")]
    #[serde(alias = "firstId")]
    pub first_id: Option<String>,
    /// Last batch ID in the result set (for pagination).
    #[php(prop, name = "lastId")]
    #[serde(alias = "lastId")]
    pub last_id: Option<String>,
}

impl Default for BatchListResponse {
    fn default() -> Self {
        <liter_llm::types::BatchListResponse as Default>::default().into()
    }
}

#[php_impl]
impl BatchListResponse {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(
        object: String,
        data: &ext_php_rs::types::ZendHashTable,
        hasMore: Option<bool>,
        firstId: Option<String>,
        lastId: Option<String>,
    ) -> PhpResult<Self> {
        let mut data_core_result: Vec<BatchObject> = Vec::new();
        for (_, item) in data.iter() {
            if let Some(parsed) = <&BatchObject as ext_php_rs::convert::FromZval>::from_zval(item) {
                data_core_result.push(parsed.clone());
            } else {
                return Err(ext_php_rs::exception::PhpException::default(
                    "Failed to convert array element to BatchObject".to_string(),
                ));
            }
        }
        let data_core: Vec<BatchObject> = data_core_result;
        Ok(Self {
            object,
            data: data_core,
            has_more: hasMore,
            first_id: firstId,
            last_id: lastId,
        })
    }

    pub fn get_object(&self) -> String {
        self.object.clone()
    }

    pub fn get_data(&self) -> Vec<BatchObject> {
        self.data.clone()
    }

    pub fn get_has_more(&self) -> Option<bool> {
        self.has_more.clone()
    }

    pub fn get_first_id(&self) -> Option<String> {
        self.first_id.clone()
    }

    pub fn get_last_id(&self) -> Option<String> {
        self.last_id.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\BatchObject")]
#[serde(default)]
pub struct BatchObject {
    /// Unique batch ID.
    #[php(prop, name = "id")]
    pub id: String,
    /// Object type (always `"batch"`).
    #[php(prop, name = "object")]
    pub object: String,
    /// API endpoint (e.g., `"/v1/chat/completions"`).
    #[php(prop, name = "endpoint")]
    pub endpoint: String,
    /// ID of the input file.
    #[php(prop, name = "inputFileId")]
    #[serde(alias = "inputFileId")]
    pub input_file_id: String,
    /// Completion window (e.g., `"24h"`).
    #[php(prop, name = "completionWindow")]
    #[serde(alias = "completionWindow")]
    pub completion_window: String,
    /// Current job status.
    #[php(prop, name = "status")]
    #[serde(skip_serializing_if = "String::is_empty")]
    pub status: String,
    /// ID of the output file (present when completed).
    #[php(prop, name = "outputFileId")]
    #[serde(alias = "outputFileId")]
    pub output_file_id: Option<String>,
    /// ID of the error file (present if some requests failed).
    #[php(prop, name = "errorFileId")]
    #[serde(alias = "errorFileId")]
    pub error_file_id: Option<String>,
    /// Unix timestamp of batch creation.
    #[php(prop, name = "createdAt")]
    #[serde(alias = "createdAt")]
    pub created_at: i64,
    /// Unix timestamp of completion (if completed).
    #[php(prop, name = "completedAt")]
    #[serde(alias = "completedAt")]
    pub completed_at: Option<i64>,
    /// Unix timestamp of failure (if failed).
    #[php(prop, name = "failedAt")]
    #[serde(alias = "failedAt")]
    pub failed_at: Option<i64>,
    /// Unix timestamp of expiration (if expired).
    #[php(prop, name = "expiredAt")]
    #[serde(alias = "expiredAt")]
    pub expired_at: Option<i64>,
    /// Request processing counts.
    #[serde(alias = "requestCounts")]
    pub request_counts: Option<BatchRequestCounts>,
    /// Metadata attached to the batch.
    pub metadata: Option<serde_json::Value>,
}

impl Default for BatchObject {
    fn default() -> Self {
        <liter_llm::types::BatchObject as Default>::default().into()
    }
}

#[php_impl]
impl BatchObject {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(
        id: String,
        object: String,
        endpoint: String,
        inputFileId: String,
        completionWindow: String,
        status: String,
        createdAt: i64,
        outputFileId: Option<String>,
        errorFileId: Option<String>,
        completedAt: Option<i64>,
        failedAt: Option<i64>,
        expiredAt: Option<i64>,
        requestCounts: Option<&BatchRequestCounts>,
        metadata: Option<String>,
    ) -> PhpResult<Self> {
        Ok(Self {
            id,
            object,
            endpoint,
            input_file_id: inputFileId,
            completion_window: completionWindow,
            status,
            output_file_id: outputFileId,
            error_file_id: errorFileId,
            created_at: createdAt,
            completed_at: completedAt,
            failed_at: failedAt,
            expired_at: expiredAt,
            request_counts: requestCounts.cloned(),
            metadata: metadata
                .map(|s| serde_json::from_str(&s))
                .transpose()
                .map_err(|e| PhpException::default(e.to_string()))?,
        })
    }

    pub fn get_id(&self) -> String {
        self.id.clone()
    }

    pub fn get_object(&self) -> String {
        self.object.clone()
    }

    pub fn get_endpoint(&self) -> String {
        self.endpoint.clone()
    }

    pub fn get_input_file_id(&self) -> String {
        self.input_file_id.clone()
    }

    pub fn get_completion_window(&self) -> String {
        self.completion_window.clone()
    }

    pub fn get_status(&self) -> String {
        self.status.clone()
    }

    pub fn get_output_file_id(&self) -> Option<String> {
        self.output_file_id.clone()
    }

    pub fn get_error_file_id(&self) -> Option<String> {
        self.error_file_id.clone()
    }

    pub fn get_created_at(&self) -> i64 {
        self.created_at.clone()
    }

    pub fn get_completed_at(&self) -> Option<i64> {
        self.completed_at.clone()
    }

    pub fn get_failed_at(&self) -> Option<i64> {
        self.failed_at.clone()
    }

    pub fn get_expired_at(&self) -> Option<i64> {
        self.expired_at.clone()
    }

    pub fn get_request_counts(&self) -> Option<BatchRequestCounts> {
        self.request_counts.clone().map(Into::into)
    }

    pub fn set_request_counts(&mut self, value: Option<&BatchRequestCounts>) {
        self.request_counts = value.map(|value| value.clone().into());
    }

    pub fn get_metadata(&self) -> Option<String> {
        self.metadata.as_ref().and_then(|v| serde_json::to_string(v).ok())
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\BatchRequestCounts")]
#[serde(default)]
pub struct BatchRequestCounts {
    /// Total requests in the batch.
    #[php(prop, name = "total")]
    pub total: i64,
    /// Completed requests.
    #[php(prop, name = "completed")]
    pub completed: i64,
    /// Failed requests.
    #[php(prop, name = "failed")]
    pub failed: i64,
}

impl Default for BatchRequestCounts {
    fn default() -> Self {
        <liter_llm::types::BatchRequestCounts as Default>::default().into()
    }
}

#[php_impl]
impl BatchRequestCounts {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(total: i64, completed: i64, failed: i64) -> Self {
        Self {
            total,
            completed,
            failed,
        }
    }

    pub fn get_total(&self) -> i64 {
        self.total.clone()
    }

    pub fn get_completed(&self) -> i64 {
        self.completed.clone()
    }

    pub fn get_failed(&self) -> i64 {
        self.failed.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\BedrockConfig")]
#[serde(default)]
pub struct BedrockConfig {
    /// AWS region (e.g. `"us-east-1"`).
    #[php(prop, name = "region")]
    pub region: Option<String>,
    /// Cross-region inference profile prefix (e.g. `"us"`).
    #[php(prop, name = "crossRegionPrefix")]
    #[serde(alias = "crossRegionPrefix")]
    pub cross_region_prefix: Option<String>,
    /// Explicit AWS access key ID.
    #[php(prop, name = "accessKeyId")]
    #[serde(alias = "accessKeyId")]
    pub access_key_id: Option<String>,
    /// Explicit AWS secret access key.
    #[php(prop, name = "secretAccessKey")]
    #[serde(alias = "secretAccessKey")]
    pub secret_access_key: Option<String>,
    /// Explicit AWS session token (temporary credentials).
    #[php(prop, name = "sessionToken")]
    #[serde(alias = "sessionToken")]
    pub session_token: Option<String>,
}

impl Default for BedrockConfig {
    fn default() -> Self {
        <liter_llm::client::BedrockConfig as Default>::default().into()
    }
}

#[php_impl]
impl BedrockConfig {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    pub fn get_region(&self) -> Option<String> {
        self.region.clone()
    }

    pub fn get_cross_region_prefix(&self) -> Option<String> {
        self.cross_region_prefix.clone()
    }

    pub fn get_access_key_id(&self) -> Option<String> {
        self.access_key_id.clone()
    }

    pub fn get_secret_access_key(&self) -> Option<String> {
        self.secret_access_key.clone()
    }

    pub fn get_session_token(&self) -> Option<String> {
        self.session_token.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\BudgetConfig")]
#[serde(default)]
pub struct BudgetConfig {
    /// Maximum total spend across all models, in USD.  `None` means unlimited.
    #[php(prop, name = "globalLimit")]
    #[serde(alias = "globalLimit")]
    pub global_limit: Option<f64>,
    /// Per-model spending limits in USD.  Models not listed here are only
    /// constrained by `global_limit`.
    #[php(prop, name = "modelLimits")]
    #[serde(alias = "modelLimits")]
    pub model_limits: HashMap<String, f64>,
    /// Whether to reject requests or merely warn when a limit is exceeded.
    #[php(prop, name = "enforcement")]
    #[serde(skip_serializing_if = "String::is_empty")]
    pub enforcement: String,
}

impl Default for BudgetConfig {
    fn default() -> Self {
        <liter_llm::BudgetConfig as Default>::default().into()
    }
}

#[php_impl]
impl BudgetConfig {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(modelLimits: HashMap<String, f64>, enforcement: String, globalLimit: Option<f64>) -> Self {
        Self {
            global_limit: globalLimit,
            model_limits: modelLimits,
            enforcement,
        }
    }

    pub fn get_global_limit(&self) -> Option<f64> {
        self.global_limit.clone()
    }

    pub fn get_model_limits(&self) -> HashMap<String, f64> {
        self.model_limits.clone()
    }

    pub fn get_enforcement(&self) -> String {
        self.enforcement.clone()
    }

    #[allow(clippy::should_implement_trait)]
    #[php(name = "default")]
    pub fn default() -> BudgetConfig {
        liter_llm::BudgetConfig::default().into()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\CacheConfig")]
#[serde(default)]
pub struct CacheConfig {
    /// Maximum number of cached entries.
    #[php(prop, name = "maxEntries")]
    #[serde(default = "crate::serde_defaults::cache_config_max_entries")]
    #[serde(alias = "maxEntries")]
    pub max_entries: i64,
    /// Time-to-live for each cached entry.
    #[php(prop, name = "ttl")]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub ttl: Option<i64>,
    /// Storage backend to use.
    pub backend: CacheBackend,
}

impl Default for CacheConfig {
    fn default() -> Self {
        <liter_llm::CacheConfig as Default>::default().into()
    }
}

#[php_impl]
impl CacheConfig {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(maxEntries: i64, backend: &CacheBackend, ttl: Option<i64>) -> Self {
        Self {
            max_entries: maxEntries,
            ttl,
            backend: backend.clone(),
        }
    }

    pub fn get_max_entries(&self) -> i64 {
        self.max_entries.clone()
    }

    pub fn get_ttl(&self) -> Option<i64> {
        self.ttl.clone()
    }

    pub fn get_backend(&self) -> CacheBackend {
        self.backend.clone()
    }

    #[allow(clippy::should_implement_trait)]
    #[php(name = "default")]
    pub fn default() -> CacheConfig {
        liter_llm::CacheConfig::default().into()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\CatalogRefreshConfig")]
#[serde(default)]
pub struct CatalogRefreshConfig {
    /// Runtime catalog refresh is entirely opt-in: when `false`,
    /// `refresh_catalog` is a no-op that returns
    /// `Ok(``RefreshOutcome.Disabled``)` without touching the network,
    /// the filesystem, or the overlay registry.
    #[php(prop, name = "enabled")]
    #[serde(default = "crate::serde_defaults::catalog_refresh_config_enabled")]
    pub enabled: bool,
    /// Source URL to fetch `catalog.json` from. Must be `https`. Defaults to
    /// `DEFAULT_CATALOG_URL`; configurable so self-hosted mirrors work.
    #[php(prop, name = "sourceUrl")]
    #[serde(default = "crate::serde_defaults::catalog_refresh_config_source_url")]
    #[serde(alias = "sourceUrl")]
    pub source_url: String,
    /// How long a cached `catalog.json` remains valid before a network
    /// refetch is attempted, in seconds.
    #[php(prop, name = "ttlSeconds")]
    #[serde(default = "crate::serde_defaults::catalog_refresh_config_ttl_seconds")]
    #[serde(alias = "ttlSeconds")]
    pub ttl_seconds: i64,
    /// Filesystem path for the on-disk cache. `None` uses a default path
    /// under `std.env.temp_dir()`.
    #[php(prop, name = "cachePath")]
    #[serde(alias = "cachePath")]
    pub cache_path: Option<String>,
}

impl Default for CatalogRefreshConfig {
    fn default() -> Self {
        <liter_llm::cost::refresh::CatalogRefreshConfig as Default>::default().into()
    }
}

#[php_impl]
impl CatalogRefreshConfig {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(enabled: bool, sourceUrl: String, ttlSeconds: i64, cachePath: Option<String>) -> Self {
        Self {
            enabled,
            source_url: sourceUrl,
            ttl_seconds: ttlSeconds,
            cache_path: cachePath,
        }
    }

    pub fn get_enabled(&self) -> bool {
        self.enabled.clone()
    }

    pub fn get_source_url(&self) -> String {
        self.source_url.clone()
    }

    pub fn get_ttl_seconds(&self) -> i64 {
        self.ttl_seconds.clone()
    }

    pub fn get_cache_path(&self) -> Option<String> {
        self.cache_path.clone()
    }

    #[allow(clippy::should_implement_trait)]
    #[php(name = "default")]
    pub fn default() -> CatalogRefreshConfig {
        liter_llm::cost::refresh::CatalogRefreshConfig::default().into()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\ChatCompletionChunk")]
#[serde(default)]
pub struct ChatCompletionChunk {
    /// Unique identifier for this stream.
    #[php(prop, name = "id")]
    pub id: String,
    /// Always `"chat.completion.chunk"` from OpenAI-compatible APIs.  Stored
    /// as a plain `String` so non-standard provider values do not fail parsing.
    #[php(prop, name = "object")]
    pub object: String,
    /// Unix timestamp of chunk creation.
    #[php(prop, name = "created")]
    pub created: i64,
    /// Model used to generate the chunk.
    #[php(prop, name = "model")]
    pub model: String,
    /// Streaming choices (delta updates).
    pub choices: Vec<StreamChoice>,
    /// Token usage (typically only in the final chunk).
    pub usage: Option<Usage>,
    /// Fingerprint of the system configuration (OpenAI-specific).
    #[php(prop, name = "systemFingerprint")]
    #[serde(alias = "systemFingerprint")]
    pub system_fingerprint: Option<String>,
    /// Service tier used (OpenAI-specific).
    #[php(prop, name = "serviceTier")]
    #[serde(alias = "serviceTier")]
    pub service_tier: Option<String>,
}

impl Default for ChatCompletionChunk {
    fn default() -> Self {
        <liter_llm::types::ChatCompletionChunk as Default>::default().into()
    }
}

#[php_impl]
impl ChatCompletionChunk {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(
        id: String,
        object: String,
        created: i64,
        model: String,
        choices: &ext_php_rs::types::ZendHashTable,
        usage: Option<&Usage>,
        systemFingerprint: Option<String>,
        serviceTier: Option<String>,
    ) -> PhpResult<Self> {
        let mut choices_core_result: Vec<StreamChoice> = Vec::new();
        for (_, item) in choices.iter() {
            if let Some(parsed) = <&StreamChoice as ext_php_rs::convert::FromZval>::from_zval(item) {
                choices_core_result.push(parsed.clone());
            } else {
                return Err(ext_php_rs::exception::PhpException::default(
                    "Failed to convert array element to StreamChoice".to_string(),
                ));
            }
        }
        let choices_core: Vec<StreamChoice> = choices_core_result;
        Ok(Self {
            id,
            object,
            created,
            model,
            choices: choices_core,
            usage: usage.cloned(),
            system_fingerprint: systemFingerprint,
            service_tier: serviceTier,
        })
    }

    pub fn get_id(&self) -> String {
        self.id.clone()
    }

    pub fn get_object(&self) -> String {
        self.object.clone()
    }

    pub fn get_created(&self) -> i64 {
        self.created.clone()
    }

    pub fn get_model(&self) -> String {
        self.model.clone()
    }

    pub fn get_choices(&self) -> Vec<StreamChoice> {
        self.choices.clone()
    }

    pub fn get_usage(&self) -> Option<Usage> {
        self.usage.clone().map(Into::into)
    }

    pub fn set_usage(&mut self, value: Option<&Usage>) {
        self.usage = value.map(|value| value.clone().into());
    }

    pub fn get_system_fingerprint(&self) -> Option<String> {
        self.system_fingerprint.clone()
    }

    pub fn get_service_tier(&self) -> Option<String> {
        self.service_tier.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\ChatCompletionRequest")]
#[serde(default)]
pub struct ChatCompletionRequest {
    /// Model ID (e.g., `"gpt-4o-mini"`, `"claude-3-5-sonnet"`).
    #[php(prop, name = "model")]
    pub model: String,
    /// Conversation history from oldest to newest.
    pub messages: Vec<Message>,
    /// Sampling temperature. Higher increases randomness, lower is more deterministic.
    /// Defaults to 1.0.
    ///
    /// The accepted range depends on the provider the request is routed to. OpenAI-compatible
    /// providers accept `[0.0, 2.0]`; Anthropic and Amazon Bedrock both cap it at `1.0`, and
    /// for those two a value above the cap is rejected with a `BadRequest` error before the
    /// request is sent, rather than being silently clamped or left for the provider to reject.
    ///
    /// No range is enforced for providers whose own documentation does not state one — the
    /// value is forwarded and the provider decides. Consult the target provider's reference
    /// rather than assuming `[0.0, 2.0]` is portable.
    #[php(prop, name = "temperature")]
    pub temperature: Option<f64>,
    /// Nucleus sampling parameter. Lower is more focused.
    ///
    /// Accepted ranges vary by provider (most document `[0.0, 1.0]`, but this is not
    /// universal — check the target provider's own documentation for its exact bounds).
    #[php(prop, name = "topP")]
    #[serde(alias = "topP")]
    pub top_p: Option<f64>,
    /// Number of chat completions to generate. Defaults to 1.
    #[php(prop, name = "n")]
    pub n: Option<u32>,
    /// Whether to stream the response.
    ///
    /// Managed by the client layer — do not set directly.
    #[php(prop, name = "stream")]
    pub stream: Option<bool>,
    /// Stop sequence(s) that halt token generation.
    pub stop: Option<serde_json::Value>,
    /// Max output tokens. Different from max_completion_tokens in some providers.
    #[php(prop, name = "maxTokens")]
    #[serde(alias = "maxTokens")]
    pub max_tokens: Option<i64>,
    /// Presence penalty in `[-2.0, 2.0]`. Positive discourages repeated topics.
    #[php(prop, name = "presencePenalty")]
    #[serde(alias = "presencePenalty")]
    pub presence_penalty: Option<f64>,
    /// Frequency penalty in `[-2.0, 2.0]`. Positive discourages repeated tokens.
    #[php(prop, name = "frequencyPenalty")]
    #[serde(alias = "frequencyPenalty")]
    pub frequency_penalty: Option<f64>,
    /// Token bias map.  Uses `BTreeMap` (sorted keys) for deterministic
    /// serialization order — important when hashing or signing requests.
    #[php(prop, name = "logitBias")]
    #[serde(alias = "logitBias")]
    pub logit_bias: Option<HashMap<String, f64>>,
    /// User identifier for request tracking and abuse detection.
    #[php(prop, name = "user")]
    pub user: Option<String>,
    /// Tools the model can invoke.
    pub tools: Option<Vec<ChatCompletionTool>>,
    /// Tool usage mode (auto, required, none, or specific tool).
    #[serde(alias = "toolChoice")]
    pub tool_choice: Option<serde_json::Value>,
    /// Whether the model can call multiple tools in parallel. Defaults to true.
    #[php(prop, name = "parallelToolCalls")]
    #[serde(alias = "parallelToolCalls")]
    pub parallel_tool_calls: Option<bool>,
    /// Output format constraint (text, JSON, JSON schema).
    #[serde(alias = "responseFormat")]
    pub response_format: Option<ResponseFormat>,
    /// Streaming options (e.g., include_usage).
    #[serde(alias = "streamOptions")]
    pub stream_options: Option<StreamOptions>,
    /// Random seed for reproducible outputs. Provider support varies.
    #[php(prop, name = "seed")]
    pub seed: Option<i64>,
    /// Reasoning effort level (minimal, low, medium, high, max) for extended-thinking models.
    #[php(prop, name = "reasoningEffort")]
    #[serde(skip_serializing_if = "Option::is_none")]
    #[serde(alias = "reasoningEffort")]
    pub reasoning_effort: Option<String>,
    /// Output modalities to request from the model.
    ///
    /// For OpenAI audio models, pass `["text", "audio"]`. Vertex AI / Gemini
    /// translates these to `generationConfig.responseModalities` (uppercase).
    #[php(prop, name = "modalities")]
    pub modalities: Option<Vec<String>>,
    /// Whether to return log probabilities of the output tokens.
    #[php(prop, name = "logprobs")]
    pub logprobs: Option<bool>,
    /// Number of most-likely tokens to return log probabilities for, `0..=20`.
    /// Requires `logprobs` to be `true`.
    #[php(prop, name = "topLogprobs")]
    #[serde(alias = "topLogprobs")]
    pub top_logprobs: Option<u32>,
    /// Upper bound on generated tokens, including reasoning tokens.
    ///
    /// Supersedes `max_tokens` on OpenAI reasoning models, which reject
    /// `max_tokens` outright.
    #[php(prop, name = "maxCompletionTokens")]
    #[serde(alias = "maxCompletionTokens")]
    pub max_completion_tokens: Option<i64>,
    /// Latency tier to process the request under (e.g. `"auto"`, `"default"`,
    /// `"flex"`).
    #[php(prop, name = "serviceTier")]
    #[serde(alias = "serviceTier")]
    pub service_tier: Option<String>,
    /// Whether to store the completion for later retrieval by the provider.
    #[php(prop, name = "store")]
    pub store: Option<bool>,
    /// Developer-defined tags attached to the completion.
    #[php(prop, name = "metadata")]
    pub metadata: Option<HashMap<String, String>>,
    /// Predicted output, for latency reduction when much of the response is
    /// known ahead of time.
    ///
    /// Untyped: the shape is provider-defined and still evolving, and the
    /// value is forwarded verbatim.
    pub prediction: Option<serde_json::Value>,
    /// Audio output parameters, required when `modalities` includes `audio`.
    ///
    /// Untyped for the same reason as `prediction`.
    pub audio: Option<serde_json::Value>,
    /// Web-search tool configuration for search-enabled models.
    ///
    /// Untyped for the same reason as `prediction`.
    #[serde(alias = "webSearchOptions")]
    pub web_search_options: Option<serde_json::Value>,
    /// Provider-specific extra parameters merged into the request body.
    /// Use for guardrails, safety settings, grounding config, etc.
    #[serde(alias = "extraBody")]
    pub extra_body: Option<serde_json::Value>,
}

impl Default for ChatCompletionRequest {
    fn default() -> Self {
        <liter_llm::types::ChatCompletionRequest as Default>::default().into()
    }
}

#[php_impl]
impl ChatCompletionRequest {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(
        model: String,
        messages: &ext_php_rs::types::ZendHashTable,
        temperature: Option<f64>,
        topP: Option<f64>,
        n: Option<u32>,
        stream: Option<bool>,
        maxTokens: Option<i64>,
        presencePenalty: Option<f64>,
        frequencyPenalty: Option<f64>,
        logitBias: Option<HashMap<String, f64>>,
        user: Option<String>,
        tools: Option<&ext_php_rs::types::ZendHashTable>,
        parallelToolCalls: Option<bool>,
        responseFormat: Option<&ResponseFormat>,
        streamOptions: Option<&StreamOptions>,
        seed: Option<i64>,
        reasoningEffort: Option<String>,
        modalities: Option<Vec<String>>,
        logprobs: Option<bool>,
        topLogprobs: Option<u32>,
        maxCompletionTokens: Option<i64>,
        serviceTier: Option<String>,
        store: Option<bool>,
        metadata: Option<HashMap<String, String>>,
        prediction: Option<String>,
        audio: Option<String>,
        webSearchOptions: Option<String>,
        extraBody: Option<String>,
    ) -> PhpResult<Self> {
        let mut messages_core_result: Vec<Message> = Vec::new();
        for (_, item) in messages.iter() {
            if let Some(parsed) = <&Message as ext_php_rs::convert::FromZval>::from_zval(item) {
                messages_core_result.push(parsed.clone());
            } else {
                return Err(ext_php_rs::exception::PhpException::default(
                    "Failed to convert array element to Message".to_string(),
                ));
            }
        }
        let messages_core: Vec<Message> = messages_core_result;
        let tools_core: Option<Vec<ChatCompletionTool>> = if let Some(ht) = tools {
            let mut result = Vec::new();
            for (_, item) in ht.iter() {
                if let Some(parsed) = <&ChatCompletionTool as ext_php_rs::convert::FromZval>::from_zval(item) {
                    result.push(parsed.clone());
                } else {
                    return Err(ext_php_rs::exception::PhpException::default(
                        "Failed to convert array element to ChatCompletionTool".to_string(),
                    ));
                }
            }
            Some(result)
        } else {
            None
        };
        Ok(Self {
            model,
            messages: messages_core,
            temperature,
            top_p: topP,
            n,
            stream,
            stop: Default::default(),
            max_tokens: maxTokens,
            presence_penalty: presencePenalty,
            frequency_penalty: frequencyPenalty,
            logit_bias: logitBias,
            user,
            tools: tools_core,
            tool_choice: Default::default(),
            parallel_tool_calls: parallelToolCalls,
            response_format: responseFormat.cloned(),
            stream_options: streamOptions.cloned(),
            seed,
            reasoning_effort: reasoningEffort,
            modalities,
            logprobs,
            top_logprobs: topLogprobs,
            max_completion_tokens: maxCompletionTokens,
            service_tier: serviceTier,
            store,
            metadata,
            prediction: prediction
                .map(|s| serde_json::from_str(&s))
                .transpose()
                .map_err(|e| PhpException::default(e.to_string()))?,
            audio: audio
                .map(|s| serde_json::from_str(&s))
                .transpose()
                .map_err(|e| PhpException::default(e.to_string()))?,
            web_search_options: webSearchOptions
                .map(|s| serde_json::from_str(&s))
                .transpose()
                .map_err(|e| PhpException::default(e.to_string()))?,
            extra_body: extraBody
                .map(|s| serde_json::from_str(&s))
                .transpose()
                .map_err(|e| PhpException::default(e.to_string()))?,
        })
    }

    pub fn get_model(&self) -> String {
        self.model.clone()
    }

    pub fn get_messages(&self) -> Vec<Message> {
        self.messages.clone()
    }

    pub fn get_temperature(&self) -> Option<f64> {
        self.temperature.clone()
    }

    pub fn get_top_p(&self) -> Option<f64> {
        self.top_p.clone()
    }

    pub fn get_n(&self) -> Option<u32> {
        self.n.clone()
    }

    pub fn get_stream(&self) -> Option<bool> {
        self.stream.clone()
    }

    pub fn get_stop(&self) -> Option<String> {
        self.stop.as_ref().and_then(|v| serde_json::to_string(v).ok())
    }

    pub fn get_max_tokens(&self) -> Option<i64> {
        self.max_tokens.clone()
    }

    pub fn get_presence_penalty(&self) -> Option<f64> {
        self.presence_penalty.clone()
    }

    pub fn get_frequency_penalty(&self) -> Option<f64> {
        self.frequency_penalty.clone()
    }

    pub fn get_logit_bias(&self) -> Option<HashMap<String, f64>> {
        self.logit_bias.clone()
    }

    pub fn get_user(&self) -> Option<String> {
        self.user.clone()
    }

    pub fn get_tools(&self) -> Option<Vec<ChatCompletionTool>> {
        self.tools.clone()
    }

    pub fn get_tool_choice(&self) -> Option<String> {
        self.tool_choice.as_ref().and_then(|v| serde_json::to_string(v).ok())
    }

    pub fn get_parallel_tool_calls(&self) -> Option<bool> {
        self.parallel_tool_calls.clone()
    }

    pub fn get_response_format(&self) -> Option<ResponseFormat> {
        self.response_format.clone().map(Into::into)
    }

    pub fn set_response_format(&mut self, value: Option<&ResponseFormat>) {
        self.response_format = value.map(|value| value.clone().into());
    }

    pub fn get_stream_options(&self) -> Option<StreamOptions> {
        self.stream_options.clone().map(Into::into)
    }

    pub fn set_stream_options(&mut self, value: Option<&StreamOptions>) {
        self.stream_options = value.map(|value| value.clone().into());
    }

    pub fn get_seed(&self) -> Option<i64> {
        self.seed.clone()
    }

    pub fn get_reasoning_effort(&self) -> Option<String> {
        self.reasoning_effort.clone().map(Into::into)
    }

    pub fn get_modalities(&self) -> Option<Vec<String>> {
        self.modalities.clone()
    }

    pub fn get_logprobs(&self) -> Option<bool> {
        self.logprobs.clone()
    }

    pub fn get_top_logprobs(&self) -> Option<u32> {
        self.top_logprobs.clone()
    }

    pub fn get_max_completion_tokens(&self) -> Option<i64> {
        self.max_completion_tokens.clone()
    }

    pub fn get_service_tier(&self) -> Option<String> {
        self.service_tier.clone()
    }

    pub fn get_store(&self) -> Option<bool> {
        self.store.clone()
    }

    pub fn get_metadata(&self) -> Option<HashMap<String, String>> {
        self.metadata.clone()
    }

    pub fn get_prediction(&self) -> Option<String> {
        self.prediction.as_ref().and_then(|v| serde_json::to_string(v).ok())
    }

    pub fn get_audio(&self) -> Option<String> {
        self.audio.as_ref().and_then(|v| serde_json::to_string(v).ok())
    }

    pub fn get_web_search_options(&self) -> Option<String> {
        self.web_search_options
            .as_ref()
            .and_then(|v| serde_json::to_string(v).ok())
    }

    pub fn get_extra_body(&self) -> Option<String> {
        self.extra_body.as_ref().and_then(|v| serde_json::to_string(v).ok())
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\ChatCompletionResponse")]
#[serde(default)]
pub struct ChatCompletionResponse {
    /// Unique identifier for this response.
    #[php(prop, name = "id")]
    pub id: String,
    /// Always `"chat.completion"` from OpenAI-compatible APIs.  Stored as a
    /// plain `String` so non-standard provider values do not break deserialization.
    #[php(prop, name = "object")]
    pub object: String,
    /// Unix timestamp of response creation.
    #[php(prop, name = "created")]
    pub created: i64,
    /// Model used to generate the response.
    #[php(prop, name = "model")]
    pub model: String,
    /// List of completion choices.
    pub choices: Vec<Choice>,
    /// Token usage statistics.
    pub usage: Option<Usage>,
    /// Fingerprint of the system configuration (OpenAI-specific).
    #[php(prop, name = "systemFingerprint")]
    #[serde(alias = "systemFingerprint")]
    pub system_fingerprint: Option<String>,
    /// Service tier used (OpenAI-specific).
    #[php(prop, name = "serviceTier")]
    #[serde(alias = "serviceTier")]
    pub service_tier: Option<String>,
}

impl Default for ChatCompletionResponse {
    fn default() -> Self {
        <liter_llm::types::ChatCompletionResponse as Default>::default().into()
    }
}

#[php_impl]
impl ChatCompletionResponse {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(
        id: String,
        object: String,
        created: i64,
        model: String,
        choices: &ext_php_rs::types::ZendHashTable,
        usage: Option<&Usage>,
        systemFingerprint: Option<String>,
        serviceTier: Option<String>,
    ) -> PhpResult<Self> {
        let mut choices_core_result: Vec<Choice> = Vec::new();
        for (_, item) in choices.iter() {
            if let Some(parsed) = <&Choice as ext_php_rs::convert::FromZval>::from_zval(item) {
                choices_core_result.push(parsed.clone());
            } else {
                return Err(ext_php_rs::exception::PhpException::default(
                    "Failed to convert array element to Choice".to_string(),
                ));
            }
        }
        let choices_core: Vec<Choice> = choices_core_result;
        Ok(Self {
            id,
            object,
            created,
            model,
            choices: choices_core,
            usage: usage.cloned(),
            system_fingerprint: systemFingerprint,
            service_tier: serviceTier,
        })
    }

    pub fn get_id(&self) -> String {
        self.id.clone()
    }

    pub fn get_object(&self) -> String {
        self.object.clone()
    }

    pub fn get_created(&self) -> i64 {
        self.created.clone()
    }

    pub fn get_model(&self) -> String {
        self.model.clone()
    }

    pub fn get_choices(&self) -> Vec<Choice> {
        self.choices.clone()
    }

    pub fn get_usage(&self) -> Option<Usage> {
        self.usage.clone().map(Into::into)
    }

    pub fn set_usage(&mut self, value: Option<&Usage>) {
        self.usage = value.map(|value| value.clone().into());
    }

    pub fn get_system_fingerprint(&self) -> Option<String> {
        self.system_fingerprint.clone()
    }

    pub fn get_service_tier(&self) -> Option<String> {
        self.service_tier.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize, Default)]
#[php_class]
#[php(name = "Liter\\Llm\\ChatCompletionTool")]
#[serde(default)]
pub struct ChatCompletionTool {
    /// Tool type (always "function" in OpenAI spec).
    #[php(prop, name = "toolType")]
    #[serde(skip_serializing_if = "String::is_empty")]
    #[serde(alias = "toolType")]
    pub tool_type: String,
    /// Function definition with name, description, and JSON schema parameters.
    pub function: FunctionDefinition,
}

#[php_impl]
impl ChatCompletionTool {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(toolType: String, function: &FunctionDefinition) -> Self {
        Self {
            tool_type: toolType,
            function: function.clone(),
        }
    }

    pub fn get_tool_type(&self) -> String {
        self.tool_type.clone()
    }

    pub fn get_function(&self) -> FunctionDefinition {
        self.function.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\Choice")]
#[serde(default)]
pub struct Choice {
    /// Index of this choice in the choices array.
    #[php(prop, name = "index")]
    pub index: u32,
    /// The assistant's message response.
    ///
    /// Serialized with an explicit `role: "assistant"`. The field is not stored
    /// on `AssistantMessage` because `Message` is an internally-tagged enum
    /// keyed on `role`, so a stored field would emit the key twice inside a
    /// request. OpenAI's response schema requires it here.
    pub message: AssistantMessage,
    /// Why the model stopped generating (stop, length, tool_calls, content_filter, etc.).
    #[php(prop, name = "finishReason")]
    #[serde(skip_serializing_if = "Option::is_none")]
    #[serde(alias = "finishReason")]
    pub finish_reason: Option<String>,
    /// Per-token log probabilities, when the request asked for them.
    ///
    /// Required by OpenAI's response schema as an always-present, nullable key,
    /// so this is deliberately not `skip_serializing_if`.
    pub logprobs: Option<serde_json::Value>,
}

impl Default for Choice {
    fn default() -> Self {
        <liter_llm::types::Choice as Default>::default().into()
    }
}

#[php_impl]
impl Choice {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(
        index: u32,
        message: &AssistantMessage,
        finishReason: Option<String>,
        logprobs: Option<String>,
    ) -> PhpResult<Self> {
        Ok(Self {
            index,
            message: message.clone(),
            finish_reason: finishReason,
            logprobs: logprobs
                .map(|s| serde_json::from_str(&s))
                .transpose()
                .map_err(|e| PhpException::default(e.to_string()))?,
        })
    }

    pub fn get_index(&self) -> u32 {
        self.index.clone()
    }

    pub fn get_message(&self) -> AssistantMessage {
        self.message.clone()
    }

    pub fn get_finish_reason(&self) -> Option<String> {
        self.finish_reason.clone().map(Into::into)
    }

    pub fn get_logprobs(&self) -> Option<String> {
        self.logprobs.as_ref().and_then(|v| serde_json::to_string(v).ok())
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\CreateBatchRequest")]
#[serde(default)]
pub struct CreateBatchRequest {
    /// ID of the uploaded input file (JSONL format).
    #[php(prop, name = "inputFileId")]
    #[serde(alias = "inputFileId")]
    pub input_file_id: String,
    /// API endpoint (e.g., `"/v1/chat/completions"`).
    #[php(prop, name = "endpoint")]
    pub endpoint: String,
    /// Completion window (e.g., `"24h"`).
    #[php(prop, name = "completionWindow")]
    #[serde(alias = "completionWindow")]
    pub completion_window: String,
    /// Optional metadata to attach to the batch.
    pub metadata: Option<serde_json::Value>,
}

impl Default for CreateBatchRequest {
    fn default() -> Self {
        <liter_llm::types::CreateBatchRequest as Default>::default().into()
    }
}

#[php_impl]
impl CreateBatchRequest {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(
        inputFileId: String,
        endpoint: String,
        completionWindow: String,
        metadata: Option<String>,
    ) -> PhpResult<Self> {
        Ok(Self {
            input_file_id: inputFileId,
            endpoint,
            completion_window: completionWindow,
            metadata: metadata
                .map(|s| serde_json::from_str(&s))
                .transpose()
                .map_err(|e| PhpException::default(e.to_string()))?,
        })
    }

    pub fn get_input_file_id(&self) -> String {
        self.input_file_id.clone()
    }

    pub fn get_endpoint(&self) -> String {
        self.endpoint.clone()
    }

    pub fn get_completion_window(&self) -> String {
        self.completion_window.clone()
    }

    pub fn get_metadata(&self) -> Option<String> {
        self.metadata.as_ref().and_then(|v| serde_json::to_string(v).ok())
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\CreateFileRequest")]
#[serde(default)]
pub struct CreateFileRequest {
    /// Base64-encoded file data.
    #[php(prop, name = "file")]
    pub file: String,
    /// Purpose for the file.
    #[php(prop, name = "purpose")]
    #[serde(skip_serializing_if = "String::is_empty")]
    pub purpose: String,
    /// Optional filename to associate with the upload.
    #[php(prop, name = "filename")]
    pub filename: Option<String>,
}

impl Default for CreateFileRequest {
    fn default() -> Self {
        <liter_llm::types::CreateFileRequest as Default>::default().into()
    }
}

#[php_impl]
impl CreateFileRequest {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(file: String, purpose: String, filename: Option<String>) -> Self {
        Self {
            file,
            purpose,
            filename,
        }
    }

    pub fn get_file(&self) -> String {
        self.file.clone()
    }

    pub fn get_purpose(&self) -> String {
        self.purpose.clone()
    }

    pub fn get_filename(&self) -> Option<String> {
        self.filename.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\CreateImageRequest")]
#[serde(default)]
pub struct CreateImageRequest {
    /// Text description of the image to generate.
    #[php(prop, name = "prompt")]
    pub prompt: String,
    /// Model ID (e.g., `"dall-e-3"`). Optional; API may use default if unset.
    #[php(prop, name = "model")]
    pub model: Option<String>,
    /// Number of images to generate. Defaults to 1.
    #[php(prop, name = "n")]
    pub n: Option<u32>,
    /// Image size (e.g., `"1024x1024"`, `"1792x1024"`).
    #[php(prop, name = "size")]
    pub size: Option<String>,
    /// Image quality: `"standard"` or `"hd"`.
    #[php(prop, name = "quality")]
    pub quality: Option<String>,
    /// Style: `"natural"` or `"vivid"` (DALL-E 3 only).
    #[php(prop, name = "style")]
    pub style: Option<String>,
    /// Response format: `"url"` or `"b64_json"`.
    #[php(prop, name = "responseFormat")]
    #[serde(alias = "responseFormat")]
    pub response_format: Option<String>,
    /// User identifier for request tracking.
    #[php(prop, name = "user")]
    pub user: Option<String>,
}

impl Default for CreateImageRequest {
    fn default() -> Self {
        <liter_llm::types::CreateImageRequest as Default>::default().into()
    }
}

#[php_impl]
impl CreateImageRequest {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(
        prompt: String,
        model: Option<String>,
        n: Option<u32>,
        size: Option<String>,
        quality: Option<String>,
        style: Option<String>,
        responseFormat: Option<String>,
        user: Option<String>,
    ) -> Self {
        Self {
            prompt,
            model,
            n,
            size,
            quality,
            style,
            response_format: responseFormat,
            user,
        }
    }

    pub fn get_prompt(&self) -> String {
        self.prompt.clone()
    }

    pub fn get_model(&self) -> Option<String> {
        self.model.clone()
    }

    pub fn get_n(&self) -> Option<u32> {
        self.n.clone()
    }

    pub fn get_size(&self) -> Option<String> {
        self.size.clone()
    }

    pub fn get_quality(&self) -> Option<String> {
        self.quality.clone()
    }

    pub fn get_style(&self) -> Option<String> {
        self.style.clone()
    }

    pub fn get_response_format(&self) -> Option<String> {
        self.response_format.clone()
    }

    pub fn get_user(&self) -> Option<String> {
        self.user.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\CreateResponseRequest")]
#[serde(default)]
pub struct CreateResponseRequest {
    /// Model ID, as named by the OpenAI Responses API (e.g. `"gpt-5"`).
    ///
    /// Sent to the wire exactly as given. The Responses path performs none of the
    /// chat path's model handling: a `provider/model` routing prefix is **not**
    /// stripped and does **not** re-route the request, which stays pinned to the
    /// provider the client was constructed with.
    #[php(prop, name = "model")]
    pub model: String,
    /// Input data to process (e.g., a document to extract from).
    pub input: serde_json::Value,
    /// Instructions for processing the input.
    #[php(prop, name = "instructions")]
    pub instructions: Option<String>,
    /// Available tools the model can use.
    pub tools: Option<Vec<ResponseTool>>,
    /// Sampling temperature in `[0.0, 2.0]`. Defaults to 1.0.
    #[php(prop, name = "temperature")]
    pub temperature: Option<f64>,
    /// Maximum output tokens.
    #[php(prop, name = "maxOutputTokens")]
    #[serde(alias = "maxOutputTokens")]
    pub max_output_tokens: Option<i64>,
    /// Optional metadata.
    pub metadata: Option<serde_json::Value>,
    /// Extra top-level parameters shallow-merged into the request body, OpenAI-Python
    /// style (`{**body, **extra_body}`) — keys here override identically named fields
    /// above. Use it for OpenAI Responses fields this struct does not model directly,
    /// such as the top-level `reasoning.effort`.
    ///
    /// This is an OpenAI escape hatch, not a cross-provider one. On the chat path the
    /// providers that consume `extra_body` natively (Anthropic, Vertex, Bedrock) claim
    /// it inside their own `transform_request`; here no provider transform runs, so the
    /// merged keys always travel to the wire as literal OpenAI Responses fields.
    ///
    /// A non-object value cannot be merged into the body root and is dropped with a
    /// warning rather than sent.
    #[serde(alias = "extraBody")]
    pub extra_body: Option<serde_json::Value>,
    /// Whether to stream the response.
    ///
    /// Managed by the client layer — do not set directly.
    #[php(prop, name = "stream")]
    pub stream: Option<bool>,
}

impl Default for CreateResponseRequest {
    fn default() -> Self {
        <liter_llm::types::CreateResponseRequest as Default>::default().into()
    }
}

#[php_impl]
impl CreateResponseRequest {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(
        model: String,
        input: String,
        instructions: Option<String>,
        tools: Option<&ext_php_rs::types::ZendHashTable>,
        temperature: Option<f64>,
        maxOutputTokens: Option<i64>,
        metadata: Option<String>,
        extraBody: Option<String>,
        stream: Option<bool>,
    ) -> PhpResult<Self> {
        let tools_core: Option<Vec<ResponseTool>> = if let Some(ht) = tools {
            let mut result = Vec::new();
            for (_, item) in ht.iter() {
                if let Some(parsed) = <&ResponseTool as ext_php_rs::convert::FromZval>::from_zval(item) {
                    result.push(parsed.clone());
                } else {
                    return Err(ext_php_rs::exception::PhpException::default(
                        "Failed to convert array element to ResponseTool".to_string(),
                    ));
                }
            }
            Some(result)
        } else {
            None
        };
        Ok(Self {
            model,
            input: serde_json::from_str(&input).map_err(|e| PhpException::default(e.to_string()))?,
            instructions,
            tools: tools_core,
            temperature,
            max_output_tokens: maxOutputTokens,
            metadata: metadata
                .map(|s| serde_json::from_str(&s))
                .transpose()
                .map_err(|e| PhpException::default(e.to_string()))?,
            extra_body: extraBody
                .map(|s| serde_json::from_str(&s))
                .transpose()
                .map_err(|e| PhpException::default(e.to_string()))?,
            stream,
        })
    }

    pub fn get_model(&self) -> String {
        self.model.clone()
    }

    pub fn get_input(&self) -> Option<String> {
        serde_json::to_string(&self.input).ok()
    }

    pub fn get_instructions(&self) -> Option<String> {
        self.instructions.clone()
    }

    pub fn get_tools(&self) -> Option<Vec<ResponseTool>> {
        self.tools.clone()
    }

    pub fn get_temperature(&self) -> Option<f64> {
        self.temperature.clone()
    }

    pub fn get_max_output_tokens(&self) -> Option<i64> {
        self.max_output_tokens.clone()
    }

    pub fn get_metadata(&self) -> Option<String> {
        self.metadata.as_ref().and_then(|v| serde_json::to_string(v).ok())
    }

    pub fn get_extra_body(&self) -> Option<String> {
        self.extra_body.as_ref().and_then(|v| serde_json::to_string(v).ok())
    }

    pub fn get_stream(&self) -> Option<bool> {
        self.stream.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\CreateSpeechRequest")]
#[serde(default)]
pub struct CreateSpeechRequest {
    /// Model ID (e.g., `"tts-1"`, `"tts-1-hd"`).
    #[php(prop, name = "model")]
    pub model: String,
    /// Text to synthesize into speech.
    #[php(prop, name = "input")]
    pub input: String,
    /// Voice name (e.g., `"alloy"`, `"echo"`, `"fable"`, `"onyx"`, `"nova"`, `"shimmer"`).
    #[php(prop, name = "voice")]
    pub voice: String,
    /// Audio format (e.g., `"mp3"`, `"opus"`, `"aac"`, `"flac"`, `"wav"`, `"pcm"`).
    #[php(prop, name = "responseFormat")]
    #[serde(alias = "responseFormat")]
    pub response_format: Option<String>,
    /// Playback speed in `[0.25, 4.0]`. Defaults to 1.0.
    #[php(prop, name = "speed")]
    pub speed: Option<f64>,
}

impl Default for CreateSpeechRequest {
    fn default() -> Self {
        <liter_llm::types::CreateSpeechRequest as Default>::default().into()
    }
}

#[php_impl]
impl CreateSpeechRequest {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(
        model: String,
        input: String,
        voice: String,
        responseFormat: Option<String>,
        speed: Option<f64>,
    ) -> Self {
        Self {
            model,
            input,
            voice,
            response_format: responseFormat,
            speed,
        }
    }

    pub fn get_model(&self) -> String {
        self.model.clone()
    }

    pub fn get_input(&self) -> String {
        self.input.clone()
    }

    pub fn get_voice(&self) -> String {
        self.voice.clone()
    }

    pub fn get_response_format(&self) -> Option<String> {
        self.response_format.clone()
    }

    pub fn get_speed(&self) -> Option<f64> {
        self.speed.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\CreateTranscriptionRequest")]
#[serde(default)]
pub struct CreateTranscriptionRequest {
    /// Model ID (e.g., `"whisper-1"`).
    #[php(prop, name = "model")]
    pub model: String,
    /// Base64-encoded audio file data.
    #[php(prop, name = "file")]
    pub file: String,
    /// Language ISO-639-1 code (e.g., `"en"`, `"fr"`, `"de"`). Optional; model auto-detects.
    #[php(prop, name = "language")]
    pub language: Option<String>,
    /// Optional text to guide the model (improves accuracy for domain-specific terms).
    #[php(prop, name = "prompt")]
    pub prompt: Option<String>,
    /// Output format (e.g., `"json"`, `"text"`, `"vtt"`, `"srt"`, `"verbose_json"`).
    #[php(prop, name = "responseFormat")]
    #[serde(alias = "responseFormat")]
    pub response_format: Option<String>,
    /// Sampling temperature in `[0.0, 1.0]`. Higher increases variability. Defaults to 0.
    #[php(prop, name = "temperature")]
    pub temperature: Option<f64>,
}

impl Default for CreateTranscriptionRequest {
    fn default() -> Self {
        <liter_llm::types::CreateTranscriptionRequest as Default>::default().into()
    }
}

#[php_impl]
impl CreateTranscriptionRequest {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(
        model: String,
        file: String,
        language: Option<String>,
        prompt: Option<String>,
        responseFormat: Option<String>,
        temperature: Option<f64>,
    ) -> Self {
        Self {
            model,
            file,
            language,
            prompt,
            response_format: responseFormat,
            temperature,
        }
    }

    pub fn get_model(&self) -> String {
        self.model.clone()
    }

    pub fn get_file(&self) -> String {
        self.file.clone()
    }

    pub fn get_language(&self) -> Option<String> {
        self.language.clone()
    }

    pub fn get_prompt(&self) -> Option<String> {
        self.prompt.clone()
    }

    pub fn get_response_format(&self) -> Option<String> {
        self.response_format.clone()
    }

    pub fn get_temperature(&self) -> Option<f64> {
        self.temperature.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize, Default)]
#[php_class]
#[php(name = "Liter\\Llm\\CustomProviderConfig")]
#[serde(default)]
pub struct CustomProviderConfig {
    /// Unique name for this provider (e.g., "my-provider").
    #[php(prop, name = "name")]
    pub name: String,
    /// Base URL for the provider's API (e.g., `<https://api.my-provider.com/v1>`).
    #[php(prop, name = "baseUrl")]
    #[serde(alias = "baseUrl")]
    pub base_url: String,
    /// Authentication header format.
    #[php(prop, name = "authHeader")]
    #[serde(skip_serializing_if = "String::is_empty")]
    #[serde(alias = "authHeader")]
    pub auth_header: String,
    /// Model name prefixes that route to this provider (e.g., `["my-"]`).
    #[php(prop, name = "modelPrefixes")]
    #[serde(alias = "modelPrefixes")]
    pub model_prefixes: Vec<String>,
}

#[php_impl]
impl CustomProviderConfig {
    #[php(constructor)]
    pub fn new(name: String, baseUrl: String, authHeader: String, modelPrefixes: Vec<String>) -> Self {
        Self {
            name,
            base_url: baseUrl,
            auth_header: authHeader,
            model_prefixes: modelPrefixes,
        }
    }

    pub fn get_name(&self) -> String {
        self.name.clone()
    }

    pub fn get_base_url(&self) -> String {
        self.base_url.clone()
    }

    pub fn get_auth_header(&self) -> String {
        self.auth_header.clone()
    }

    pub fn get_model_prefixes(&self) -> Vec<String> {
        self.model_prefixes.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\DecodedDataUrl")]
#[serde(default)]
pub struct DecodedDataUrl {
    /// MIME type extracted from the URL prefix (verbatim, not normalised).
    #[php(prop, name = "mime")]
    pub mime: String,
    /// Decoded base64 payload.
    pub data: Vec<u8>,
}

impl Default for DecodedDataUrl {
    fn default() -> Self {
        <liter_llm::image::DecodedDataUrl as Default>::default().into()
    }
}

#[php_impl]
impl DecodedDataUrl {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(mime: String, data: PhpBytes) -> Self {
        Self { mime, data: data.0 }
    }

    pub fn get_mime(&self) -> String {
        self.mime.clone()
    }

    pub fn get_data(&self) -> Vec<u8> {
        self.data.clone()
    }
}

#[derive(Clone)]
#[php_class]
#[php(name = "Liter\\Llm\\DefaultClient")]
pub struct DefaultClient {
    inner: Arc<liter_llm::client::DefaultClient>,
}

#[php_impl]
impl DefaultClient {
    pub fn chat(&self, req: &ChatCompletionRequest) -> PhpResult<ChatCompletionResponse> {
        WORKER_RUNTIME
            .block_on(async { self.inner.chat(req.clone().into()).await })
            .map(ChatCompletionResponse::from)
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))
    }

    pub fn embed(&self, req: &EmbeddingRequest) -> PhpResult<EmbeddingResponse> {
        WORKER_RUNTIME
            .block_on(async { self.inner.embed(req.clone().into()).await })
            .map(EmbeddingResponse::from)
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))
    }

    pub fn list_models(&self) -> PhpResult<ModelsListResponse> {
        WORKER_RUNTIME
            .block_on(async { self.inner.list_models().await })
            .map(ModelsListResponse::from)
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))
    }

    pub fn image_generate(&self, req: &CreateImageRequest) -> PhpResult<ImagesResponse> {
        WORKER_RUNTIME
            .block_on(async { self.inner.image_generate(req.clone().into()).await })
            .map(ImagesResponse::from)
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))
    }

    pub fn speech(&self, req: &CreateSpeechRequest) -> PhpResult<Vec<u8>> {
        WORKER_RUNTIME
            .block_on(async { self.inner.speech(req.clone().into()).await })
            .map(|b| b.to_vec())
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))
    }

    pub fn transcribe(&self, req: &CreateTranscriptionRequest) -> PhpResult<TranscriptionResponse> {
        WORKER_RUNTIME
            .block_on(async { self.inner.transcribe(req.clone().into()).await })
            .map(TranscriptionResponse::from)
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))
    }

    pub fn moderate(&self, req: &ModerationRequest) -> PhpResult<ModerationResponse> {
        WORKER_RUNTIME
            .block_on(async { self.inner.moderate(req.clone().into()).await })
            .map(ModerationResponse::from)
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))
    }

    pub fn rerank(&self, req: &RerankRequest) -> PhpResult<RerankResponse> {
        WORKER_RUNTIME
            .block_on(async { self.inner.rerank(req.clone().into()).await })
            .map(RerankResponse::from)
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))
    }

    pub fn search(&self, req: &SearchRequest) -> PhpResult<SearchResponse> {
        WORKER_RUNTIME
            .block_on(async { self.inner.search(req.clone().into()).await })
            .map(SearchResponse::from)
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))
    }

    pub fn ocr(&self, req: &OcrRequest) -> PhpResult<OcrResponse> {
        WORKER_RUNTIME
            .block_on(async { self.inner.ocr(req.clone().into()).await })
            .map(OcrResponse::from)
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))
    }

    pub fn create_file(&self, req: &CreateFileRequest) -> PhpResult<FileObject> {
        WORKER_RUNTIME
            .block_on(async { self.inner.create_file(req.clone().into()).await })
            .map(FileObject::from)
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))
    }

    pub fn retrieve_file(&self, fileId: String) -> PhpResult<FileObject> {
        WORKER_RUNTIME
            .block_on(async { self.inner.retrieve_file(fileId.as_str()).await })
            .map(FileObject::from)
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))
    }

    pub fn delete_file(&self, fileId: String) -> PhpResult<DeleteResponse> {
        WORKER_RUNTIME
            .block_on(async { self.inner.delete_file(fileId.as_str()).await })
            .map(DeleteResponse::from)
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))
    }

    pub fn list_files(&self, query: Option<&FileListQuery>) -> PhpResult<FileListResponse> {
        WORKER_RUNTIME
            .block_on(async { self.inner.list_files(query.map(|v| v.clone().into())).await })
            .map(FileListResponse::from)
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))
    }

    pub fn file_content(&self, fileId: String) -> PhpResult<Vec<u8>> {
        WORKER_RUNTIME
            .block_on(async { self.inner.file_content(fileId.as_str()).await })
            .map(|b| b.to_vec())
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))
    }

    pub fn create_batch(&self, req: &CreateBatchRequest) -> PhpResult<BatchObject> {
        WORKER_RUNTIME
            .block_on(async { self.inner.create_batch(req.clone().into()).await })
            .map(BatchObject::from)
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))
    }

    pub fn retrieve_batch(&self, batchId: String) -> PhpResult<BatchObject> {
        WORKER_RUNTIME
            .block_on(async { self.inner.retrieve_batch(batchId.as_str()).await })
            .map(BatchObject::from)
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))
    }

    pub fn list_batches(&self, query: Option<&BatchListQuery>) -> PhpResult<BatchListResponse> {
        WORKER_RUNTIME
            .block_on(async { self.inner.list_batches(query.map(|v| v.clone().into())).await })
            .map(BatchListResponse::from)
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))
    }

    pub fn cancel_batch(&self, batchId: String) -> PhpResult<BatchObject> {
        WORKER_RUNTIME
            .block_on(async { self.inner.cancel_batch(batchId.as_str()).await })
            .map(BatchObject::from)
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))
    }

    pub fn fetch_batch_for_polling(&self, batchId: String) -> PhpResult<BatchObject> {
        let inner = self.inner.clone();
        WORKER_RUNTIME.block_on(async {
            let result = inner
                .fetch_batch_for_polling(&batchId)
                .await
                .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))?;
            Ok(result.into())
        })
    }

    /// Poll a batch until it reaches a terminal status (Completed, Failed, Expired, Cancelled).
    ///
    /// Uses exponential backoff with configurable initial interval, maximum interval, and backoff multiplier.
    /// Optionally supports a timeout that aborts polling if exceeded.
    ///
    /// # Errors
    ///
    /// Returns `BatchWaitError::Failed` if the batch reaches a failure terminal status.
    /// Returns `BatchWaitError::Timeout` if the configured timeout is exceeded.
    /// Returns `BatchWaitError::Client` for underlying client errors.
    ///
    /// # Example
    ///
    /// ```no_run
    /// # use liter_llm::client::{DefaultClient, ClientConfig, WaitForBatchConfig};
    /// # async fn run() -> Result<(), Box<dyn std::error::Error>> {
    /// let client = DefaultClient::new(ClientConfig::new("api-key"), None)?;
    /// let batch = client.wait_for_batch("b-123", WaitForBatchConfig::default()).await?;
    /// println!("Batch completed: {:?}", batch.status);
    /// # Ok(())
    /// # }
    /// ```
    pub fn wait_for_batch(&self, batchId: String, config: &WaitForBatchConfig) -> PhpResult<BatchObject> {
        let inner = self.inner.clone();
        WORKER_RUNTIME.block_on(async {
            let result = inner
                .wait_for_batch(&batchId, config.clone().into())
                .await
                .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))?;
            Ok(result.into())
        })
    }

    pub fn create_response(&self, req: &CreateResponseRequest) -> PhpResult<ResponseObject> {
        WORKER_RUNTIME
            .block_on(async { self.inner.create_response(req.clone().into()).await })
            .map(ResponseObject::from)
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))
    }

    pub fn retrieve_response(&self, responseId: String) -> PhpResult<ResponseObject> {
        WORKER_RUNTIME
            .block_on(async { self.inner.retrieve_response(responseId.as_str()).await })
            .map(ResponseObject::from)
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))
    }

    pub fn cancel_response(&self, responseId: String) -> PhpResult<ResponseObject> {
        WORKER_RUNTIME
            .block_on(async { self.inner.cancel_response(responseId.as_str()).await })
            .map(ResponseObject::from)
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))
    }

    #[php(name = "chatStream")]
    pub fn chat_stream(
        &self,
        req: &ChatCompletionRequest,
    ) -> std::result::Result<Vec<String>, ext_php_rs::exception::PhpException> {
        use futures_util::StreamExt;
        let rt =
            tokio::runtime::Runtime::new().map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))?;
        let core_req: liter_llm::ChatCompletionRequest = req.clone().into();
        rt.block_on(async {
            let stream = self
                .inner
                .chat_stream(core_req)
                .await
                .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))?;
            let chunks: Vec<String> = stream
                .collect::<Vec<_>>()
                .await
                .into_iter()
                .collect::<std::result::Result<Vec<_>, _>>()
                .map(|chunks| {
                    chunks
                        .into_iter()
                        .map(|c| serde_json::to_string(&c).unwrap_or_default())
                        .collect()
                })
                .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))?;
            Ok(chunks)
        })
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\DeleteResponse")]
#[serde(default)]
pub struct DeleteResponse {
    /// ID of the deleted resource.
    #[php(prop, name = "id")]
    pub id: String,
    /// Object type.
    #[php(prop, name = "object")]
    pub object: String,
    /// Confirmation that the resource was deleted.
    #[php(prop, name = "deleted")]
    pub deleted: bool,
}

impl Default for DeleteResponse {
    fn default() -> Self {
        <liter_llm::types::DeleteResponse as Default>::default().into()
    }
}

#[php_impl]
impl DeleteResponse {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(id: String, object: String, deleted: bool) -> Self {
        Self { id, object, deleted }
    }

    pub fn get_id(&self) -> String {
        self.id.clone()
    }

    pub fn get_object(&self) -> String {
        self.object.clone()
    }

    pub fn get_deleted(&self) -> bool {
        self.deleted.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\DeveloperMessage")]
#[serde(default)]
pub struct DeveloperMessage {
    /// Developer-specific instructions or context.
    #[php(prop, name = "content")]
    pub content: String,
    /// Optional name for the developer message source.
    #[php(prop, name = "name")]
    pub name: Option<String>,
}

impl Default for DeveloperMessage {
    fn default() -> Self {
        <liter_llm::types::DeveloperMessage as Default>::default().into()
    }
}

#[php_impl]
impl DeveloperMessage {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(content: String, name: Option<String>) -> Self {
        Self { content, name }
    }

    pub fn get_content(&self) -> String {
        self.content.clone()
    }

    pub fn get_name(&self) -> Option<String> {
        self.name.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\DocumentContent")]
#[serde(default)]
pub struct DocumentContent {
    /// Base64-encoded document data or URL.
    #[php(prop, name = "data")]
    pub data: String,
    /// MIME type (e.g., "application/pdf", "text/csv").
    #[php(prop, name = "mediaType")]
    #[serde(alias = "mediaType")]
    pub media_type: String,
}

impl Default for DocumentContent {
    fn default() -> Self {
        <liter_llm::types::DocumentContent as Default>::default().into()
    }
}

#[php_impl]
impl DocumentContent {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(data: String, mediaType: String) -> Self {
        Self {
            data,
            media_type: mediaType,
        }
    }

    pub fn get_data(&self) -> String {
        self.data.clone()
    }

    pub fn get_media_type(&self) -> String {
        self.media_type.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize, Default)]
#[php_class]
#[php(name = "Liter\\Llm\\EmbeddingObject")]
#[serde(default)]
pub struct EmbeddingObject {
    /// Always `"embedding"` from OpenAI-compatible APIs.  Stored as a plain
    /// `String` so non-standard provider values do not break deserialization.
    #[php(prop, name = "object")]
    pub object: String,
    /// The embedding vector.
    ///
    /// Providers may return this as a JSON float array or, when
    /// `encoding_format: "base64"` was requested, as a base64 string of
    /// little-endian `f32` bytes. Base64 responses are decoded on read; this
    /// field always serializes back out as a JSON float array.
    #[php(prop, name = "embedding")]
    pub embedding: Vec<f32>,
    /// Index in the batch (corresponds to input order).
    #[php(prop, name = "index")]
    pub index: u32,
}

#[php_impl]
impl EmbeddingObject {
    #[php(constructor)]
    pub fn new(object: String, embedding: Vec<f32>, index: u32) -> Self {
        Self {
            object,
            embedding,
            index,
        }
    }

    pub fn get_object(&self) -> String {
        self.object.clone()
    }

    pub fn get_embedding(&self) -> Vec<f32> {
        self.embedding.clone()
    }

    pub fn get_index(&self) -> u32 {
        self.index.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\EmbeddingRequest")]
#[serde(default)]
pub struct EmbeddingRequest {
    /// Model ID (e.g., `"text-embedding-3-small"`).
    #[php(prop, name = "model")]
    pub model: String,
    /// Text, texts, or multimodal content to embed.
    pub input: serde_json::Value,
    /// Output format: float (native) or base64.
    #[php(prop, name = "encodingFormat")]
    #[serde(skip_serializing_if = "Option::is_none")]
    #[serde(alias = "encodingFormat")]
    pub encoding_format: Option<String>,
    /// Requested embedding dimensions (if supported by the model).
    #[php(prop, name = "dimensions")]
    pub dimensions: Option<u32>,
    /// User identifier for request tracking.
    #[php(prop, name = "user")]
    pub user: Option<String>,
}

impl Default for EmbeddingRequest {
    fn default() -> Self {
        <liter_llm::types::EmbeddingRequest as Default>::default().into()
    }
}

#[php_impl]
impl EmbeddingRequest {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(model: String, encodingFormat: Option<String>, dimensions: Option<u32>, user: Option<String>) -> Self {
        Self {
            model,
            input: Default::default(),
            encoding_format: encodingFormat,
            dimensions,
            user,
        }
    }

    pub fn get_model(&self) -> String {
        self.model.clone()
    }

    pub fn get_input(&self) -> Option<String> {
        serde_json::to_string(&self.input).ok()
    }

    pub fn get_encoding_format(&self) -> Option<String> {
        self.encoding_format.clone().map(Into::into)
    }

    pub fn get_dimensions(&self) -> Option<u32> {
        self.dimensions.clone()
    }

    pub fn get_user(&self) -> Option<String> {
        self.user.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize, Default)]
#[php_class]
#[php(name = "Liter\\Llm\\EmbeddingResponse")]
#[serde(default)]
pub struct EmbeddingResponse {
    /// Always `"list"` from OpenAI-compatible APIs.  Stored as a plain
    /// `String` so non-standard provider values do not break deserialization.
    #[php(prop, name = "object")]
    pub object: String,
    /// List of embeddings.
    pub data: Vec<EmbeddingObject>,
    /// Model used to generate embeddings.
    #[php(prop, name = "model")]
    pub model: String,
    /// Token usage (input tokens only; embeddings have zero output tokens).
    pub usage: Option<Usage>,
}

#[php_impl]
impl EmbeddingResponse {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(
        object: String,
        data: &ext_php_rs::types::ZendHashTable,
        model: String,
        usage: Option<&Usage>,
    ) -> PhpResult<Self> {
        let mut data_core_result: Vec<EmbeddingObject> = Vec::new();
        for (_, item) in data.iter() {
            if let Some(parsed) = <&EmbeddingObject as ext_php_rs::convert::FromZval>::from_zval(item) {
                data_core_result.push(parsed.clone());
            } else {
                return Err(ext_php_rs::exception::PhpException::default(
                    "Failed to convert array element to EmbeddingObject".to_string(),
                ));
            }
        }
        let data_core: Vec<EmbeddingObject> = data_core_result;
        Ok(Self {
            object,
            data: data_core,
            model,
            usage: usage.cloned(),
        })
    }

    pub fn get_object(&self) -> String {
        self.object.clone()
    }

    pub fn get_data(&self) -> Vec<EmbeddingObject> {
        self.data.clone()
    }

    pub fn get_model(&self) -> String {
        self.model.clone()
    }

    pub fn get_usage(&self) -> Option<Usage> {
        self.usage.clone().map(Into::into)
    }

    pub fn set_usage(&mut self, value: Option<&Usage>) {
        self.usage = value.map(|value| value.clone().into());
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\FileListQuery")]
#[serde(default)]
pub struct FileListQuery {
    /// Filter by file purpose (e.g., `"batch"`, `"fine-tune"`).
    #[php(prop, name = "purpose")]
    pub purpose: Option<String>,
    /// Maximum number of results to return. Defaults to 20.
    #[php(prop, name = "limit")]
    pub limit: Option<u32>,
    /// Pagination cursor: return results after this file ID.
    #[php(prop, name = "after")]
    pub after: Option<String>,
}

impl Default for FileListQuery {
    fn default() -> Self {
        <liter_llm::types::FileListQuery as Default>::default().into()
    }
}

#[php_impl]
impl FileListQuery {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    pub fn get_purpose(&self) -> Option<String> {
        self.purpose.clone()
    }

    pub fn get_limit(&self) -> Option<u32> {
        self.limit.clone()
    }

    pub fn get_after(&self) -> Option<String> {
        self.after.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\FileListResponse")]
#[serde(default)]
pub struct FileListResponse {
    /// Object type (always `"list"`).
    #[php(prop, name = "object")]
    pub object: String,
    /// List of file objects.
    pub data: Vec<FileObject>,
    /// Whether more results are available.
    #[php(prop, name = "hasMore")]
    #[serde(alias = "hasMore")]
    pub has_more: Option<bool>,
}

impl Default for FileListResponse {
    fn default() -> Self {
        <liter_llm::types::FileListResponse as Default>::default().into()
    }
}

#[php_impl]
impl FileListResponse {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(object: String, data: &ext_php_rs::types::ZendHashTable, hasMore: Option<bool>) -> PhpResult<Self> {
        let mut data_core_result: Vec<FileObject> = Vec::new();
        for (_, item) in data.iter() {
            if let Some(parsed) = <&FileObject as ext_php_rs::convert::FromZval>::from_zval(item) {
                data_core_result.push(parsed.clone());
            } else {
                return Err(ext_php_rs::exception::PhpException::default(
                    "Failed to convert array element to FileObject".to_string(),
                ));
            }
        }
        let data_core: Vec<FileObject> = data_core_result;
        Ok(Self {
            object,
            data: data_core,
            has_more: hasMore,
        })
    }

    pub fn get_object(&self) -> String {
        self.object.clone()
    }

    pub fn get_data(&self) -> Vec<FileObject> {
        self.data.clone()
    }

    pub fn get_has_more(&self) -> Option<bool> {
        self.has_more.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\FileObject")]
#[serde(default)]
pub struct FileObject {
    /// Unique file ID.
    #[php(prop, name = "id")]
    pub id: String,
    /// Object type (always `"file"`).
    #[php(prop, name = "object")]
    pub object: String,
    /// File size in bytes.
    #[php(prop, name = "bytes")]
    pub bytes: i64,
    /// Unix timestamp of file creation.
    #[php(prop, name = "createdAt")]
    #[serde(alias = "createdAt")]
    pub created_at: i64,
    /// Filename.
    #[php(prop, name = "filename")]
    pub filename: String,
    /// File purpose.
    #[php(prop, name = "purpose")]
    pub purpose: String,
    /// Processing status (e.g., `"uploaded"`, `"processed"`).
    #[php(prop, name = "status")]
    pub status: Option<String>,
}

impl Default for FileObject {
    fn default() -> Self {
        <liter_llm::types::FileObject as Default>::default().into()
    }
}

#[php_impl]
impl FileObject {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(
        id: String,
        object: String,
        bytes: i64,
        createdAt: i64,
        filename: String,
        purpose: String,
        status: Option<String>,
    ) -> Self {
        Self {
            id,
            object,
            bytes,
            created_at: createdAt,
            filename,
            purpose,
            status,
        }
    }

    pub fn get_id(&self) -> String {
        self.id.clone()
    }

    pub fn get_object(&self) -> String {
        self.object.clone()
    }

    pub fn get_bytes(&self) -> i64 {
        self.bytes.clone()
    }

    pub fn get_created_at(&self) -> i64 {
        self.created_at.clone()
    }

    pub fn get_filename(&self) -> String {
        self.filename.clone()
    }

    pub fn get_purpose(&self) -> String {
        self.purpose.clone()
    }

    pub fn get_status(&self) -> Option<String> {
        self.status.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize, Default)]
#[php_class]
#[php(name = "Liter\\Llm\\FunctionCall")]
#[serde(default)]
pub struct FunctionCall {
    /// Function name.
    #[php(prop, name = "name")]
    pub name: String,
    /// Arguments as a JSON string (parse with serde_json.from_str).
    #[php(prop, name = "arguments")]
    pub arguments: String,
}

#[php_impl]
impl FunctionCall {
    #[php(constructor)]
    pub fn new(name: String, arguments: String) -> Self {
        Self { name, arguments }
    }

    pub fn get_name(&self) -> String {
        self.name.clone()
    }

    pub fn get_arguments(&self) -> String {
        self.arguments.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize, Default)]
#[php_class]
#[php(name = "Liter\\Llm\\FunctionDefinition")]
#[serde(default)]
pub struct FunctionDefinition {
    /// Name of the function. Required and must be alphanumeric + underscores.
    #[php(prop, name = "name")]
    pub name: String,
    /// Human-readable description explaining what the function does.
    #[php(prop, name = "description")]
    pub description: Option<String>,
    /// JSON Schema defining the function's parameters.
    pub parameters: Option<serde_json::Value>,
    /// If true, enforce strict JSON schema validation for arguments.
    #[php(prop, name = "strict")]
    pub strict: Option<bool>,
}

#[php_impl]
impl FunctionDefinition {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(
        name: String,
        description: Option<String>,
        parameters: Option<String>,
        strict: Option<bool>,
    ) -> PhpResult<Self> {
        Ok(Self {
            name,
            description,
            parameters: parameters
                .map(|s| serde_json::from_str(&s))
                .transpose()
                .map_err(|e| PhpException::default(e.to_string()))?,
            strict,
        })
    }

    pub fn get_name(&self) -> String {
        self.name.clone()
    }

    pub fn get_description(&self) -> Option<String> {
        self.description.clone()
    }

    pub fn get_parameters(&self) -> Option<String> {
        self.parameters.as_ref().and_then(|v| serde_json::to_string(v).ok())
    }

    pub fn get_strict(&self) -> Option<bool> {
        self.strict.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\FunctionMessage")]
#[serde(default)]
pub struct FunctionMessage {
    #[php(prop, name = "content")]
    pub content: String,
    #[php(prop, name = "name")]
    pub name: String,
}

impl Default for FunctionMessage {
    fn default() -> Self {
        <liter_llm::types::FunctionMessage as Default>::default().into()
    }
}

#[php_impl]
impl FunctionMessage {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(content: String, name: String) -> Self {
        Self { content, name }
    }

    pub fn get_content(&self) -> String {
        self.content.clone()
    }

    pub fn get_name(&self) -> String {
        self.name.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\Image")]
#[serde(default)]
pub struct Image {
    /// Image URL (if response_format was "url").
    #[php(prop, name = "url")]
    pub url: Option<String>,
    /// Base64-encoded image data (if response_format was "b64_json").
    #[php(prop, name = "b64Json")]
    #[serde(alias = "b64Json")]
    pub b64_json: Option<String>,
    /// The final prompt used to generate the image (DALL-E 3).
    #[php(prop, name = "revisedPrompt")]
    #[serde(alias = "revisedPrompt")]
    pub revised_prompt: Option<String>,
}

impl Default for Image {
    fn default() -> Self {
        <liter_llm::types::Image as Default>::default().into()
    }
}

#[php_impl]
impl Image {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    pub fn get_url(&self) -> Option<String> {
        self.url.clone()
    }

    pub fn get_b64_json(&self) -> Option<String> {
        self.b64_json.clone()
    }

    pub fn get_revised_prompt(&self) -> Option<String> {
        self.revised_prompt.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\ImageUrl")]
#[serde(default)]
pub struct ImageUrl {
    /// URL of the image (data URI or HTTP/HTTPS URL).
    #[php(prop, name = "url")]
    pub url: String,
    /// Detail level: low (512x512), high (2x2 tiles), or auto (model-selected).
    #[php(prop, name = "detail")]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub detail: Option<String>,
}

impl Default for ImageUrl {
    fn default() -> Self {
        <liter_llm::types::ImageUrl as Default>::default().into()
    }
}

#[php_impl]
impl ImageUrl {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(url: String, detail: Option<String>) -> Self {
        Self { url, detail }
    }

    pub fn get_url(&self) -> String {
        self.url.clone()
    }

    pub fn get_detail(&self) -> Option<String> {
        self.detail.clone().map(Into::into)
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\ImagesResponse")]
#[serde(default)]
pub struct ImagesResponse {
    /// Unix timestamp of image creation.
    #[php(prop, name = "created")]
    pub created: i64,
    /// List of generated images.
    pub data: Vec<Image>,
}

impl Default for ImagesResponse {
    fn default() -> Self {
        <liter_llm::types::ImagesResponse as Default>::default().into()
    }
}

#[php_impl]
impl ImagesResponse {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(created: i64, data: &ext_php_rs::types::ZendHashTable) -> PhpResult<Self> {
        let mut data_core_result: Vec<Image> = Vec::new();
        for (_, item) in data.iter() {
            if let Some(parsed) = <&Image as ext_php_rs::convert::FromZval>::from_zval(item) {
                data_core_result.push(parsed.clone());
            } else {
                return Err(ext_php_rs::exception::PhpException::default(
                    "Failed to convert array element to Image".to_string(),
                ));
            }
        }
        let data_core: Vec<Image> = data_core_result;
        Ok(Self {
            created,
            data: data_core,
        })
    }

    pub fn get_created(&self) -> i64 {
        self.created.clone()
    }

    pub fn get_data(&self) -> Vec<Image> {
        self.data.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\InFlightLimitConfig")]
#[serde(default)]
pub struct InFlightLimitConfig {
    /// Maximum simultaneously outstanding provider requests. `None` means unlimited.
    #[php(prop, name = "maxInFlight")]
    #[serde(alias = "maxInFlight")]
    pub max_in_flight: Option<i64>,
}

impl Default for InFlightLimitConfig {
    fn default() -> Self {
        <liter_llm::InFlightLimitConfig as Default>::default().into()
    }
}

#[php_impl]
impl InFlightLimitConfig {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    pub fn get_max_in_flight(&self) -> Option<i64> {
        self.max_in_flight.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize, Default)]
#[php_class]
#[php(name = "Liter\\Llm\\IntentPrototype")]
#[serde(default)]
pub struct IntentPrototype {
    /// Human-readable name for the intent (used in logs/metrics).
    #[php(prop, name = "name")]
    pub name: String,
    /// Pre-computed embedding vector for this intent.
    #[php(prop, name = "embedding")]
    pub embedding: Vec<f64>,
    /// Model to route to when this intent is detected.
    #[php(prop, name = "model")]
    pub model: String,
}

#[php_impl]
impl IntentPrototype {
    #[php(constructor)]
    pub fn new(name: String, embedding: Vec<f64>, model: String) -> Self {
        Self { name, embedding, model }
    }

    pub fn get_name(&self) -> String {
        self.name.clone()
    }

    pub fn get_embedding(&self) -> Vec<f64> {
        self.embedding.clone()
    }

    pub fn get_model(&self) -> String {
        self.model.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\JsonSchemaFormat")]
#[serde(default)]
pub struct JsonSchemaFormat {
    /// Name of the schema (must be unique in the request).
    #[php(prop, name = "name")]
    pub name: String,
    /// Description of what the schema represents.
    #[php(prop, name = "description")]
    pub description: Option<String>,
    /// JSON Schema object defining the output structure.
    pub schema: serde_json::Value,
    /// If true, enforce strict schema validation.
    #[php(prop, name = "strict")]
    pub strict: Option<bool>,
}

impl Default for JsonSchemaFormat {
    fn default() -> Self {
        <liter_llm::types::JsonSchemaFormat as Default>::default().into()
    }
}

#[php_impl]
impl JsonSchemaFormat {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(name: String, schema: String, description: Option<String>, strict: Option<bool>) -> PhpResult<Self> {
        Ok(Self {
            name,
            description,
            schema: serde_json::from_str(&schema).map_err(|e| PhpException::default(e.to_string()))?,
            strict,
        })
    }

    pub fn get_name(&self) -> String {
        self.name.clone()
    }

    pub fn get_description(&self) -> Option<String> {
        self.description.clone()
    }

    pub fn get_schema(&self) -> Option<String> {
        serde_json::to_string(&self.schema).ok()
    }

    pub fn get_strict(&self) -> Option<bool> {
        self.strict.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\LlmBudgetConfig")]
#[serde(default)]
pub struct LlmBudgetConfig {
    /// Global spend limit in USD.
    #[php(prop, name = "globalLimit")]
    #[serde(alias = "globalLimit")]
    pub global_limit: Option<f64>,
    /// Per-model spend limits in USD, keyed by model name.
    #[php(prop, name = "modelLimits")]
    #[serde(alias = "modelLimits")]
    pub model_limits: Option<HashMap<String, f64>>,
    /// Enforcement mode: `"hard"` (reject over-budget requests) or `"soft"` (log only).
    #[php(prop, name = "enforcement")]
    pub enforcement: Option<String>,
}

impl Default for LlmBudgetConfig {
    fn default() -> Self {
        <liter_llm::client::LlmBudgetConfig as Default>::default().into()
    }
}

#[php_impl]
impl LlmBudgetConfig {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    pub fn get_global_limit(&self) -> Option<f64> {
        self.global_limit.clone()
    }

    pub fn get_model_limits(&self) -> Option<HashMap<String, f64>> {
        self.model_limits.clone()
    }

    pub fn get_enforcement(&self) -> Option<String> {
        self.enforcement.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\LlmCacheConfig")]
#[serde(default)]
pub struct LlmCacheConfig {
    /// Maximum number of cached entries.
    #[php(prop, name = "maxEntries")]
    #[serde(alias = "maxEntries")]
    pub max_entries: Option<i64>,
    /// Cache entry time-to-live, in seconds.
    #[php(prop, name = "ttlSeconds")]
    #[serde(alias = "ttlSeconds")]
    pub ttl_seconds: Option<i64>,
    /// Cache backend name (e.g. `"memory"`, or an `opendal` scheme).
    #[php(prop, name = "backend")]
    pub backend: Option<String>,
    /// Backend-specific configuration key/value pairs.
    #[php(prop, name = "backendConfig")]
    #[serde(alias = "backendConfig")]
    pub backend_config: Option<HashMap<String, String>>,
}

impl Default for LlmCacheConfig {
    fn default() -> Self {
        <liter_llm::client::LlmCacheConfig as Default>::default().into()
    }
}

#[php_impl]
impl LlmCacheConfig {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    pub fn get_max_entries(&self) -> Option<i64> {
        self.max_entries.clone()
    }

    pub fn get_ttl_seconds(&self) -> Option<i64> {
        self.ttl_seconds.clone()
    }

    pub fn get_backend(&self) -> Option<String> {
        self.backend.clone()
    }

    pub fn get_backend_config(&self) -> Option<HashMap<String, String>> {
        self.backend_config.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\LlmConfig")]
#[serde(default)]
pub struct LlmConfig {
    /// Model identifier (e.g. `"gpt-4o"`, `"bedrock/anthropic.claude-3-sonnet-20240229-v1:0"`).
    #[php(prop, name = "model")]
    pub model: String,
    /// API key for authentication.
    #[php(prop, name = "apiKey")]
    #[serde(alias = "apiKey")]
    pub api_key: Option<String>,
    /// Override base URL. When set, all requests go here and provider
    /// auto-detection is skipped.
    #[php(prop, name = "baseUrl")]
    #[serde(alias = "baseUrl")]
    pub base_url: Option<String>,
    /// Request timeout, in seconds.
    #[php(prop, name = "timeoutSecs")]
    #[serde(alias = "timeoutSecs")]
    pub timeout_secs: Option<i64>,
    /// Maximum number of retries on 429 / 5xx responses.
    #[php(prop, name = "maxRetries")]
    #[serde(alias = "maxRetries")]
    pub max_retries: Option<u32>,
    /// Sampling temperature for requests built from this config.
    #[php(prop, name = "temperature")]
    pub temperature: Option<f64>,
    /// Maximum number of tokens to generate for requests built from this config.
    #[php(prop, name = "maxTokens")]
    #[serde(alias = "maxTokens")]
    pub max_tokens: Option<i64>,
    /// Automatically load the API key from the provider's environment variable
    /// when no explicit key is provided (default: `true`).
    #[php(prop, name = "loadEnv")]
    #[serde(alias = "loadEnv")]
    pub load_env: Option<bool>,
    /// Extra headers sent on every request.
    #[php(prop, name = "headers")]
    pub headers: Option<HashMap<String, String>>,
    /// Custom provider configurations, in addition to the built-in providers.
    pub providers: Option<Vec<LlmProviderConfig>>,
    /// Response cache configuration.
    pub cache: Option<LlmCacheConfig>,
    /// Budget enforcement configuration.
    pub budget: Option<LlmBudgetConfig>,
    /// Per-model rate limiting configuration.
    #[serde(alias = "rateLimit")]
    pub rate_limit: Option<LlmRateLimitConfig>,
    /// Global per-client in-flight provider request limit.
    #[serde(alias = "inFlightLimit")]
    pub in_flight_limit: Option<LlmInFlightLimitConfig>,
    /// Enable per-request cost tracking.
    #[php(prop, name = "costTracking")]
    #[serde(alias = "costTracking")]
    pub cost_tracking: Option<bool>,
    /// Enable OpenTelemetry-compatible tracing spans.
    #[php(prop, name = "tracing")]
    pub tracing: Option<bool>,
    /// Cooldown duration after transient errors, in seconds.
    #[php(prop, name = "cooldownSecs")]
    #[serde(alias = "cooldownSecs")]
    pub cooldown_secs: Option<i64>,
    /// Background health check interval, in seconds.
    #[php(prop, name = "healthCheckSecs")]
    #[serde(alias = "healthCheckSecs")]
    pub health_check_secs: Option<i64>,
    /// AWS Bedrock configuration (region, credentials, cross-region routing).
    pub bedrock: Option<BedrockConfig>,
}

impl Default for LlmConfig {
    fn default() -> Self {
        <liter_llm::client::LlmConfig as Default>::default().into()
    }
}

#[php_impl]
impl LlmConfig {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(
        model: String,
        apiKey: Option<String>,
        baseUrl: Option<String>,
        timeoutSecs: Option<i64>,
        maxRetries: Option<u32>,
        temperature: Option<f64>,
        maxTokens: Option<i64>,
        loadEnv: Option<bool>,
        headers: Option<HashMap<String, String>>,
        providers: Option<&ext_php_rs::types::ZendHashTable>,
        cache: Option<&LlmCacheConfig>,
        budget: Option<&LlmBudgetConfig>,
        rateLimit: Option<&LlmRateLimitConfig>,
        inFlightLimit: Option<&LlmInFlightLimitConfig>,
        costTracking: Option<bool>,
        tracing: Option<bool>,
        cooldownSecs: Option<i64>,
        healthCheckSecs: Option<i64>,
        bedrock: Option<&BedrockConfig>,
    ) -> PhpResult<Self> {
        let providers_core: Option<Vec<LlmProviderConfig>> = if let Some(ht) = providers {
            let mut result = Vec::new();
            for (_, item) in ht.iter() {
                if let Some(parsed) = <&LlmProviderConfig as ext_php_rs::convert::FromZval>::from_zval(item) {
                    result.push(parsed.clone());
                } else {
                    return Err(ext_php_rs::exception::PhpException::default(
                        "Failed to convert array element to LlmProviderConfig".to_string(),
                    ));
                }
            }
            Some(result)
        } else {
            None
        };
        Ok(Self {
            model,
            api_key: apiKey,
            base_url: baseUrl,
            timeout_secs: timeoutSecs,
            max_retries: maxRetries,
            temperature,
            max_tokens: maxTokens,
            load_env: loadEnv,
            headers,
            providers: providers_core,
            cache: cache.cloned(),
            budget: budget.cloned(),
            rate_limit: rateLimit.cloned(),
            in_flight_limit: inFlightLimit.cloned(),
            cost_tracking: costTracking,
            tracing,
            cooldown_secs: cooldownSecs,
            health_check_secs: healthCheckSecs,
            bedrock: bedrock.cloned(),
        })
    }

    pub fn get_model(&self) -> String {
        self.model.clone()
    }

    pub fn get_api_key(&self) -> Option<String> {
        self.api_key.clone()
    }

    pub fn get_base_url(&self) -> Option<String> {
        self.base_url.clone()
    }

    pub fn get_timeout_secs(&self) -> Option<i64> {
        self.timeout_secs.clone()
    }

    pub fn get_max_retries(&self) -> Option<u32> {
        self.max_retries.clone()
    }

    pub fn get_temperature(&self) -> Option<f64> {
        self.temperature.clone()
    }

    pub fn get_max_tokens(&self) -> Option<i64> {
        self.max_tokens.clone()
    }

    pub fn get_load_env(&self) -> Option<bool> {
        self.load_env.clone()
    }

    pub fn get_headers(&self) -> Option<HashMap<String, String>> {
        self.headers.clone()
    }

    pub fn get_providers(&self) -> Option<Vec<LlmProviderConfig>> {
        self.providers.clone()
    }

    pub fn get_cache(&self) -> Option<LlmCacheConfig> {
        self.cache.clone().map(Into::into)
    }

    pub fn set_cache(&mut self, value: Option<&LlmCacheConfig>) {
        self.cache = value.map(|value| value.clone().into());
    }

    pub fn get_budget(&self) -> Option<LlmBudgetConfig> {
        self.budget.clone().map(Into::into)
    }

    pub fn set_budget(&mut self, value: Option<&LlmBudgetConfig>) {
        self.budget = value.map(|value| value.clone().into());
    }

    pub fn get_rate_limit(&self) -> Option<LlmRateLimitConfig> {
        self.rate_limit.clone().map(Into::into)
    }

    pub fn set_rate_limit(&mut self, value: Option<&LlmRateLimitConfig>) {
        self.rate_limit = value.map(|value| value.clone().into());
    }

    pub fn get_in_flight_limit(&self) -> Option<LlmInFlightLimitConfig> {
        self.in_flight_limit.clone().map(Into::into)
    }

    pub fn set_in_flight_limit(&mut self, value: Option<&LlmInFlightLimitConfig>) {
        self.in_flight_limit = value.map(|value| value.clone().into());
    }

    pub fn get_cost_tracking(&self) -> Option<bool> {
        self.cost_tracking.clone()
    }

    pub fn get_tracing(&self) -> Option<bool> {
        self.tracing.clone()
    }

    pub fn get_cooldown_secs(&self) -> Option<i64> {
        self.cooldown_secs.clone()
    }

    pub fn get_health_check_secs(&self) -> Option<i64> {
        self.health_check_secs.clone()
    }

    pub fn get_bedrock(&self) -> Option<BedrockConfig> {
        self.bedrock.clone().map(Into::into)
    }

    pub fn set_bedrock(&mut self, value: Option<&BedrockConfig>) {
        self.bedrock = value.map(|value| value.clone().into());
    }

    pub fn providers(&self) -> Vec<LlmProviderConfig> {
        #[allow(clippy::needless_update)]
        let core_self = liter_llm::client::LlmConfig {
            model: self.model.clone(),
            api_key: self.api_key.clone(),
            base_url: self.base_url.clone(),
            timeout_secs: self.timeout_secs.map(|v| v as u64),
            max_retries: self.max_retries,
            temperature: self.temperature,
            max_tokens: self.max_tokens.map(|v| v as u64),
            load_env: self.load_env,
            headers: self.headers.clone().map(|m| m.into_iter().collect()),
            providers: self.providers.clone().map(|v| v.into_iter().map(Into::into).collect()),
            cache: self.cache.clone().map(Into::into),
            budget: self.budget.clone().map(Into::into),
            rate_limit: self.rate_limit.clone().map(Into::into),
            in_flight_limit: self.in_flight_limit.clone().map(Into::into),
            cost_tracking: self.cost_tracking,
            tracing: self.tracing,
            cooldown_secs: self.cooldown_secs.map(|v| v as u64),
            health_check_secs: self.health_check_secs.map(|v| v as u64),
            bedrock: self.bedrock.clone().map(Into::into),
            ..Default::default()
        };
        core_self.providers().iter().cloned().map(Into::into).collect()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\LlmInFlightLimitConfig")]
#[serde(default)]
pub struct LlmInFlightLimitConfig {
    /// Maximum simultaneously outstanding provider requests. `None` means unlimited.
    #[php(prop, name = "maxInFlight")]
    #[serde(alias = "maxInFlight")]
    pub max_in_flight: Option<i64>,
}

impl Default for LlmInFlightLimitConfig {
    fn default() -> Self {
        <liter_llm::client::LlmInFlightLimitConfig as Default>::default().into()
    }
}

#[php_impl]
impl LlmInFlightLimitConfig {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    pub fn get_max_in_flight(&self) -> Option<i64> {
        self.max_in_flight.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\LlmProviderConfig")]
#[serde(default)]
pub struct LlmProviderConfig {
    /// Provider name, used to key model prefix matching.
    #[php(prop, name = "name")]
    pub name: String,
    /// Base URL for the provider's OpenAI-compatible API.
    #[php(prop, name = "baseUrl")]
    #[serde(alias = "baseUrl")]
    pub base_url: String,
    /// Header name used to carry the API key (defaults to `Authorization` when unset).
    #[php(prop, name = "authHeader")]
    #[serde(alias = "authHeader")]
    pub auth_header: Option<String>,
    /// Model name prefixes routed to this provider (e.g. `["my-provider/"]`).
    #[php(prop, name = "modelPrefixes")]
    #[serde(alias = "modelPrefixes")]
    pub model_prefixes: Vec<String>,
}

impl Default for LlmProviderConfig {
    fn default() -> Self {
        <liter_llm::client::LlmProviderConfig as Default>::default().into()
    }
}

#[php_impl]
impl LlmProviderConfig {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(name: String, baseUrl: String, modelPrefixes: Vec<String>, authHeader: Option<String>) -> Self {
        Self {
            name,
            base_url: baseUrl,
            auth_header: authHeader,
            model_prefixes: modelPrefixes,
        }
    }

    pub fn get_name(&self) -> String {
        self.name.clone()
    }

    pub fn get_base_url(&self) -> String {
        self.base_url.clone()
    }

    pub fn get_auth_header(&self) -> Option<String> {
        self.auth_header.clone()
    }

    pub fn get_model_prefixes(&self) -> Vec<String> {
        self.model_prefixes.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\LlmRateLimitConfig")]
#[serde(default)]
#[allow(clippy::similar_names)]
pub struct LlmRateLimitConfig {
    /// Requests per minute limit.
    #[php(prop, name = "rpm")]
    pub rpm: Option<u32>,
    /// Tokens per minute limit.
    #[php(prop, name = "tpm")]
    pub tpm: Option<i64>,
    /// Rate limit window, in seconds.
    #[php(prop, name = "windowSeconds")]
    #[serde(alias = "windowSeconds")]
    pub window_seconds: Option<i64>,
}

impl Default for LlmRateLimitConfig {
    fn default() -> Self {
        <liter_llm::client::LlmRateLimitConfig as Default>::default().into()
    }
}

#[php_impl]
impl LlmRateLimitConfig {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    pub fn get_rpm(&self) -> Option<u32> {
        self.rpm.clone()
    }

    pub fn get_tpm(&self) -> Option<i64> {
        self.tpm.clone()
    }

    pub fn get_window_seconds(&self) -> Option<i64> {
        self.window_seconds.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\ModelInfo")]
#[serde(default)]
pub struct ModelInfo {
    /// Cost in USD per input (prompt) token.
    #[php(prop, name = "inputCostPerToken")]
    #[serde(alias = "inputCostPerToken")]
    pub input_cost_per_token: f64,
    /// Cost in USD per output (completion) token.
    #[php(prop, name = "outputCostPerToken")]
    #[serde(alias = "outputCostPerToken")]
    pub output_cost_per_token: f64,
    /// Cost in USD per cached input token (cache hit / read).
    #[php(prop, name = "cacheReadInputTokenCost")]
    #[serde(alias = "cacheReadInputTokenCost")]
    pub cache_read_input_token_cost: Option<f64>,
    /// Cost in USD per token written to the prompt cache.
    #[php(prop, name = "cacheCreationInputTokenCost")]
    #[serde(alias = "cacheCreationInputTokenCost")]
    pub cache_creation_input_token_cost: Option<f64>,
    /// Cost in USD per input audio token.
    #[php(prop, name = "inputCostPerAudioToken")]
    #[serde(alias = "inputCostPerAudioToken")]
    pub input_cost_per_audio_token: Option<f64>,
    /// Cost in USD per output audio token.
    #[php(prop, name = "outputCostPerAudioToken")]
    #[serde(alias = "outputCostPerAudioToken")]
    pub output_cost_per_audio_token: Option<f64>,
    /// Cost in USD per reasoning (extended-thinking) output token.
    #[php(prop, name = "outputCostPerReasoningToken")]
    #[serde(alias = "outputCostPerReasoningToken")]
    pub output_cost_per_reasoning_token: Option<f64>,
    /// Total context window size in tokens (input + output).
    #[php(prop, name = "maxTokens")]
    #[serde(alias = "maxTokens")]
    pub max_tokens: Option<i64>,
    /// Maximum input (prompt) tokens accepted.
    #[php(prop, name = "maxInputTokens")]
    #[serde(alias = "maxInputTokens")]
    pub max_input_tokens: Option<i64>,
    /// Maximum output (completion) tokens the model can generate.
    #[php(prop, name = "maxOutputTokens")]
    #[serde(alias = "maxOutputTokens")]
    pub max_output_tokens: Option<i64>,
    /// Best-effort operating mode, e.g. `"chat"`, `"embedding"`.
    #[php(prop, name = "mode")]
    pub mode: Option<String>,
    /// The model accepts image input.
    #[php(prop, name = "supportsVision")]
    #[serde(alias = "supportsVision")]
    pub supports_vision: Option<bool>,
    /// The model supports tool / function calling.
    #[php(prop, name = "supportsFunctionCalling")]
    #[serde(alias = "supportsFunctionCalling")]
    pub supports_function_calling: Option<bool>,
    /// The model supports extended-thinking / reasoning tokens.
    #[php(prop, name = "supportsReasoning")]
    #[serde(alias = "supportsReasoning")]
    pub supports_reasoning: Option<bool>,
    /// The model supports JSON-mode or `response_format` structured output.
    #[php(prop, name = "supportsStructuredOutput")]
    #[serde(alias = "supportsStructuredOutput")]
    pub supports_structured_output: Option<bool>,
    /// The model accepts audio input.
    #[php(prop, name = "supportsAudioInput")]
    #[serde(alias = "supportsAudioInput")]
    pub supports_audio_input: Option<bool>,
    /// The model can generate audio output.
    #[php(prop, name = "supportsAudioOutput")]
    #[serde(alias = "supportsAudioOutput")]
    pub supports_audio_output: Option<bool>,
    /// The model supports prompt caching.
    #[php(prop, name = "supportsPromptCaching")]
    #[serde(alias = "supportsPromptCaching")]
    pub supports_prompt_caching: Option<bool>,
    /// Context-tiered pricing overrides, sorted by ascending
    /// `min_context_tokens`. Empty when the model has flat pricing.
    pub tiers: Vec<ModelTier>,
}

impl Default for ModelInfo {
    fn default() -> Self {
        <liter_llm::ModelInfo as Default>::default().into()
    }
}

#[php_impl]
impl ModelInfo {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(
        inputCostPerToken: f64,
        outputCostPerToken: f64,
        tiers: &ext_php_rs::types::ZendHashTable,
        cacheReadInputTokenCost: Option<f64>,
        cacheCreationInputTokenCost: Option<f64>,
        inputCostPerAudioToken: Option<f64>,
        outputCostPerAudioToken: Option<f64>,
        outputCostPerReasoningToken: Option<f64>,
        maxTokens: Option<i64>,
        maxInputTokens: Option<i64>,
        maxOutputTokens: Option<i64>,
        mode: Option<String>,
        supportsVision: Option<bool>,
        supportsFunctionCalling: Option<bool>,
        supportsReasoning: Option<bool>,
        supportsStructuredOutput: Option<bool>,
        supportsAudioInput: Option<bool>,
        supportsAudioOutput: Option<bool>,
        supportsPromptCaching: Option<bool>,
    ) -> PhpResult<Self> {
        let mut tiers_core_result: Vec<ModelTier> = Vec::new();
        for (_, item) in tiers.iter() {
            if let Some(parsed) = <&ModelTier as ext_php_rs::convert::FromZval>::from_zval(item) {
                tiers_core_result.push(parsed.clone());
            } else {
                return Err(ext_php_rs::exception::PhpException::default(
                    "Failed to convert array element to ModelTier".to_string(),
                ));
            }
        }
        let tiers_core: Vec<ModelTier> = tiers_core_result;
        Ok(Self {
            input_cost_per_token: inputCostPerToken,
            output_cost_per_token: outputCostPerToken,
            cache_read_input_token_cost: cacheReadInputTokenCost,
            cache_creation_input_token_cost: cacheCreationInputTokenCost,
            input_cost_per_audio_token: inputCostPerAudioToken,
            output_cost_per_audio_token: outputCostPerAudioToken,
            output_cost_per_reasoning_token: outputCostPerReasoningToken,
            max_tokens: maxTokens,
            max_input_tokens: maxInputTokens,
            max_output_tokens: maxOutputTokens,
            mode,
            supports_vision: supportsVision,
            supports_function_calling: supportsFunctionCalling,
            supports_reasoning: supportsReasoning,
            supports_structured_output: supportsStructuredOutput,
            supports_audio_input: supportsAudioInput,
            supports_audio_output: supportsAudioOutput,
            supports_prompt_caching: supportsPromptCaching,
            tiers: tiers_core,
        })
    }

    pub fn get_input_cost_per_token(&self) -> f64 {
        self.input_cost_per_token.clone()
    }

    pub fn get_output_cost_per_token(&self) -> f64 {
        self.output_cost_per_token.clone()
    }

    pub fn get_cache_read_input_token_cost(&self) -> Option<f64> {
        self.cache_read_input_token_cost.clone()
    }

    pub fn get_cache_creation_input_token_cost(&self) -> Option<f64> {
        self.cache_creation_input_token_cost.clone()
    }

    pub fn get_input_cost_per_audio_token(&self) -> Option<f64> {
        self.input_cost_per_audio_token.clone()
    }

    pub fn get_output_cost_per_audio_token(&self) -> Option<f64> {
        self.output_cost_per_audio_token.clone()
    }

    pub fn get_output_cost_per_reasoning_token(&self) -> Option<f64> {
        self.output_cost_per_reasoning_token.clone()
    }

    pub fn get_max_tokens(&self) -> Option<i64> {
        self.max_tokens.clone()
    }

    pub fn get_max_input_tokens(&self) -> Option<i64> {
        self.max_input_tokens.clone()
    }

    pub fn get_max_output_tokens(&self) -> Option<i64> {
        self.max_output_tokens.clone()
    }

    pub fn get_mode(&self) -> Option<String> {
        self.mode.clone()
    }

    pub fn get_supports_vision(&self) -> Option<bool> {
        self.supports_vision.clone()
    }

    pub fn get_supports_function_calling(&self) -> Option<bool> {
        self.supports_function_calling.clone()
    }

    pub fn get_supports_reasoning(&self) -> Option<bool> {
        self.supports_reasoning.clone()
    }

    pub fn get_supports_structured_output(&self) -> Option<bool> {
        self.supports_structured_output.clone()
    }

    pub fn get_supports_audio_input(&self) -> Option<bool> {
        self.supports_audio_input.clone()
    }

    pub fn get_supports_audio_output(&self) -> Option<bool> {
        self.supports_audio_output.clone()
    }

    pub fn get_supports_prompt_caching(&self) -> Option<bool> {
        self.supports_prompt_caching.clone()
    }

    pub fn get_tiers(&self) -> Vec<ModelTier> {
        self.tiers.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\ModelObject")]
#[serde(default)]
pub struct ModelObject {
    /// Model ID (e.g., `"gpt-4o"`, `"claude-3-5-sonnet"`).
    #[php(prop, name = "id")]
    pub id: String,
    /// Always `"model"` from OpenAI-compatible APIs.  Stored as a plain
    /// `String` so non-standard provider values do not break deserialization.
    /// Defaults to empty when a provider omits the field.
    #[php(prop, name = "object")]
    pub object: String,
    /// Unix timestamp of model creation (or release date).
    ///
    /// Defaults to `0` when a provider omits it — DeepSeek and some other
    /// OpenAI-compatible providers do not return `created` from `/v1/models`.
    #[php(prop, name = "created")]
    pub created: i64,
    /// Organization or entity that owns the model.
    /// Defaults to empty when a provider omits the field.
    #[php(prop, name = "ownedBy")]
    #[serde(alias = "ownedBy")]
    pub owned_by: String,
}

impl Default for ModelObject {
    fn default() -> Self {
        <liter_llm::types::ModelObject as Default>::default().into()
    }
}

#[php_impl]
impl ModelObject {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(id: String, object: String, created: i64, ownedBy: String) -> Self {
        Self {
            id,
            object,
            created,
            owned_by: ownedBy,
        }
    }

    pub fn get_id(&self) -> String {
        self.id.clone()
    }

    pub fn get_object(&self) -> String {
        self.object.clone()
    }

    pub fn get_created(&self) -> i64 {
        self.created.clone()
    }

    pub fn get_owned_by(&self) -> String {
        self.owned_by.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\ModelTier")]
#[serde(default)]
pub struct ModelTier {
    /// The tier applies when the prompt/context token count is at least
    /// this value.
    #[php(prop, name = "minContextTokens")]
    #[serde(alias = "minContextTokens")]
    pub min_context_tokens: i64,
    /// Cost in USD per input (prompt) token within this tier.
    #[php(prop, name = "inputCostPerToken")]
    #[serde(alias = "inputCostPerToken")]
    pub input_cost_per_token: f64,
    /// Cost in USD per output (completion) token within this tier.
    #[php(prop, name = "outputCostPerToken")]
    #[serde(alias = "outputCostPerToken")]
    pub output_cost_per_token: f64,
    /// Cost in USD per cached input token within this tier.
    #[php(prop, name = "cacheReadInputTokenCost")]
    #[serde(alias = "cacheReadInputTokenCost")]
    pub cache_read_input_token_cost: Option<f64>,
    /// Cost in USD per cache-write token within this tier.
    #[php(prop, name = "cacheCreationInputTokenCost")]
    #[serde(alias = "cacheCreationInputTokenCost")]
    pub cache_creation_input_token_cost: Option<f64>,
    /// Cost in USD per input audio token within this tier.
    #[php(prop, name = "inputCostPerAudioToken")]
    #[serde(alias = "inputCostPerAudioToken")]
    pub input_cost_per_audio_token: Option<f64>,
    /// Cost in USD per output audio token within this tier.
    #[php(prop, name = "outputCostPerAudioToken")]
    #[serde(alias = "outputCostPerAudioToken")]
    pub output_cost_per_audio_token: Option<f64>,
    /// Cost in USD per reasoning output token within this tier.
    #[php(prop, name = "outputCostPerReasoningToken")]
    #[serde(alias = "outputCostPerReasoningToken")]
    pub output_cost_per_reasoning_token: Option<f64>,
}

impl Default for ModelTier {
    fn default() -> Self {
        <liter_llm::ModelTier as Default>::default().into()
    }
}

#[php_impl]
impl ModelTier {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(
        minContextTokens: i64,
        inputCostPerToken: f64,
        outputCostPerToken: f64,
        cacheReadInputTokenCost: Option<f64>,
        cacheCreationInputTokenCost: Option<f64>,
        inputCostPerAudioToken: Option<f64>,
        outputCostPerAudioToken: Option<f64>,
        outputCostPerReasoningToken: Option<f64>,
    ) -> Self {
        Self {
            min_context_tokens: minContextTokens,
            input_cost_per_token: inputCostPerToken,
            output_cost_per_token: outputCostPerToken,
            cache_read_input_token_cost: cacheReadInputTokenCost,
            cache_creation_input_token_cost: cacheCreationInputTokenCost,
            input_cost_per_audio_token: inputCostPerAudioToken,
            output_cost_per_audio_token: outputCostPerAudioToken,
            output_cost_per_reasoning_token: outputCostPerReasoningToken,
        }
    }

    pub fn get_min_context_tokens(&self) -> i64 {
        self.min_context_tokens.clone()
    }

    pub fn get_input_cost_per_token(&self) -> f64 {
        self.input_cost_per_token.clone()
    }

    pub fn get_output_cost_per_token(&self) -> f64 {
        self.output_cost_per_token.clone()
    }

    pub fn get_cache_read_input_token_cost(&self) -> Option<f64> {
        self.cache_read_input_token_cost.clone()
    }

    pub fn get_cache_creation_input_token_cost(&self) -> Option<f64> {
        self.cache_creation_input_token_cost.clone()
    }

    pub fn get_input_cost_per_audio_token(&self) -> Option<f64> {
        self.input_cost_per_audio_token.clone()
    }

    pub fn get_output_cost_per_audio_token(&self) -> Option<f64> {
        self.output_cost_per_audio_token.clone()
    }

    pub fn get_output_cost_per_reasoning_token(&self) -> Option<f64> {
        self.output_cost_per_reasoning_token.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\ModelsListResponse")]
#[serde(default)]
pub struct ModelsListResponse {
    /// Always `"list"` from OpenAI-compatible APIs.  Stored as a plain
    /// `String` so non-standard provider values do not break deserialization.
    /// Defaults to empty when a provider omits the field.
    #[php(prop, name = "object")]
    pub object: String,
    /// List of available models.
    pub data: Vec<ModelObject>,
}

impl Default for ModelsListResponse {
    fn default() -> Self {
        <liter_llm::types::ModelsListResponse as Default>::default().into()
    }
}

#[php_impl]
impl ModelsListResponse {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(object: String, data: &ext_php_rs::types::ZendHashTable) -> PhpResult<Self> {
        let mut data_core_result: Vec<ModelObject> = Vec::new();
        for (_, item) in data.iter() {
            if let Some(parsed) = <&ModelObject as ext_php_rs::convert::FromZval>::from_zval(item) {
                data_core_result.push(parsed.clone());
            } else {
                return Err(ext_php_rs::exception::PhpException::default(
                    "Failed to convert array element to ModelObject".to_string(),
                ));
            }
        }
        let data_core: Vec<ModelObject> = data_core_result;
        Ok(Self {
            object,
            data: data_core,
        })
    }

    pub fn get_object(&self) -> String {
        self.object.clone()
    }

    pub fn get_data(&self) -> Vec<ModelObject> {
        self.data.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\ModerationCategories")]
#[serde(default)]
pub struct ModerationCategories {
    /// Sexual content.
    #[php(prop, name = "sexual")]
    pub sexual: bool,
    /// Hate speech.
    #[php(prop, name = "hate")]
    pub hate: bool,
    /// Harassment.
    #[php(prop, name = "harassment")]
    pub harassment: bool,
    /// Self-harm content.
    #[php(prop, name = "selfHarm")]
    #[serde(alias = "selfHarm")]
    pub self_harm: bool,
    /// Sexual content involving minors.
    #[php(prop, name = "sexualMinors")]
    #[serde(alias = "sexualMinors")]
    pub sexual_minors: bool,
    /// Hate speech that threatens violence.
    #[php(prop, name = "hateThreatening")]
    #[serde(alias = "hateThreatening")]
    pub hate_threatening: bool,
    /// Graphic violence.
    #[php(prop, name = "violenceGraphic")]
    #[serde(alias = "violenceGraphic")]
    pub violence_graphic: bool,
    /// Intent to self-harm.
    #[php(prop, name = "selfHarmIntent")]
    #[serde(alias = "selfHarmIntent")]
    pub self_harm_intent: bool,
    /// Instructions for self-harm.
    #[php(prop, name = "selfHarmInstructions")]
    #[serde(alias = "selfHarmInstructions")]
    pub self_harm_instructions: bool,
    /// Harassment that threatens violence.
    #[php(prop, name = "harassmentThreatening")]
    #[serde(alias = "harassmentThreatening")]
    pub harassment_threatening: bool,
    /// Non-graphic violence.
    #[php(prop, name = "violence")]
    pub violence: bool,
}

impl Default for ModerationCategories {
    fn default() -> Self {
        <liter_llm::types::ModerationCategories as Default>::default().into()
    }
}

#[php_impl]
impl ModerationCategories {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(
        sexual: bool,
        hate: bool,
        harassment: bool,
        selfHarm: bool,
        sexualMinors: bool,
        hateThreatening: bool,
        violenceGraphic: bool,
        selfHarmIntent: bool,
        selfHarmInstructions: bool,
        harassmentThreatening: bool,
        violence: bool,
    ) -> Self {
        Self {
            sexual,
            hate,
            harassment,
            self_harm: selfHarm,
            sexual_minors: sexualMinors,
            hate_threatening: hateThreatening,
            violence_graphic: violenceGraphic,
            self_harm_intent: selfHarmIntent,
            self_harm_instructions: selfHarmInstructions,
            harassment_threatening: harassmentThreatening,
            violence,
        }
    }

    pub fn get_sexual(&self) -> bool {
        self.sexual.clone()
    }

    pub fn get_hate(&self) -> bool {
        self.hate.clone()
    }

    pub fn get_harassment(&self) -> bool {
        self.harassment.clone()
    }

    pub fn get_self_harm(&self) -> bool {
        self.self_harm.clone()
    }

    pub fn get_sexual_minors(&self) -> bool {
        self.sexual_minors.clone()
    }

    pub fn get_hate_threatening(&self) -> bool {
        self.hate_threatening.clone()
    }

    pub fn get_violence_graphic(&self) -> bool {
        self.violence_graphic.clone()
    }

    pub fn get_self_harm_intent(&self) -> bool {
        self.self_harm_intent.clone()
    }

    pub fn get_self_harm_instructions(&self) -> bool {
        self.self_harm_instructions.clone()
    }

    pub fn get_harassment_threatening(&self) -> bool {
        self.harassment_threatening.clone()
    }

    pub fn get_violence(&self) -> bool {
        self.violence.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\ModerationCategoryScores")]
#[serde(default)]
pub struct ModerationCategoryScores {
    /// Sexual content score.
    #[php(prop, name = "sexual")]
    pub sexual: f64,
    /// Hate speech score.
    #[php(prop, name = "hate")]
    pub hate: f64,
    /// Harassment score.
    #[php(prop, name = "harassment")]
    pub harassment: f64,
    /// Self-harm content score.
    #[php(prop, name = "selfHarm")]
    #[serde(alias = "selfHarm")]
    pub self_harm: f64,
    /// Sexual content involving minors score.
    #[php(prop, name = "sexualMinors")]
    #[serde(alias = "sexualMinors")]
    pub sexual_minors: f64,
    /// Hate speech that threatens violence score.
    #[php(prop, name = "hateThreatening")]
    #[serde(alias = "hateThreatening")]
    pub hate_threatening: f64,
    /// Graphic violence score.
    #[php(prop, name = "violenceGraphic")]
    #[serde(alias = "violenceGraphic")]
    pub violence_graphic: f64,
    /// Intent to self-harm score.
    #[php(prop, name = "selfHarmIntent")]
    #[serde(alias = "selfHarmIntent")]
    pub self_harm_intent: f64,
    /// Instructions for self-harm score.
    #[php(prop, name = "selfHarmInstructions")]
    #[serde(alias = "selfHarmInstructions")]
    pub self_harm_instructions: f64,
    /// Harassment that threatens violence score.
    #[php(prop, name = "harassmentThreatening")]
    #[serde(alias = "harassmentThreatening")]
    pub harassment_threatening: f64,
    /// Non-graphic violence score.
    #[php(prop, name = "violence")]
    pub violence: f64,
}

impl Default for ModerationCategoryScores {
    fn default() -> Self {
        <liter_llm::types::ModerationCategoryScores as Default>::default().into()
    }
}

#[php_impl]
impl ModerationCategoryScores {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(
        sexual: f64,
        hate: f64,
        harassment: f64,
        selfHarm: f64,
        sexualMinors: f64,
        hateThreatening: f64,
        violenceGraphic: f64,
        selfHarmIntent: f64,
        selfHarmInstructions: f64,
        harassmentThreatening: f64,
        violence: f64,
    ) -> Self {
        Self {
            sexual,
            hate,
            harassment,
            self_harm: selfHarm,
            sexual_minors: sexualMinors,
            hate_threatening: hateThreatening,
            violence_graphic: violenceGraphic,
            self_harm_intent: selfHarmIntent,
            self_harm_instructions: selfHarmInstructions,
            harassment_threatening: harassmentThreatening,
            violence,
        }
    }

    pub fn get_sexual(&self) -> f64 {
        self.sexual.clone()
    }

    pub fn get_hate(&self) -> f64 {
        self.hate.clone()
    }

    pub fn get_harassment(&self) -> f64 {
        self.harassment.clone()
    }

    pub fn get_self_harm(&self) -> f64 {
        self.self_harm.clone()
    }

    pub fn get_sexual_minors(&self) -> f64 {
        self.sexual_minors.clone()
    }

    pub fn get_hate_threatening(&self) -> f64 {
        self.hate_threatening.clone()
    }

    pub fn get_violence_graphic(&self) -> f64 {
        self.violence_graphic.clone()
    }

    pub fn get_self_harm_intent(&self) -> f64 {
        self.self_harm_intent.clone()
    }

    pub fn get_self_harm_instructions(&self) -> f64 {
        self.self_harm_instructions.clone()
    }

    pub fn get_harassment_threatening(&self) -> f64 {
        self.harassment_threatening.clone()
    }

    pub fn get_violence(&self) -> f64 {
        self.violence.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\ModerationRequest")]
#[serde(default)]
pub struct ModerationRequest {
    /// Text or texts to check.
    pub input: serde_json::Value,
    /// Model ID (e.g., `"text-moderation-latest"`). Optional; API uses default if unset.
    #[php(prop, name = "model")]
    pub model: Option<String>,
}

impl Default for ModerationRequest {
    fn default() -> Self {
        <liter_llm::types::ModerationRequest as Default>::default().into()
    }
}

#[php_impl]
impl ModerationRequest {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    pub fn get_input(&self) -> Option<String> {
        serde_json::to_string(&self.input).ok()
    }

    pub fn get_model(&self) -> Option<String> {
        self.model.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize, Default)]
#[php_class]
#[php(name = "Liter\\Llm\\ModerationResponse")]
#[serde(default)]
pub struct ModerationResponse {
    /// Unique identifier for this moderation request.
    #[php(prop, name = "id")]
    pub id: String,
    /// Model used for classification.
    #[php(prop, name = "model")]
    pub model: String,
    /// Results for each input string.
    pub results: Vec<ModerationResult>,
}

#[php_impl]
impl ModerationResponse {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(id: String, model: String, results: &ext_php_rs::types::ZendHashTable) -> PhpResult<Self> {
        let mut results_core_result: Vec<ModerationResult> = Vec::new();
        for (_, item) in results.iter() {
            if let Some(parsed) = <&ModerationResult as ext_php_rs::convert::FromZval>::from_zval(item) {
                results_core_result.push(parsed.clone());
            } else {
                return Err(ext_php_rs::exception::PhpException::default(
                    "Failed to convert array element to ModerationResult".to_string(),
                ));
            }
        }
        let results_core: Vec<ModerationResult> = results_core_result;
        Ok(Self {
            id,
            model,
            results: results_core,
        })
    }

    pub fn get_id(&self) -> String {
        self.id.clone()
    }

    pub fn get_model(&self) -> String {
        self.model.clone()
    }

    pub fn get_results(&self) -> Vec<ModerationResult> {
        self.results.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize, Default)]
#[php_class]
#[php(name = "Liter\\Llm\\ModerationResult")]
#[serde(default)]
pub struct ModerationResult {
    /// True if any category was flagged.
    #[php(prop, name = "flagged")]
    pub flagged: bool,
    /// Boolean flags for each moderation category.
    pub categories: ModerationCategories,
    /// Confidence scores for each category.
    #[serde(alias = "categoryScores")]
    pub category_scores: ModerationCategoryScores,
}

#[php_impl]
impl ModerationResult {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(flagged: bool, categories: &ModerationCategories, categoryScores: &ModerationCategoryScores) -> Self {
        Self {
            flagged,
            categories: categories.clone(),
            category_scores: categoryScores.clone(),
        }
    }

    pub fn get_flagged(&self) -> bool {
        self.flagged.clone()
    }

    pub fn get_categories(&self) -> ModerationCategories {
        self.categories.clone()
    }

    pub fn get_category_scores(&self) -> ModerationCategoryScores {
        self.category_scores.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize, Default)]
#[php_class]
#[php(name = "Liter\\Llm\\OcrImage")]
#[serde(default)]
pub struct OcrImage {
    /// Unique image identifier within the document.
    #[php(prop, name = "id")]
    pub id: String,
    /// Base64-encoded image data (if `include_image_base64` was true).
    #[php(prop, name = "imageBase64")]
    #[serde(alias = "imageBase64")]
    pub image_base64: Option<String>,
}

#[php_impl]
impl OcrImage {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(id: String, imageBase64: Option<String>) -> Self {
        Self {
            id,
            image_base64: imageBase64,
        }
    }

    pub fn get_id(&self) -> String {
        self.id.clone()
    }

    pub fn get_image_base64(&self) -> Option<String> {
        self.image_base64.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize, Default)]
#[php_class]
#[php(name = "Liter\\Llm\\OcrPage")]
#[serde(default)]
pub struct OcrPage {
    /// Page index (0-based).
    #[php(prop, name = "index")]
    pub index: u32,
    /// Extracted page content as Markdown.
    #[php(prop, name = "markdown")]
    pub markdown: String,
    /// Embedded images extracted from the page (if `include_image_base64` was true).
    pub images: Option<Vec<OcrImage>>,
    /// Page dimensions in pixels, if available.
    pub dimensions: Option<PageDimensions>,
}

#[php_impl]
impl OcrPage {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(
        index: u32,
        markdown: String,
        images: Option<&ext_php_rs::types::ZendHashTable>,
        dimensions: Option<&PageDimensions>,
    ) -> PhpResult<Self> {
        let images_core: Option<Vec<OcrImage>> = if let Some(ht) = images {
            let mut result = Vec::new();
            for (_, item) in ht.iter() {
                if let Some(parsed) = <&OcrImage as ext_php_rs::convert::FromZval>::from_zval(item) {
                    result.push(parsed.clone());
                } else {
                    return Err(ext_php_rs::exception::PhpException::default(
                        "Failed to convert array element to OcrImage".to_string(),
                    ));
                }
            }
            Some(result)
        } else {
            None
        };
        Ok(Self {
            index,
            markdown,
            images: images_core,
            dimensions: dimensions.cloned(),
        })
    }

    pub fn get_index(&self) -> u32 {
        self.index.clone()
    }

    pub fn get_markdown(&self) -> String {
        self.markdown.clone()
    }

    pub fn get_images(&self) -> Option<Vec<OcrImage>> {
        self.images.clone()
    }

    pub fn get_dimensions(&self) -> Option<PageDimensions> {
        self.dimensions.clone().map(Into::into)
    }

    pub fn set_dimensions(&mut self, value: Option<&PageDimensions>) {
        self.dimensions = value.map(|value| value.clone().into());
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\OcrRequest")]
#[serde(default)]
pub struct OcrRequest {
    /// The model/provider to use (e.g. `"mistral/mistral-ocr-latest"`).
    #[php(prop, name = "model")]
    pub model: String,
    /// The document to process (URL or base64).
    pub document: OcrDocument,
    /// Specific pages to process (1-indexed). `None` means all pages.
    #[php(prop, name = "pages")]
    pub pages: Option<Vec<u32>>,
    /// Whether to include base64-encoded images of each processed page.
    #[php(prop, name = "includeImageBase64")]
    #[serde(alias = "includeImageBase64")]
    pub include_image_base64: Option<bool>,
}

impl Default for OcrRequest {
    fn default() -> Self {
        <liter_llm::types::OcrRequest as Default>::default().into()
    }
}

#[php_impl]
impl OcrRequest {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(
        model: String,
        document: &OcrDocument,
        pages: Option<Vec<u32>>,
        includeImageBase64: Option<bool>,
    ) -> Self {
        Self {
            model,
            document: document.clone(),
            pages,
            include_image_base64: includeImageBase64,
        }
    }

    pub fn get_model(&self) -> String {
        self.model.clone()
    }

    pub fn get_document(&self) -> OcrDocument {
        self.document.clone()
    }

    pub fn get_pages(&self) -> Option<Vec<u32>> {
        self.pages.clone()
    }

    pub fn get_include_image_base64(&self) -> Option<bool> {
        self.include_image_base64.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize, Default)]
#[php_class]
#[php(name = "Liter\\Llm\\OcrResponse")]
#[serde(default)]
pub struct OcrResponse {
    /// Extracted pages in order.
    pub pages: Vec<OcrPage>,
    /// Model/provider used for OCR.
    #[php(prop, name = "model")]
    pub model: String,
    /// Token usage, if reported by the provider.
    pub usage: Option<Usage>,
}

#[php_impl]
impl OcrResponse {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(pages: &ext_php_rs::types::ZendHashTable, model: String, usage: Option<&Usage>) -> PhpResult<Self> {
        let mut pages_core_result: Vec<OcrPage> = Vec::new();
        for (_, item) in pages.iter() {
            if let Some(parsed) = <&OcrPage as ext_php_rs::convert::FromZval>::from_zval(item) {
                pages_core_result.push(parsed.clone());
            } else {
                return Err(ext_php_rs::exception::PhpException::default(
                    "Failed to convert array element to OcrPage".to_string(),
                ));
            }
        }
        let pages_core: Vec<OcrPage> = pages_core_result;
        Ok(Self {
            pages: pages_core,
            model,
            usage: usage.cloned(),
        })
    }

    pub fn get_pages(&self) -> Vec<OcrPage> {
        self.pages.clone()
    }

    pub fn get_model(&self) -> String {
        self.model.clone()
    }

    pub fn get_usage(&self) -> Option<Usage> {
        self.usage.clone().map(Into::into)
    }

    pub fn set_usage(&mut self, value: Option<&Usage>) {
        self.usage = value.map(|value| value.clone().into());
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize, Default)]
#[php_class]
#[php(name = "Liter\\Llm\\PageDimensions")]
#[serde(default)]
pub struct PageDimensions {
    /// Width in pixels.
    #[php(prop, name = "width")]
    pub width: u32,
    /// Height in pixels.
    #[php(prop, name = "height")]
    pub height: u32,
}

#[php_impl]
impl PageDimensions {
    #[php(constructor)]
    pub fn new(width: u32, height: u32) -> Self {
        Self { width, height }
    }

    pub fn get_width(&self) -> u32 {
        self.width.clone()
    }

    pub fn get_height(&self) -> u32 {
        self.height.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\PromptTokensDetails")]
#[serde(default)]
pub struct PromptTokensDetails {
    /// Cached tokens present in the prompt. Defaults to 0 when absent.
    #[php(prop, name = "cachedTokens")]
    #[serde(alias = "cachedTokens")]
    pub cached_tokens: i64,
    /// Audio input tokens present in the prompt. Defaults to 0 when absent.
    #[php(prop, name = "audioTokens")]
    #[serde(alias = "audioTokens")]
    pub audio_tokens: i64,
}

impl Default for PromptTokensDetails {
    fn default() -> Self {
        <liter_llm::types::PromptTokensDetails as Default>::default().into()
    }
}

#[php_impl]
impl PromptTokensDetails {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(cachedTokens: i64, audioTokens: i64) -> Self {
        Self {
            cached_tokens: cachedTokens,
            audio_tokens: audioTokens,
        }
    }

    pub fn get_cached_tokens(&self) -> i64 {
        self.cached_tokens.clone()
    }

    pub fn get_audio_tokens(&self) -> i64 {
        self.audio_tokens.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\ProviderCapabilities")]
#[serde(default)]
pub struct ProviderCapabilities {
    /// The provider accepts image input in chat messages.
    #[php(prop, name = "vision")]
    pub vision: bool,
    /// The provider supports extended-thinking / reasoning tokens.
    #[php(prop, name = "reasoning")]
    pub reasoning: bool,
    /// The provider supports JSON-mode or `response_format` structured output.
    #[php(prop, name = "structuredOutput")]
    #[serde(alias = "structuredOutput")]
    pub structured_output: bool,
    /// The provider supports tool / function calling.
    #[php(prop, name = "functionCalling")]
    #[serde(alias = "functionCalling")]
    pub function_calling: bool,
    /// The provider accepts audio as input.
    #[php(prop, name = "audioIn")]
    #[serde(alias = "audioIn")]
    pub audio_in: bool,
    /// The provider can generate audio / TTS output.
    #[php(prop, name = "audioOut")]
    #[serde(alias = "audioOut")]
    pub audio_out: bool,
    /// The provider accepts video as input.
    #[php(prop, name = "videoIn")]
    #[serde(alias = "videoIn")]
    pub video_in: bool,
}

impl Default for ProviderCapabilities {
    fn default() -> Self {
        <liter_llm::provider::ProviderCapabilities as Default>::default().into()
    }
}

#[php_impl]
impl ProviderCapabilities {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(
        vision: bool,
        reasoning: bool,
        structuredOutput: bool,
        functionCalling: bool,
        audioIn: bool,
        audioOut: bool,
        videoIn: bool,
    ) -> Self {
        Self {
            vision,
            reasoning,
            structured_output: structuredOutput,
            function_calling: functionCalling,
            audio_in: audioIn,
            audio_out: audioOut,
            video_in: videoIn,
        }
    }

    pub fn get_vision(&self) -> bool {
        self.vision.clone()
    }

    pub fn get_reasoning(&self) -> bool {
        self.reasoning.clone()
    }

    pub fn get_structured_output(&self) -> bool {
        self.structured_output.clone()
    }

    pub fn get_function_calling(&self) -> bool {
        self.function_calling.clone()
    }

    pub fn get_audio_in(&self) -> bool {
        self.audio_in.clone()
    }

    pub fn get_audio_out(&self) -> bool {
        self.audio_out.clone()
    }

    pub fn get_video_in(&self) -> bool {
        self.video_in.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize, Default)]
#[php_class]
#[php(name = "Liter\\Llm\\ProviderConfig")]
#[serde(default)]
pub struct ProviderConfig {
    /// Provider identifier (matches the entry key in providers.json).
    #[php(prop, name = "name")]
    pub name: String,
    /// Human-readable provider name shown in UIs.
    #[php(prop, name = "displayName")]
    #[serde(alias = "displayName")]
    pub display_name: Option<String>,
    /// Base URL used as the default for this provider's HTTP client.
    #[php(prop, name = "baseUrl")]
    #[serde(alias = "baseUrl")]
    pub base_url: Option<String>,
    /// Authentication scheme metadata (auth type + env var holding the key).
    pub auth: Option<AuthConfig>,
    /// Supported endpoint kinds (e.g. `chat`, `embeddings`).
    #[php(prop, name = "endpoints")]
    pub endpoints: Option<Vec<String>>,
    /// Model-name prefixes claimed by this provider (e.g. `["gpt-", "o1-"]`).
    #[php(prop, name = "modelPrefixes")]
    #[serde(alias = "modelPrefixes")]
    pub model_prefixes: Option<Vec<String>>,
    /// Parameter key renaming for this provider.
    ///
    /// Each entry maps an OpenAI-spec field name (e.g. `"max_completion_tokens"`)
    /// to the name this provider expects (e.g. `"max_tokens"`).  Applied
    /// automatically by `ConfigDrivenProvider.transform_request`.
    #[php(prop, name = "paramMappings")]
    #[serde(alias = "paramMappings")]
    pub param_mappings: Option<HashMap<String, String>>,
}

#[php_impl]
impl ProviderConfig {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(
        name: String,
        displayName: Option<String>,
        baseUrl: Option<String>,
        auth: Option<&AuthConfig>,
        endpoints: Option<Vec<String>>,
        modelPrefixes: Option<Vec<String>>,
        paramMappings: Option<HashMap<String, String>>,
    ) -> Self {
        Self {
            name,
            display_name: displayName,
            base_url: baseUrl,
            auth: auth.cloned(),
            endpoints,
            model_prefixes: modelPrefixes,
            param_mappings: paramMappings,
        }
    }

    pub fn get_name(&self) -> String {
        self.name.clone()
    }

    pub fn get_display_name(&self) -> Option<String> {
        self.display_name.clone()
    }

    pub fn get_base_url(&self) -> Option<String> {
        self.base_url.clone()
    }

    pub fn get_auth(&self) -> Option<AuthConfig> {
        self.auth.clone().map(Into::into)
    }

    pub fn set_auth(&mut self, value: Option<&AuthConfig>) {
        self.auth = value.map(|value| value.clone().into());
    }

    pub fn get_endpoints(&self) -> Option<Vec<String>> {
        self.endpoints.clone()
    }

    pub fn get_model_prefixes(&self) -> Option<Vec<String>> {
        self.model_prefixes.clone()
    }

    pub fn get_param_mappings(&self) -> Option<HashMap<String, String>> {
        self.param_mappings.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\RateLimitConfig")]
#[serde(default)]
#[allow(clippy::similar_names)]
pub struct RateLimitConfig {
    /// Maximum requests per window.  `None` means unlimited.
    #[php(prop, name = "rpm")]
    pub rpm: Option<u32>,
    /// Maximum tokens per window.  `None` means unlimited.
    #[php(prop, name = "tpm")]
    pub tpm: Option<i64>,
    /// Fixed window duration (defaults to 60 s).
    #[php(prop, name = "window")]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub window: Option<i64>,
}

impl Default for RateLimitConfig {
    fn default() -> Self {
        <liter_llm::RateLimitConfig as Default>::default().into()
    }
}

#[php_impl]
impl RateLimitConfig {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(rpm: Option<u32>, tpm: Option<i64>, window: Option<i64>) -> Self {
        Self { rpm, tpm, window }
    }

    pub fn get_rpm(&self) -> Option<u32> {
        self.rpm.clone()
    }

    pub fn get_tpm(&self) -> Option<i64> {
        self.tpm.clone()
    }

    pub fn get_window(&self) -> Option<i64> {
        self.window.clone()
    }

    #[allow(clippy::should_implement_trait)]
    #[php(name = "default")]
    pub fn default() -> RateLimitConfig {
        liter_llm::RateLimitConfig::default().into()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\RerankRequest")]
#[serde(default)]
pub struct RerankRequest {
    /// Model ID (e.g., `"cohere/rerank-english-v3.0"`).
    #[php(prop, name = "model")]
    pub model: String,
    /// The search query.
    #[php(prop, name = "query")]
    pub query: String,
    /// Documents to rerank.
    pub documents: Vec<serde_json::Value>,
    /// Return only the top N results. Optional.
    #[php(prop, name = "topN")]
    #[serde(alias = "topN")]
    pub top_n: Option<u32>,
    /// Include the document content in results. Defaults to false.
    #[php(prop, name = "returnDocuments")]
    #[serde(alias = "returnDocuments")]
    pub return_documents: Option<bool>,
}

impl Default for RerankRequest {
    fn default() -> Self {
        <liter_llm::types::RerankRequest as Default>::default().into()
    }
}

#[php_impl]
impl RerankRequest {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(model: String, query: String, topN: Option<u32>, returnDocuments: Option<bool>) -> Self {
        Self {
            model,
            query,
            documents: Default::default(),
            top_n: topN,
            return_documents: returnDocuments,
        }
    }

    pub fn get_model(&self) -> String {
        self.model.clone()
    }

    pub fn get_query(&self) -> String {
        self.query.clone()
    }

    pub fn get_documents(&self) -> Option<String> {
        serde_json::to_string(&self.documents).ok()
    }

    pub fn get_top_n(&self) -> Option<u32> {
        self.top_n.clone()
    }

    pub fn get_return_documents(&self) -> Option<bool> {
        self.return_documents.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize, Default)]
#[php_class]
#[php(name = "Liter\\Llm\\RerankResponse")]
#[serde(default)]
pub struct RerankResponse {
    /// Unique identifier for this rerank request.
    #[php(prop, name = "id")]
    pub id: Option<String>,
    /// Reranked documents in order of relevance.
    pub results: Vec<RerankResult>,
    /// Optional metadata about the reranking operation.
    pub meta: Option<serde_json::Value>,
}

#[php_impl]
impl RerankResponse {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(
        results: &ext_php_rs::types::ZendHashTable,
        id: Option<String>,
        meta: Option<String>,
    ) -> PhpResult<Self> {
        let mut results_core_result: Vec<RerankResult> = Vec::new();
        for (_, item) in results.iter() {
            if let Some(parsed) = <&RerankResult as ext_php_rs::convert::FromZval>::from_zval(item) {
                results_core_result.push(parsed.clone());
            } else {
                return Err(ext_php_rs::exception::PhpException::default(
                    "Failed to convert array element to RerankResult".to_string(),
                ));
            }
        }
        let results_core: Vec<RerankResult> = results_core_result;
        Ok(Self {
            id,
            results: results_core,
            meta: meta
                .map(|s| serde_json::from_str(&s))
                .transpose()
                .map_err(|e| PhpException::default(e.to_string()))?,
        })
    }

    pub fn get_id(&self) -> Option<String> {
        self.id.clone()
    }

    pub fn get_results(&self) -> Vec<RerankResult> {
        self.results.clone()
    }

    pub fn get_meta(&self) -> Option<String> {
        self.meta.as_ref().and_then(|v| serde_json::to_string(v).ok())
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize, Default)]
#[php_class]
#[php(name = "Liter\\Llm\\RerankResult")]
#[serde(default)]
pub struct RerankResult {
    /// Original document index in the input list.
    #[php(prop, name = "index")]
    pub index: u32,
    /// Relevance score in `[0, 1]`. Higher indicates more relevant.
    #[php(prop, name = "relevanceScore")]
    #[serde(alias = "relevanceScore")]
    pub relevance_score: f64,
    /// Original document content (if `return_documents` was true).
    pub document: Option<RerankResultDocument>,
}

#[php_impl]
impl RerankResult {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(index: u32, relevanceScore: f64, document: Option<&RerankResultDocument>) -> Self {
        Self {
            index,
            relevance_score: relevanceScore,
            document: document.cloned(),
        }
    }

    pub fn get_index(&self) -> u32 {
        self.index.clone()
    }

    pub fn get_relevance_score(&self) -> f64 {
        self.relevance_score.clone()
    }

    pub fn get_document(&self) -> Option<RerankResultDocument> {
        self.document.clone().map(Into::into)
    }

    pub fn set_document(&mut self, value: Option<&RerankResultDocument>) {
        self.document = value.map(|value| value.clone().into());
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize, Default)]
#[php_class]
#[php(name = "Liter\\Llm\\RerankResultDocument")]
#[serde(default)]
pub struct RerankResultDocument {
    /// Document text.
    #[php(prop, name = "text")]
    pub text: String,
}

#[php_impl]
impl RerankResultDocument {
    #[php(constructor)]
    pub fn new(text: String) -> Self {
        Self { text }
    }

    pub fn get_text(&self) -> String {
        self.text.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\ResponseObject")]
#[serde(default)]
pub struct ResponseObject {
    /// Unique response ID.
    #[php(prop, name = "id")]
    pub id: String,
    /// Object type (e.g., `"response"`).
    #[php(prop, name = "object")]
    pub object: String,
    /// Unix timestamp of response creation.
    #[php(prop, name = "createdAt")]
    #[serde(alias = "createdAt")]
    pub created_at: i64,
    /// Model used to generate the response.
    #[php(prop, name = "model")]
    pub model: String,
    /// Status (e.g., `"succeeded"`, `"failed"`).
    #[php(prop, name = "status")]
    pub status: String,
    /// Output items from the response.
    pub output: Vec<ResponseOutputItem>,
    /// Token usage.
    pub usage: Option<ResponseUsage>,
    /// Error details (if status is "failed").
    pub error: Option<serde_json::Value>,
}

impl Default for ResponseObject {
    fn default() -> Self {
        <liter_llm::types::ResponseObject as Default>::default().into()
    }
}

#[php_impl]
impl ResponseObject {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(
        id: String,
        object: String,
        createdAt: i64,
        model: String,
        status: String,
        output: &ext_php_rs::types::ZendHashTable,
        usage: Option<&ResponseUsage>,
        error: Option<String>,
    ) -> PhpResult<Self> {
        let mut output_core_result: Vec<ResponseOutputItem> = Vec::new();
        for (_, item) in output.iter() {
            if let Some(parsed) = <&ResponseOutputItem as ext_php_rs::convert::FromZval>::from_zval(item) {
                output_core_result.push(parsed.clone());
            } else {
                return Err(ext_php_rs::exception::PhpException::default(
                    "Failed to convert array element to ResponseOutputItem".to_string(),
                ));
            }
        }
        let output_core: Vec<ResponseOutputItem> = output_core_result;
        Ok(Self {
            id,
            object,
            created_at: createdAt,
            model,
            status,
            output: output_core,
            usage: usage.cloned(),
            error: error
                .map(|s| serde_json::from_str(&s))
                .transpose()
                .map_err(|e| PhpException::default(e.to_string()))?,
        })
    }

    pub fn get_id(&self) -> String {
        self.id.clone()
    }

    pub fn get_object(&self) -> String {
        self.object.clone()
    }

    pub fn get_created_at(&self) -> i64 {
        self.created_at.clone()
    }

    pub fn get_model(&self) -> String {
        self.model.clone()
    }

    pub fn get_status(&self) -> String {
        self.status.clone()
    }

    pub fn get_output(&self) -> Vec<ResponseOutputItem> {
        self.output.clone()
    }

    pub fn get_usage(&self) -> Option<ResponseUsage> {
        self.usage.clone().map(Into::into)
    }

    pub fn set_usage(&mut self, value: Option<&ResponseUsage>) {
        self.usage = value.map(|value| value.clone().into());
    }

    pub fn get_error(&self) -> Option<String> {
        self.error.as_ref().and_then(|v| serde_json::to_string(v).ok())
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\ResponseOutputItem")]
#[serde(default)]
pub struct ResponseOutputItem {
    /// Output type (e.g., `"text"`, `"object"`, `"error"`).
    #[php(prop, name = "itemType")]
    #[serde(alias = "itemType")]
    pub item_type: String,
    /// Output content (flattened into the object).
    pub content: serde_json::Value,
}

impl Default for ResponseOutputItem {
    fn default() -> Self {
        <liter_llm::types::ResponseOutputItem as Default>::default().into()
    }
}

#[php_impl]
impl ResponseOutputItem {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(itemType: String, content: String) -> PhpResult<Self> {
        Ok(Self {
            item_type: itemType,
            content: serde_json::from_str(&content).map_err(|e| PhpException::default(e.to_string()))?,
        })
    }

    pub fn get_item_type(&self) -> String {
        self.item_type.clone()
    }

    pub fn get_content(&self) -> Option<String> {
        serde_json::to_string(&self.content).ok()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\ResponseTool")]
#[serde(default)]
pub struct ResponseTool {
    /// Tool type (e.g., "extractor", "search").
    #[php(prop, name = "toolType")]
    #[serde(alias = "toolType")]
    pub tool_type: String,
    /// Tool configuration (flattened into the object).
    pub config: serde_json::Value,
}

impl Default for ResponseTool {
    fn default() -> Self {
        <liter_llm::types::ResponseTool as Default>::default().into()
    }
}

#[php_impl]
impl ResponseTool {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(toolType: String, config: String) -> PhpResult<Self> {
        Ok(Self {
            tool_type: toolType,
            config: serde_json::from_str(&config).map_err(|e| PhpException::default(e.to_string()))?,
        })
    }

    pub fn get_tool_type(&self) -> String {
        self.tool_type.clone()
    }

    pub fn get_config(&self) -> Option<String> {
        serde_json::to_string(&self.config).ok()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\ResponseUsage")]
#[serde(default)]
pub struct ResponseUsage {
    /// Input tokens used.
    #[php(prop, name = "inputTokens")]
    #[serde(alias = "inputTokens")]
    pub input_tokens: i64,
    /// Output tokens used.
    #[php(prop, name = "outputTokens")]
    #[serde(alias = "outputTokens")]
    pub output_tokens: i64,
    /// Total tokens used.
    #[php(prop, name = "totalTokens")]
    #[serde(alias = "totalTokens")]
    pub total_tokens: i64,
}

impl Default for ResponseUsage {
    fn default() -> Self {
        <liter_llm::types::ResponseUsage as Default>::default().into()
    }
}

#[php_impl]
impl ResponseUsage {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(inputTokens: i64, outputTokens: i64, totalTokens: i64) -> Self {
        Self {
            input_tokens: inputTokens,
            output_tokens: outputTokens,
            total_tokens: totalTokens,
        }
    }

    pub fn get_input_tokens(&self) -> i64 {
        self.input_tokens.clone()
    }

    pub fn get_output_tokens(&self) -> i64 {
        self.output_tokens.clone()
    }

    pub fn get_total_tokens(&self) -> i64 {
        self.total_tokens.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\SearchRequest")]
#[serde(default)]
pub struct SearchRequest {
    /// The model/provider to use (e.g. `"brave/web-search"`, `"tavily/search"`).
    #[php(prop, name = "model")]
    pub model: String,
    /// The search query string.
    #[php(prop, name = "query")]
    pub query: String,
    /// Maximum number of results to return.
    #[php(prop, name = "maxResults")]
    #[serde(alias = "maxResults")]
    pub max_results: Option<u32>,
    /// Domain filter — restrict results to specific domains.
    #[php(prop, name = "searchDomainFilter")]
    #[serde(alias = "searchDomainFilter")]
    pub search_domain_filter: Option<Vec<String>>,
    /// Country code for localized results (ISO 3166-1 alpha-2, e.g., `"US"`, `"FR"`).
    #[php(prop, name = "country")]
    pub country: Option<String>,
}

impl Default for SearchRequest {
    fn default() -> Self {
        <liter_llm::types::SearchRequest as Default>::default().into()
    }
}

#[php_impl]
impl SearchRequest {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(
        model: String,
        query: String,
        maxResults: Option<u32>,
        searchDomainFilter: Option<Vec<String>>,
        country: Option<String>,
    ) -> Self {
        Self {
            model,
            query,
            max_results: maxResults,
            search_domain_filter: searchDomainFilter,
            country,
        }
    }

    pub fn get_model(&self) -> String {
        self.model.clone()
    }

    pub fn get_query(&self) -> String {
        self.query.clone()
    }

    pub fn get_max_results(&self) -> Option<u32> {
        self.max_results.clone()
    }

    pub fn get_search_domain_filter(&self) -> Option<Vec<String>> {
        self.search_domain_filter.clone()
    }

    pub fn get_country(&self) -> Option<String> {
        self.country.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize, Default)]
#[php_class]
#[php(name = "Liter\\Llm\\SearchResponse")]
#[serde(default)]
pub struct SearchResponse {
    /// List of search results.
    pub results: Vec<SearchResult>,
    /// Model/provider that performed the search.
    #[php(prop, name = "model")]
    pub model: String,
}

#[php_impl]
impl SearchResponse {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(results: &ext_php_rs::types::ZendHashTable, model: String) -> PhpResult<Self> {
        let mut results_core_result: Vec<SearchResult> = Vec::new();
        for (_, item) in results.iter() {
            if let Some(parsed) = <&SearchResult as ext_php_rs::convert::FromZval>::from_zval(item) {
                results_core_result.push(parsed.clone());
            } else {
                return Err(ext_php_rs::exception::PhpException::default(
                    "Failed to convert array element to SearchResult".to_string(),
                ));
            }
        }
        let results_core: Vec<SearchResult> = results_core_result;
        Ok(Self {
            results: results_core,
            model,
        })
    }

    pub fn get_results(&self) -> Vec<SearchResult> {
        self.results.clone()
    }

    pub fn get_model(&self) -> String {
        self.model.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize, Default)]
#[php_class]
#[php(name = "Liter\\Llm\\SearchResult")]
#[serde(default)]
pub struct SearchResult {
    /// Result title.
    #[php(prop, name = "title")]
    pub title: String,
    /// Result URL.
    #[php(prop, name = "url")]
    pub url: String,
    /// Text snippet or excerpt from the page.
    #[php(prop, name = "snippet")]
    pub snippet: String,
    /// Publication or last-updated date, if available.
    #[php(prop, name = "date")]
    pub date: Option<String>,
}

#[php_impl]
impl SearchResult {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(title: String, url: String, snippet: String, date: Option<String>) -> Self {
        Self {
            title,
            url,
            snippet,
            date,
        }
    }

    pub fn get_title(&self) -> String {
        self.title.clone()
    }

    pub fn get_url(&self) -> String {
        self.url.clone()
    }

    pub fn get_snippet(&self) -> String {
        self.snippet.clone()
    }

    pub fn get_date(&self) -> Option<String> {
        self.date.clone()
    }
}

#[derive(Clone)]
#[php_class]
#[php(name = "Liter\\Llm\\SingleflightResult")]
pub struct SingleflightResult {
    inner: Arc<liter_llm::tower::SingleflightResult>,
}

#[php_impl]
impl SingleflightResult {}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\SpecificFunction")]
#[serde(default)]
pub struct SpecificFunction {
    /// Function name.
    #[php(prop, name = "name")]
    pub name: String,
}

impl Default for SpecificFunction {
    fn default() -> Self {
        <liter_llm::types::SpecificFunction as Default>::default().into()
    }
}

#[php_impl]
impl SpecificFunction {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(name: String) -> Self {
        Self { name }
    }

    pub fn get_name(&self) -> String {
        self.name.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\SpecificToolChoice")]
#[serde(default)]
pub struct SpecificToolChoice {
    /// Tool type (always "function").
    #[php(prop, name = "choiceType")]
    #[serde(skip_serializing_if = "String::is_empty")]
    #[serde(alias = "choiceType")]
    pub choice_type: String,
    /// The specific function to invoke.
    pub function: SpecificFunction,
}

impl Default for SpecificToolChoice {
    fn default() -> Self {
        <liter_llm::types::SpecificToolChoice as Default>::default().into()
    }
}

#[php_impl]
impl SpecificToolChoice {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(choiceType: String, function: &SpecificFunction) -> Self {
        Self {
            choice_type: choiceType,
            function: function.clone(),
        }
    }

    pub fn get_choice_type(&self) -> String {
        self.choice_type.clone()
    }

    pub fn get_function(&self) -> SpecificFunction {
        self.function.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\StreamChoice")]
#[serde(default)]
pub struct StreamChoice {
    /// Index of this choice in the choices array.
    #[php(prop, name = "index")]
    pub index: u32,
    /// Incremental update to the message (content, tool calls, etc.).
    pub delta: StreamDelta,
    /// Why the stream ended (present only in final chunk).
    #[php(prop, name = "finishReason")]
    #[serde(skip_serializing_if = "Option::is_none")]
    #[serde(alias = "finishReason")]
    pub finish_reason: Option<String>,
}

impl Default for StreamChoice {
    fn default() -> Self {
        <liter_llm::types::StreamChoice as Default>::default().into()
    }
}

#[php_impl]
impl StreamChoice {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(index: u32, delta: &StreamDelta, finishReason: Option<String>) -> Self {
        Self {
            index,
            delta: delta.clone(),
            finish_reason: finishReason,
        }
    }

    pub fn get_index(&self) -> u32 {
        self.index.clone()
    }

    pub fn get_delta(&self) -> StreamDelta {
        self.delta.clone()
    }

    pub fn get_finish_reason(&self) -> Option<String> {
        self.finish_reason.clone().map(Into::into)
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\StreamDelta")]
#[serde(default)]
pub struct StreamDelta {
    /// Role (typically present only in the first chunk).
    #[php(prop, name = "role")]
    pub role: Option<String>,
    /// Partial content chunk (e.g., a few words of the response).
    #[php(prop, name = "content")]
    pub content: Option<String>,
    /// Partial tool calls being streamed.
    #[serde(alias = "toolCalls")]
    pub tool_calls: Option<Vec<StreamToolCall>>,
    /// Deprecated legacy function_call delta; retained for API compatibility.
    #[serde(alias = "functionCall")]
    pub function_call: Option<StreamFunctionCall>,
    /// Partial refusal message.
    #[php(prop, name = "refusal")]
    pub refusal: Option<String>,
    /// Partial reasoning/thinking tokens (OpenAI-compatible extension used by DeepSeek R1, Qwen, etc.).
    #[php(prop, name = "reasoningContent")]
    #[serde(alias = "reasoningContent")]
    pub reasoning_content: Option<String>,
}

impl Default for StreamDelta {
    fn default() -> Self {
        <liter_llm::types::StreamDelta as Default>::default().into()
    }
}

#[php_impl]
impl StreamDelta {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    pub fn get_role(&self) -> Option<String> {
        self.role.clone()
    }

    pub fn get_content(&self) -> Option<String> {
        self.content.clone()
    }

    pub fn get_tool_calls(&self) -> Option<Vec<StreamToolCall>> {
        self.tool_calls.clone()
    }

    pub fn get_function_call(&self) -> Option<StreamFunctionCall> {
        self.function_call.clone().map(Into::into)
    }

    pub fn set_function_call(&mut self, value: Option<&StreamFunctionCall>) {
        self.function_call = value.map(|value| value.clone().into());
    }

    pub fn get_refusal(&self) -> Option<String> {
        self.refusal.clone()
    }

    pub fn get_reasoning_content(&self) -> Option<String> {
        self.reasoning_content.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\StreamFunctionCall")]
#[serde(default)]
pub struct StreamFunctionCall {
    /// Function name (typically in the first chunk).
    #[php(prop, name = "name")]
    pub name: Option<String>,
    /// Partial JSON arguments chunk.
    #[php(prop, name = "arguments")]
    pub arguments: Option<String>,
}

impl Default for StreamFunctionCall {
    fn default() -> Self {
        <liter_llm::types::StreamFunctionCall as Default>::default().into()
    }
}

#[php_impl]
impl StreamFunctionCall {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    pub fn get_name(&self) -> Option<String> {
        self.name.clone()
    }

    pub fn get_arguments(&self) -> Option<String> {
        self.arguments.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\StreamOptions")]
#[serde(default)]
pub struct StreamOptions {
    /// If true, include token usage in the final stream chunk.
    #[php(prop, name = "includeUsage")]
    #[serde(alias = "includeUsage")]
    pub include_usage: Option<bool>,
}

impl Default for StreamOptions {
    fn default() -> Self {
        <liter_llm::types::StreamOptions as Default>::default().into()
    }
}

#[php_impl]
impl StreamOptions {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    pub fn get_include_usage(&self) -> Option<bool> {
        self.include_usage.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\StreamToolCall")]
#[serde(default)]
pub struct StreamToolCall {
    /// Index of this tool call in the tool_calls array.
    #[php(prop, name = "index")]
    pub index: u32,
    /// Tool call ID (typically in the first chunk for this call).
    #[php(prop, name = "id")]
    pub id: Option<String>,
    /// Tool type (typically "function").
    #[php(prop, name = "callType")]
    #[serde(skip_serializing_if = "Option::is_none")]
    #[serde(alias = "callType")]
    pub call_type: Option<String>,
    /// Partial function name and arguments.
    pub function: Option<StreamFunctionCall>,
}

impl Default for StreamToolCall {
    fn default() -> Self {
        <liter_llm::types::StreamToolCall as Default>::default().into()
    }
}

#[php_impl]
impl StreamToolCall {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(
        index: u32,
        id: Option<String>,
        callType: Option<String>,
        function: Option<&StreamFunctionCall>,
    ) -> Self {
        Self {
            index,
            id,
            call_type: callType,
            function: function.cloned(),
        }
    }

    pub fn get_index(&self) -> u32 {
        self.index.clone()
    }

    pub fn get_id(&self) -> Option<String> {
        self.id.clone()
    }

    pub fn get_call_type(&self) -> Option<String> {
        self.call_type.clone().map(Into::into)
    }

    pub fn get_function(&self) -> Option<StreamFunctionCall> {
        self.function.clone().map(Into::into)
    }

    pub fn set_function(&mut self, value: Option<&StreamFunctionCall>) {
        self.function = value.map(|value| value.clone().into());
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\SystemMessage")]
#[serde(default)]
pub struct SystemMessage {
    /// Instructions or context that apply throughout the conversation.
    ///
    /// Accepts either a plain text string or an array of content parts,
    /// mirroring `UserContent` so that `Message.system_with_parts` works.
    pub content: serde_json::Value,
    /// Optional name for the system message source.
    #[php(prop, name = "name")]
    pub name: Option<String>,
}

impl Default for SystemMessage {
    fn default() -> Self {
        <liter_llm::types::SystemMessage as Default>::default().into()
    }
}

#[php_impl]
impl SystemMessage {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    pub fn get_content(&self) -> Option<String> {
        serde_json::to_string(&self.content).ok()
    }

    pub fn get_name(&self) -> Option<String> {
        self.name.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize, Default)]
#[php_class]
#[php(name = "Liter\\Llm\\ToolCall")]
#[serde(default)]
pub struct ToolCall {
    /// Unique ID for this call, used to reference in tool result messages.
    #[php(prop, name = "id")]
    pub id: String,
    /// Tool type (always "function").
    #[php(prop, name = "callType")]
    #[serde(skip_serializing_if = "String::is_empty")]
    #[serde(alias = "callType")]
    pub call_type: String,
    /// Function name and arguments.
    pub function: FunctionCall,
}

#[php_impl]
impl ToolCall {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(id: String, callType: String, function: &FunctionCall) -> Self {
        Self {
            id,
            call_type: callType,
            function: function.clone(),
        }
    }

    pub fn get_id(&self) -> String {
        self.id.clone()
    }

    pub fn get_call_type(&self) -> String {
        self.call_type.clone()
    }

    pub fn get_function(&self) -> FunctionCall {
        self.function.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\ToolMessage")]
#[serde(default)]
pub struct ToolMessage {
    /// Result of the tool execution as plain text or an array of content parts
    /// (text, images, documents, audio), mirroring `UserMessage.content`.
    ///
    /// `#[serde(untagged)]` on `UserContent` means a bare JSON string still
    /// deserialises into `Text`, so tool results persisted before this field
    /// carried structured content continue to round-trip.
    pub content: serde_json::Value,
    /// ID of the tool call this result responds to.
    #[php(prop, name = "toolCallId")]
    #[serde(alias = "toolCallId")]
    pub tool_call_id: String,
    /// Optional tool/function name.
    #[php(prop, name = "name")]
    pub name: Option<String>,
}

impl Default for ToolMessage {
    fn default() -> Self {
        <liter_llm::types::ToolMessage as Default>::default().into()
    }
}

#[php_impl]
impl ToolMessage {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(toolCallId: String, name: Option<String>) -> Self {
        Self {
            content: Default::default(),
            tool_call_id: toolCallId,
            name,
        }
    }

    pub fn get_content(&self) -> Option<String> {
        serde_json::to_string(&self.content).ok()
    }

    pub fn get_tool_call_id(&self) -> String {
        self.tool_call_id.clone()
    }

    pub fn get_name(&self) -> Option<String> {
        self.name.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\TranscriptionResponse")]
#[serde(default)]
pub struct TranscriptionResponse {
    /// The transcribed text.
    #[php(prop, name = "text")]
    pub text: String,
    /// Detected language (ISO-639-1 code).
    #[php(prop, name = "language")]
    pub language: Option<String>,
    /// Total audio duration in seconds.
    #[php(prop, name = "duration")]
    pub duration: Option<f64>,
    /// Detailed segment-level transcription (if response_format is "verbose_json").
    pub segments: Option<Vec<TranscriptionSegment>>,
}

impl Default for TranscriptionResponse {
    fn default() -> Self {
        <liter_llm::types::TranscriptionResponse as Default>::default().into()
    }
}

#[php_impl]
impl TranscriptionResponse {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(
        text: String,
        language: Option<String>,
        duration: Option<f64>,
        segments: Option<&ext_php_rs::types::ZendHashTable>,
    ) -> PhpResult<Self> {
        let segments_core: Option<Vec<TranscriptionSegment>> = if let Some(ht) = segments {
            let mut result = Vec::new();
            for (_, item) in ht.iter() {
                if let Some(parsed) = <&TranscriptionSegment as ext_php_rs::convert::FromZval>::from_zval(item) {
                    result.push(parsed.clone());
                } else {
                    return Err(ext_php_rs::exception::PhpException::default(
                        "Failed to convert array element to TranscriptionSegment".to_string(),
                    ));
                }
            }
            Some(result)
        } else {
            None
        };
        Ok(Self {
            text,
            language,
            duration,
            segments: segments_core,
        })
    }

    pub fn get_text(&self) -> String {
        self.text.clone()
    }

    pub fn get_language(&self) -> Option<String> {
        self.language.clone()
    }

    pub fn get_duration(&self) -> Option<f64> {
        self.duration.clone()
    }

    pub fn get_segments(&self) -> Option<Vec<TranscriptionSegment>> {
        self.segments.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\TranscriptionSegment")]
#[serde(default)]
pub struct TranscriptionSegment {
    /// Segment index (0-based).
    #[php(prop, name = "id")]
    pub id: u32,
    /// Start time in seconds.
    #[php(prop, name = "start")]
    pub start: f64,
    /// End time in seconds.
    #[php(prop, name = "end")]
    pub end: f64,
    /// Transcribed text for this segment.
    #[php(prop, name = "text")]
    pub text: String,
}

impl Default for TranscriptionSegment {
    fn default() -> Self {
        <liter_llm::types::TranscriptionSegment as Default>::default().into()
    }
}

#[php_impl]
impl TranscriptionSegment {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(id: u32, start: f64, end: f64, text: String) -> Self {
        Self { id, start, end, text }
    }

    pub fn get_id(&self) -> u32 {
        self.id.clone()
    }

    pub fn get_start(&self) -> f64 {
        self.start.clone()
    }

    pub fn get_end(&self) -> f64 {
        self.end.clone()
    }

    pub fn get_text(&self) -> String {
        self.text.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\Usage")]
#[serde(default)]
pub struct Usage {
    /// Prompt tokens used. Defaults to 0 when absent (some providers omit this).
    #[php(prop, name = "promptTokens")]
    #[serde(alias = "promptTokens")]
    pub prompt_tokens: i64,
    /// Completion tokens used. Defaults to 0 when absent (e.g. embedding responses).
    #[php(prop, name = "completionTokens")]
    #[serde(alias = "completionTokens")]
    pub completion_tokens: i64,
    /// Total tokens used. Defaults to 0 when absent (some providers omit this).
    #[php(prop, name = "totalTokens")]
    #[serde(alias = "totalTokens")]
    pub total_tokens: i64,
    /// Breakdown of tokens used in the prompt, including cached tokens served
    /// at the provider's discounted cache-read rate. Absent when the provider
    /// does not return prompt-token details.
    #[serde(alias = "promptTokensDetails")]
    pub prompt_tokens_details: Option<PromptTokensDetails>,
}

impl Default for Usage {
    fn default() -> Self {
        <liter_llm::types::Usage as Default>::default().into()
    }
}

#[php_impl]
impl Usage {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(
        promptTokens: i64,
        completionTokens: i64,
        totalTokens: i64,
        promptTokensDetails: Option<&PromptTokensDetails>,
    ) -> Self {
        Self {
            prompt_tokens: promptTokens,
            completion_tokens: completionTokens,
            total_tokens: totalTokens,
            prompt_tokens_details: promptTokensDetails.cloned(),
        }
    }

    pub fn get_prompt_tokens(&self) -> i64 {
        self.prompt_tokens.clone()
    }

    pub fn get_completion_tokens(&self) -> i64 {
        self.completion_tokens.clone()
    }

    pub fn get_total_tokens(&self) -> i64 {
        self.total_tokens.clone()
    }

    pub fn get_prompt_tokens_details(&self) -> Option<PromptTokensDetails> {
        self.prompt_tokens_details.clone().map(Into::into)
    }

    pub fn set_prompt_tokens_details(&mut self, value: Option<&PromptTokensDetails>) {
        self.prompt_tokens_details = value.map(|value| value.clone().into());
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\UserMessage")]
#[serde(default)]
pub struct UserMessage {
    /// Message content as plain text or array of content parts (text, images, documents, audio).
    pub content: serde_json::Value,
    /// Optional name for the user.
    #[php(prop, name = "name")]
    pub name: Option<String>,
}

impl Default for UserMessage {
    fn default() -> Self {
        <liter_llm::types::UserMessage as Default>::default().into()
    }
}

#[php_impl]
impl UserMessage {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    pub fn get_content(&self) -> Option<String> {
        serde_json::to_string(&self.content).ok()
    }

    pub fn get_name(&self) -> Option<String> {
        self.name.clone()
    }
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
#[php_class]
#[php(name = "Liter\\Llm\\WaitForBatchConfig")]
#[serde(default)]
pub struct WaitForBatchConfig {
    /// Initial interval between polls, in seconds.
    #[php(prop, name = "initialIntervalSecs")]
    #[serde(default = "crate::serde_defaults::wait_for_batch_config_initial_interval_secs")]
    #[serde(alias = "initialIntervalSecs")]
    pub initial_interval_secs: f64,
    /// Maximum interval between polls (backoff plateau), in seconds.
    #[php(prop, name = "maxIntervalSecs")]
    #[serde(default = "crate::serde_defaults::wait_for_batch_config_max_interval_secs")]
    #[serde(alias = "maxIntervalSecs")]
    pub max_interval_secs: f64,
    /// Exponential backoff multiplier (e.g., 1.5 increases delay by 50% each poll).
    #[php(prop, name = "backoffMultiplier")]
    #[serde(default = "crate::serde_defaults::wait_for_batch_config_backoff_multiplier")]
    #[serde(alias = "backoffMultiplier")]
    pub backoff_multiplier: f32,
    /// Optional timeout in seconds — polling fails if this duration is exceeded.
    #[php(prop, name = "timeoutSecs")]
    #[serde(alias = "timeoutSecs")]
    pub timeout_secs: Option<f64>,
}

impl Default for WaitForBatchConfig {
    fn default() -> Self {
        <liter_llm::client::WaitForBatchConfig as Default>::default().into()
    }
}

#[php_impl]
impl WaitForBatchConfig {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))
    }

    #[php(constructor)]
    pub fn new(
        initialIntervalSecs: f64,
        maxIntervalSecs: f64,
        backoffMultiplier: f32,
        timeoutSecs: Option<f64>,
    ) -> Self {
        Self {
            initial_interval_secs: initialIntervalSecs,
            max_interval_secs: maxIntervalSecs,
            backoff_multiplier: backoffMultiplier,
            timeout_secs: timeoutSecs,
        }
    }

    pub fn get_initial_interval_secs(&self) -> f64 {
        self.initial_interval_secs.clone()
    }

    pub fn get_max_interval_secs(&self) -> f64 {
        self.max_interval_secs.clone()
    }

    pub fn get_backoff_multiplier(&self) -> f32 {
        self.backoff_multiplier.clone()
    }

    pub fn get_timeout_secs(&self) -> Option<f64> {
        self.timeout_secs.clone()
    }

    #[allow(clippy::should_implement_trait)]
    #[php(name = "default")]
    pub fn default() -> WaitForBatchConfig {
        liter_llm::client::WaitForBatchConfig::default().into()
    }
}

#[php_class]
#[php(name = "Liter\\Llm\\AssistantContent")]
pub struct AssistantContent {}

#[php_impl]
impl AssistantContent {
    pub const TEXT: &str = "Text";
    pub const PARTS: &str = "Parts";
}

#[php_class]
#[php(name = "Liter\\Llm\\AssistantPart")]
#[derive(Clone, Default, serde::Serialize, serde::Deserialize)]
#[serde(default)]
pub struct AssistantPart {
    #[serde(rename = "type")]
    type_tag: String,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub text: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub refusal: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub image_url: Option<ImageUrl>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub audio: Option<AudioContent>,
}

#[php_impl]
impl AssistantPart {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        let value: Self = serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))?;
        if !matches!(
            value.type_tag.as_str(),
            "text" | "refusal" | "output_image" | "output_audio"
        ) {
            return Err(PhpException::default(format!(
                "Invalid AssistantPart tag: {}",
                value.type_tag,
            )));
        }
        Ok(value)
    }

    #[php(name = "text")]
    pub fn _factory_text(text: String) -> Self {
        liter_llm::types::AssistantPart::Text { text }.into()
    }

    #[php(name = "refusal")]
    pub fn _factory_refusal(refusal: String) -> Self {
        liter_llm::types::AssistantPart::Refusal { refusal }.into()
    }

    #[php(name = "outputImage")]
    pub fn _factory_output_image(imageUrl: &ImageUrl) -> Self {
        liter_llm::types::AssistantPart::OutputImage {
            image_url: imageUrl.clone().into(),
        }
        .into()
    }

    #[php(name = "outputAudio")]
    pub fn _factory_output_audio(audio: &AudioContent) -> Self {
        liter_llm::types::AssistantPart::OutputAudio {
            audio: audio.clone().into(),
        }
        .into()
    }

    #[php(getter)]
    pub fn get_type_tag(&self) -> String {
        self.type_tag.clone()
    }

    #[php(getter)]
    pub fn get_text(&self) -> Option<String> {
        self.text.clone()
    }

    #[php(getter)]
    pub fn get_refusal(&self) -> Option<String> {
        self.refusal.clone()
    }

    #[php(getter)]
    pub fn get_image_url(&self) -> Option<ImageUrl> {
        self.image_url.clone()
    }

    #[php(getter)]
    pub fn get_audio(&self) -> Option<AudioContent> {
        self.audio.clone()
    }
}

#[php_class]
#[php(name = "Liter\\Llm\\AuthHeaderFormat")]
pub struct AuthHeaderFormat {}

#[php_impl]
impl AuthHeaderFormat {
    pub const BEARER: &str = "Bearer";
    pub const APIKEY: &str = "ApiKey";
    pub const NONE: &str = "None";
}

#[php_class]
#[php(name = "Liter\\Llm\\AuthType")]
pub struct AuthType {}

#[php_impl]
impl AuthType {
    pub const BEARER: &str = "bearer";
    pub const APIKEY: &str = "api-key";
    pub const NONE: &str = "none";
    pub const UNKNOWN: &str = "unknown";
}

#[php_class]
#[php(name = "Liter\\Llm\\BatchStatus")]
pub struct BatchStatus {}

#[php_impl]
impl BatchStatus {
    pub const VALIDATING: &str = "validating";
    pub const FAILED: &str = "failed";
    pub const INPROGRESS: &str = "in_progress";
    pub const FINALIZING: &str = "finalizing";
    pub const COMPLETED: &str = "completed";
    pub const EXPIRED: &str = "expired";
    pub const CANCELLING: &str = "cancelling";
    pub const CANCELLED: &str = "cancelled";
}

#[php_class]
#[php(name = "Liter\\Llm\\CacheBackend")]
#[derive(Clone, Default, serde::Serialize, serde::Deserialize)]
#[serde(default)]
pub struct CacheBackend {
    #[serde(rename = "type")]
    type_tag: String,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub scheme: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub config: Option<HashMap<String, String>>,
}

#[php_impl]
impl CacheBackend {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        let value: Self = serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))?;
        if !matches!(value.type_tag.as_str(), "memory" | "open_dal") {
            return Err(PhpException::default(format!(
                "Invalid CacheBackend tag: {}",
                value.type_tag,
            )));
        }
        Ok(value)
    }

    #[php(name = "openDal")]
    pub fn _factory_open_dal(scheme: String, config: HashMap<String, String>) -> Self {
        liter_llm::CacheBackend::OpenDal { scheme, config }.into()
    }

    #[php(getter)]
    pub fn get_type_tag(&self) -> String {
        self.type_tag.clone()
    }

    #[php(getter)]
    pub fn get_scheme(&self) -> Option<String> {
        self.scheme.clone()
    }

    #[php(getter)]
    pub fn get_config(&self) -> Option<HashMap<String, String>> {
        self.config.clone()
    }
}

#[php_class]
#[php(name = "Liter\\Llm\\CircuitState")]
pub struct CircuitState {}

#[php_impl]
impl CircuitState {
    pub const CLOSED: &str = "Closed";
    pub const OPEN: &str = "Open";
    pub const HALFOPEN: &str = "HalfOpen";
}

#[php_class]
#[php(name = "Liter\\Llm\\ContentPart")]
#[derive(Clone, Default, serde::Serialize, serde::Deserialize)]
#[serde(default)]
pub struct ContentPart {
    #[serde(rename = "type")]
    type_tag: String,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub text: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub image_url: Option<ImageUrl>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub document: Option<DocumentContent>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub input_audio: Option<AudioContent>,
}

#[php_impl]
impl ContentPart {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        let value: Self = serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))?;
        if !matches!(
            value.type_tag.as_str(),
            "text" | "image_url" | "document" | "input_audio"
        ) {
            return Err(PhpException::default(format!(
                "Invalid ContentPart tag: {}",
                value.type_tag,
            )));
        }
        Ok(value)
    }

    #[php(name = "text")]
    pub fn _factory_text(text: String) -> Self {
        liter_llm::types::ContentPart::Text { text }.into()
    }

    #[php(name = "imageUrl")]
    pub fn _factory_image_url(imageUrl: &ImageUrl) -> Self {
        liter_llm::types::ContentPart::ImageUrl {
            image_url: imageUrl.clone().into(),
        }
        .into()
    }

    #[php(name = "document")]
    pub fn _factory_document(document: &DocumentContent) -> Self {
        liter_llm::types::ContentPart::Document {
            document: document.clone().into(),
        }
        .into()
    }

    #[php(name = "inputAudio")]
    pub fn _factory_input_audio(inputAudio: &AudioContent) -> Self {
        liter_llm::types::ContentPart::InputAudio {
            input_audio: inputAudio.clone().into(),
        }
        .into()
    }

    #[php(getter)]
    pub fn get_type_tag(&self) -> String {
        self.type_tag.clone()
    }

    #[php(getter)]
    pub fn get_text(&self) -> Option<String> {
        self.text.clone()
    }

    #[php(getter)]
    pub fn get_image_url(&self) -> Option<ImageUrl> {
        self.image_url.clone()
    }

    #[php(getter)]
    pub fn get_document(&self) -> Option<DocumentContent> {
        self.document.clone()
    }

    #[php(getter)]
    pub fn get_input_audio(&self) -> Option<AudioContent> {
        self.input_audio.clone()
    }
}

#[php_class]
#[php(name = "Liter\\Llm\\EmbeddingContentPart")]
#[derive(Clone, Default, serde::Serialize, serde::Deserialize)]
#[serde(default)]
pub struct EmbeddingContentPart {
    #[serde(rename = "type")]
    type_tag: String,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub text: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub image_url: Option<ImageUrl>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub image_base64: Option<String>,
}

#[php_impl]
impl EmbeddingContentPart {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        let value: Self = serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))?;
        if !matches!(value.type_tag.as_str(), "text" | "image_url" | "image_base64") {
            return Err(PhpException::default(format!(
                "Invalid EmbeddingContentPart tag: {}",
                value.type_tag,
            )));
        }
        Ok(value)
    }

    #[php(name = "text")]
    pub fn _factory_text(text: String) -> Self {
        liter_llm::types::EmbeddingContentPart::Text { text }.into()
    }

    #[php(name = "imageUrl")]
    pub fn _factory_image_url(imageUrl: &ImageUrl) -> Self {
        liter_llm::types::EmbeddingContentPart::ImageUrl {
            image_url: imageUrl.clone().into(),
        }
        .into()
    }

    #[php(name = "imageBase64")]
    pub fn _factory_image_base64(imageBase64: String) -> Self {
        liter_llm::types::EmbeddingContentPart::ImageBase64 {
            image_base64: imageBase64,
        }
        .into()
    }

    #[php(getter)]
    pub fn get_type_tag(&self) -> String {
        self.type_tag.clone()
    }

    #[php(getter)]
    pub fn get_text(&self) -> Option<String> {
        self.text.clone()
    }

    #[php(getter)]
    pub fn get_image_url(&self) -> Option<ImageUrl> {
        self.image_url.clone()
    }

    #[php(getter)]
    pub fn get_image_base64(&self) -> Option<String> {
        self.image_base64.clone()
    }
}

#[php_class]
#[php(name = "Liter\\Llm\\EmbeddingFormat")]
pub struct EmbeddingFormat {}

#[php_impl]
impl EmbeddingFormat {
    pub const FLOAT: &str = "float";
    pub const BASE64: &str = "base64";
}

#[php_class]
#[php(name = "Liter\\Llm\\EmbeddingInput")]
pub struct EmbeddingInput {}

#[php_impl]
impl EmbeddingInput {
    pub const SINGLE: &str = "Single";
    pub const MULTIPLE: &str = "Multiple";
    pub const MULTIMODAL: &str = "Multimodal";
}

#[php_class]
#[php(name = "Liter\\Llm\\Enforcement")]
pub struct Enforcement {}

#[php_impl]
impl Enforcement {
    pub const HARD: &str = "Hard";
    pub const SOFT: &str = "Soft";
}

#[php_class]
#[php(name = "Liter\\Llm\\FilePurpose")]
pub struct FilePurpose {}

#[php_impl]
impl FilePurpose {
    pub const ASSISTANTS: &str = "assistants";
    pub const BATCH: &str = "batch";
    pub const FINETUNE: &str = "fine-tune";
    pub const VISION: &str = "vision";
}

#[php_class]
#[php(name = "Liter\\Llm\\FinishReason")]
pub struct FinishReason {}

#[php_impl]
impl FinishReason {
    pub const STOP: &str = "stop";
    pub const LENGTH: &str = "length";
    pub const TOOLCALLS: &str = "tool_calls";
    pub const CONTENTFILTER: &str = "content_filter";
    pub const FUNCTIONCALL: &str = "function_call";
    pub const OTHER: &str = "other";
}

#[php_class]
#[php(name = "Liter\\Llm\\HealthStatus")]
pub struct HealthStatus {}

#[php_impl]
impl HealthStatus {
    pub const HEALTHY: &str = "Healthy";
    pub const UNHEALTHY: &str = "Unhealthy";
}

#[php_class]
#[php(name = "Liter\\Llm\\ImageDetail")]
pub struct ImageDetail {}

#[php_impl]
impl ImageDetail {
    pub const LOW: &str = "low";
    pub const HIGH: &str = "high";
    pub const AUTO: &str = "auto";
}

#[php_class]
#[php(name = "Liter\\Llm\\Message")]
#[derive(Clone, Default, serde::Serialize, serde::Deserialize)]
#[serde(default)]
pub struct Message {
    #[serde(rename = "role")]
    role_tag: String,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub system: Option<SystemMessage>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub user: Option<UserMessage>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub assistant: Option<AssistantMessage>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub tool: Option<ToolMessage>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub developer: Option<DeveloperMessage>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub function: Option<FunctionMessage>,
}

#[php_impl]
impl Message {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        let value: Self = serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))?;
        if !matches!(
            value.role_tag.as_str(),
            "system" | "user" | "assistant" | "tool" | "developer" | "function"
        ) {
            return Err(PhpException::default(format!(
                "Invalid Message tag: {}",
                value.role_tag,
            )));
        }
        Ok(value)
    }

    #[php(getter)]
    pub fn get_role_tag(&self) -> String {
        self.role_tag.clone()
    }

    #[php(getter)]
    pub fn get_system(&self) -> Option<SystemMessage> {
        self.system.clone()
    }

    #[php(getter)]
    pub fn get_user(&self) -> Option<UserMessage> {
        self.user.clone()
    }

    #[php(getter)]
    pub fn get_assistant(&self) -> Option<AssistantMessage> {
        self.assistant.clone()
    }

    #[php(getter)]
    pub fn get_tool(&self) -> Option<ToolMessage> {
        self.tool.clone()
    }

    #[php(getter)]
    pub fn get_developer(&self) -> Option<DeveloperMessage> {
        self.developer.clone()
    }

    #[php(getter)]
    pub fn get_function(&self) -> Option<FunctionMessage> {
        self.function.clone()
    }
}

#[php_class]
#[php(name = "Liter\\Llm\\Modality")]
pub struct Modality {}

#[php_impl]
impl Modality {
    pub const TEXT: &str = "text";
    pub const AUDIO: &str = "audio";
    pub const IMAGE: &str = "image";
}

#[php_class]
#[php(name = "Liter\\Llm\\ModerationInput")]
pub struct ModerationInput {}

#[php_impl]
impl ModerationInput {
    pub const SINGLE: &str = "Single";
    pub const MULTIPLE: &str = "Multiple";
}

#[php_class]
#[php(name = "Liter\\Llm\\OcrDocument")]
#[derive(Clone, Default, serde::Serialize, serde::Deserialize)]
#[serde(default)]
pub struct OcrDocument {
    #[serde(rename = "type")]
    type_tag: String,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub url: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub data: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub media_type: Option<String>,
}

#[php_impl]
impl OcrDocument {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        let value: Self = serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))?;
        if !matches!(value.type_tag.as_str(), "document_url" | "base64") {
            return Err(PhpException::default(format!(
                "Invalid OcrDocument tag: {}",
                value.type_tag,
            )));
        }
        Ok(value)
    }

    #[php(name = "url")]
    pub fn _factory_url(url: String) -> Self {
        liter_llm::types::OcrDocument::Url { url }.into()
    }

    #[php(name = "base64")]
    pub fn _factory_base64(data: String, mediaType: String) -> Self {
        liter_llm::types::OcrDocument::Base64 {
            data,
            media_type: mediaType,
        }
        .into()
    }

    #[php(getter)]
    pub fn get_type_tag(&self) -> String {
        self.type_tag.clone()
    }

    #[php(getter)]
    pub fn get_url(&self) -> Option<String> {
        self.url.clone()
    }

    #[php(getter)]
    pub fn get_data(&self) -> Option<String> {
        self.data.clone()
    }

    #[php(getter)]
    pub fn get_media_type(&self) -> Option<String> {
        self.media_type.clone()
    }
}

#[php_class]
#[php(name = "Liter\\Llm\\ReasoningEffort")]
pub struct ReasoningEffort {}

#[php_impl]
impl ReasoningEffort {
    pub const LOW: &str = "low";
    pub const MEDIUM: &str = "medium";
    pub const HIGH: &str = "high";
    pub const MINIMAL: &str = "minimal";
    pub const MAX: &str = "max";
}

#[php_class]
#[php(name = "Liter\\Llm\\RefreshOutcome")]
pub struct RefreshOutcome {}

#[php_impl]
impl RefreshOutcome {
    pub const DISABLED: &str = "Disabled";
    pub const FROMCACHE: &str = "FromCache";
    pub const FETCHED: &str = "Fetched";
}

#[php_class]
#[php(name = "Liter\\Llm\\RerankDocument")]
pub struct RerankDocument {}

#[php_impl]
impl RerankDocument {
    pub const TEXT: &str = "Text";
    pub const OBJECT: &str = "Object";
}

#[php_class]
#[php(name = "Liter\\Llm\\ResponseFormat")]
#[derive(Clone, Default, serde::Serialize, serde::Deserialize)]
#[serde(default)]
pub struct ResponseFormat {
    #[serde(rename = "type")]
    type_tag: String,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub json_schema: Option<JsonSchemaFormat>,
}

#[php_impl]
impl ResponseFormat {
    #[php(name = "from_json")]
    pub fn from_json(json: String) -> PhpResult<Self> {
        let value: Self = serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))?;
        if !matches!(value.type_tag.as_str(), "text" | "json_object" | "json_schema") {
            return Err(PhpException::default(format!(
                "Invalid ResponseFormat tag: {}",
                value.type_tag,
            )));
        }
        Ok(value)
    }

    #[php(name = "jsonSchema")]
    pub fn _factory_json_schema(jsonSchema: &JsonSchemaFormat) -> Self {
        liter_llm::types::ResponseFormat::JsonSchema {
            json_schema: jsonSchema.clone().into(),
        }
        .into()
    }

    #[php(getter)]
    pub fn get_type_tag(&self) -> String {
        self.type_tag.clone()
    }

    #[php(getter)]
    pub fn get_json_schema(&self) -> Option<JsonSchemaFormat> {
        self.json_schema.clone()
    }
}

#[php_class]
#[php(name = "Liter\\Llm\\StopSequence")]
pub struct StopSequence {}

#[php_impl]
impl StopSequence {
    pub const SINGLE: &str = "Single";
    pub const MULTIPLE: &str = "Multiple";
}

#[php_class]
#[php(name = "Liter\\Llm\\StreamFormat")]
pub struct StreamFormat {}

#[php_impl]
impl StreamFormat {
    pub const SSE: &str = "sse";
    pub const AWSEVENTSTREAM: &str = "aws_event_stream";
}

#[php_class]
#[php(name = "Liter\\Llm\\ToolChoice")]
pub struct ToolChoice {}

#[php_impl]
impl ToolChoice {
    pub const MODE: &str = "Mode";
    pub const SPECIFIC: &str = "Specific";
}

#[php_class]
#[php(name = "Liter\\Llm\\ToolChoiceMode")]
pub struct ToolChoiceMode {}

#[php_impl]
impl ToolChoiceMode {
    pub const AUTO: &str = "auto";
    pub const REQUIRED: &str = "required";
    pub const NONE: &str = "none";
}

#[php_class]
#[php(name = "Liter\\Llm\\ToolType")]
pub struct ToolType {}

#[php_impl]
impl ToolType {
    pub const FUNCTION_: &str = "function";
}

#[php_class]
#[php(name = "Liter\\Llm\\UserContent")]
pub struct UserContent {}

#[php_impl]
impl UserContent {
    pub const TEXT: &str = "Text";
    pub const PARTS: &str = "Parts";
}

#[php_class]
#[php(name = "Liter\\Llm\\LiterLlmApi")]
pub struct LiterLlmApi;

#[php_impl]
impl LiterLlmApi {
    /// Return all provider configs from the registry.
    ///
    /// Useful for tooling, documentation generation, or runtime enumeration.
    /// Returns the public `ProviderConfig` slice (without capability flags).
    /// To query capability flags for a specific provider use `capabilities`.
    #[php(name = "allProviders")]
    pub fn all_providers() -> PhpResult<Vec<ProviderConfig>> {
        let result = liter_llm::provider::all_providers()
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))?;
        Ok(result.iter().cloned().map(Into::into).collect())
    }

    /// Return the capability flags for a named provider.
    ///
    /// Performs an O(n) linear scan over the embedded registry (165 entries).
    /// Returns an owned value so bindings can pass capability data without
    /// borrowing registry internals.
    ///
    /// For unknown `provider_name` values the function returns an all-`false`
    /// sentinel so callers never need to handle `Option`.
    #[php(name = "capabilities")]
    pub fn capabilities(providerName: String) -> ProviderCapabilities {
        liter_llm::provider::capabilities(&providerName).into()
    }

    /// Assert that `current_len + incoming` does not exceed `limit`.
    ///
    /// Call this before appending `incoming` bytes to any buffer that must
    /// stay below `limit`.  Returns `Err(LiterLlmError::Streaming)` on overflow
    /// and emits a `tracing::warn!` with context.
    ///
    /// # Example
    ///
    /// ```ignore
    /// check_bound("SSE buffer", buffer.len(), chunk.len(), SSE_BUFFER_MAX_BYTES)?;
    /// buffer.push_str(chunk_str);
    /// ```
    #[php(name = "checkBound")]
    pub fn check_bound(context: String, currentLen: i64, incoming: i64, limit: i64) -> PhpResult<()> {
        let result =
            liter_llm::util::bounds::check_bound(&context, currentLen as usize, incoming as usize, limit as usize)
                .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))?;
        Ok(result)
    }

    /// Remove all guardrails from the global registry.
    ///
    /// Primarily useful in tests to reset state between test cases.
    ///
    /// If the lock was poisoned by a panicking guardrail on a previous access,
    /// the poisoned state is recovered rather than propagating the panic.
    #[php(name = "clear")]
    pub fn clear() {
        liter_llm::guardrail::registry::clear()
    }

    /// Clear the overlay registry, reverting `completion_cost`,
    /// `completion_cost_with_cache`, and `model_info` to the
    /// embedded catalog.
    ///
    /// Primarily a test seam (see `install_catalog_overlay_from_str`); also
    /// usable by long-running processes that want to abandon a runtime refresh.
    #[php(name = "clearCatalogOverlay")]
    pub fn clear_catalog_overlay() {
        liter_llm::cost::refresh::clear_catalog_overlay()
    }

    /// Calculate the estimated cost of a completion given a model name and token
    /// counts.
    ///
    /// Returns `None` if the model is not present in the embedded pricing registry.
    /// Returns `Some(cost_usd)` otherwise, where the value is in US dollars.
    ///
    /// When an exact model name match is not found, progressively shorter prefixes
    /// are tried by stripping from the last `-` or `.` separator.  For example,
    /// `gpt-4-0613` will match `gpt-4` if no `gpt-4-0613` entry exists.
    ///
    /// # Example
    ///
    /// ```rust
    /// use liter_llm::cost;
    ///
    /// let usd = cost::completion_cost("gpt-4o", 1_000, 500).expect("gpt-4o is a known model");
    /// // 1000 * 0.0000025 + 500 * 0.00001 = 0.0025 + 0.005 = 0.0075
    /// assert!((usd - 0.0075).abs() < 1e-9);
    /// ```
    #[php(name = "completionCost")]
    pub fn completion_cost(model: String, promptTokens: i64, completionTokens: i64) -> Option<f64> {
        liter_llm::completion_cost(&model, promptTokens as u64, completionTokens as u64)
    }

    /// Calculate the estimated cost of a completion, accounting for cached
    /// (cache-hit) prompt tokens billed at the provider's discounted rate.
    ///
    /// `cached_tokens` is the count of prompt tokens served from the provider's
    /// prompt cache. It must be `<= prompt_tokens` (cached tokens are a subset of
    /// the prompt). The non-cached portion is billed at `input_cost_per_token`
    /// and the cached portion at `cache_read_input_token_cost` when the model
    /// has cache pricing; otherwise the entire prompt is billed at the regular
    /// input rate.
    ///
    /// Returns `None` if the model is not present in the embedded pricing
    /// registry, mirroring `completion_cost`.
    ///
    /// When the model has `ModelPricing::tiers`, the tier whose
    /// `min_context_tokens` is the highest value `<= prompt_tokens` supplies the
    /// input/output/cache rates for the whole call; models without tiers (or
    /// when `prompt_tokens` is below every tier threshold) use the base rates
    /// unchanged, matching the original flat-rate behaviour.
    #[php(name = "completionCostWithCache")]
    pub fn completion_cost_with_cache(
        model: String,
        promptTokens: i64,
        cachedTokens: i64,
        completionTokens: i64,
    ) -> Option<f64> {
        liter_llm::completion_cost_with_cache(
            &model,
            promptTokens as u64,
            cachedTokens as u64,
            completionTokens as u64,
        )
    }

    /// Return the set of complex provider names.
    ///
    /// Complex providers require custom auth/routing logic beyond simple bearer
    /// tokens (e.g. AWS Bedrock SigV4, Vertex AI OAuth2).
    ///
    /// The returned reference points into the static registry — no allocation.
    #[php(name = "complexProviderNames")]
    pub fn complex_provider_names() -> PhpResult<Vec<String>> {
        let result = liter_llm::provider::complex_provider_names()
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))?;
        Ok(result.iter().map(|s| s.to_string()).collect())
    }

    /// Count tokens for a full `ChatCompletionRequest`.
    ///
    /// Sums tokens across all message text contents plus a per-message overhead
    /// of ~4 tokens (for role, separators, and formatting metadata). Tool
    /// definitions and multimodal content parts (images, audio, documents) are
    /// not counted — only textual content contributes to the token total.
    ///
    /// # Errors
    ///
    /// Returns `LiterLlmError::BadRequest` if the tokenizer cannot be loaded or
    /// if tokenization fails for any message.
    #[php(name = "countRequestTokens")]
    pub fn count_request_tokens(model: String, req: Option<&ChatCompletionRequest>) -> PhpResult<i64> {
        let req_core: Option<liter_llm::ChatCompletionRequest> = req.map(|v| v.clone().into());
        let result = liter_llm::count_request_tokens(&model, &req_core.unwrap_or_default())
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))?;
        Ok(result as i64)
    }

    /// Count tokens in a text string using the tokenizer for the given model.
    ///
    /// The tokenizer is resolved from the model name prefix (e.g. `"gpt-4o"` maps
    /// to the `Xenova/gpt-4o` HuggingFace tokenizer). Tokenizers are cached after
    /// first load.
    ///
    /// # Errors
    ///
    /// Returns `LiterLlmError::BadRequest` if the tokenizer cannot be loaded
    /// (e.g. network failure on first use) or if tokenization itself fails.
    #[php(name = "countTokens")]
    pub fn count_tokens(model: String, text: String) -> PhpResult<i64> {
        let result = liter_llm::count_tokens(&model, &text)
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))?;
        Ok(result as i64)
    }

    /// Create a new LLM client with simple scalar configuration.
    ///
    /// This is the primary binding entry-point. All parameters except `api_key`
    /// are optional — omitting them uses the same defaults as
    /// `ClientConfigBuilder`.
    ///
    /// # Errors
    ///
    /// Returns `LiterLlmError` if the underlying HTTP client cannot be
    /// constructed, or if the resolved provider configuration is invalid.
    #[php(name = "createClient")]
    pub fn create_client(
        apiKey: String,
        baseUrl: Option<String>,
        timeoutSecs: Option<i64>,
        maxRetries: Option<u32>,
        modelHint: Option<String>,
    ) -> PhpResult<DefaultClient> {
        let result = liter_llm::create_client(apiKey, baseUrl, timeoutSecs.map(|v| v as u64), maxRetries, modelHint)
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))?;
        Ok(DefaultClient {
            inner: Arc::new(result),
        })
    }

    /// Create a new LLM client from a JSON string.
    ///
    /// The JSON object accepts the same fields as `liter-llm.toml` (snake_case).
    ///
    /// # Errors
    ///
    /// Returns `LiterLlmError::BadRequest` if `json` is not valid JSON or
    /// contains unknown fields.
    #[php(name = "createClientFromJson")]
    pub fn create_client_from_json(json: String) -> PhpResult<DefaultClient> {
        let result = liter_llm::create_client_from_json(&json)
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))?;
        Ok(DefaultClient {
            inner: Arc::new(result),
        })
    }

    /// Decode a base64 data URL into `DecodedDataUrl`.
    ///
    /// Returns `None` for:
    /// - Non-data URLs (strings that do not start with `"data:"`).
    /// - Malformed prefixes (missing `";base64,"` marker).
    /// - Invalid base64 payloads.
    ///
    /// The returned MIME string is extracted verbatim from the URL prefix —
    /// it is not validated or normalised.
    ///
    /// # Example
    ///
    /// ```
    /// use liter_llm::image::{encode_data_url, decode_data_url, IMAGE_PNG};
    ///
    /// let url = encode_data_url(b"hello", Some(IMAGE_PNG));
    /// let decoded = decode_data_url(&url).expect("valid data URL");
    /// assert_eq!(decoded.mime, IMAGE_PNG);
    /// assert_eq!(decoded.data, b"hello");
    ///
    /// // Non-data URLs return None.
    /// assert!(decode_data_url("https://example.com/img.png").is_none());
    ///
    /// // Missing ;base64, marker returns None.
    /// assert!(decode_data_url("data:image/png,plaintext").is_none());
    /// ```
    #[php(name = "decodeDataUrl")]
    pub fn decode_data_url(url: String) -> Option<DecodedDataUrl> {
        liter_llm::image::decode_data_url(&url).map(Into::into)
    }

    /// Encode bytes as a base64 data URL: `data:<mime>;base64,<b64>`.
    ///
    /// `mime` defaults to `IMAGE_PNG` when `None`.
    ///
    /// # Example
    ///
    /// ```
    /// use liter_llm::image::{encode_data_url, IMAGE_PNG, IMAGE_JPEG};
    ///
    /// let url = encode_data_url(b"\x89PNG", Some(IMAGE_PNG));
    /// assert!(url.starts_with("data:image/png;base64,"));
    ///
    /// let url_default = encode_data_url(b"\x89PNG", None);
    /// assert!(url_default.starts_with("data:image/png;base64,"));
    ///
    /// let jpeg_url = encode_data_url(b"\xff\xd8\xff", Some(IMAGE_JPEG));
    /// assert!(jpeg_url.starts_with("data:image/jpeg;base64,"));
    /// ```
    #[php(name = "encodeDataUrl")]
    pub fn encode_data_url(bytes: PhpBytes, mime: Option<String>) -> String {
        liter_llm::image::encode_data_url(&bytes.0[..], mime.as_deref())
    }

    /// Install the overlay registry from a raw catalog JSON string, bypassing
    /// the network and disk cache entirely.
    ///
    /// Parses and flattens `catalog_json` with the same
    /// `registry_from_catalog_str` logic used for the embedded catalog and the
    /// network refresh path, then atomically swaps it in as the active overlay.
    /// A parse failure returns `CatalogRefreshError::Parse` and leaves any
    /// existing overlay untouched.
    ///
    /// This is primarily a testable seam: it lets tests exercise overlay
    /// installation and the embedded/overlay fallback behavior in
    /// `completion_cost` / `model_info` without a real network
    /// call.
    #[php(name = "installCatalogOverlayFromStr")]
    pub fn install_catalog_overlay_from_str(catalogJson: String) -> PhpResult<()> {
        let result = liter_llm::cost::refresh::install_catalog_overlay_from_str(&catalogJson)
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))?;
        Ok(result)
    }

    /// Look up FFI-friendly pricing and capability metadata for a model.
    ///
    /// Returns `None` if the model is not present in the active pricing
    /// registry. Uses the same exact-match-then-prefix-fallback resolution as
    /// `model_pricing`; unlike `model_pricing`, the result is an owned
    /// `ModelInfo` value safe to hand across the FFI boundary.
    ///
    /// When a runtime catalog refresh has succeeded, this reflects the refreshed
    /// (overlay) catalog; otherwise it reflects the embedded catalog. See
    /// `model_pricing` for the embedded-only alternative.
    #[php(name = "modelInfo")]
    pub fn model_info(model: String) -> Option<ModelInfo> {
        liter_llm::model_info(&model).map(Into::into)
    }

    /// Record the estimated USD cost of a completion.
    ///
    /// Call from `CostTrackingService` once a
    /// completion's cost has been computed. Emits `gen_ai.client.cost.usd`.
    /// If the meter has not been initialized, this call is a no-op.
    #[php(name = "recordCostUsd")]
    pub fn record_cost_usd(system: String, model: String, operation: String, costUsd: f64) {
        liter_llm::tower::metrics::record_cost_usd(&system, &model, &operation, costUsd)
    }

    /// Refresh the runtime catalog overlay per `config`.
    ///
    /// - `config.enabled == false`: returns `Ok(``RefreshOutcome::Disabled``)`
    ///   immediately. No network, filesystem, or overlay activity.
    /// - A fresh on-disk cache (age < `config.ttl_seconds`) exists at the
    ///   resolved cache path (`config.cache_path`, or a default under
    ///   `std::env::temp_dir()`): read + flatten it and install the overlay,
    ///   returning `Ok(``RefreshOutcome::FromCache``)`. No network request is
    ///   made.
    /// - Otherwise: validate `config.source_url` uses `https`
    ///   (`CatalogRefreshError::InsecureUrl` otherwise), fetch it, flatten it,
    ///   install the overlay, best-effort write the raw JSON to the cache path
    ///   (a cache write failure does not fail the refresh), and return
    ///   `Ok(``RefreshOutcome::Fetched``)`.
    ///
    /// On any error return, the overlay is left untouched: the previously
    /// active registry (a prior successful overlay, or the embedded catalog if
    /// none was ever installed) remains in effect. This is what makes the
    /// feature air-gap-safe — an unreachable or invalid `source_url` never
    /// degrades `completion_cost` / `model_info` below embedded-catalog
    /// availability.
    #[php(name = "refreshCatalog")]
    pub fn refresh_catalog(config: Option<&CatalogRefreshConfig>) -> PhpResult<String> {
        let config_core: Option<liter_llm::CatalogRefreshConfig> = config.map(|v| v.clone().into());
        WORKER_RUNTIME.block_on(async {
            let result = liter_llm::cost::refresh::refresh_catalog(&config_core.unwrap_or_default())
                .await
                .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))?;
            Ok(format!("{:?}", result))
        })
    }

    /// Register a custom provider in the global runtime registry.
    ///
    /// The provider will be checked **before** all built-in providers during model
    /// detection. If a provider with the same `name` already exists it is replaced.
    ///
    /// # Errors
    ///
    /// Returns an error if the config is invalid (empty name, empty base_url, or
    /// no model prefixes).
    #[php(name = "registerCustomProvider")]
    pub fn register_custom_provider(config: &CustomProviderConfig) -> PhpResult<()> {
        let config_core: liter_llm::CustomProviderConfig = config.clone().into();
        let result = liter_llm::provider::custom::register_custom_provider(config_core)
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))?;
        Ok(result)
    }

    /// Remove a previously registered custom provider by name.
    ///
    /// Returns `true` if a provider with the given name was found and removed,
    /// `false` if no such provider existed.
    ///
    /// # Errors
    ///
    /// Returns an error if the custom-provider registry cannot be updated.
    #[php(name = "unregisterCustomProvider")]
    pub fn unregister_custom_provider(name: String) -> PhpResult<bool> {
        let result = liter_llm::provider::custom::unregister_custom_provider(&name)
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))?;
        Ok(result)
    }

    #[php(name = "chatStream")]
    pub fn chatStream(
        engine: &DefaultClient,
        req: &ChatCompletionRequest,
    ) -> std::result::Result<Vec<String>, ext_php_rs::exception::PhpException> {
        engine.chat_stream(&req)
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::useless_conversion)]
impl From<AssistantMessage> for liter_llm::types::AssistantMessage {
    fn from(val: AssistantMessage) -> Self {
        Self {
            content: val.content.and_then(|v| serde_json::from_value(v).ok()),
            name: val.name,
            tool_calls: val.tool_calls.map(|v| v.into_iter().map(Into::into).collect()),
            refusal: val.refusal,
            function_call: val.function_call.map(Into::into),
            reasoning_content: val.reasoning_content,
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<liter_llm::types::AssistantMessage> for AssistantMessage {
    fn from(val: liter_llm::types::AssistantMessage) -> Self {
        Self {
            content: val.content.as_ref().and_then(|v| serde_json::to_value(v).ok()),
            name: val.name.map(|v| v.to_string()),
            tool_calls: val.tool_calls.map(|v| v.into_iter().map(Into::into).collect()),
            refusal: val.refusal.map(|v| v.to_string()),
            function_call: val.function_call.map(Into::into),
            reasoning_content: val.reasoning_content.map(|v| v.to_string()),
        }
    }
}

#[allow(clippy::needless_update)]
impl From<AudioContent> for liter_llm::types::AudioContent {
    fn from(val: AudioContent) -> Self {
        Self {
            data: val.data,
            format: val.format,
            ..Default::default()
        }
    }
}

impl From<liter_llm::types::AudioContent> for AudioContent {
    fn from(val: liter_llm::types::AudioContent) -> Self {
        Self {
            data: val.data.to_string(),
            format: val.format.to_string(),
        }
    }
}

#[allow(clippy::useless_conversion)]
impl From<AuthConfig> for liter_llm::provider::AuthConfig {
    fn from(val: AuthConfig) -> Self {
        Self {
            auth_type: match val.auth_type.as_str() {
                "bearer" => liter_llm::provider::AuthType::Bearer,
                "api-key" => liter_llm::provider::AuthType::ApiKey,
                "none" => liter_llm::provider::AuthType::None,
                "unknown" => liter_llm::provider::AuthType::Unknown,
                other => {
                    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'auth_type' of AuthType: expected one of [bearer, api-key, none, unknown]"
    ))
    .throw()
    .expect("failed to throw PHP exception for invalid enum string value");
                    liter_llm::provider::AuthType::Bearer
                }
            },
            env_var: val.env_var,
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::provider::AuthConfig> for AuthConfig {
    fn from(val: liter_llm::provider::AuthConfig) -> Self {
        Self {
            auth_type: serde_json::to_value(val.auth_type)
                .ok()
                .and_then(|s| s.as_str().map(String::from))
                .unwrap_or_default(),
            env_var: val.env_var.map(|v| v.to_string()),
        }
    }
}

#[allow(clippy::needless_update)]
impl From<BatchListQuery> for liter_llm::types::BatchListQuery {
    fn from(val: BatchListQuery) -> Self {
        Self {
            limit: val.limit,
            after: val.after,
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::types::BatchListQuery> for BatchListQuery {
    fn from(val: liter_llm::types::BatchListQuery) -> Self {
        Self {
            limit: val.limit,
            after: val.after.map(|v| v.to_string()),
        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::useless_conversion)]
impl From<BatchListResponse> for liter_llm::types::BatchListResponse {
    fn from(val: BatchListResponse) -> Self {
        Self {
            object: val.object,
            data: val.data.into_iter().map(Into::into).collect(),
            has_more: val.has_more,
            first_id: val.first_id,
            last_id: val.last_id,
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<liter_llm::types::BatchListResponse> for BatchListResponse {
    fn from(val: liter_llm::types::BatchListResponse) -> Self {
        Self {
            object: val.object.to_string(),
            data: val.data.into_iter().map(Into::into).collect(),
            has_more: val.has_more,
            first_id: val.first_id.map(|v| v.to_string()),
            last_id: val.last_id.map(|v| v.to_string()),
        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::useless_conversion)]
impl From<BatchObject> for liter_llm::types::BatchObject {
    fn from(val: BatchObject) -> Self {
        Self {
            id: val.id,
            object: val.object,
            endpoint: val.endpoint,
            input_file_id: val.input_file_id,
            completion_window: val.completion_window,
            status: match val.status.as_str() {
                "validating" => liter_llm::types::BatchStatus::Validating,
                "failed" => liter_llm::types::BatchStatus::Failed,
                "in_progress" => liter_llm::types::BatchStatus::InProgress,
                "finalizing" => liter_llm::types::BatchStatus::Finalizing,
                "completed" => liter_llm::types::BatchStatus::Completed,
                "expired" => liter_llm::types::BatchStatus::Expired,
                "cancelling" => liter_llm::types::BatchStatus::Cancelling,
                "cancelled" => liter_llm::types::BatchStatus::Cancelled,
                other => {
                    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'status' of BatchStatus: expected one of [validating, failed, in_progress, finalizing, completed, expired, cancelling, cancelled]"
    ))
    .throw()
    .expect("failed to throw PHP exception for invalid enum string value");
                    Default::default()
                }
            },
            output_file_id: val.output_file_id,
            error_file_id: val.error_file_id,
            created_at: val.created_at as u64,
            completed_at: val.completed_at.map(|v| v as u64),
            failed_at: val.failed_at.map(|v| v as u64),
            expired_at: val.expired_at.map(|v| v as u64),
            request_counts: val.request_counts.map(Into::into),
            metadata: val.metadata,
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<liter_llm::types::BatchObject> for BatchObject {
    fn from(val: liter_llm::types::BatchObject) -> Self {
        Self {
            id: val.id.to_string(),
            object: val.object.to_string(),
            endpoint: val.endpoint.to_string(),
            input_file_id: val.input_file_id.to_string(),
            completion_window: val.completion_window.to_string(),
            status: serde_json::to_value(val.status)
                .ok()
                .and_then(|s| s.as_str().map(String::from))
                .unwrap_or_default(),
            output_file_id: val.output_file_id.map(|v| v.to_string()),
            error_file_id: val.error_file_id.map(|v| v.to_string()),
            created_at: val.created_at as i64,
            completed_at: val.completed_at.map(|v| v as i64),
            failed_at: val.failed_at.map(|v| v as i64),
            expired_at: val.expired_at.map(|v| v as i64),
            request_counts: val.request_counts.map(Into::into),
            metadata: val.metadata,
        }
    }
}

#[allow(clippy::needless_update)]
impl From<BatchRequestCounts> for liter_llm::types::BatchRequestCounts {
    fn from(val: BatchRequestCounts) -> Self {
        Self {
            total: val.total as u64,
            completed: val.completed as u64,
            failed: val.failed as u64,
            ..Default::default()
        }
    }
}

impl From<liter_llm::types::BatchRequestCounts> for BatchRequestCounts {
    fn from(val: liter_llm::types::BatchRequestCounts) -> Self {
        Self {
            total: val.total as i64,
            completed: val.completed as i64,
            failed: val.failed as i64,
        }
    }
}

#[allow(clippy::needless_update)]
impl From<BedrockConfig> for liter_llm::client::BedrockConfig {
    fn from(val: BedrockConfig) -> Self {
        Self {
            region: val.region,
            cross_region_prefix: val.cross_region_prefix,
            access_key_id: val.access_key_id,
            secret_access_key: val.secret_access_key,
            session_token: val.session_token,
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::client::BedrockConfig> for BedrockConfig {
    fn from(val: liter_llm::client::BedrockConfig) -> Self {
        Self {
            region: val.region.map(|v| v.to_string()),
            cross_region_prefix: val.cross_region_prefix.map(|v| v.to_string()),
            access_key_id: val.access_key_id.map(|v| v.to_string()),
            secret_access_key: val.secret_access_key.map(|v| v.to_string()),
            session_token: val.session_token.map(|v| v.to_string()),
        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::useless_conversion)]
impl From<BudgetConfig> for liter_llm::BudgetConfig {
    fn from(val: BudgetConfig) -> Self {
        Self {
            global_limit: val.global_limit,
            model_limits: val.model_limits.into_iter().collect(),
            enforcement: match val.enforcement.as_str() {
                "Hard" | "hard" => liter_llm::Enforcement::Hard,
                "Soft" | "soft" => liter_llm::Enforcement::Soft,
                other => {
                    ext_php_rs::exception::PhpException::default(format!(
                        "invalid value {other:?} for field 'enforcement' of Enforcement: expected one of [Hard, Soft]"
                    ))
                    .throw()
                    .expect("failed to throw PHP exception for invalid enum string value");
                    liter_llm::Enforcement::Hard
                }
            },
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::BudgetConfig> for BudgetConfig {
    fn from(val: liter_llm::BudgetConfig) -> Self {
        Self {
            global_limit: val.global_limit,
            model_limits: val.model_limits.into_iter().collect(),
            enforcement: serde_json::to_value(val.enforcement)
                .ok()
                .and_then(|s| s.as_str().map(String::from))
                .unwrap_or_default(),
        }
    }
}

#[allow(clippy::field_reassign_with_default, clippy::let_and_return)]
#[allow(clippy::useless_conversion)]
impl From<CacheConfig> for liter_llm::CacheConfig {
    fn from(val: CacheConfig) -> Self {
        let mut __result = liter_llm::CacheConfig::default();
        __result.max_entries = val.max_entries as usize;
        if let Some(__v) = val.ttl {
            __result.ttl = std::time::Duration::from_millis(__v as u64);
        }
        __result.backend = val.backend.into();
        __result
    }
}

#[allow(clippy::useless_conversion)]
impl From<liter_llm::CacheConfig> for CacheConfig {
    fn from(val: liter_llm::CacheConfig) -> Self {
        Self {
            max_entries: val.max_entries as i64,
            ttl: Some(val.ttl.as_millis() as u64 as i64),
            backend: val.backend.into(),
        }
    }
}

#[allow(clippy::needless_update)]
impl From<CatalogRefreshConfig> for liter_llm::cost::refresh::CatalogRefreshConfig {
    fn from(val: CatalogRefreshConfig) -> Self {
        Self {
            enabled: val.enabled,
            source_url: val.source_url,
            ttl_seconds: val.ttl_seconds as u64,
            cache_path: val.cache_path,
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::cost::refresh::CatalogRefreshConfig> for CatalogRefreshConfig {
    fn from(val: liter_llm::cost::refresh::CatalogRefreshConfig) -> Self {
        Self {
            enabled: val.enabled,
            source_url: val.source_url.to_string(),
            ttl_seconds: val.ttl_seconds as i64,
            cache_path: val.cache_path.map(|v| v.to_string()),
        }
    }
}

#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<liter_llm::types::ChatCompletionChunk> for ChatCompletionChunk {
    fn from(val: liter_llm::types::ChatCompletionChunk) -> Self {
        Self {
            id: val.id.to_string(),
            object: val.object.to_string(),
            created: val.created as i64,
            model: val.model.to_string(),
            choices: val.choices.into_iter().map(Into::into).collect(),
            usage: val.usage.map(Into::into),
            system_fingerprint: val.system_fingerprint.map(|v| v.to_string()),
            service_tier: val.service_tier.map(|v| v.to_string()),
        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::useless_conversion)]
impl From<ChatCompletionRequest> for liter_llm::types::ChatCompletionRequest {
    fn from(val: ChatCompletionRequest) -> Self {
        Self {
            model: val.model,
            messages: val.messages.into_iter().map(Into::into).collect(),
            temperature: val.temperature,
            top_p: val.top_p,
            n: val.n,
            stream: val.stream,
            stop: val.stop.and_then(|v| serde_json::from_value(v).ok()),
            max_tokens: val.max_tokens.map(|v| v as u64),
            presence_penalty: val.presence_penalty,
            frequency_penalty: val.frequency_penalty,
            logit_bias: val.logit_bias.map(|m| m.into_iter().collect()),
            user: val.user,
            tools: val.tools.map(|v| v.into_iter().map(Into::into).collect()),
            tool_choice: val.tool_choice.and_then(|v| serde_json::from_value(v).ok()),
            parallel_tool_calls: val.parallel_tool_calls,
            response_format: val.response_format.map(Into::into),
            stream_options: val.stream_options.map(Into::into),
            seed: val.seed,
            reasoning_effort: val.reasoning_effort.as_deref().map(|s| match s { "low" => liter_llm::types::ReasoningEffort::Low,"medium" => liter_llm::types::ReasoningEffort::Medium,"high" => liter_llm::types::ReasoningEffort::High,"minimal" => liter_llm::types::ReasoningEffort::Minimal,"max" => liter_llm::types::ReasoningEffort::Max,other => {
    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'reasoning_effort' of ReasoningEffort: expected one of [low, medium, high, minimal, max]"
    ))
    .throw()
    .expect("failed to throw PHP exception for invalid enum string value");
    Default::default()
}
 })
,
            modalities: val.modalities.map(|v| v.into_iter().map(|s| match s.as_str() { "text" => liter_llm::types::Modality::Text,"audio" => liter_llm::types::Modality::Audio,"image" => liter_llm::types::Modality::Image,other => {
    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'modalities' of Modality: expected one of [text, audio, image]"
    ))
    .throw()
    .expect("failed to throw PHP exception for invalid enum string value");
    liter_llm::types::Modality::Text
}
 }
).collect()),
            logprobs: val.logprobs,
            top_logprobs: val.top_logprobs,
            max_completion_tokens: val.max_completion_tokens.map(|v| v as u64),
            service_tier: val.service_tier,
            store: val.store,
            metadata: val.metadata.map(|m| m.into_iter().map(|(k, v)| (k.into(), v.into())).collect()),
            prediction: val.prediction,
            audio: val.audio,
            web_search_options: val.web_search_options,
            extra_body: val.extra_body,
            ..Default::default()        }
    }
}

#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<liter_llm::types::ChatCompletionRequest> for ChatCompletionRequest {
    fn from(val: liter_llm::types::ChatCompletionRequest) -> Self {
        Self {
            model: val.model.to_string(),
            messages: val.messages.into_iter().map(Into::into).collect(),
            temperature: val.temperature,
            top_p: val.top_p,
            n: val.n,
            stream: val.stream,
            stop: val.stop.as_ref().and_then(|v| serde_json::to_value(v).ok()),
            max_tokens: val.max_tokens.map(|v| v as i64),
            presence_penalty: val.presence_penalty,
            frequency_penalty: val.frequency_penalty,
            logit_bias: val.logit_bias.map(|m| m.into_iter().collect()),
            user: val.user.map(|v| v.to_string()),
            tools: val.tools.map(|v| v.into_iter().map(Into::into).collect()),
            tool_choice: val.tool_choice.as_ref().and_then(|v| serde_json::to_value(v).ok()),
            parallel_tool_calls: val.parallel_tool_calls,
            response_format: val.response_format.map(Into::into),
            stream_options: val.stream_options.map(Into::into),
            seed: val.seed,
            reasoning_effort: val.reasoning_effort.as_ref().map(|v| {
                serde_json::to_value(v)
                    .ok()
                    .and_then(|s| s.as_str().map(String::from))
                    .unwrap_or_default()
            }),
            modalities: val.modalities.as_ref().map(|v| {
                v.iter()
                    .map(|x| {
                        serde_json::to_value(x)
                            .ok()
                            .and_then(|s| s.as_str().map(String::from))
                            .unwrap_or_default()
                    })
                    .collect()
            }),
            logprobs: val.logprobs,
            top_logprobs: val.top_logprobs,
            max_completion_tokens: val.max_completion_tokens.map(|v| v as i64),
            service_tier: val.service_tier.map(|v| v.to_string()),
            store: val.store,
            metadata: val
                .metadata
                .map(|m| m.into_iter().map(|(k, v)| (k.into(), v.into())).collect()),
            prediction: val.prediction,
            audio: val.audio,
            web_search_options: val.web_search_options,
            extra_body: val.extra_body,
        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::useless_conversion)]
impl From<ChatCompletionResponse> for liter_llm::types::ChatCompletionResponse {
    fn from(val: ChatCompletionResponse) -> Self {
        Self {
            id: val.id,
            object: val.object,
            created: val.created as u64,
            model: val.model,
            choices: val.choices.into_iter().map(Into::into).collect(),
            usage: val.usage.map(Into::into),
            system_fingerprint: val.system_fingerprint,
            service_tier: val.service_tier,
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<liter_llm::types::ChatCompletionResponse> for ChatCompletionResponse {
    fn from(val: liter_llm::types::ChatCompletionResponse) -> Self {
        Self {
            id: val.id.to_string(),
            object: val.object.to_string(),
            created: val.created as i64,
            model: val.model.to_string(),
            choices: val.choices.into_iter().map(Into::into).collect(),
            usage: val.usage.map(Into::into),
            system_fingerprint: val.system_fingerprint.map(|v| v.to_string()),
            service_tier: val.service_tier.map(|v| v.to_string()),
        }
    }
}

#[allow(clippy::useless_conversion)]
impl From<ChatCompletionTool> for liter_llm::types::ChatCompletionTool {
    fn from(val: ChatCompletionTool) -> Self {
        Self {
            tool_type: match val.tool_type.as_str() {
                "function" => liter_llm::types::ToolType::Function,
                other => {
                    ext_php_rs::exception::PhpException::default(format!(
                        "invalid value {other:?} for field 'tool_type' of ToolType: expected one of [function]"
                    ))
                    .throw()
                    .expect("failed to throw PHP exception for invalid enum string value");
                    Default::default()
                }
            },
            function: val.function.into(),
        }
    }
}

#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<liter_llm::types::ChatCompletionTool> for ChatCompletionTool {
    fn from(val: liter_llm::types::ChatCompletionTool) -> Self {
        Self {
            tool_type: serde_json::to_value(val.tool_type)
                .ok()
                .and_then(|s| s.as_str().map(String::from))
                .unwrap_or_default(),
            function: val.function.into(),
        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::useless_conversion)]
impl From<Choice> for liter_llm::types::Choice {
    fn from(val: Choice) -> Self {
        Self {
            index: val.index,
            message: val.message.into(),
            finish_reason: val.finish_reason.as_deref().map(|s| match s { "stop" => liter_llm::types::FinishReason::Stop,"length" => liter_llm::types::FinishReason::Length,"tool_calls" => liter_llm::types::FinishReason::ToolCalls,"content_filter" => liter_llm::types::FinishReason::ContentFilter,"function_call" => liter_llm::types::FinishReason::FunctionCall,"other" => liter_llm::types::FinishReason::Other,other => {
    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'finish_reason' of FinishReason: expected one of [stop, length, tool_calls, content_filter, function_call, other]"
    ))
    .throw()
    .expect("failed to throw PHP exception for invalid enum string value");
    Default::default()
}
 })
,
            logprobs: val.logprobs,
            ..Default::default()        }
    }
}

#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<liter_llm::types::Choice> for Choice {
    fn from(val: liter_llm::types::Choice) -> Self {
        Self {
            index: val.index,
            message: val.message.into(),
            finish_reason: val.finish_reason.as_ref().map(|v| {
                serde_json::to_value(v)
                    .ok()
                    .and_then(|s| s.as_str().map(String::from))
                    .unwrap_or_default()
            }),
            logprobs: val.logprobs,
        }
    }
}

#[allow(clippy::needless_update)]
impl From<CreateBatchRequest> for liter_llm::types::CreateBatchRequest {
    fn from(val: CreateBatchRequest) -> Self {
        Self {
            input_file_id: val.input_file_id,
            endpoint: val.endpoint,
            completion_window: val.completion_window,
            metadata: val.metadata,
            ..Default::default()
        }
    }
}

impl From<liter_llm::types::CreateBatchRequest> for CreateBatchRequest {
    fn from(val: liter_llm::types::CreateBatchRequest) -> Self {
        Self {
            input_file_id: val.input_file_id.to_string(),
            endpoint: val.endpoint.to_string(),
            completion_window: val.completion_window.to_string(),
            metadata: val.metadata,
        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::useless_conversion)]
impl From<CreateFileRequest> for liter_llm::types::CreateFileRequest {
    fn from(val: CreateFileRequest) -> Self {
        Self {
            file: val.file,
            purpose: match val.purpose.as_str() {
                "assistants" => liter_llm::types::FilePurpose::Assistants,
                "batch" => liter_llm::types::FilePurpose::Batch,
                "fine-tune" => liter_llm::types::FilePurpose::FineTune,
                "vision" => liter_llm::types::FilePurpose::Vision,
                other => {
                    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'purpose' of FilePurpose: expected one of [assistants, batch, fine-tune, vision]"
    ))
    .throw()
    .expect("failed to throw PHP exception for invalid enum string value");
                    Default::default()
                }
            },
            filename: val.filename,
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::types::CreateFileRequest> for CreateFileRequest {
    fn from(val: liter_llm::types::CreateFileRequest) -> Self {
        Self {
            file: val.file.to_string(),
            purpose: serde_json::to_value(val.purpose)
                .ok()
                .and_then(|s| s.as_str().map(String::from))
                .unwrap_or_default(),
            filename: val.filename.map(|v| v.to_string()),
        }
    }
}

#[allow(clippy::needless_update)]
impl From<CreateImageRequest> for liter_llm::types::CreateImageRequest {
    fn from(val: CreateImageRequest) -> Self {
        Self {
            prompt: val.prompt,
            model: val.model,
            n: val.n,
            size: val.size,
            quality: val.quality,
            style: val.style,
            response_format: val.response_format,
            user: val.user,
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::types::CreateImageRequest> for CreateImageRequest {
    fn from(val: liter_llm::types::CreateImageRequest) -> Self {
        Self {
            prompt: val.prompt.to_string(),
            model: val.model.map(|v| v.to_string()),
            n: val.n,
            size: val.size.map(|v| v.to_string()),
            quality: val.quality.map(|v| v.to_string()),
            style: val.style.map(|v| v.to_string()),
            response_format: val.response_format.map(|v| v.to_string()),
            user: val.user.map(|v| v.to_string()),
        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<CreateResponseRequest> for liter_llm::types::CreateResponseRequest {
    fn from(val: CreateResponseRequest) -> Self {
        Self {
            model: val.model,
            input: val.input,
            instructions: val.instructions,
            tools: val.tools.map(|v| v.into_iter().map(Into::into).collect()),
            temperature: val.temperature,
            max_output_tokens: val.max_output_tokens.map(|v| v as u64),
            metadata: val.metadata,
            extra_body: val.extra_body,
            stream: val.stream,
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<liter_llm::types::CreateResponseRequest> for CreateResponseRequest {
    fn from(val: liter_llm::types::CreateResponseRequest) -> Self {
        Self {
            model: val.model.to_string(),
            input: val.input,
            instructions: val.instructions.map(|v| v.to_string()),
            tools: val.tools.map(|v| v.into_iter().map(Into::into).collect()),
            temperature: val.temperature,
            max_output_tokens: val.max_output_tokens.map(|v| v as i64),
            metadata: val.metadata,
            extra_body: val.extra_body,
            stream: val.stream,
        }
    }
}

#[allow(clippy::needless_update)]
impl From<CreateSpeechRequest> for liter_llm::types::CreateSpeechRequest {
    fn from(val: CreateSpeechRequest) -> Self {
        Self {
            model: val.model,
            input: val.input,
            voice: val.voice,
            response_format: val.response_format,
            speed: val.speed,
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::types::CreateSpeechRequest> for CreateSpeechRequest {
    fn from(val: liter_llm::types::CreateSpeechRequest) -> Self {
        Self {
            model: val.model.to_string(),
            input: val.input.to_string(),
            voice: val.voice.to_string(),
            response_format: val.response_format.map(|v| v.to_string()),
            speed: val.speed,
        }
    }
}

#[allow(clippy::needless_update)]
impl From<CreateTranscriptionRequest> for liter_llm::types::CreateTranscriptionRequest {
    fn from(val: CreateTranscriptionRequest) -> Self {
        Self {
            model: val.model,
            file: val.file,
            language: val.language,
            prompt: val.prompt,
            response_format: val.response_format,
            temperature: val.temperature,
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::types::CreateTranscriptionRequest> for CreateTranscriptionRequest {
    fn from(val: liter_llm::types::CreateTranscriptionRequest) -> Self {
        Self {
            model: val.model.to_string(),
            file: val.file.to_string(),
            language: val.language.map(|v| v.to_string()),
            prompt: val.prompt.map(|v| v.to_string()),
            response_format: val.response_format.map(|v| v.to_string()),
            temperature: val.temperature,
        }
    }
}

#[allow(clippy::useless_conversion)]
impl From<CustomProviderConfig> for liter_llm::provider::custom::CustomProviderConfig {
    fn from(val: CustomProviderConfig) -> Self {
        Self {
            name: val.name,
            base_url: val.base_url,
            auth_header: match val.auth_header.as_str() {
                "Bearer" | "bearer" => liter_llm::provider::custom::AuthHeaderFormat::Bearer,
                "ApiKey" | "apikey" => liter_llm::provider::custom::AuthHeaderFormat::ApiKey(Default::default()),
                "None" | "none" => liter_llm::provider::custom::AuthHeaderFormat::None,
                other => {
                    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'auth_header' of AuthHeaderFormat: expected one of [Bearer, ApiKey, None]"
    ))
    .throw()
    .expect("failed to throw PHP exception for invalid enum string value");
                    Default::default()
                }
            },
            model_prefixes: val.model_prefixes.into_iter().collect(),
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::provider::custom::CustomProviderConfig> for CustomProviderConfig {
    fn from(val: liter_llm::provider::custom::CustomProviderConfig) -> Self {
        Self {
            name: val.name.to_string(),
            base_url: val.base_url.to_string(),
            auth_header: serde_json::to_value(val.auth_header)
                .ok()
                .and_then(|s| s.as_str().map(String::from))
                .unwrap_or_default(),
            model_prefixes: val.model_prefixes.into_iter().collect(),
        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::useless_conversion)]
impl From<DecodedDataUrl> for liter_llm::image::DecodedDataUrl {
    fn from(val: DecodedDataUrl) -> Self {
        Self {
            mime: val.mime,
            data: val.data.to_vec().into(),
            ..Default::default()
        }
    }
}

#[allow(clippy::useless_conversion)]
impl From<liter_llm::image::DecodedDataUrl> for DecodedDataUrl {
    fn from(val: liter_llm::image::DecodedDataUrl) -> Self {
        Self {
            mime: val.mime.to_string(),
            data: val.data.to_vec().into(),
        }
    }
}

#[allow(clippy::needless_update)]
impl From<DeleteResponse> for liter_llm::types::DeleteResponse {
    fn from(val: DeleteResponse) -> Self {
        Self {
            id: val.id,
            object: val.object,
            deleted: val.deleted,
            ..Default::default()
        }
    }
}

impl From<liter_llm::types::DeleteResponse> for DeleteResponse {
    fn from(val: liter_llm::types::DeleteResponse) -> Self {
        Self {
            id: val.id.to_string(),
            object: val.object.to_string(),
            deleted: val.deleted,
        }
    }
}

#[allow(clippy::needless_update)]
impl From<DeveloperMessage> for liter_llm::types::DeveloperMessage {
    fn from(val: DeveloperMessage) -> Self {
        Self {
            content: val.content,
            name: val.name,
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::types::DeveloperMessage> for DeveloperMessage {
    fn from(val: liter_llm::types::DeveloperMessage) -> Self {
        Self {
            content: val.content.to_string(),
            name: val.name.map(|v| v.to_string()),
        }
    }
}

#[allow(clippy::needless_update)]
impl From<DocumentContent> for liter_llm::types::DocumentContent {
    fn from(val: DocumentContent) -> Self {
        Self {
            data: val.data,
            media_type: val.media_type,
            ..Default::default()
        }
    }
}

impl From<liter_llm::types::DocumentContent> for DocumentContent {
    fn from(val: liter_llm::types::DocumentContent) -> Self {
        Self {
            data: val.data.to_string(),
            media_type: val.media_type.to_string(),
        }
    }
}

impl From<EmbeddingObject> for liter_llm::types::EmbeddingObject {
    fn from(val: EmbeddingObject) -> Self {
        Self {
            object: val.object,
            embedding: val.embedding.into_iter().collect(),
            index: val.index,
        }
    }
}

impl From<liter_llm::types::EmbeddingObject> for EmbeddingObject {
    fn from(val: liter_llm::types::EmbeddingObject) -> Self {
        Self {
            object: val.object.to_string(),
            embedding: val.embedding.into_iter().collect(),
            index: val.index,
        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::useless_conversion)]
impl From<EmbeddingRequest> for liter_llm::types::EmbeddingRequest {
    fn from(val: EmbeddingRequest) -> Self {
        Self {
            model: val.model,
            input: serde_json::from_value(val.input).unwrap_or_default(),
            encoding_format: val.encoding_format.as_deref().map(|s| match s {
                "float" => liter_llm::types::EmbeddingFormat::Float,
                "base64" => liter_llm::types::EmbeddingFormat::Base64,
                other => {
                    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'encoding_format' of EmbeddingFormat: expected one of [float, base64]"
    ))
    .throw()
    .expect("failed to throw PHP exception for invalid enum string value");
                    liter_llm::types::EmbeddingFormat::Float
                }
            }),
            dimensions: val.dimensions,
            user: val.user,
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::types::EmbeddingRequest> for EmbeddingRequest {
    fn from(val: liter_llm::types::EmbeddingRequest) -> Self {
        Self {
            model: val.model.to_string(),
            input: serde_json::to_value(&val.input).unwrap_or(serde_json::Value::Null),
            encoding_format: val.encoding_format.as_ref().map(|v| {
                serde_json::to_value(v)
                    .ok()
                    .and_then(|s| s.as_str().map(String::from))
                    .unwrap_or_default()
            }),
            dimensions: val.dimensions,
            user: val.user.map(|v| v.to_string()),
        }
    }
}

#[allow(clippy::useless_conversion)]
impl From<EmbeddingResponse> for liter_llm::types::EmbeddingResponse {
    fn from(val: EmbeddingResponse) -> Self {
        Self {
            object: val.object,
            data: val.data.into_iter().map(Into::into).collect(),
            model: val.model,
            usage: val.usage.map(Into::into),
        }
    }
}

#[allow(clippy::useless_conversion)]
impl From<liter_llm::types::EmbeddingResponse> for EmbeddingResponse {
    fn from(val: liter_llm::types::EmbeddingResponse) -> Self {
        Self {
            object: val.object.to_string(),
            data: val.data.into_iter().map(Into::into).collect(),
            model: val.model.to_string(),
            usage: val.usage.map(Into::into),
        }
    }
}

#[allow(clippy::needless_update)]
impl From<FileListQuery> for liter_llm::types::FileListQuery {
    fn from(val: FileListQuery) -> Self {
        Self {
            purpose: val.purpose,
            limit: val.limit,
            after: val.after,
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::types::FileListQuery> for FileListQuery {
    fn from(val: liter_llm::types::FileListQuery) -> Self {
        Self {
            purpose: val.purpose.map(|v| v.to_string()),
            limit: val.limit,
            after: val.after.map(|v| v.to_string()),
        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::useless_conversion)]
impl From<FileListResponse> for liter_llm::types::FileListResponse {
    fn from(val: FileListResponse) -> Self {
        Self {
            object: val.object,
            data: val.data.into_iter().map(Into::into).collect(),
            has_more: val.has_more,
            ..Default::default()
        }
    }
}

#[allow(clippy::useless_conversion)]
impl From<liter_llm::types::FileListResponse> for FileListResponse {
    fn from(val: liter_llm::types::FileListResponse) -> Self {
        Self {
            object: val.object.to_string(),
            data: val.data.into_iter().map(Into::into).collect(),
            has_more: val.has_more,
        }
    }
}

#[allow(clippy::needless_update)]
impl From<FileObject> for liter_llm::types::FileObject {
    fn from(val: FileObject) -> Self {
        Self {
            id: val.id,
            object: val.object,
            bytes: val.bytes as u64,
            created_at: val.created_at as u64,
            filename: val.filename,
            purpose: val.purpose,
            status: val.status,
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::types::FileObject> for FileObject {
    fn from(val: liter_llm::types::FileObject) -> Self {
        Self {
            id: val.id.to_string(),
            object: val.object.to_string(),
            bytes: val.bytes as i64,
            created_at: val.created_at as i64,
            filename: val.filename.to_string(),
            purpose: val.purpose.to_string(),
            status: val.status.map(|v| v.to_string()),
        }
    }
}

impl From<FunctionCall> for liter_llm::types::FunctionCall {
    fn from(val: FunctionCall) -> Self {
        Self {
            name: val.name,
            arguments: val.arguments,
        }
    }
}

impl From<liter_llm::types::FunctionCall> for FunctionCall {
    fn from(val: liter_llm::types::FunctionCall) -> Self {
        Self {
            name: val.name.to_string(),
            arguments: val.arguments.to_string(),
        }
    }
}

impl From<FunctionDefinition> for liter_llm::types::FunctionDefinition {
    fn from(val: FunctionDefinition) -> Self {
        Self {
            name: val.name,
            description: val.description,
            parameters: val.parameters,
            strict: val.strict,
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::types::FunctionDefinition> for FunctionDefinition {
    fn from(val: liter_llm::types::FunctionDefinition) -> Self {
        Self {
            name: val.name.to_string(),
            description: val.description.map(|v| v.to_string()),
            parameters: val.parameters,
            strict: val.strict,
        }
    }
}

#[allow(clippy::needless_update)]
impl From<FunctionMessage> for liter_llm::types::FunctionMessage {
    fn from(val: FunctionMessage) -> Self {
        Self {
            content: val.content,
            name: val.name,
            ..Default::default()
        }
    }
}

impl From<liter_llm::types::FunctionMessage> for FunctionMessage {
    fn from(val: liter_llm::types::FunctionMessage) -> Self {
        Self {
            content: val.content.to_string(),
            name: val.name.to_string(),
        }
    }
}

#[allow(clippy::needless_update)]
impl From<Image> for liter_llm::types::Image {
    fn from(val: Image) -> Self {
        Self {
            url: val.url,
            b64_json: val.b64_json,
            revised_prompt: val.revised_prompt,
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::types::Image> for Image {
    fn from(val: liter_llm::types::Image) -> Self {
        Self {
            url: val.url.map(|v| v.to_string()),
            b64_json: val.b64_json.map(|v| v.to_string()),
            revised_prompt: val.revised_prompt.map(|v| v.to_string()),
        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::useless_conversion)]
impl From<ImageUrl> for liter_llm::types::ImageUrl {
    fn from(val: ImageUrl) -> Self {
        Self {
            url: val.url,
            detail: val.detail.as_deref().map(|s| match s {
                "low" => liter_llm::types::ImageDetail::Low,
                "high" => liter_llm::types::ImageDetail::High,
                "auto" => liter_llm::types::ImageDetail::Auto,
                other => {
                    ext_php_rs::exception::PhpException::default(format!(
                        "invalid value {other:?} for field 'detail' of ImageDetail: expected one of [low, high, auto]"
                    ))
                    .throw()
                    .expect("failed to throw PHP exception for invalid enum string value");
                    liter_llm::types::ImageDetail::Low
                }
            }),
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::types::ImageUrl> for ImageUrl {
    fn from(val: liter_llm::types::ImageUrl) -> Self {
        Self {
            url: val.url.to_string(),
            detail: val.detail.as_ref().map(|v| {
                serde_json::to_value(v)
                    .ok()
                    .and_then(|s| s.as_str().map(String::from))
                    .unwrap_or_default()
            }),
        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::useless_conversion)]
impl From<ImagesResponse> for liter_llm::types::ImagesResponse {
    fn from(val: ImagesResponse) -> Self {
        Self {
            created: val.created as u64,
            data: val.data.into_iter().map(Into::into).collect(),
            ..Default::default()
        }
    }
}

#[allow(clippy::useless_conversion)]
impl From<liter_llm::types::ImagesResponse> for ImagesResponse {
    fn from(val: liter_llm::types::ImagesResponse) -> Self {
        Self {
            created: val.created as i64,
            data: val.data.into_iter().map(Into::into).collect(),
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::InFlightLimitConfig> for InFlightLimitConfig {
    fn from(val: liter_llm::InFlightLimitConfig) -> Self {
        Self {
            max_in_flight: val.max_in_flight.map(|v| v as i64),
        }
    }
}

impl From<liter_llm::tower::IntentPrototype> for IntentPrototype {
    fn from(val: liter_llm::tower::IntentPrototype) -> Self {
        Self {
            name: val.name.to_string(),
            embedding: val.embedding.into_iter().collect(),
            model: val.model.to_string(),
        }
    }
}

#[allow(clippy::needless_update)]
impl From<JsonSchemaFormat> for liter_llm::types::JsonSchemaFormat {
    fn from(val: JsonSchemaFormat) -> Self {
        Self {
            name: val.name,
            description: val.description,
            schema: val.schema,
            strict: val.strict,
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::types::JsonSchemaFormat> for JsonSchemaFormat {
    fn from(val: liter_llm::types::JsonSchemaFormat) -> Self {
        Self {
            name: val.name.to_string(),
            description: val.description.map(|v| v.to_string()),
            schema: val.schema,
            strict: val.strict,
        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::redundant_closure)]
impl From<LlmBudgetConfig> for liter_llm::client::LlmBudgetConfig {
    fn from(val: LlmBudgetConfig) -> Self {
        Self {
            global_limit: val.global_limit,
            model_limits: val.model_limits.map(|m| m.into_iter().collect()),
            enforcement: val.enforcement,
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::client::LlmBudgetConfig> for LlmBudgetConfig {
    fn from(val: liter_llm::client::LlmBudgetConfig) -> Self {
        Self {
            global_limit: val.global_limit,
            model_limits: val.model_limits.map(|m| m.into_iter().collect()),
            enforcement: val.enforcement.map(|v| v.to_string()),
        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<LlmCacheConfig> for liter_llm::client::LlmCacheConfig {
    fn from(val: LlmCacheConfig) -> Self {
        Self {
            max_entries: val.max_entries.map(|v| v as usize),
            ttl_seconds: val.ttl_seconds.map(|v| v as u64),
            backend: val.backend,
            backend_config: val
                .backend_config
                .map(|m| m.into_iter().map(|(k, v)| (k.into(), v.into())).collect()),
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<liter_llm::client::LlmCacheConfig> for LlmCacheConfig {
    fn from(val: liter_llm::client::LlmCacheConfig) -> Self {
        Self {
            max_entries: val.max_entries.map(|v| v as i64),
            ttl_seconds: val.ttl_seconds.map(|v| v as i64),
            backend: val.backend.map(|v| v.to_string()),
            backend_config: val
                .backend_config
                .map(|m| m.into_iter().map(|(k, v)| (k.into(), v.into())).collect()),
        }
    }
}

#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<liter_llm::client::LlmConfig> for LlmConfig {
    fn from(val: liter_llm::client::LlmConfig) -> Self {
        Self {
            model: val.model.to_string(),
            api_key: val.api_key.map(|v| v.to_string()),
            base_url: val.base_url.map(|v| v.to_string()),
            timeout_secs: val.timeout_secs.map(|v| v as i64),
            max_retries: val.max_retries,
            temperature: val.temperature,
            max_tokens: val.max_tokens.map(|v| v as i64),
            load_env: val.load_env,
            headers: val
                .headers
                .map(|m| m.into_iter().map(|(k, v)| (k.into(), v.into())).collect()),
            providers: val.providers.map(|v| v.into_iter().map(Into::into).collect()),
            cache: val.cache.map(Into::into),
            budget: val.budget.map(Into::into),
            rate_limit: val.rate_limit.map(Into::into),
            in_flight_limit: val.in_flight_limit.map(Into::into),
            cost_tracking: val.cost_tracking,
            tracing: val.tracing,
            cooldown_secs: val.cooldown_secs.map(|v| v as i64),
            health_check_secs: val.health_check_secs.map(|v| v as i64),
            bedrock: val.bedrock.map(Into::into),
        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::redundant_closure)]
impl From<LlmInFlightLimitConfig> for liter_llm::client::LlmInFlightLimitConfig {
    fn from(val: LlmInFlightLimitConfig) -> Self {
        Self {
            max_in_flight: val.max_in_flight.map(|v| v as usize),
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::client::LlmInFlightLimitConfig> for LlmInFlightLimitConfig {
    fn from(val: liter_llm::client::LlmInFlightLimitConfig) -> Self {
        Self {
            max_in_flight: val.max_in_flight.map(|v| v as i64),
        }
    }
}

#[allow(clippy::needless_update)]
impl From<LlmProviderConfig> for liter_llm::client::LlmProviderConfig {
    fn from(val: LlmProviderConfig) -> Self {
        Self {
            name: val.name,
            base_url: val.base_url,
            auth_header: val.auth_header,
            model_prefixes: val.model_prefixes.into_iter().collect(),
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::client::LlmProviderConfig> for LlmProviderConfig {
    fn from(val: liter_llm::client::LlmProviderConfig) -> Self {
        Self {
            name: val.name.to_string(),
            base_url: val.base_url.to_string(),
            auth_header: val.auth_header.map(|v| v.to_string()),
            model_prefixes: val.model_prefixes.into_iter().collect(),
        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::redundant_closure)]
impl From<LlmRateLimitConfig> for liter_llm::client::LlmRateLimitConfig {
    fn from(val: LlmRateLimitConfig) -> Self {
        Self {
            rpm: val.rpm,
            tpm: val.tpm.map(|v| v as u64),
            window_seconds: val.window_seconds.map(|v| v as u64),
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::client::LlmRateLimitConfig> for LlmRateLimitConfig {
    fn from(val: liter_llm::client::LlmRateLimitConfig) -> Self {
        Self {
            rpm: val.rpm,
            tpm: val.tpm.map(|v| v as i64),
            window_seconds: val.window_seconds.map(|v| v as i64),
        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<ModelInfo> for liter_llm::ModelInfo {
    fn from(val: ModelInfo) -> Self {
        Self {
            input_cost_per_token: val.input_cost_per_token,
            output_cost_per_token: val.output_cost_per_token,
            cache_read_input_token_cost: val.cache_read_input_token_cost,
            cache_creation_input_token_cost: val.cache_creation_input_token_cost,
            input_cost_per_audio_token: val.input_cost_per_audio_token,
            output_cost_per_audio_token: val.output_cost_per_audio_token,
            output_cost_per_reasoning_token: val.output_cost_per_reasoning_token,
            max_tokens: val.max_tokens.map(|v| v as u64),
            max_input_tokens: val.max_input_tokens.map(|v| v as u64),
            max_output_tokens: val.max_output_tokens.map(|v| v as u64),
            mode: val.mode,
            supports_vision: val.supports_vision,
            supports_function_calling: val.supports_function_calling,
            supports_reasoning: val.supports_reasoning,
            supports_structured_output: val.supports_structured_output,
            supports_audio_input: val.supports_audio_input,
            supports_audio_output: val.supports_audio_output,
            supports_prompt_caching: val.supports_prompt_caching,
            tiers: val.tiers.into_iter().map(Into::into).collect(),
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<liter_llm::ModelInfo> for ModelInfo {
    fn from(val: liter_llm::ModelInfo) -> Self {
        Self {
            input_cost_per_token: val.input_cost_per_token,
            output_cost_per_token: val.output_cost_per_token,
            cache_read_input_token_cost: val.cache_read_input_token_cost,
            cache_creation_input_token_cost: val.cache_creation_input_token_cost,
            input_cost_per_audio_token: val.input_cost_per_audio_token,
            output_cost_per_audio_token: val.output_cost_per_audio_token,
            output_cost_per_reasoning_token: val.output_cost_per_reasoning_token,
            max_tokens: val.max_tokens.map(|v| v as i64),
            max_input_tokens: val.max_input_tokens.map(|v| v as i64),
            max_output_tokens: val.max_output_tokens.map(|v| v as i64),
            mode: val.mode.map(|v| v.to_string()),
            supports_vision: val.supports_vision,
            supports_function_calling: val.supports_function_calling,
            supports_reasoning: val.supports_reasoning,
            supports_structured_output: val.supports_structured_output,
            supports_audio_input: val.supports_audio_input,
            supports_audio_output: val.supports_audio_output,
            supports_prompt_caching: val.supports_prompt_caching,
            tiers: val.tiers.into_iter().map(Into::into).collect(),
        }
    }
}

#[allow(clippy::needless_update)]
impl From<ModelObject> for liter_llm::types::ModelObject {
    fn from(val: ModelObject) -> Self {
        Self {
            id: val.id,
            object: val.object,
            created: val.created as u64,
            owned_by: val.owned_by,
            ..Default::default()
        }
    }
}

impl From<liter_llm::types::ModelObject> for ModelObject {
    fn from(val: liter_llm::types::ModelObject) -> Self {
        Self {
            id: val.id.to_string(),
            object: val.object.to_string(),
            created: val.created as i64,
            owned_by: val.owned_by.to_string(),
        }
    }
}

#[allow(clippy::needless_update)]
impl From<ModelTier> for liter_llm::ModelTier {
    fn from(val: ModelTier) -> Self {
        Self {
            min_context_tokens: val.min_context_tokens as u64,
            input_cost_per_token: val.input_cost_per_token,
            output_cost_per_token: val.output_cost_per_token,
            cache_read_input_token_cost: val.cache_read_input_token_cost,
            cache_creation_input_token_cost: val.cache_creation_input_token_cost,
            input_cost_per_audio_token: val.input_cost_per_audio_token,
            output_cost_per_audio_token: val.output_cost_per_audio_token,
            output_cost_per_reasoning_token: val.output_cost_per_reasoning_token,
            ..Default::default()
        }
    }
}

impl From<liter_llm::ModelTier> for ModelTier {
    fn from(val: liter_llm::ModelTier) -> Self {
        Self {
            min_context_tokens: val.min_context_tokens as i64,
            input_cost_per_token: val.input_cost_per_token,
            output_cost_per_token: val.output_cost_per_token,
            cache_read_input_token_cost: val.cache_read_input_token_cost,
            cache_creation_input_token_cost: val.cache_creation_input_token_cost,
            input_cost_per_audio_token: val.input_cost_per_audio_token,
            output_cost_per_audio_token: val.output_cost_per_audio_token,
            output_cost_per_reasoning_token: val.output_cost_per_reasoning_token,
        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::useless_conversion)]
impl From<ModelsListResponse> for liter_llm::types::ModelsListResponse {
    fn from(val: ModelsListResponse) -> Self {
        Self {
            object: val.object,
            data: val.data.into_iter().map(Into::into).collect(),
            ..Default::default()
        }
    }
}

#[allow(clippy::useless_conversion)]
impl From<liter_llm::types::ModelsListResponse> for ModelsListResponse {
    fn from(val: liter_llm::types::ModelsListResponse) -> Self {
        Self {
            object: val.object.to_string(),
            data: val.data.into_iter().map(Into::into).collect(),
        }
    }
}

#[allow(clippy::needless_update)]
impl From<ModerationCategories> for liter_llm::types::ModerationCategories {
    fn from(val: ModerationCategories) -> Self {
        Self {
            sexual: val.sexual,
            hate: val.hate,
            harassment: val.harassment,
            self_harm: val.self_harm,
            sexual_minors: val.sexual_minors,
            hate_threatening: val.hate_threatening,
            violence_graphic: val.violence_graphic,
            self_harm_intent: val.self_harm_intent,
            self_harm_instructions: val.self_harm_instructions,
            harassment_threatening: val.harassment_threatening,
            violence: val.violence,
            ..Default::default()
        }
    }
}

impl From<liter_llm::types::ModerationCategories> for ModerationCategories {
    fn from(val: liter_llm::types::ModerationCategories) -> Self {
        Self {
            sexual: val.sexual,
            hate: val.hate,
            harassment: val.harassment,
            self_harm: val.self_harm,
            sexual_minors: val.sexual_minors,
            hate_threatening: val.hate_threatening,
            violence_graphic: val.violence_graphic,
            self_harm_intent: val.self_harm_intent,
            self_harm_instructions: val.self_harm_instructions,
            harassment_threatening: val.harassment_threatening,
            violence: val.violence,
        }
    }
}

#[allow(clippy::needless_update)]
impl From<ModerationCategoryScores> for liter_llm::types::ModerationCategoryScores {
    fn from(val: ModerationCategoryScores) -> Self {
        Self {
            sexual: val.sexual,
            hate: val.hate,
            harassment: val.harassment,
            self_harm: val.self_harm,
            sexual_minors: val.sexual_minors,
            hate_threatening: val.hate_threatening,
            violence_graphic: val.violence_graphic,
            self_harm_intent: val.self_harm_intent,
            self_harm_instructions: val.self_harm_instructions,
            harassment_threatening: val.harassment_threatening,
            violence: val.violence,
            ..Default::default()
        }
    }
}

impl From<liter_llm::types::ModerationCategoryScores> for ModerationCategoryScores {
    fn from(val: liter_llm::types::ModerationCategoryScores) -> Self {
        Self {
            sexual: val.sexual,
            hate: val.hate,
            harassment: val.harassment,
            self_harm: val.self_harm,
            sexual_minors: val.sexual_minors,
            hate_threatening: val.hate_threatening,
            violence_graphic: val.violence_graphic,
            self_harm_intent: val.self_harm_intent,
            self_harm_instructions: val.self_harm_instructions,
            harassment_threatening: val.harassment_threatening,
            violence: val.violence,
        }
    }
}

#[allow(clippy::needless_update)]
impl From<ModerationRequest> for liter_llm::types::ModerationRequest {
    fn from(val: ModerationRequest) -> Self {
        Self {
            input: serde_json::from_value(val.input).unwrap_or_default(),
            model: val.model,
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::types::ModerationRequest> for ModerationRequest {
    fn from(val: liter_llm::types::ModerationRequest) -> Self {
        Self {
            input: serde_json::to_value(&val.input).unwrap_or(serde_json::Value::Null),
            model: val.model.map(|v| v.to_string()),
        }
    }
}

#[allow(clippy::useless_conversion)]
impl From<ModerationResponse> for liter_llm::types::ModerationResponse {
    fn from(val: ModerationResponse) -> Self {
        Self {
            id: val.id,
            model: val.model,
            results: val.results.into_iter().map(Into::into).collect(),
        }
    }
}

#[allow(clippy::useless_conversion)]
impl From<liter_llm::types::ModerationResponse> for ModerationResponse {
    fn from(val: liter_llm::types::ModerationResponse) -> Self {
        Self {
            id: val.id.to_string(),
            model: val.model.to_string(),
            results: val.results.into_iter().map(Into::into).collect(),
        }
    }
}

#[allow(clippy::useless_conversion)]
impl From<ModerationResult> for liter_llm::types::ModerationResult {
    fn from(val: ModerationResult) -> Self {
        Self {
            flagged: val.flagged,
            categories: val.categories.into(),
            category_scores: val.category_scores.into(),
        }
    }
}

#[allow(clippy::useless_conversion)]
impl From<liter_llm::types::ModerationResult> for ModerationResult {
    fn from(val: liter_llm::types::ModerationResult) -> Self {
        Self {
            flagged: val.flagged,
            categories: val.categories.into(),
            category_scores: val.category_scores.into(),
        }
    }
}

impl From<OcrImage> for liter_llm::types::OcrImage {
    fn from(val: OcrImage) -> Self {
        Self {
            id: val.id,
            image_base64: val.image_base64,
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::types::OcrImage> for OcrImage {
    fn from(val: liter_llm::types::OcrImage) -> Self {
        Self {
            id: val.id.to_string(),
            image_base64: val.image_base64.map(|v| v.to_string()),
        }
    }
}

#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<OcrPage> for liter_llm::types::OcrPage {
    fn from(val: OcrPage) -> Self {
        Self {
            index: val.index,
            markdown: val.markdown,
            images: val.images.map(|v| v.into_iter().map(Into::into).collect()),
            dimensions: val.dimensions.map(Into::into),
        }
    }
}

#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<liter_llm::types::OcrPage> for OcrPage {
    fn from(val: liter_llm::types::OcrPage) -> Self {
        Self {
            index: val.index,
            markdown: val.markdown.to_string(),
            images: val.images.map(|v| v.into_iter().map(Into::into).collect()),
            dimensions: val.dimensions.map(Into::into),
        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<OcrRequest> for liter_llm::types::OcrRequest {
    fn from(val: OcrRequest) -> Self {
        Self {
            model: val.model,
            document: val.document.into(),
            pages: val.pages.map(|v| v.into_iter().collect()),
            include_image_base64: val.include_image_base64,
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<liter_llm::types::OcrRequest> for OcrRequest {
    fn from(val: liter_llm::types::OcrRequest) -> Self {
        Self {
            model: val.model.to_string(),
            document: val.document.into(),
            pages: val.pages.map(|v| v.into_iter().collect()),
            include_image_base64: val.include_image_base64,
        }
    }
}

#[allow(clippy::useless_conversion)]
impl From<OcrResponse> for liter_llm::types::OcrResponse {
    fn from(val: OcrResponse) -> Self {
        Self {
            pages: val.pages.into_iter().map(Into::into).collect(),
            model: val.model,
            usage: val.usage.map(Into::into),
        }
    }
}

#[allow(clippy::useless_conversion)]
impl From<liter_llm::types::OcrResponse> for OcrResponse {
    fn from(val: liter_llm::types::OcrResponse) -> Self {
        Self {
            pages: val.pages.into_iter().map(Into::into).collect(),
            model: val.model.to_string(),
            usage: val.usage.map(Into::into),
        }
    }
}

impl From<PageDimensions> for liter_llm::types::PageDimensions {
    fn from(val: PageDimensions) -> Self {
        Self {
            width: val.width,
            height: val.height,
        }
    }
}

impl From<liter_llm::types::PageDimensions> for PageDimensions {
    fn from(val: liter_llm::types::PageDimensions) -> Self {
        Self {
            width: val.width,
            height: val.height,
        }
    }
}

#[allow(clippy::needless_update)]
impl From<PromptTokensDetails> for liter_llm::types::PromptTokensDetails {
    fn from(val: PromptTokensDetails) -> Self {
        Self {
            cached_tokens: val.cached_tokens as u64,
            audio_tokens: val.audio_tokens as u64,
            ..Default::default()
        }
    }
}

impl From<liter_llm::types::PromptTokensDetails> for PromptTokensDetails {
    fn from(val: liter_llm::types::PromptTokensDetails) -> Self {
        Self {
            cached_tokens: val.cached_tokens as i64,
            audio_tokens: val.audio_tokens as i64,
        }
    }
}

#[allow(clippy::needless_update)]
impl From<ProviderCapabilities> for liter_llm::provider::ProviderCapabilities {
    fn from(val: ProviderCapabilities) -> Self {
        Self {
            vision: val.vision,
            reasoning: val.reasoning,
            structured_output: val.structured_output,
            function_calling: val.function_calling,
            audio_in: val.audio_in,
            audio_out: val.audio_out,
            video_in: val.video_in,
            ..Default::default()
        }
    }
}

impl From<liter_llm::provider::ProviderCapabilities> for ProviderCapabilities {
    fn from(val: liter_llm::provider::ProviderCapabilities) -> Self {
        Self {
            vision: val.vision,
            reasoning: val.reasoning,
            structured_output: val.structured_output,
            function_calling: val.function_calling,
            audio_in: val.audio_in,
            audio_out: val.audio_out,
            video_in: val.video_in,
        }
    }
}

#[allow(clippy::useless_conversion)]
impl From<ProviderConfig> for liter_llm::provider::ProviderConfig {
    fn from(val: ProviderConfig) -> Self {
        Self {
            name: val.name,
            display_name: val.display_name,
            base_url: val.base_url,
            auth: val.auth.map(Into::into),
            endpoints: val.endpoints.map(|v| v.into_iter().collect()),
            model_prefixes: val.model_prefixes.map(|v| v.into_iter().collect()),
            param_mappings: val
                .param_mappings
                .map(|m| m.into_iter().map(|(k, v)| (k.into(), v.into())).collect()),
        }
    }
}

#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<liter_llm::provider::ProviderConfig> for ProviderConfig {
    fn from(val: liter_llm::provider::ProviderConfig) -> Self {
        Self {
            name: val.name.to_string(),
            display_name: val.display_name.map(|v| v.to_string()),
            base_url: val.base_url.map(|v| v.to_string()),
            auth: val.auth.map(Into::into),
            endpoints: val.endpoints.map(|v| v.into_iter().collect()),
            model_prefixes: val.model_prefixes.map(|v| v.into_iter().collect()),
            param_mappings: val
                .param_mappings
                .map(|m| m.into_iter().map(|(k, v)| (k.into(), v.into())).collect()),
        }
    }
}

#[allow(clippy::field_reassign_with_default, clippy::let_and_return)]
#[allow(clippy::redundant_closure)]
impl From<RateLimitConfig> for liter_llm::RateLimitConfig {
    fn from(val: RateLimitConfig) -> Self {
        let mut __result = liter_llm::RateLimitConfig::default();
        __result.rpm = val.rpm;
        __result.tpm = val.tpm.map(|v| v as u64);
        if let Some(__v) = val.window {
            __result.window = std::time::Duration::from_millis(__v as u64);
        }
        __result
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::RateLimitConfig> for RateLimitConfig {
    fn from(val: liter_llm::RateLimitConfig) -> Self {
        Self {
            rpm: val.rpm,
            tpm: val.tpm.map(|v| v as i64),
            window: Some(val.window.as_millis() as u64 as i64),
        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::redundant_closure)]
impl From<RerankRequest> for liter_llm::types::RerankRequest {
    fn from(val: RerankRequest) -> Self {
        Self {
            model: val.model,
            query: val.query,
            documents: val
                .documents
                .into_iter()
                .map(|x| serde_json::from_value(x).unwrap_or_default())
                .collect(),
            top_n: val.top_n,
            return_documents: val.return_documents,
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::types::RerankRequest> for RerankRequest {
    fn from(val: liter_llm::types::RerankRequest) -> Self {
        Self {
            model: val.model.to_string(),
            query: val.query.to_string(),
            documents: val
                .documents
                .iter()
                .map(|x| serde_json::to_value(x).unwrap_or(serde_json::Value::Null))
                .collect(),
            top_n: val.top_n,
            return_documents: val.return_documents,
        }
    }
}

#[allow(clippy::useless_conversion)]
impl From<RerankResponse> for liter_llm::types::RerankResponse {
    fn from(val: RerankResponse) -> Self {
        Self {
            id: val.id,
            results: val.results.into_iter().map(Into::into).collect(),
            meta: val.meta,
        }
    }
}

#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<liter_llm::types::RerankResponse> for RerankResponse {
    fn from(val: liter_llm::types::RerankResponse) -> Self {
        Self {
            id: val.id.map(|v| v.to_string()),
            results: val.results.into_iter().map(Into::into).collect(),
            meta: val.meta,
        }
    }
}

#[allow(clippy::useless_conversion)]
impl From<RerankResult> for liter_llm::types::RerankResult {
    fn from(val: RerankResult) -> Self {
        Self {
            index: val.index,
            relevance_score: val.relevance_score,
            document: val.document.map(Into::into),
        }
    }
}

#[allow(clippy::useless_conversion)]
impl From<liter_llm::types::RerankResult> for RerankResult {
    fn from(val: liter_llm::types::RerankResult) -> Self {
        Self {
            index: val.index,
            relevance_score: val.relevance_score,
            document: val.document.map(Into::into),
        }
    }
}

impl From<RerankResultDocument> for liter_llm::types::RerankResultDocument {
    fn from(val: RerankResultDocument) -> Self {
        Self { text: val.text }
    }
}

impl From<liter_llm::types::RerankResultDocument> for RerankResultDocument {
    fn from(val: liter_llm::types::RerankResultDocument) -> Self {
        Self {
            text: val.text.to_string(),
        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::useless_conversion)]
impl From<ResponseObject> for liter_llm::types::ResponseObject {
    fn from(val: ResponseObject) -> Self {
        Self {
            id: val.id,
            object: val.object,
            created_at: val.created_at as u64,
            model: val.model,
            status: val.status,
            output: val.output.into_iter().map(Into::into).collect(),
            usage: val.usage.map(Into::into),
            error: val.error,
            ..Default::default()
        }
    }
}

#[allow(clippy::useless_conversion)]
impl From<liter_llm::types::ResponseObject> for ResponseObject {
    fn from(val: liter_llm::types::ResponseObject) -> Self {
        Self {
            id: val.id.to_string(),
            object: val.object.to_string(),
            created_at: val.created_at as i64,
            model: val.model.to_string(),
            status: val.status.to_string(),
            output: val.output.into_iter().map(Into::into).collect(),
            usage: val.usage.map(Into::into),
            error: val.error,
        }
    }
}

#[allow(clippy::needless_update)]
impl From<ResponseOutputItem> for liter_llm::types::ResponseOutputItem {
    fn from(val: ResponseOutputItem) -> Self {
        Self {
            item_type: val.item_type,
            content: val.content,
            ..Default::default()
        }
    }
}

impl From<liter_llm::types::ResponseOutputItem> for ResponseOutputItem {
    fn from(val: liter_llm::types::ResponseOutputItem) -> Self {
        Self {
            item_type: val.item_type.to_string(),
            content: val.content,
        }
    }
}

#[allow(clippy::needless_update)]
impl From<ResponseTool> for liter_llm::types::ResponseTool {
    fn from(val: ResponseTool) -> Self {
        Self {
            tool_type: val.tool_type,
            config: val.config,
            ..Default::default()
        }
    }
}

impl From<liter_llm::types::ResponseTool> for ResponseTool {
    fn from(val: liter_llm::types::ResponseTool) -> Self {
        Self {
            tool_type: val.tool_type.to_string(),
            config: val.config,
        }
    }
}

#[allow(clippy::needless_update)]
impl From<ResponseUsage> for liter_llm::types::ResponseUsage {
    fn from(val: ResponseUsage) -> Self {
        Self {
            input_tokens: val.input_tokens as u64,
            output_tokens: val.output_tokens as u64,
            total_tokens: val.total_tokens as u64,
            ..Default::default()
        }
    }
}

impl From<liter_llm::types::ResponseUsage> for ResponseUsage {
    fn from(val: liter_llm::types::ResponseUsage) -> Self {
        Self {
            input_tokens: val.input_tokens as i64,
            output_tokens: val.output_tokens as i64,
            total_tokens: val.total_tokens as i64,
        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::redundant_closure)]
impl From<SearchRequest> for liter_llm::types::SearchRequest {
    fn from(val: SearchRequest) -> Self {
        Self {
            model: val.model,
            query: val.query,
            max_results: val.max_results,
            search_domain_filter: val.search_domain_filter.map(|v| v.into_iter().collect()),
            country: val.country,
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::types::SearchRequest> for SearchRequest {
    fn from(val: liter_llm::types::SearchRequest) -> Self {
        Self {
            model: val.model.to_string(),
            query: val.query.to_string(),
            max_results: val.max_results,
            search_domain_filter: val.search_domain_filter.map(|v| v.into_iter().collect()),
            country: val.country.map(|v| v.to_string()),
        }
    }
}

#[allow(clippy::useless_conversion)]
impl From<SearchResponse> for liter_llm::types::SearchResponse {
    fn from(val: SearchResponse) -> Self {
        Self {
            results: val.results.into_iter().map(Into::into).collect(),
            model: val.model,
        }
    }
}

#[allow(clippy::useless_conversion)]
impl From<liter_llm::types::SearchResponse> for SearchResponse {
    fn from(val: liter_llm::types::SearchResponse) -> Self {
        Self {
            results: val.results.into_iter().map(Into::into).collect(),
            model: val.model.to_string(),
        }
    }
}

impl From<SearchResult> for liter_llm::types::SearchResult {
    fn from(val: SearchResult) -> Self {
        Self {
            title: val.title,
            url: val.url,
            snippet: val.snippet,
            date: val.date,
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::types::SearchResult> for SearchResult {
    fn from(val: liter_llm::types::SearchResult) -> Self {
        Self {
            title: val.title.to_string(),
            url: val.url.to_string(),
            snippet: val.snippet.to_string(),
            date: val.date.map(|v| v.to_string()),
        }
    }
}

#[allow(clippy::needless_update)]
impl From<SpecificFunction> for liter_llm::types::SpecificFunction {
    fn from(val: SpecificFunction) -> Self {
        Self {
            name: val.name,
            ..Default::default()
        }
    }
}

impl From<liter_llm::types::SpecificFunction> for SpecificFunction {
    fn from(val: liter_llm::types::SpecificFunction) -> Self {
        Self {
            name: val.name.to_string(),
        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::useless_conversion)]
impl From<SpecificToolChoice> for liter_llm::types::SpecificToolChoice {
    fn from(val: SpecificToolChoice) -> Self {
        Self {
            choice_type: match val.choice_type.as_str() {
                "function" => liter_llm::types::ToolType::Function,
                other => {
                    ext_php_rs::exception::PhpException::default(format!(
                        "invalid value {other:?} for field 'choice_type' of ToolType: expected one of [function]"
                    ))
                    .throw()
                    .expect("failed to throw PHP exception for invalid enum string value");
                    Default::default()
                }
            },
            function: val.function.into(),
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<liter_llm::types::SpecificToolChoice> for SpecificToolChoice {
    fn from(val: liter_llm::types::SpecificToolChoice) -> Self {
        Self {
            choice_type: serde_json::to_value(val.choice_type)
                .ok()
                .and_then(|s| s.as_str().map(String::from))
                .unwrap_or_default(),
            function: val.function.into(),
        }
    }
}

#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<liter_llm::types::StreamChoice> for StreamChoice {
    fn from(val: liter_llm::types::StreamChoice) -> Self {
        Self {
            index: val.index,
            delta: val.delta.into(),
            finish_reason: val.finish_reason.as_ref().map(|v| {
                serde_json::to_value(v)
                    .ok()
                    .and_then(|s| s.as_str().map(String::from))
                    .unwrap_or_default()
            }),
        }
    }
}

#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<liter_llm::types::StreamDelta> for StreamDelta {
    fn from(val: liter_llm::types::StreamDelta) -> Self {
        Self {
            role: val.role.map(|v| v.to_string()),
            content: val.content.map(|v| v.to_string()),
            tool_calls: val.tool_calls.map(|v| v.into_iter().map(Into::into).collect()),
            function_call: val.function_call.map(Into::into),
            refusal: val.refusal.map(|v| v.to_string()),
            reasoning_content: val.reasoning_content.map(|v| v.to_string()),
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::types::StreamFunctionCall> for StreamFunctionCall {
    fn from(val: liter_llm::types::StreamFunctionCall) -> Self {
        Self {
            name: val.name.map(|v| v.to_string()),
            arguments: val.arguments.map(|v| v.to_string()),
        }
    }
}

#[allow(clippy::needless_update)]
impl From<StreamOptions> for liter_llm::types::StreamOptions {
    fn from(val: StreamOptions) -> Self {
        Self {
            include_usage: val.include_usage,
            ..Default::default()
        }
    }
}

impl From<liter_llm::types::StreamOptions> for StreamOptions {
    fn from(val: liter_llm::types::StreamOptions) -> Self {
        Self {
            include_usage: val.include_usage,
        }
    }
}

#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<liter_llm::types::StreamToolCall> for StreamToolCall {
    fn from(val: liter_llm::types::StreamToolCall) -> Self {
        Self {
            index: val.index,
            id: val.id.map(|v| v.to_string()),
            call_type: val.call_type.as_ref().map(|v| {
                serde_json::to_value(v)
                    .ok()
                    .and_then(|s| s.as_str().map(String::from))
                    .unwrap_or_default()
            }),
            function: val.function.map(Into::into),
        }
    }
}

#[allow(clippy::needless_update)]
impl From<SystemMessage> for liter_llm::types::SystemMessage {
    fn from(val: SystemMessage) -> Self {
        Self {
            content: serde_json::from_value(val.content).unwrap_or_default(),
            name: val.name,
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::types::SystemMessage> for SystemMessage {
    fn from(val: liter_llm::types::SystemMessage) -> Self {
        Self {
            content: serde_json::to_value(&val.content).unwrap_or(serde_json::Value::Null),
            name: val.name.map(|v| v.to_string()),
        }
    }
}

#[allow(clippy::useless_conversion)]
impl From<ToolCall> for liter_llm::types::ToolCall {
    fn from(val: ToolCall) -> Self {
        Self {
            id: val.id,
            call_type: match val.call_type.as_str() {
                "function" => liter_llm::types::ToolType::Function,
                other => {
                    ext_php_rs::exception::PhpException::default(format!(
                        "invalid value {other:?} for field 'call_type' of ToolType: expected one of [function]"
                    ))
                    .throw()
                    .expect("failed to throw PHP exception for invalid enum string value");
                    Default::default()
                }
            },
            function: val.function.into(),
        }
    }
}

#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<liter_llm::types::ToolCall> for ToolCall {
    fn from(val: liter_llm::types::ToolCall) -> Self {
        Self {
            id: val.id.to_string(),
            call_type: serde_json::to_value(val.call_type)
                .ok()
                .and_then(|s| s.as_str().map(String::from))
                .unwrap_or_default(),
            function: val.function.into(),
        }
    }
}

#[allow(clippy::needless_update)]
impl From<ToolMessage> for liter_llm::types::ToolMessage {
    fn from(val: ToolMessage) -> Self {
        Self {
            content: serde_json::from_value(val.content).unwrap_or_default(),
            tool_call_id: val.tool_call_id,
            name: val.name,
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::types::ToolMessage> for ToolMessage {
    fn from(val: liter_llm::types::ToolMessage) -> Self {
        Self {
            content: serde_json::to_value(&val.content).unwrap_or(serde_json::Value::Null),
            tool_call_id: val.tool_call_id.to_string(),
            name: val.name.map(|v| v.to_string()),
        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<TranscriptionResponse> for liter_llm::types::TranscriptionResponse {
    fn from(val: TranscriptionResponse) -> Self {
        Self {
            text: val.text,
            language: val.language,
            duration: val.duration,
            segments: val.segments.map(|v| v.into_iter().map(Into::into).collect()),
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<liter_llm::types::TranscriptionResponse> for TranscriptionResponse {
    fn from(val: liter_llm::types::TranscriptionResponse) -> Self {
        Self {
            text: val.text.to_string(),
            language: val.language.map(|v| v.to_string()),
            duration: val.duration,
            segments: val.segments.map(|v| v.into_iter().map(Into::into).collect()),
        }
    }
}

#[allow(clippy::needless_update)]
impl From<TranscriptionSegment> for liter_llm::types::TranscriptionSegment {
    fn from(val: TranscriptionSegment) -> Self {
        Self {
            id: val.id,
            start: val.start,
            end: val.end,
            text: val.text,
            ..Default::default()
        }
    }
}

impl From<liter_llm::types::TranscriptionSegment> for TranscriptionSegment {
    fn from(val: liter_llm::types::TranscriptionSegment) -> Self {
        Self {
            id: val.id,
            start: val.start,
            end: val.end,
            text: val.text.to_string(),
        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::useless_conversion)]
impl From<Usage> for liter_llm::types::Usage {
    fn from(val: Usage) -> Self {
        Self {
            prompt_tokens: val.prompt_tokens as u64,
            completion_tokens: val.completion_tokens as u64,
            total_tokens: val.total_tokens as u64,
            prompt_tokens_details: val.prompt_tokens_details.map(Into::into),
            ..Default::default()
        }
    }
}

#[allow(clippy::useless_conversion)]
impl From<liter_llm::types::Usage> for Usage {
    fn from(val: liter_llm::types::Usage) -> Self {
        Self {
            prompt_tokens: val.prompt_tokens as i64,
            completion_tokens: val.completion_tokens as i64,
            total_tokens: val.total_tokens as i64,
            prompt_tokens_details: val.prompt_tokens_details.map(Into::into),
        }
    }
}

#[allow(clippy::needless_update)]
impl From<UserMessage> for liter_llm::types::UserMessage {
    fn from(val: UserMessage) -> Self {
        Self {
            content: serde_json::from_value(val.content).unwrap_or_default(),
            name: val.name,
            ..Default::default()
        }
    }
}

#[allow(clippy::redundant_closure)]
impl From<liter_llm::types::UserMessage> for UserMessage {
    fn from(val: liter_llm::types::UserMessage) -> Self {
        Self {
            content: serde_json::to_value(&val.content).unwrap_or(serde_json::Value::Null),
            name: val.name.map(|v| v.to_string()),
        }
    }
}

#[allow(clippy::needless_update)]
impl From<WaitForBatchConfig> for liter_llm::client::WaitForBatchConfig {
    fn from(val: WaitForBatchConfig) -> Self {
        Self {
            initial_interval_secs: val.initial_interval_secs,
            max_interval_secs: val.max_interval_secs,
            backoff_multiplier: val.backoff_multiplier,
            timeout_secs: val.timeout_secs,
            ..Default::default()
        }
    }
}

impl From<liter_llm::client::WaitForBatchConfig> for WaitForBatchConfig {
    fn from(val: liter_llm::client::WaitForBatchConfig) -> Self {
        Self {
            initial_interval_secs: val.initial_interval_secs,
            max_interval_secs: val.max_interval_secs,
            backoff_multiplier: val.backoff_multiplier,
            timeout_secs: val.timeout_secs,
        }
    }
}

impl From<liter_llm::types::AssistantPart> for AssistantPart {
    fn from(val: liter_llm::types::AssistantPart) -> Self {
        match val {
            liter_llm::types::AssistantPart::Text { text } => Self {
                type_tag: "text".to_string(),
                text: Some(text),
                ..Default::default()
            },
            liter_llm::types::AssistantPart::Refusal { refusal } => Self {
                type_tag: "refusal".to_string(),
                refusal: Some(refusal),
                ..Default::default()
            },
            liter_llm::types::AssistantPart::OutputImage { image_url } => Self {
                type_tag: "output_image".to_string(),
                image_url: Some(image_url.into()),
                ..Default::default()
            },
            liter_llm::types::AssistantPart::OutputAudio { audio } => Self {
                type_tag: "output_audio".to_string(),
                audio: Some(audio.into()),
                ..Default::default()
            },
        }
    }
}

impl From<AssistantPart> for liter_llm::types::AssistantPart {
    fn from(val: AssistantPart) -> Self {
        match val.type_tag.as_str() {
            "text" => liter_llm::types::AssistantPart::Text {
                text: val.text.unwrap_or_default(),
            },
            "refusal" => liter_llm::types::AssistantPart::Refusal {
                refusal: val.refusal.unwrap_or_default(),
            },
            "output_image" => liter_llm::types::AssistantPart::OutputImage {
                image_url: val.image_url.map(Into::into).unwrap_or_default(),
            },
            "output_audio" => liter_llm::types::AssistantPart::OutputAudio {
                audio: val.audio.map(Into::into).unwrap_or_default(),
            },
            _ => unreachable!("unrecognised tag for flat enum, not constructible from PHP"),
        }
    }
}

impl From<liter_llm::CacheBackend> for CacheBackend {
    fn from(val: liter_llm::CacheBackend) -> Self {
        match val {
            liter_llm::CacheBackend::Memory => Self {
                type_tag: "memory".to_string(),
                ..Default::default()
            },
            #[cfg(feature = "opendal-cache")]
            liter_llm::CacheBackend::OpenDal { scheme, config } => Self {
                type_tag: "open_dal".to_string(),
                scheme: Some(scheme),
                config: Some(config.into()),
            },
        }
    }
}

impl From<CacheBackend> for liter_llm::CacheBackend {
    fn from(val: CacheBackend) -> Self {
        match val.type_tag.as_str() {
            "memory" => liter_llm::CacheBackend::Memory,
            #[cfg(feature = "opendal-cache")]
            "open_dal" => liter_llm::CacheBackend::OpenDal {
                scheme: val.scheme.unwrap_or_default(),
                config: val.config.map(Into::into).unwrap_or_default(),
            },
            _ => liter_llm::CacheBackend::default(),
        }
    }
}

impl From<liter_llm::types::ContentPart> for ContentPart {
    fn from(val: liter_llm::types::ContentPart) -> Self {
        match val {
            liter_llm::types::ContentPart::Text { text } => Self {
                type_tag: "text".to_string(),
                text: Some(text),
                ..Default::default()
            },
            liter_llm::types::ContentPart::ImageUrl { image_url } => Self {
                type_tag: "image_url".to_string(),
                image_url: Some(image_url.into()),
                ..Default::default()
            },
            liter_llm::types::ContentPart::Document { document } => Self {
                type_tag: "document".to_string(),
                document: Some(document.into()),
                ..Default::default()
            },
            liter_llm::types::ContentPart::InputAudio { input_audio } => Self {
                type_tag: "input_audio".to_string(),
                input_audio: Some(input_audio.into()),
                ..Default::default()
            },
        }
    }
}

impl From<ContentPart> for liter_llm::types::ContentPart {
    fn from(val: ContentPart) -> Self {
        match val.type_tag.as_str() {
            "text" => liter_llm::types::ContentPart::Text {
                text: val.text.unwrap_or_default(),
            },
            "image_url" => liter_llm::types::ContentPart::ImageUrl {
                image_url: val.image_url.map(Into::into).unwrap_or_default(),
            },
            "document" => liter_llm::types::ContentPart::Document {
                document: val.document.map(Into::into).unwrap_or_default(),
            },
            "input_audio" => liter_llm::types::ContentPart::InputAudio {
                input_audio: val.input_audio.map(Into::into).unwrap_or_default(),
            },
            _ => unreachable!("unrecognised tag for flat enum, not constructible from PHP"),
        }
    }
}

impl From<liter_llm::types::EmbeddingContentPart> for EmbeddingContentPart {
    fn from(val: liter_llm::types::EmbeddingContentPart) -> Self {
        match val {
            liter_llm::types::EmbeddingContentPart::Text { text } => Self {
                type_tag: "text".to_string(),
                text: Some(text),
                ..Default::default()
            },
            liter_llm::types::EmbeddingContentPart::ImageUrl { image_url } => Self {
                type_tag: "image_url".to_string(),
                image_url: Some(image_url.into()),
                ..Default::default()
            },
            liter_llm::types::EmbeddingContentPart::ImageBase64 { image_base64 } => Self {
                type_tag: "image_base64".to_string(),
                image_base64: Some(image_base64),
                ..Default::default()
            },
        }
    }
}

impl From<EmbeddingContentPart> for liter_llm::types::EmbeddingContentPart {
    fn from(val: EmbeddingContentPart) -> Self {
        match val.type_tag.as_str() {
            "text" => liter_llm::types::EmbeddingContentPart::Text {
                text: val.text.unwrap_or_default(),
            },
            "image_url" => liter_llm::types::EmbeddingContentPart::ImageUrl {
                image_url: val.image_url.map(Into::into).unwrap_or_default(),
            },
            "image_base64" => liter_llm::types::EmbeddingContentPart::ImageBase64 {
                image_base64: val.image_base64.unwrap_or_default(),
            },
            _ => unreachable!("unrecognised tag for flat enum, not constructible from PHP"),
        }
    }
}

impl From<liter_llm::types::Message> for Message {
    fn from(val: liter_llm::types::Message) -> Self {
        match val {
            liter_llm::types::Message::System(_0) => Self {
                role_tag: "system".to_string(),
                system: Some(_0.into()),
                ..Default::default()
            },
            liter_llm::types::Message::User(_0) => Self {
                role_tag: "user".to_string(),
                user: Some(_0.into()),
                ..Default::default()
            },
            liter_llm::types::Message::Assistant(_0) => Self {
                role_tag: "assistant".to_string(),
                assistant: Some(_0.into()),
                ..Default::default()
            },
            liter_llm::types::Message::Tool(_0) => Self {
                role_tag: "tool".to_string(),
                tool: Some(_0.into()),
                ..Default::default()
            },
            liter_llm::types::Message::Developer(_0) => Self {
                role_tag: "developer".to_string(),
                developer: Some(_0.into()),
                ..Default::default()
            },
            liter_llm::types::Message::Function(_0) => Self {
                role_tag: "function".to_string(),
                function: Some(_0.into()),
                ..Default::default()
            },
        }
    }
}

impl From<Message> for liter_llm::types::Message {
    fn from(val: Message) -> Self {
        match val.role_tag.as_str() {
            "system" => liter_llm::types::Message::System(val.system.map(Into::into).unwrap_or_default()),
            "user" => liter_llm::types::Message::User(val.user.map(Into::into).unwrap_or_default()),
            "assistant" => liter_llm::types::Message::Assistant(val.assistant.map(Into::into).unwrap_or_default()),
            "tool" => liter_llm::types::Message::Tool(val.tool.map(Into::into).unwrap_or_default()),
            "developer" => liter_llm::types::Message::Developer(val.developer.map(Into::into).unwrap_or_default()),
            "function" => liter_llm::types::Message::Function(val.function.map(Into::into).unwrap_or_default()),
            _ => unreachable!("unrecognised tag for flat enum, not constructible from PHP"),
        }
    }
}

impl From<liter_llm::types::OcrDocument> for OcrDocument {
    fn from(val: liter_llm::types::OcrDocument) -> Self {
        match val {
            liter_llm::types::OcrDocument::Url { url } => Self {
                type_tag: "document_url".to_string(),
                url: Some(url),
                ..Default::default()
            },
            liter_llm::types::OcrDocument::Base64 { data, media_type } => Self {
                type_tag: "base64".to_string(),
                data: Some(data),
                media_type: Some(media_type),
                ..Default::default()
            },
        }
    }
}

impl From<OcrDocument> for liter_llm::types::OcrDocument {
    fn from(val: OcrDocument) -> Self {
        match val.type_tag.as_str() {
            "document_url" => liter_llm::types::OcrDocument::Url {
                url: val.url.unwrap_or_default(),
            },
            "base64" => liter_llm::types::OcrDocument::Base64 {
                data: val.data.unwrap_or_default(),
                media_type: val.media_type.unwrap_or_default(),
            },
            _ => unreachable!("unrecognised tag for flat enum, not constructible from PHP"),
        }
    }
}

impl From<liter_llm::types::ResponseFormat> for ResponseFormat {
    fn from(val: liter_llm::types::ResponseFormat) -> Self {
        match val {
            liter_llm::types::ResponseFormat::Text => Self {
                type_tag: "text".to_string(),
                ..Default::default()
            },
            liter_llm::types::ResponseFormat::JsonObject => Self {
                type_tag: "json_object".to_string(),
                ..Default::default()
            },
            liter_llm::types::ResponseFormat::JsonSchema { json_schema } => Self {
                type_tag: "json_schema".to_string(),
                json_schema: Some(json_schema.into()),
            },
        }
    }
}

impl From<ResponseFormat> for liter_llm::types::ResponseFormat {
    fn from(val: ResponseFormat) -> Self {
        match val.type_tag.as_str() {
            "text" => liter_llm::types::ResponseFormat::Text,
            "json_object" => liter_llm::types::ResponseFormat::JsonObject,
            "json_schema" => liter_llm::types::ResponseFormat::JsonSchema {
                json_schema: val.json_schema.map(Into::into).unwrap_or_default(),
            },
            _ => liter_llm::types::ResponseFormat::default(),
        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::useless_conversion)]
impl From<ChatCompletionChunk> for liter_llm::types::ChatCompletionChunk {
    fn from(val: ChatCompletionChunk) -> Self {
        Self {
            id: val.id,
            object: val.object,
            created: val.created as u64,
            model: val.model,
            choices: val.choices.into_iter().map(Into::into).collect(),
            usage: val.usage.map(Into::into),
            system_fingerprint: val.system_fingerprint,
            service_tier: val.service_tier,
            ..Default::default()
        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::redundant_closure)]
impl From<InFlightLimitConfig> for liter_llm::InFlightLimitConfig {
    fn from(val: InFlightLimitConfig) -> Self {
        Self {
            max_in_flight: val.max_in_flight.map(|v| v as usize),
            ..Default::default()
        }
    }
}

impl From<IntentPrototype> for liter_llm::tower::IntentPrototype {
    fn from(val: IntentPrototype) -> Self {
        Self {
            name: val.name,
            embedding: val.embedding.into_iter().collect(),
            model: val.model,
        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<LlmConfig> for liter_llm::client::LlmConfig {
    fn from(val: LlmConfig) -> Self {
        Self {
            model: val.model,
            api_key: val.api_key,
            base_url: val.base_url,
            timeout_secs: val.timeout_secs.map(|v| v as u64),
            max_retries: val.max_retries,
            temperature: val.temperature,
            max_tokens: val.max_tokens.map(|v| v as u64),
            load_env: val.load_env,
            headers: val
                .headers
                .map(|m| m.into_iter().map(|(k, v)| (k.into(), v.into())).collect()),
            providers: val.providers.map(|v| v.into_iter().map(Into::into).collect()),
            cache: val.cache.map(Into::into),
            budget: val.budget.map(Into::into),
            rate_limit: val.rate_limit.map(Into::into),
            in_flight_limit: val.in_flight_limit.map(Into::into),
            cost_tracking: val.cost_tracking,
            tracing: val.tracing,
            cooldown_secs: val.cooldown_secs.map(|v| v as u64),
            health_check_secs: val.health_check_secs.map(|v| v as u64),
            bedrock: val.bedrock.map(Into::into),
            ..Default::default()
        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::useless_conversion)]
impl From<StreamChoice> for liter_llm::types::StreamChoice {
    fn from(val: StreamChoice) -> Self {
        Self {
            index: val.index,
            delta: val.delta.into(),
            finish_reason: val.finish_reason.as_deref().map(|s| match s { "stop" => liter_llm::types::FinishReason::Stop,"length" => liter_llm::types::FinishReason::Length,"tool_calls" => liter_llm::types::FinishReason::ToolCalls,"content_filter" => liter_llm::types::FinishReason::ContentFilter,"function_call" => liter_llm::types::FinishReason::FunctionCall,"other" => liter_llm::types::FinishReason::Other,other => {
    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'finish_reason' of FinishReason: expected one of [stop, length, tool_calls, content_filter, function_call, other]"
    ))
    .throw()
    .expect("failed to throw PHP exception for invalid enum string value");
    Default::default()
}
 })
,
            ..Default::default()        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::useless_conversion)]
impl From<StreamDelta> for liter_llm::types::StreamDelta {
    fn from(val: StreamDelta) -> Self {
        Self {
            role: val.role,
            content: val.content,
            tool_calls: val.tool_calls.map(|v| v.into_iter().map(Into::into).collect()),
            function_call: val.function_call.map(Into::into),
            refusal: val.refusal,
            reasoning_content: val.reasoning_content,
            ..Default::default()
        }
    }
}

#[allow(clippy::needless_update)]
impl From<StreamFunctionCall> for liter_llm::types::StreamFunctionCall {
    fn from(val: StreamFunctionCall) -> Self {
        Self {
            name: val.name,
            arguments: val.arguments,
            ..Default::default()
        }
    }
}

#[allow(clippy::needless_update)]
#[allow(clippy::useless_conversion)]
impl From<StreamToolCall> for liter_llm::types::StreamToolCall {
    fn from(val: StreamToolCall) -> Self {
        Self {
            index: val.index,
            id: val.id,
            call_type: val.call_type.as_deref().map(|s| match s {
                "function" => liter_llm::types::ToolType::Function,
                other => {
                    ext_php_rs::exception::PhpException::default(format!(
                        "invalid value {other:?} for field 'call_type' of ToolType: expected one of [function]"
                    ))
                    .throw()
                    .expect("failed to throw PHP exception for invalid enum string value");
                    Default::default()
                }
            }),
            function: val.function.map(Into::into),
            ..Default::default()
        }
    }
}

/// Convert a `liter_llm::cost::refresh::CatalogRefreshError` error to a PHP exception.
#[allow(dead_code)]
fn catalog_refresh_error_to_php_err(
    e: liter_llm::cost::refresh::CatalogRefreshError,
) -> ext_php_rs::exception::PhpException {
    let msg = e.to_string();
    #[allow(unreachable_patterns)]
    match &e {
        liter_llm::cost::refresh::CatalogRefreshError::Disabled => {
            ext_php_rs::exception::PhpException::default(format!("[Disabled] {}", msg))
        }
        liter_llm::cost::refresh::CatalogRefreshError::InsecureUrl { .. } => {
            ext_php_rs::exception::PhpException::default(format!("[InsecureUrl] {}", msg))
        }
        liter_llm::cost::refresh::CatalogRefreshError::Fetch { .. } => {
            ext_php_rs::exception::PhpException::default(format!("[Fetch] {}", msg))
        }
        liter_llm::cost::refresh::CatalogRefreshError::Parse { .. } => {
            ext_php_rs::exception::PhpException::default(format!("[Parse] {}", msg))
        }
        liter_llm::cost::refresh::CatalogRefreshError::Cache { .. } => {
            ext_php_rs::exception::PhpException::default(format!("[Cache] {}", msg))
        }
        _ => ext_php_rs::exception::PhpException::default(msg),
    }
}

/// Convert a `liter_llm::error::LiterLlmError` error to a PHP exception.
#[allow(dead_code)]
fn liter_llm_error_to_php_err(e: liter_llm::error::LiterLlmError) -> ext_php_rs::exception::PhpException {
    let msg = e.to_string();
    #[allow(unreachable_patterns)]
    match &e {
        liter_llm::error::LiterLlmError::Authentication { .. } => {
            ext_php_rs::exception::PhpException::default(format!("[Authentication] {}", msg))
        }
        liter_llm::error::LiterLlmError::RateLimited { .. } => {
            ext_php_rs::exception::PhpException::default(format!("[RateLimited] {}", msg))
        }
        liter_llm::error::LiterLlmError::BadRequest { .. } => {
            ext_php_rs::exception::PhpException::default(format!("[BadRequest] {}", msg))
        }
        liter_llm::error::LiterLlmError::ContextWindowExceeded { .. } => {
            ext_php_rs::exception::PhpException::default(format!("[ContextWindowExceeded] {}", msg))
        }
        liter_llm::error::LiterLlmError::ContentPolicy { .. } => {
            ext_php_rs::exception::PhpException::default(format!("[ContentPolicy] {}", msg))
        }
        liter_llm::error::LiterLlmError::NotFound { .. } => {
            ext_php_rs::exception::PhpException::default(format!("[NotFound] {}", msg))
        }
        liter_llm::error::LiterLlmError::ServerError { .. } => {
            ext_php_rs::exception::PhpException::default(format!("[ServerError] {}", msg))
        }
        liter_llm::error::LiterLlmError::ServiceUnavailable { .. } => {
            ext_php_rs::exception::PhpException::default(format!("[ServiceUnavailable] {}", msg))
        }
        liter_llm::error::LiterLlmError::Timeout => {
            ext_php_rs::exception::PhpException::default(format!("[Timeout] {}", msg))
        }
        liter_llm::error::LiterLlmError::Streaming { .. } => {
            ext_php_rs::exception::PhpException::default(format!("[Streaming] {}", msg))
        }
        liter_llm::error::LiterLlmError::EndpointNotSupported { .. } => {
            ext_php_rs::exception::PhpException::default(format!("[EndpointNotSupported] {}", msg))
        }
        liter_llm::error::LiterLlmError::InvalidHeader { .. } => {
            ext_php_rs::exception::PhpException::default(format!("[InvalidHeader] {}", msg))
        }
        liter_llm::error::LiterLlmError::Serialization(..) => {
            ext_php_rs::exception::PhpException::default(format!("[Serialization] {}", msg))
        }
        liter_llm::error::LiterLlmError::BudgetExceeded { .. } => {
            ext_php_rs::exception::PhpException::default(format!("[BudgetExceeded] {}", msg))
        }
        liter_llm::error::LiterLlmError::HookRejected { .. } => {
            ext_php_rs::exception::PhpException::default(format!("[HookRejected] {}", msg))
        }
        liter_llm::error::LiterLlmError::InternalError { .. } => {
            ext_php_rs::exception::PhpException::default(format!("[InternalError] {}", msg))
        }
        liter_llm::error::LiterLlmError::OutboundForbidden { .. } => {
            ext_php_rs::exception::PhpException::default(format!("[OutboundForbidden] {}", msg))
        }
        liter_llm::error::LiterLlmError::IdempotencyConflict { .. } => {
            ext_php_rs::exception::PhpException::default(format!("[IdempotencyConflict] {}", msg))
        }
        liter_llm::error::LiterLlmError::IdempotencyInFlight { .. } => {
            ext_php_rs::exception::PhpException::default(format!("[IdempotencyInFlight] {}", msg))
        }
        _ => ext_php_rs::exception::PhpException::default(msg),
    }
}

#[php_class]
pub struct LiterLlmErrorInfo {
    pub status_code: u16,
    pub is_transient: bool,
    pub error_type: String,
}

#[allow(dead_code)]
fn liter_llm_error_info(e: &liter_llm::error::LiterLlmError) -> LiterLlmErrorInfo {
    LiterLlmErrorInfo {
        status_code: e.status_code(),
        is_transient: e.is_transient(),
        error_type: e.error_type().to_string(),
    }
}

#[php_impl]
impl LiterLlmErrorInfo {
    /// HTTP status code for this error (0 means no associated status).
    pub fn status_code(&self) -> u16 {
        self.status_code
    }

    /// Returns `true` if the error is transient and a retry may succeed.
    pub fn is_transient(&self) -> bool {
        self.is_transient
    }

    /// Machine-readable error category string for matching and logging.
    pub fn error_type(&self) -> String {
        self.error_type.clone()
    }
}

mod serde_defaults {
    pub fn cache_config_max_entries() -> i64 {
        256
    }
    pub fn catalog_refresh_config_enabled() -> bool {
        false
    }
    pub fn catalog_refresh_config_source_url() -> String {
        "https://github.com/xberg-io/liter-llm/releases/download/model-catalog/catalog.json".to_string()
    }
    pub fn catalog_refresh_config_ttl_seconds() -> i64 {
        86400
    }
    pub fn wait_for_batch_config_initial_interval_secs() -> f64 {
        5.0
    }
    pub fn wait_for_batch_config_max_interval_secs() -> f64 {
        60.0
    }
    pub fn wait_for_batch_config_backoff_multiplier() -> f32 {
        1.5
    }
}

static __EXT_PHP_RS_MODULE_STARTUP: ::std::sync::Mutex<::std::option::Option<::ext_php_rs::builders::ModuleStartup>> =
    ::std::sync::Mutex::new(::std::option::Option::None);

unsafe extern "C" fn __ext_php_rs_module_startup(ty: i32, mod_num: i32) -> i32 {
    let startup = match __EXT_PHP_RS_MODULE_STARTUP.lock() {
        Ok(mut guard) => guard.take(),
        Err(_) => return 1,
    };
    match startup {
        Some(s) => match s.startup(ty, mod_num) {
            Ok(_) => 0,
            Err(_) => 1,
        },
        None => 1,
    }
}

#[doc(hidden)]
#[unsafe(no_mangle)]
pub extern "C" fn get_module() -> *mut ::ext_php_rs::zend::ModuleEntry {
    static __EXT_PHP_RS_MODULE_ENTRY: ::ext_php_rs::zend::StaticModuleEntry =
        ::ext_php_rs::zend::StaticModuleEntry::new();
    __EXT_PHP_RS_MODULE_ENTRY.get_or_init(|| {
        let builder = ::ext_php_rs::builders::ModuleBuilder::new("liter_llm", "1.19.1")
            .startup_function(__ext_php_rs_module_startup);
        let builder = builder
            .class::<AssistantMessage>()
            .class::<AudioContent>()
            .class::<AuthConfig>()
            .class::<BatchListQuery>()
            .class::<BatchListResponse>()
            .class::<BatchObject>()
            .class::<BatchRequestCounts>()
            .class::<BedrockConfig>()
            .class::<BudgetConfig>()
            .class::<CacheConfig>()
            .class::<CatalogRefreshConfig>()
            .class::<ChatCompletionChunk>()
            .class::<ChatCompletionRequest>()
            .class::<ChatCompletionResponse>()
            .class::<ChatCompletionTool>()
            .class::<Choice>()
            .class::<CreateBatchRequest>()
            .class::<CreateFileRequest>()
            .class::<CreateImageRequest>()
            .class::<CreateResponseRequest>()
            .class::<CreateSpeechRequest>()
            .class::<CreateTranscriptionRequest>()
            .class::<CustomProviderConfig>()
            .class::<DecodedDataUrl>()
            .class::<DefaultClient>()
            .class::<DeleteResponse>()
            .class::<DeveloperMessage>()
            .class::<DocumentContent>()
            .class::<EmbeddingObject>()
            .class::<EmbeddingRequest>()
            .class::<EmbeddingResponse>()
            .class::<FileListQuery>()
            .class::<FileListResponse>()
            .class::<FileObject>()
            .class::<FunctionCall>()
            .class::<FunctionDefinition>()
            .class::<FunctionMessage>()
            .class::<Image>()
            .class::<ImageUrl>()
            .class::<ImagesResponse>()
            .class::<InFlightLimitConfig>()
            .class::<IntentPrototype>()
            .class::<JsonSchemaFormat>()
            .class::<LlmBudgetConfig>()
            .class::<LlmCacheConfig>()
            .class::<LlmConfig>()
            .class::<LlmInFlightLimitConfig>()
            .class::<LlmProviderConfig>()
            .class::<LlmRateLimitConfig>()
            .class::<ModelInfo>()
            .class::<ModelObject>()
            .class::<ModelTier>()
            .class::<ModelsListResponse>()
            .class::<ModerationCategories>()
            .class::<ModerationCategoryScores>()
            .class::<ModerationRequest>()
            .class::<ModerationResponse>()
            .class::<ModerationResult>()
            .class::<OcrImage>()
            .class::<OcrPage>()
            .class::<OcrRequest>()
            .class::<OcrResponse>()
            .class::<PageDimensions>()
            .class::<PromptTokensDetails>()
            .class::<ProviderCapabilities>()
            .class::<ProviderConfig>()
            .class::<RateLimitConfig>()
            .class::<RerankRequest>()
            .class::<RerankResponse>()
            .class::<RerankResult>()
            .class::<RerankResultDocument>()
            .class::<ResponseObject>()
            .class::<ResponseOutputItem>()
            .class::<ResponseTool>()
            .class::<ResponseUsage>()
            .class::<SearchRequest>()
            .class::<SearchResponse>()
            .class::<SearchResult>()
            .class::<SingleflightResult>()
            .class::<SpecificFunction>()
            .class::<SpecificToolChoice>()
            .class::<StreamChoice>()
            .class::<StreamDelta>()
            .class::<StreamFunctionCall>()
            .class::<StreamOptions>()
            .class::<StreamToolCall>()
            .class::<SystemMessage>()
            .class::<ToolCall>()
            .class::<ToolMessage>()
            .class::<TranscriptionResponse>()
            .class::<TranscriptionSegment>()
            .class::<Usage>()
            .class::<UserMessage>()
            .class::<WaitForBatchConfig>()
            .class::<LiterLlmApi>()
            .class::<AssistantContent>()
            .class::<AssistantPart>()
            .class::<AuthHeaderFormat>()
            .class::<AuthType>()
            .class::<BatchStatus>()
            .class::<CacheBackend>()
            .class::<CircuitState>()
            .class::<ContentPart>()
            .class::<EmbeddingContentPart>()
            .class::<EmbeddingFormat>()
            .class::<EmbeddingInput>()
            .class::<Enforcement>()
            .class::<FilePurpose>()
            .class::<FinishReason>()
            .class::<HealthStatus>()
            .class::<ImageDetail>()
            .class::<Message>()
            .class::<Modality>()
            .class::<ModerationInput>()
            .class::<OcrDocument>()
            .class::<ReasoningEffort>()
            .class::<RefreshOutcome>()
            .class::<RerankDocument>()
            .class::<ResponseFormat>()
            .class::<StopSequence>()
            .class::<StreamFormat>()
            .class::<ToolChoice>()
            .class::<ToolChoiceMode>()
            .class::<ToolType>()
            .class::<UserContent>()
            .class::<LiterLlmErrorInfo>();
        match builder.try_into() {
            Ok((entry, startup)) => {
                *__EXT_PHP_RS_MODULE_STARTUP
                    .lock()
                    .expect("module startup mutex poisoned") = Some(startup);
                entry
            }
            Err(e) => panic!("Failed to build PHP module: {:?}", e),
        }
    })
}
