...
// alef:hash:632f5d968db8037f6e74610c272872e8d8cf8f6cb0321a41108ee5f4812d124c
...
tasks.register("buildHostJni", Exec::class) {
    if (project.providers.gradleProperty("alef.skipHostJni").orNull != "true") {
        ...
        val jniManifest = configuredManifest?.let(::file) ?: generateSequence(projectDir) { it.parentFile }
            .map { it.resolve("crates/liter-llm-jni/Cargo.toml") }
            .firstOrNull { it.isFile }
            ?: throw GradleException(
                "Cannot locate crates/liter-llm-jni/Cargo.toml from $projectDir; " +
...
tasks.register("copyHostJni", Copy::class) {
    if (project.providers.gradleProperty("alef.skipHostJni").orNull != "true") {
        ...
        val jniManifest = configuredManifest?.let(::file) ?: generateSequence(projectDir) { it.parentFile }
            .map { it.resolve("crates/liter-llm-jni/Cargo.toml") }
            .firstOrNull { it.isFile }
            ?: throw GradleException("Cannot locate crates/liter-llm-jni/Cargo.toml from $projectDir")
...
tasks.register("validateJniLibsForRelease") {
    doFirst {
        ...
        if (releaseAssemble) {
            ...
            // lib*_ffi.so instead of libliterllm_jni.so) passes the emptiness
            // check above but fails at runtime with UnsatisfiedLinkError, because
            // System.loadLibrary("literllm_jni") needs the JNI entry points.
            val expectedJniLib = "libliterllm_jni.so"
            ...
            if (abisMissingJniLib.isNotEmpty()) {
                throw GradleException(
                    ...
                    "export the JNI symbols System.loadLibrary(\"literllm_jni\") requires and " +
