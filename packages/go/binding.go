...
// alef:hash:45baab1ac15637b3c1b7d358cbe24c1ee4fff0b01578273fe988d6658cdfe4cf
...
var (
	// ErrAuthentication is returned when authentication failed: {message}.
	ErrAuthentication = errors.New("authentication failed: {message}")
	// ErrRateLimited is returned when rate limited: {message}.
	ErrRateLimited = errors.New("rate limited: {message}")
	// ErrBadRequest is returned when bad request: {message}.
	ErrBadRequest = errors.New("bad request: {message}")
	// ErrContextWindowExceeded is returned when context window exceeded: {message}.
	ErrContextWindowExceeded = errors.New("context window exceeded: {message}")
	// ErrContentPolicy is returned when content policy violation: {message}.
	ErrContentPolicy = errors.New("content policy violation: {message}")
	// ErrNotFound is returned when not found: {message}.
	ErrNotFound = errors.New("not found: {message}")
	// ErrServerError is returned when server error: {message}.
	ErrServerError = errors.New("server error: {message}")
	// ErrServiceUnavailable is returned when service unavailable: {message}.
	ErrServiceUnavailable = errors.New("service unavailable: {message}")
	// ErrTimeout is returned when request timeout.
	ErrTimeout = errors.New("request timeout")
	// ErrStreaming is returned when streaming error: {message}.
	ErrStreaming = errors.New("streaming error: {message}")
	// ErrEndpointNotSupported is returned when provider {provider} does not support {endpoint}.
	ErrEndpointNotSupported = errors.New("provider {provider} does not support {endpoint}")
	// ErrInvalidHeader is returned when invalid header {name:?}: {reason}.
	ErrInvalidHeader = errors.New("invalid header {name:?}: {reason}")
	// ErrSerialization is returned when serialization error.
	ErrSerialization = errors.New("serialization error")
	// ErrBudgetExceeded is returned when budget exceeded: {message}.
	ErrBudgetExceeded = errors.New("budget exceeded: {message}")
	// ErrHookRejected is returned when hook rejected: {message}.
	ErrHookRejected = errors.New("hook rejected: {message}")
	// ErrInternalError is returned when internal error: {message}.
...
const (
	// ImageDetailLow is the Low variant of ImageDetail.
	ImageDetailLow ImageDetail = "low"
	// ImageDetailHigh is the High variant of ImageDetail.
	ImageDetailHigh ImageDetail = "high"
	// ImageDetailAuto is the Auto variant of ImageDetail.
	ImageDetailAuto ImageDetail = "auto"
)

// ToolType type discriminator for tool/tool-call objects.
//
...
const (
	// ToolTypeFunction is the Function variant of ToolType.
...
const (
	// ToolChoiceModeAuto is the Auto variant of ToolChoiceMode.
	ToolChoiceModeAuto ToolChoiceMode = "auto"
	// ToolChoiceModeRequired is the Required variant of ToolChoiceMode.
	ToolChoiceModeRequired ToolChoiceMode = "required"
	// ToolChoiceModeNone is the None variant of ToolChoiceMode.
...
// FinishReason why a choice stopped generating tokens.
type FinishReason string

const (
	// FinishReasonStop is the Stop variant of FinishReason.
	FinishReasonStop FinishReason = "stop"
	// FinishReasonLength is the Length variant of FinishReason.
	FinishReasonLength FinishReason = "length"
	// FinishReasonToolCalls is the ToolCalls variant of FinishReason.
	FinishReasonToolCalls FinishReason = "tool_calls"
	// FinishReasonContentFilter is the ContentFilter variant of FinishReason.
	FinishReasonContentFilter FinishReason = "content_filter"
	// FinishReasonFunctionCall deprecated legacy finish reason; retained for API compatibility.
	FinishReasonFunctionCall FinishReason = "function_call"
	// FinishReasonOther catch-all for unknown finish reasons returned by non-OpenAI providers.
	//
...
const (
	// ReasoningEffortLow is the Low variant of ReasoningEffort.
	ReasoningEffortLow ReasoningEffort = "low"
	// ReasoningEffortMedium is the Medium variant of ReasoningEffort.
	ReasoningEffortMedium ReasoningEffort = "medium"
	// ReasoningEffortHigh is the High variant of ReasoningEffort.
	ReasoningEffortHigh ReasoningEffort = "high"
)

// EmbeddingFormat format in which the embedding vectors are returned.
type EmbeddingFormat string

const (
	// EmbeddingFormatFloat 32-bit floating-point numbers (default).
	EmbeddingFormatFloat EmbeddingFormat = "float"
	// EmbeddingFormatBase64 base64-encoded string representation of the floats.
...
func (e *AuthHeaderFormat) UnmarshalJSON(data []byte) error {
	var value string
	if err := json.Unmarshal(data, &value); err == nil {
		*e = AuthHeaderFormat(value)
		return nil
	}
...
// DefaultClient default client implementation backed by `reqwest`.
//
...
func (h *DefaultClient) Speech(req CreateSpeechRequest) (*[]byte, error) {
	...
	ptr := C.literllm_default_client_speech((*C.LITERLLMDefaultClient)(unsafe.Pointer(h.ptr)), cReq)
	if err := lastError(); err != nil {
		return nil, err
	}
...
func (h *DefaultClient) CreateFile(req string) (*string, error) {
	cReq := C.CString(req)
...
func (h *DefaultClient) RetrieveFile(fileID string) (*string, error) {
...
func (h *DefaultClient) DeleteFile(fileID string) (*string, error) {
	cFileID := C.CString(fileID)
	defer C.free(unsafe.Pointer(cFileID))
...
func (h *DefaultClient) ListFiles(query *string) (*string, error) {
	var cQuery *C.char
	if query != nil {
...
func (h *DefaultClient) FileContent(fileID string) (*[]byte, error) {
	...
	ptr := C.literllm_default_client_file_content((*C.LITERLLMDefaultClient)(unsafe.Pointer(h.ptr)), cFileID)
	if err := lastError(); err != nil {
		return nil, err
	}
	return unmarshalBytes(ptr), nil
...
func (h *DefaultClient) CreateBatch(req string) (*string, error) {
	cReq := C.CString(req)
	defer C.free(unsafe.Pointer(cReq))
...
func (h *DefaultClient) RetrieveBatch(batchID string) (*string, error) {
...
func (h *DefaultClient) ListBatches(query *string) (*string, error) {
	var cQuery *C.char
	if query != nil {
		cQuery = C.CString(*query)
		defer C.free(unsafe.Pointer(cQuery))
...
func (h *DefaultClient) CancelBatch(batchID string) (*string, error) {
	cBatchID := C.CString(batchID)
	defer C.free(unsafe.Pointer(cBatchID))
...
func (h *DefaultClient) CreateResponse(req string) (*string, error) {
	cReq := C.CString(req)
	defer C.free(unsafe.Pointer(cReq))
...
func (h *DefaultClient) RetrieveResponse(id string) (*string, error) {
	cid := C.CString(id)
...
func (h *DefaultClient) CancelResponse(id string) (*string, error) {
	cid := C.CString(id)
	defer C.free(unsafe.Pointer(cid))
