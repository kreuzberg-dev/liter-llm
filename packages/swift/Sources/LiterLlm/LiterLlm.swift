...
// alef:hash:ad56513e71a573fe391908fc7ecaa14ba892b629c57dc5632ad45dd90a8da273
...
public struct AssistantMessage: Codable, Sendable, Hashable {
    ...
    public init(content: AssistantContent? = nil, name: String? = nil, toolCalls: [ToolCall]? = nil, refusal: String? = nil, functionCall: FunctionCall? = nil, reasoningContent: String? = nil) {
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.reasoningContent = try container.decodeIfPresent(String.self, forKey: .reasoningContent) ?? nil
    }
    // MARK: - Instance Methods
    public func text() throws  -> String? {
        let jsonSelf = try JSONEncoder().encode(self)
        let selfString = String(data: jsonSelf, encoding: .utf8) ?? "{}"
        let resultJson = try RustBridge.assistantMessageTextFromJson(selfString).toString()
        let data = resultJson.data(using: .utf8) ?? Data()
        return try JSONDecoder().decode(String?.self, from: data)
    }

    public func refusalText() throws  -> String? {
        let jsonSelf = try JSONEncoder().encode(self)
        let selfString = String(data: jsonSelf, encoding: .utf8) ?? "{}"
        let resultJson = try RustBridge.assistantMessageRefusalTextFromJson(selfString).toString()
        let data = resultJson.data(using: .utf8) ?? Data()
        return try JSONDecoder().decode(String?.self, from: data)
    }

    public func reasoningText() throws  -> String? {
        let jsonSelf = try JSONEncoder().encode(self)
        let selfString = String(data: jsonSelf, encoding: .utf8) ?? "{}"
        let resultJson = try RustBridge.assistantMessageReasoningTextFromJson(selfString).toString()
        let data = resultJson.data(using: .utf8) ?? Data()
        return try JSONDecoder().decode(String?.self, from: data)
    }

    public func outputImages() throws  -> [ImageUrl] {
        let jsonSelf = try JSONEncoder().encode(self)
        let selfString = String(data: jsonSelf, encoding: .utf8) ?? "{}"
        let resultJson = try RustBridge.assistantMessageOutputImagesFromJson(selfString).toString()
        let data = resultJson.data(using: .utf8) ?? Data()
        return try JSONDecoder().decode([ImageUrl].self, from: data)
    }

    public func outputAudio() throws  -> [AudioContent] {
        let jsonSelf = try JSONEncoder().encode(self)
        let selfString = String(data: jsonSelf, encoding: .utf8) ?? "{}"
        let resultJson = try RustBridge.assistantMessageOutputAudioFromJson(selfString).toString()
        let data = resultJson.data(using: .utf8) ?? Data()
...
internal extension AssistantMessage {
    init(_ rb: RustBridge.AssistantMessageRef) throws {
        ...
        self.toolCalls = try JSONDecoder().decode([ToolCall]?.self, from: ((rb.toolCalls().toString()).data(using: .utf8) ?? Data("null".utf8)))
        self.refusal = rb.refusal()?.toString()
        self.functionCall = try rb.functionCall().map { try FunctionCall($0) }
        self.reasoningContent = rb.reasoningContent()?.toString()
    }
...
internal extension AuthConfig {
    init(_ rb: RustBridge.AuthConfigRef) throws {
        self.authType = try { let rawValue = rb.authType().toString(); guard let value = AuthType(rawValue: rawValue) else { throw CatalogRefreshError.validation(message: "Unknown AuthType variant", source: rawValue) }; return value }()
...
public struct CatalogRefreshConfig: Codable, Sendable, Hashable {
    ...
    public init(enabled: Bool = false, sourceUrl: String = "https://github.com/xberg-io/liter-llm/releases/download/model-catalog/catalog.json", ttlSeconds: UInt64 = 86400, cachePath: String? = nil) {
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.sourceUrl = try container.decodeIfPresent(String.self, forKey: .sourceUrl) ?? "https://github.com/xberg-io/liter-llm/releases/download/model-catalog/catalog.json"
        self.ttlSeconds = try container.decodeIfPresent(UInt64.self, forKey: .ttlSeconds) ?? 86400
        self.cachePath = try container.decodeIfPresent(String.self, forKey: .cachePath) ?? nil
    }
}

// MARK: - Internal FFI conversions for CatalogRefreshConfig
internal extension CatalogRefreshConfig {
    init(_ rb: RustBridge.CatalogRefreshConfigRef) throws {
        self.enabled = rb.enabled()
        self.sourceUrl = rb.sourceUrl().toString()
        self.ttlSeconds = rb.ttlSeconds()
        self.cachePath = rb.cachePath()?.toString()
    }

    func intoRust() throws -> RustBridge.CatalogRefreshConfig {
        return RustBridge.CatalogRefreshConfig(self.enabled, RustString(self.sourceUrl), self.ttlSeconds, self.cachePath.map(RustString.init))
    }
}
...
internal extension ChatCompletionChunk {
    init(_ rb: RustBridge.ChatCompletionChunkRef) throws {
        ...
        self.choices = try rb.choices().map { (s: RustStringRef) -> StreamChoice in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(StreamChoice.self, from: d) }
...
internal extension CreateFileRequest {
    init(_ rb: RustBridge.CreateFileRequestRef) throws {
        ...
        self.purpose = try { let rawValue = rb.purpose().toString(); guard let value = FilePurpose(rawValue: rawValue) else { throw CatalogRefreshError.validation(message: "Unknown FilePurpose variant", source: rawValue) }; return value }()
...
public struct EmbeddingObject: Codable, Sendable, Hashable {
    ...
    public init(object: String, embedding: [Float], index: UInt32) {
        self.object = object
...
internal extension EmbeddingResponse {
    init(_ rb: RustBridge.EmbeddingResponseRef) throws {
        self.object = rb.object().toString()
        self.data = try rb.data().map { (s: RustStringRef) -> EmbeddingObject in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(EmbeddingObject.self, from: d) }
        self.model = rb.model().toString()
        self.usage = try rb.usage().map { try Usage($0) }
    }
...
internal extension FileListResponse {
    init(_ rb: RustBridge.FileListResponseRef) throws {
        self.object = rb.object().toString()
        self.data = try rb.data().map { (s: RustStringRef) -> FileObject in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(FileObject.self, from: d) }
...
internal extension ImagesResponse {
    init(_ rb: RustBridge.ImagesResponseRef) throws {
        self.created = rb.created()
        self.data = try rb.data().map { (s: RustStringRef) -> Image in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(Image.self, from: d) }
    }
...
public struct InFlightLimitConfig: Codable, Sendable, Hashable {
    /// Maximum simultaneously outstanding provider requests. `None` means unlimited.
    public let maxInFlight: UInt?
    public init(maxInFlight: UInt? = nil) {
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.maxInFlight = try container.decodeIfPresent(UInt.self, forKey: .maxInFlight) ?? nil
    }
}

// MARK: - Internal FFI conversions for InFlightLimitConfig
internal extension InFlightLimitConfig {
    init(_ rb: RustBridge.InFlightLimitConfigRef) throws {
        self.maxInFlight = rb.maxInFlight()
    }

    func intoRust() throws -> RustBridge.InFlightLimitConfig {
        return RustBridge.InFlightLimitConfig(self.maxInFlight)
    }
}
...
public struct LlmInFlightLimitConfig: Codable, Sendable, Hashable {
    /// Maximum simultaneously outstanding provider requests. `None` means unlimited.
    public let maxInFlight: UInt?
    public init(maxInFlight: UInt? = nil) {
        self.maxInFlight = maxInFlight
...
// MARK: - Internal FFI conversions for LlmInFlightLimitConfig
internal extension LlmInFlightLimitConfig {
    init(_ rb: RustBridge.LlmInFlightLimitConfigRef) throws {
        self.maxInFlight = rb.maxInFlight()
    }

    func intoRust() throws -> RustBridge.LlmInFlightLimitConfig {
        return RustBridge.LlmInFlightLimitConfig(self.maxInFlight)
    }
}
...
public struct ModelInfo: Codable, Sendable, Hashable {
    ...
    public init(inputCostPerToken: Double, outputCostPerToken: Double, cacheReadInputTokenCost: Double? = nil, cacheCreationInputTokenCost: Double? = nil, inputCostPerAudioToken: Double? = nil, outputCostPerAudioToken: Double? = nil, outputCostPerReasoningToken: Double? = nil, maxTokens: UInt64? = nil, maxInputTokens: UInt64? = nil, maxOutputTokens: UInt64? = nil, mode: String? = nil, supportsVision: Bool? = nil, supportsFunctionCalling: Bool? = nil, supportsReasoning: Bool? = nil, supportsStructuredOutput: Bool? = nil, supportsAudioInput: Bool? = nil, supportsAudioOutput: Bool? = nil, supportsPromptCaching: Bool? = nil, tiers: [ModelTier]) {
        ...
        self.inputCostPerAudioToken = inputCostPerAudioToken
        self.outputCostPerAudioToken = outputCostPerAudioToken
        self.outputCostPerReasoningToken = outputCostPerReasoningToken
    ...
    private enum CodingKeys: String, CodingKey {
        ...
        case inputCostPerAudioToken = "input_cost_per_audio_token"
        case outputCostPerAudioToken = "output_cost_per_audio_token"
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.inputCostPerToken = try container.decodeIfPresent(Double.self, forKey: .inputCostPerToken) ?? 0
        self.outputCostPerToken = try container.decodeIfPresent(Double.self, forKey: .outputCostPerToken) ?? 0
        self.cacheReadInputTokenCost = try container.decodeIfPresent(Double.self, forKey: .cacheReadInputTokenCost) ?? nil
        self.cacheCreationInputTokenCost = try container.decodeIfPresent(Double.self, forKey: .cacheCreationInputTokenCost) ?? nil
        self.inputCostPerAudioToken = try container.decodeIfPresent(Double.self, forKey: .inputCostPerAudioToken) ?? nil
        self.outputCostPerAudioToken = try container.decodeIfPresent(Double.self, forKey: .outputCostPerAudioToken) ?? nil
        self.outputCostPerReasoningToken = try container.decodeIfPresent(Double.self, forKey: .outputCostPerReasoningToken) ?? nil
        self.maxTokens = try container.decodeIfPresent(UInt64.self, forKey: .maxTokens) ?? nil
        self.maxInputTokens = try container.decodeIfPresent(UInt64.self, forKey: .maxInputTokens) ?? nil
        self.maxOutputTokens = try container.decodeIfPresent(UInt64.self, forKey: .maxOutputTokens) ?? nil
        self.mode = try container.decodeIfPresent(String.self, forKey: .mode) ?? nil
        self.supportsVision = try container.decodeIfPresent(Bool.self, forKey: .supportsVision) ?? nil
        self.supportsFunctionCalling = try container.decodeIfPresent(Bool.self, forKey: .supportsFunctionCalling) ?? nil
        self.supportsReasoning = try container.decodeIfPresent(Bool.self, forKey: .supportsReasoning) ?? nil
        self.supportsStructuredOutput = try container.decodeIfPresent(Bool.self, forKey: .supportsStructuredOutput) ?? nil
        self.supportsAudioInput = try container.decodeIfPresent(Bool.self, forKey: .supportsAudioInput) ?? nil
        self.supportsAudioOutput = try container.decodeIfPresent(Bool.self, forKey: .supportsAudioOutput) ?? nil
        self.supportsPromptCaching = try container.decodeIfPresent(Bool.self, forKey: .supportsPromptCaching) ?? nil
        self.tiers = try container.decodeIfPresent([ModelTier].self, forKey: .tiers) ?? []
    }
}

// MARK: - Internal FFI conversions for ModelInfo
internal extension ModelInfo {
    init(_ rb: RustBridge.ModelInfoRef) throws {
        self.inputCostPerToken = rb.inputCostPerToken()
        self.outputCostPerToken = rb.outputCostPerToken()
        self.cacheReadInputTokenCost = rb.cacheReadInputTokenCost()
        self.cacheCreationInputTokenCost = rb.cacheCreationInputTokenCost()
        self.inputCostPerAudioToken = rb.inputCostPerAudioToken()
        self.outputCostPerAudioToken = rb.outputCostPerAudioToken()
        self.outputCostPerReasoningToken = rb.outputCostPerReasoningToken()
        self.maxTokens = rb.maxTokens()
        self.maxInputTokens = rb.maxInputTokens()
        self.maxOutputTokens = rb.maxOutputTokens()
        self.mode = rb.mode()?.toString()
        self.supportsVision = rb.supportsVision()
        self.supportsFunctionCalling = rb.supportsFunctionCalling()
        self.supportsReasoning = rb.supportsReasoning()
        self.supportsStructuredOutput = rb.supportsStructuredOutput()
        self.supportsAudioInput = rb.supportsAudioInput()
        self.supportsAudioOutput = rb.supportsAudioOutput()
        self.supportsPromptCaching = rb.supportsPromptCaching()
        self.tiers = try rb.tiers().map { (s: RustStringRef) -> ModelTier in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(ModelTier.self, from: d) }
    }

    func intoRust() throws -> RustBridge.ModelInfo {
        let __tiers = RustVec<RustBridge.ModelTier>()
        for __elem in self.tiers { __tiers.push(value: try __elem.intoRust()) }
        return RustBridge.ModelInfo(self.inputCostPerToken, self.outputCostPerToken, self.cacheReadInputTokenCost, self.cacheCreationInputTokenCost, self.inputCostPerAudioToken, self.outputCostPerAudioToken, self.outputCostPerReasoningToken, self.maxTokens, self.maxInputTokens, self.maxOutputTokens, self.mode.map(RustString.init), self.supportsVision, self.supportsFunctionCalling, self.supportsReasoning, self.supportsStructuredOutput, self.supportsAudioInput, self.supportsAudioOutput, self.supportsPromptCaching, __tiers)
    }
}
...
public struct ModelTier: Codable, Sendable, Hashable {
    /// The tier applies when the prompt/context token count is at least
    /// this value.
    public let minContextTokens: UInt64
    ...
    public let outputCostPerReasoningToken: Double?
    public init(minContextTokens: UInt64, inputCostPerToken: Double, outputCostPerToken: Double, cacheReadInputTokenCost: Double? = nil, cacheCreationInputTokenCost: Double? = nil, inputCostPerAudioToken: Double? = nil, outputCostPerAudioToken: Double? = nil, outputCostPerReasoningToken: Double? = nil) {
    ...
    private enum CodingKeys: String, CodingKey {
        case minContextTokens = "min_context_tokens"
        case inputCostPerToken = "input_cost_per_token"
        case outputCostPerToken = "output_cost_per_token"
        case cacheReadInputTokenCost = "cache_read_input_token_cost"
        case cacheCreationInputTokenCost = "cache_creation_input_token_cost"
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.minContextTokens = try container.decodeIfPresent(UInt64.self, forKey: .minContextTokens) ?? 0
        self.inputCostPerToken = try container.decodeIfPresent(Double.self, forKey: .inputCostPerToken) ?? 0
        self.outputCostPerToken = try container.decodeIfPresent(Double.self, forKey: .outputCostPerToken) ?? 0
        self.cacheReadInputTokenCost = try container.decodeIfPresent(Double.self, forKey: .cacheReadInputTokenCost) ?? nil
        self.cacheCreationInputTokenCost = try container.decodeIfPresent(Double.self, forKey: .cacheCreationInputTokenCost) ?? nil
...
// MARK: - Internal FFI conversions for ModelTier
internal extension ModelTier {
    init(_ rb: RustBridge.ModelTierRef) throws {
        self.minContextTokens = rb.minContextTokens()
        self.inputCostPerToken = rb.inputCostPerToken()
        self.outputCostPerToken = rb.outputCostPerToken()
        self.cacheReadInputTokenCost = rb.cacheReadInputTokenCost()
        self.cacheCreationInputTokenCost = rb.cacheCreationInputTokenCost()
    ...
    func intoRust() throws -> RustBridge.ModelTier {
        return RustBridge.ModelTier(self.minContextTokens, self.inputCostPerToken, self.outputCostPerToken, self.cacheReadInputTokenCost, self.cacheCreationInputTokenCost, self.inputCostPerAudioToken, self.outputCostPerAudioToken, self.outputCostPerReasoningToken)
    }
}
...
internal extension ModelsListResponse {
    init(_ rb: RustBridge.ModelsListResponseRef) throws {
        self.object = rb.object().toString()
        self.data = try rb.data().map { (s: RustStringRef) -> ModelObject in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(ModelObject.self, from: d) }
    }
...
internal extension ModerationResponse {
    init(_ rb: RustBridge.ModerationResponseRef) throws {
        self.id = rb.id().toString()
        self.model = rb.model().toString()
        self.results = try rb.results().map { (s: RustStringRef) -> ModerationResult in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(ModerationResult.self, from: d) }
    }
...
internal extension OcrPage {
    init(_ rb: RustBridge.OcrPageRef) throws {
        ...
        self.images = try JSONDecoder().decode([OcrImage]?.self, from: ((rb.images().toString()).data(using: .utf8) ?? Data("null".utf8)))
...
internal extension OcrRequest {
    init(_ rb: RustBridge.OcrRequestRef) throws {
        ...
        self.pages = try JSONDecoder().decode([UInt32]?.self, from: ((rb.pages().toString()).data(using: .utf8) ?? Data("null".utf8)))
...
internal extension OcrResponse {
    init(_ rb: RustBridge.OcrResponseRef) throws {
        self.pages = try rb.pages().map { (s: RustStringRef) -> OcrPage in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(OcrPage.self, from: d) }
        self.model = rb.model().toString()
        self.usage = try rb.usage().map { try Usage($0) }
    }
...
internal extension SearchRequest {
    init(_ rb: RustBridge.SearchRequestRef) throws {
        ...
        self.searchDomainFilter = try JSONDecoder().decode([String]?.self, from: ((rb.searchDomainFilter().toString()).data(using: .utf8) ?? Data("null".utf8)))
...
internal extension SearchResponse {
    init(_ rb: RustBridge.SearchResponseRef) throws {
        self.results = try rb.results().map { (s: RustStringRef) -> SearchResult in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(SearchResult.self, from: d) }
        self.model = rb.model().toString()
    }
...
internal extension SpecificToolChoice {
    init(_ rb: RustBridge.SpecificToolChoiceRef) throws {
        self.choiceType = try { let rawValue = rb.choiceType().toString(); guard let value = ToolType(rawValue: rawValue) else { throw CatalogRefreshError.validation(message: "Unknown ToolType variant", source: rawValue) }; return value }()
...
public struct StreamDelta: Codable, Sendable, Hashable {
    ...
    public let reasoningContent: String?
    public init(role: String? = nil, content: String? = nil, toolCalls: [StreamToolCall]? = nil, functionCall: StreamFunctionCall? = nil, refusal: String? = nil, reasoningContent: String? = nil) {
        self.role = role
        self.content = content
        self.toolCalls = toolCalls
        self.functionCall = functionCall
...
internal extension StreamDelta {
    init(_ rb: RustBridge.StreamDeltaRef) throws {
        ...
        self.content = rb.content()?.toString()
        self.toolCalls = try JSONDecoder().decode([StreamToolCall]?.self, from: ((rb.toolCalls().toString()).data(using: .utf8) ?? Data("null".utf8)))
...
internal extension ToolCall {
    init(_ rb: RustBridge.ToolCallRef) throws {
        self.id = rb.id().toString()
        self.callType = try { let rawValue = rb.callType().toString(); guard let value = ToolType(rawValue: rawValue) else { throw CatalogRefreshError.validation(message: "Unknown ToolType variant", source: rawValue) }; return value }()
...
public struct ToolMessage: Codable, Sendable, Hashable {
    ...
    public let name: String?
    public init(content: UserContent, toolCallId: String, name: String? = nil) {
        self.content = content
...
internal extension ToolMessage {
    ...
    func intoRust() throws -> RustBridge.ToolMessage {
        return RustBridge.ToolMessage(try self.content.intoRust(), RustString(self.toolCallId), self.name.map(RustString.init))
    }
}
...
internal extension TranscriptionResponse {
    init(_ rb: RustBridge.TranscriptionResponseRef) throws {
        ...
        self.segments = try JSONDecoder().decode([TranscriptionSegment]?.self, from: ((rb.segments().toString()).data(using: .utf8) ?? Data("null".utf8)))
    }
...
public struct WaitForBatchConfig: Codable, Sendable, Hashable {
    ...
    public init(initialIntervalSecs: Double = 5.0, maxIntervalSecs: Double = 60.0, backoffMultiplier: Float = 1.5, timeoutSecs: Double? = nil) {
...
public typealias CatalogRefreshConfigRef = RustBridge.CatalogRefreshConfigRef

public typealias CatalogRefreshConfigRefMut = RustBridge.CatalogRefreshConfigRefMut
...
public typealias InFlightLimitConfigRef = RustBridge.InFlightLimitConfigRef

public typealias InFlightLimitConfigRefMut = RustBridge.InFlightLimitConfigRefMut

public typealias LlmInFlightLimitConfigRef = RustBridge.LlmInFlightLimitConfigRef

public typealias LlmInFlightLimitConfigRefMut = RustBridge.LlmInFlightLimitConfigRefMut
...
public typealias ModelInfoRef = RustBridge.ModelInfoRef

public typealias ModelInfoRefMut = RustBridge.ModelInfoRefMut

public typealias ModelObjectRef = RustBridge.ModelObjectRef

public typealias ModelObjectRefMut = RustBridge.ModelObjectRefMut

public typealias ModelTierRef = RustBridge.ModelTierRef

public typealias ModelTierRefMut = RustBridge.ModelTierRefMut
...
public enum EmbeddingContentPart: Codable, Sendable, Hashable {
    /// Plain text.
    case text(text: String)
    /// Image identified by a data URL or HTTP/HTTPS URL.
    case imageUrl(imageUrl: ImageUrl)
    /// Image encoded as a complete data URL.
    case imageBase64(imageBase64: String)
    ...
    public init(from decoder: Decoder) throws {
        ...
        case "image_url":
            self = .imageUrl(imageUrl: try container.decode(ImageUrl.self, forKey: .imageUrl))
        case "image_base64":
            self = .imageBase64(imageBase64: try container.decode(String.self, forKey: .imageBase64))
        default:
            throw DecodingError.dataCorruptedError(
                forKey: .type,
                in: container,
                debugDescription: "Unknown EmbeddingContentPart type: \(type)"
            )
        }
    }
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        case .imageBase64(let imageBase64):
...
extension EmbeddingContentPart {
    func intoRust() throws -> RustBridge.EmbeddingContentPart {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "null"
        return try RustBridge.embeddingContentPartFromJson(json)
    }
}
...
public enum EmbeddingInput: Codable, Sendable, Hashable {
    ...
    case multimodal(field0: [EmbeddingContentPart])
    ...
    public init(from decoder: Decoder) throws {
        ...
        if let value = try? container.decode([EmbeddingContentPart].self) {
            self = .multimodal(field0: value)
            return
        }
        throw DecodingError.dataCorruptedError(in: container, debugDescription: "no matching untagged variant")
    }
...
public enum RefreshOutcome: String, Codable, Sendable, Hashable {
...
extension RefreshOutcome {
    func intoRust() throws -> RustBridge.RefreshOutcome {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "null"
        return try RustBridge.refreshOutcomeFromJson(json)
    }
}
...
public enum CatalogRefreshError: Swift.Error {
    ...
    case insecureUrl(message: String, url: String)
    /// The network fetch failed, timed out, or returned a non-success
    /// status.
    case fetch(url: String, message: String)
    ...
    /// (see [`refresh_catalog`]).
    case cache(path: String, message: String)
...
public func refreshCatalog(_ configJson: String) async throws -> RefreshOutcome {
    let config = try catalogRefreshConfigFromJson(configJson)
    return try await refreshCatalog(config: config)
}
...
public func catalogRefreshConfigFromJson(_ json: String) throws -> CatalogRefreshConfig {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(CatalogRefreshConfig.self, from: data)
}
...
public func inFlightLimitConfigFromJson(_ json: String) throws -> InFlightLimitConfig {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(InFlightLimitConfig.self, from: data)
}
...
public func llmInFlightLimitConfigFromJson(_ json: String) throws -> LlmInFlightLimitConfig {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(LlmInFlightLimitConfig.self, from: data)
}
...
public func modelInfoFromJson(_ json: String) throws -> ModelInfo {
    let data = json.data(using: .utf8) ?? Data()
...
public func modelTierFromJson(_ json: String) throws -> ModelTier {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(ModelTier.self, from: data)
}
...
public func embeddingContentPartFromJson(_ json: String) throws -> EmbeddingContentPart {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(EmbeddingContentPart.self, from: data)
}
...
public func refreshOutcomeFromJson(_ json: String) throws -> RefreshOutcome {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(RefreshOutcome.self, from: data)
}
...
public func clearCatalogOverlay() {
    return RustBridge.clearCatalogOverlay()
}
...
/// call.
public func installCatalogOverlayFromStr(catalogJson: String) throws {
        let _rb_catalogJson = RustString(catalogJson)
    return try RustBridge.installCatalogOverlayFromStr(_rb_catalogJson)
}
...
public func modelInfo(model: String) throws -> ModelInfo? {
        let _rb_model = RustString(model)
    return try RustBridge.modelInfo(_rb_model).map { try ModelInfo($0) }
}
...
public func recordCostUsd(system: String, model: String, operation: String, costUsd: Double) {
        let _rb_system = RustString(system)
        let _rb_model = RustString(model)
        let _rb_operation = RustString(operation)
    return RustBridge.recordCostUsd(_rb_system, _rb_model, _rb_operation, costUsd)
}
...
/// availability.
public func refreshCatalog(config: CatalogRefreshConfig) async throws -> RefreshOutcome {
    return try await Task.detached(priority: .userInitiated) {
        let _rb_config = try config.intoRust()
        let _rb_obj = try RustBridge.refreshCatalog(_rb_config)
        let _rbRawValue = _rb_obj.to_string().toString()
        guard let _rbValue = RefreshOutcome(rawValue: _rbRawValue) else {
            throw CatalogRefreshError.validation(message: "Unknown RefreshOutcome variant", source: _rbRawValue)
        }
        return _rbValue
    }.value
}
...
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.CatalogRefreshConfig: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
...
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.InFlightLimitConfig: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
...
extension RustBridge.RefreshOutcome: @unchecked Sendable {}
