...
// alef:hash:80c3fd299d77e8c991217ce77003d6bf1b8bfff9bc1f8a144c9123ab013c0589
...
pub struct ChatCompletionRequest {
    ...
    ///
    /// Untyped for the same reason as `prediction`.
    pub web_search_options: Option<String>,
...
/// Global per-client in-flight provider request limit.
#[frb(mirror(LlmInFlightLimitConfig))]
...
/// ```
#[frb(mirror(ProviderCapabilities))]
...
pub struct SingleflightResult {
    pub(crate) inner: liter_llm::tower::SingleflightResult,
}

#[cfg(feature = "tower")]
impl From<liter_llm::tower::SingleflightResult> for SingleflightResult {
    fn from(inner: liter_llm::tower::SingleflightResult) -> Self {
        Self { inner }
    }
}

#[cfg(feature = "tower")]
impl From<SingleflightResult> for liter_llm::tower::SingleflightResult {
    fn from(value: SingleflightResult) -> Self {
        value.inner
    }
}

/// Configuration for the global per-client in-flight request limit.
#[frb(mirror(InFlightLimitConfig))]
...
#[frb(mirror(CatalogRefreshConfig))]
...
impl DefaultClient {
    ...
    #[frb]
    pub async fn fetch_batch_for_polling(&self, batch_id: String) -> Result<BatchObject, String> {
        self.inner
    ...
    #[frb]
    pub async fn wait_for_batch(&self, batch_id: String, config: WaitForBatchConfig) -> Result<BatchObject, String> {
        self.inner
            .wait_for_batch(&batch_id, liter_llm::WaitForBatchConfig::from(config))
            .await
            .map(|v| BatchObject::from(v))
            .map_err(|e| e.to_string())
    }
...
#[frb(mirror(AssistantPart), unignore)]
...
pub enum EmbeddingInput {
    ...
    Multimodal { field0: Vec<EmbeddingContentPart> },
}

/// A content part in a multimodal embedding input.
#[frb(mirror(EmbeddingContentPart), unignore)]
...
#[frb(mirror(StreamFormat), unignore)]
...
#[frb(mirror(CircuitState), unignore)]
...
#[frb(mirror(HealthStatus), unignore)]
...
#[frb(mirror(RefreshOutcome), unignore)]
...
impl From<&LiterLlmError> for liter_llm::error::LiterLlmError {
    fn from(m: &LiterLlmError) -> Self {
        match m {
            ...
            LiterLlmError::IdempotencyConflict { key: f_key } => Self::IdempotencyConflict { key: f_key.clone() },
            LiterLlmError::IdempotencyInFlight { key: f_key } => Self::IdempotencyInFlight { key: f_key.clone() },
        }
    }
}
...
#[frb(mirror(CatalogRefreshError), unignore)]
...
impl From<liter_llm::types::AssistantMessage> for AssistantMessage {
    fn from(v: liter_llm::types::AssistantMessage) -> Self {
        AssistantMessage {
            content: v.content.map(AssistantContent::from),
            name: v.name.map(|s| s.into()),
            tool_calls: v
                .tool_calls
            ...
            reasoning_content: v.reasoning_content.map(|s| s.into()),
        }
    }
}
...
impl From<liter_llm::types::ChatCompletionRequest> for ChatCompletionRequest {
    fn from(v: liter_llm::types::ChatCompletionRequest) -> Self {
        ChatCompletionRequest {
            ...
            modalities: v
                .modalities
                .map(|vec| vec.into_iter().map(Modality::from).collect::<Vec<_>>()),
            logprobs: v.logprobs.map(|x| x as _),
            top_logprobs: v.top_logprobs.map(|x| x as _),
            max_completion_tokens: v.max_completion_tokens.map(|x| x as _),
            service_tier: v.service_tier.map(|s| s.into()),
            store: v.store.map(|x| x as _),
            metadata: v
                .metadata
                .map(|m| m.into_iter().map(|(k, v)| (k.into(), v.into())).collect()),
            prediction: v.prediction.map(|j| serde_json::to_string(&j).unwrap_or_default()),
            audio: v.audio.map(|j| serde_json::to_string(&j).unwrap_or_default()),
            web_search_options: v
                .web_search_options
...
impl From<liter_llm::types::Choice> for Choice {
    fn from(v: liter_llm::types::Choice) -> Self {
        Choice {
            ...
            logprobs: v.logprobs.map(|j| serde_json::to_string(&j).unwrap_or_default()),
        }
    }
}
...
impl From<liter_llm::image::DecodedDataUrl> for DecodedDataUrl {
    fn from(v: liter_llm::image::DecodedDataUrl) -> Self {
        DecodedDataUrl {
            mime: v.mime.into(),
            data: v.data.into(),
        }
...
impl From<liter_llm::client::LlmConfig> for LlmConfig {
    fn from(v: liter_llm::client::LlmConfig) -> Self {
        LlmConfig {
            model: v.model.into(),
            ...
            cache: v.cache.map(LlmCacheConfig::from),
            budget: v.budget.map(LlmBudgetConfig::from),
            rate_limit: v.rate_limit.map(LlmRateLimitConfig::from),
            in_flight_limit: v.in_flight_limit.map(LlmInFlightLimitConfig::from),
            ...
            bedrock: v.bedrock.map(BedrockConfig::from),
        }
    }
}

impl From<liter_llm::client::LlmCacheConfig> for LlmCacheConfig {
    fn from(v: liter_llm::client::LlmCacheConfig) -> Self {
        LlmCacheConfig {
...
impl From<liter_llm::client::LlmBudgetConfig> for LlmBudgetConfig {
    fn from(v: liter_llm::client::LlmBudgetConfig) -> Self {
        LlmBudgetConfig {
...
impl From<liter_llm::client::LlmRateLimitConfig> for LlmRateLimitConfig {
    fn from(v: liter_llm::client::LlmRateLimitConfig) -> Self {
        LlmRateLimitConfig {
...
impl From<liter_llm::client::LlmInFlightLimitConfig> for LlmInFlightLimitConfig {
    fn from(v: liter_llm::client::LlmInFlightLimitConfig) -> Self {
        LlmInFlightLimitConfig {
            max_in_flight: v.max_in_flight.map(|x| x as _),
        }
    }
}

impl From<liter_llm::client::LlmProviderConfig> for LlmProviderConfig {
    fn from(v: liter_llm::client::LlmProviderConfig) -> Self {
        LlmProviderConfig {
            name: v.name.into(),
...
impl From<liter_llm::client::BedrockConfig> for BedrockConfig {
    fn from(v: liter_llm::client::BedrockConfig) -> Self {
        BedrockConfig {
...
impl From<liter_llm::client::WaitForBatchConfig> for WaitForBatchConfig {
    fn from(v: liter_llm::client::WaitForBatchConfig) -> Self {
        WaitForBatchConfig {
            initial_interval_secs: v.initial_interval_secs as _,
            max_interval_secs: v.max_interval_secs as _,
            backoff_multiplier: v.backoff_multiplier as _,
            timeout_secs: v.timeout_secs.map(|x| x as _),
        }
    }
}
...
impl From<liter_llm::provider::ProviderCapabilities> for ProviderCapabilities {
    fn from(v: liter_llm::provider::ProviderCapabilities) -> Self {
        ProviderCapabilities {
            vision: v.vision as _,
            reasoning: v.reasoning as _,
            structured_output: v.structured_output as _,
            function_calling: v.function_calling as _,
            audio_in: v.audio_in as _,
            audio_out: v.audio_out as _,
            video_in: v.video_in as _,
        }
    }
}
...
impl From<liter_llm::ModelInfo> for ModelInfo {
    fn from(v: liter_llm::ModelInfo) -> Self {
        ModelInfo {
            input_cost_per_token: v.input_cost_per_token as _,
            output_cost_per_token: v.output_cost_per_token as _,
            cache_read_input_token_cost: v.cache_read_input_token_cost.map(|x| x as _),
            cache_creation_input_token_cost: v.cache_creation_input_token_cost.map(|x| x as _),
            input_cost_per_audio_token: v.input_cost_per_audio_token.map(|x| x as _),
            output_cost_per_audio_token: v.output_cost_per_audio_token.map(|x| x as _),
            output_cost_per_reasoning_token: v.output_cost_per_reasoning_token.map(|x| x as _),
            max_tokens: v.max_tokens.map(|x| x as _),
            max_input_tokens: v.max_input_tokens.map(|x| x as _),
            max_output_tokens: v.max_output_tokens.map(|x| x as _),
            mode: v.mode.map(|s| s.into()),
            supports_vision: v.supports_vision.map(|x| x as _),
            supports_function_calling: v.supports_function_calling.map(|x| x as _),
            supports_reasoning: v.supports_reasoning.map(|x| x as _),
            supports_structured_output: v.supports_structured_output.map(|x| x as _),
            supports_audio_input: v.supports_audio_input.map(|x| x as _),
            supports_audio_output: v.supports_audio_output.map(|x| x as _),
            supports_prompt_caching: v.supports_prompt_caching.map(|x| x as _),
            tiers: v.tiers.into_iter().map(ModelTier::from).collect::<Vec<_>>(),
        }
    }
}

impl From<liter_llm::ModelTier> for ModelTier {
    fn from(v: liter_llm::ModelTier) -> Self {
        ModelTier {
            min_context_tokens: v.min_context_tokens as _,
            input_cost_per_token: v.input_cost_per_token as _,
            output_cost_per_token: v.output_cost_per_token as _,
            cache_read_input_token_cost: v.cache_read_input_token_cost.map(|x| x as _),
            cache_creation_input_token_cost: v.cache_creation_input_token_cost.map(|x| x as _),
...
impl From<liter_llm::InFlightLimitConfig> for InFlightLimitConfig {
    fn from(v: liter_llm::InFlightLimitConfig) -> Self {
        InFlightLimitConfig {
            max_in_flight: v.max_in_flight.map(|x| x as _),
        }
    }
...
impl From<liter_llm::tower::IntentPrototype> for IntentPrototype {
    fn from(v: liter_llm::tower::IntentPrototype) -> Self {
        IntentPrototype {
            name: v.name.into(),
            embedding: v.embedding.into_iter().map(|x| x as _).collect::<Vec<_>>(),
            model: v.model.into(),
...
impl From<liter_llm::cost::refresh::CatalogRefreshConfig> for CatalogRefreshConfig {
    fn from(v: liter_llm::cost::refresh::CatalogRefreshConfig) -> Self {
        CatalogRefreshConfig {
            enabled: v.enabled as _,
            source_url: v.source_url.into(),
            ttl_seconds: v.ttl_seconds as _,
            cache_path: v.cache_path.map(|s| s.into()),
        }
    }
}
...
fn try_convert_message_from_core(value: liter_llm::types::Message) -> Result<Message, String> {
    let representable = match &value {
        liter_llm::types::Message::System(..) => true,
        liter_llm::types::Message::User(..) => true,
        liter_llm::types::Message::Assistant(..) => true,
        liter_llm::types::Message::Tool(..) => true,
        liter_llm::types::Message::Developer(..) => true,
        liter_llm::types::Message::Function(..) => true,
    };
    if !representable {
        return Err("Message contains a variant unavailable in the Dart binding".to_string());
    }
...
fn try_convert_user_content_from_core(value: liter_llm::types::UserContent) -> Result<UserContent, String> {
    let representable = match &value {
        liter_llm::types::UserContent::Text(..) => true,
        liter_llm::types::UserContent::Parts(..) => true,
    };
    if !representable {
        return Err("UserContent contains a variant unavailable in the Dart binding".to_string());
    }
    Ok(UserContent::from(value))
}
...
fn try_convert_content_part_from_core(value: liter_llm::types::ContentPart) -> Result<ContentPart, String> {
    let representable = match &value {
        liter_llm::types::ContentPart::Text { .. } => true,
        liter_llm::types::ContentPart::ImageUrl { .. } => true,
        liter_llm::types::ContentPart::Document { .. } => true,
        liter_llm::types::ContentPart::InputAudio { .. } => true,
    };
    if !representable {
        return Err("ContentPart contains a variant unavailable in the Dart binding".to_string());
    }
    Ok(ContentPart::from(value))
}
...
fn try_convert_image_detail_from_core(value: liter_llm::types::ImageDetail) -> Result<ImageDetail, String> {
    let representable = match &value {
        liter_llm::types::ImageDetail::Low => true,
        liter_llm::types::ImageDetail::High => true,
        liter_llm::types::ImageDetail::Auto => true,
    };
    if !representable {
        return Err("ImageDetail contains a variant unavailable in the Dart binding".to_string());
    }
    Ok(ImageDetail::from(value))
}

impl From<liter_llm::types::AssistantContent> for AssistantContent {
    fn from(v: liter_llm::types::AssistantContent) -> Self {
        match v {
            liter_llm::types::AssistantContent::Text(f0) => AssistantContent::Text { field0: f0 },
            liter_llm::types::AssistantContent::Parts(f0) => AssistantContent::Parts {
                field0: f0.into_iter().map(AssistantPart::from).collect(),
            },
        }
    }
}
fn try_convert_assistant_content_from_core(
    value: liter_llm::types::AssistantContent,
) -> Result<AssistantContent, String> {
    let representable = match &value {
        liter_llm::types::AssistantContent::Text(..) => true,
        liter_llm::types::AssistantContent::Parts(..) => true,
    };
    if !representable {
        return Err("AssistantContent contains a variant unavailable in the Dart binding".to_string());
    }
    Ok(AssistantContent::from(value))
}

impl From<liter_llm::types::AssistantPart> for AssistantPart {
    fn from(v: liter_llm::types::AssistantPart) -> Self {
        match v {
            liter_llm::types::AssistantPart::Text { text } => AssistantPart::Text { text },
            liter_llm::types::AssistantPart::Refusal { refusal } => AssistantPart::Refusal { refusal },
            liter_llm::types::AssistantPart::OutputImage { image_url } => AssistantPart::OutputImage {
                image_url: ImageUrl::from(image_url),
            },
            liter_llm::types::AssistantPart::OutputAudio { audio } => AssistantPart::OutputAudio {
                audio: AudioContent::from(audio),
            },
        }
    }
}
fn try_convert_assistant_part_from_core(value: liter_llm::types::AssistantPart) -> Result<AssistantPart, String> {
    let representable = match &value {
        liter_llm::types::AssistantPart::Text { .. } => true,
        liter_llm::types::AssistantPart::Refusal { .. } => true,
        liter_llm::types::AssistantPart::OutputImage { .. } => true,
        liter_llm::types::AssistantPart::OutputAudio { .. } => true,
    };
    if !representable {
        return Err("AssistantPart contains a variant unavailable in the Dart binding".to_string());
    }
    Ok(AssistantPart::from(value))
}
...
fn try_convert_tool_type_from_core(value: liter_llm::types::ToolType) -> Result<ToolType, String> {
    let representable = match &value {
        liter_llm::types::ToolType::Function => true,
    };
    if !representable {
        return Err("ToolType contains a variant unavailable in the Dart binding".to_string());
    }
    Ok(ToolType::from(value))
}
...
fn try_convert_tool_choice_from_core(value: liter_llm::types::ToolChoice) -> Result<ToolChoice, String> {
    let representable = match &value {
        liter_llm::types::ToolChoice::Mode(..) => true,
        liter_llm::types::ToolChoice::Specific(..) => true,
    };
    if !representable {
        return Err("ToolChoice contains a variant unavailable in the Dart binding".to_string());
    }
    Ok(ToolChoice::from(value))
}
...
fn try_convert_tool_choice_mode_from_core(value: liter_llm::types::ToolChoiceMode) -> Result<ToolChoiceMode, String> {
    let representable = match &value {
        liter_llm::types::ToolChoiceMode::Auto => true,
        liter_llm::types::ToolChoiceMode::Required => true,
        liter_llm::types::ToolChoiceMode::None => true,
    };
    if !representable {
        return Err("ToolChoiceMode contains a variant unavailable in the Dart binding".to_string());
    }
    Ok(ToolChoiceMode::from(value))
}
...
fn try_convert_response_format_from_core(value: liter_llm::types::ResponseFormat) -> Result<ResponseFormat, String> {
    let representable = match &value {
        liter_llm::types::ResponseFormat::Text => true,
        liter_llm::types::ResponseFormat::JsonObject => true,
        liter_llm::types::ResponseFormat::JsonSchema { .. } => true,
    };
    if !representable {
        return Err("ResponseFormat contains a variant unavailable in the Dart binding".to_string());
    }
    Ok(ResponseFormat::from(value))
}
...
fn try_convert_stop_sequence_from_core(value: liter_llm::types::StopSequence) -> Result<StopSequence, String> {
    let representable = match &value {
        liter_llm::types::StopSequence::Single(..) => true,
        liter_llm::types::StopSequence::Multiple(..) => true,
    };
    if !representable {
        return Err("StopSequence contains a variant unavailable in the Dart binding".to_string());
    }
    Ok(StopSequence::from(value))
}

impl From<liter_llm::types::Modality> for Modality {
    fn from(v: liter_llm::types::Modality) -> Self {
        match v {
            liter_llm::types::Modality::Text => Modality::Text,
            liter_llm::types::Modality::Audio => Modality::Audio,
            liter_llm::types::Modality::Image => Modality::Image,
        }
    }
}
fn try_convert_modality_from_core(value: liter_llm::types::Modality) -> Result<Modality, String> {
    let representable = match &value {
        liter_llm::types::Modality::Text => true,
        liter_llm::types::Modality::Audio => true,
        liter_llm::types::Modality::Image => true,
    };
    if !representable {
        return Err("Modality contains a variant unavailable in the Dart binding".to_string());
    }
    Ok(Modality::from(value))
}
...
fn try_convert_finish_reason_from_core(value: liter_llm::types::FinishReason) -> Result<FinishReason, String> {
    let representable = match &value {
        liter_llm::types::FinishReason::Stop => true,
        liter_llm::types::FinishReason::Length => true,
        liter_llm::types::FinishReason::ToolCalls => true,
        liter_llm::types::FinishReason::ContentFilter => true,
        liter_llm::types::FinishReason::FunctionCall => true,
        liter_llm::types::FinishReason::Other => true,
    };
    if !representable {
        return Err("FinishReason contains a variant unavailable in the Dart binding".to_string());
    }
...
impl From<liter_llm::types::ReasoningEffort> for ReasoningEffort {
    fn from(v: liter_llm::types::ReasoningEffort) -> Self {
        match v {
            ...
            liter_llm::types::ReasoningEffort::Minimal => ReasoningEffort::Minimal,
            liter_llm::types::ReasoningEffort::Max => ReasoningEffort::Max,
        }
    }
}
fn try_convert_reasoning_effort_from_core(value: liter_llm::types::ReasoningEffort) -> Result<ReasoningEffort, String> {
    let representable = match &value {
        liter_llm::types::ReasoningEffort::Low => true,
        liter_llm::types::ReasoningEffort::Medium => true,
        liter_llm::types::ReasoningEffort::High => true,
        liter_llm::types::ReasoningEffort::Minimal => true,
        liter_llm::types::ReasoningEffort::Max => true,
    };
    if !representable {
        return Err("ReasoningEffort contains a variant unavailable in the Dart binding".to_string());
    }
    Ok(ReasoningEffort::from(value))
}
...
fn try_convert_embedding_format_from_core(value: liter_llm::types::EmbeddingFormat) -> Result<EmbeddingFormat, String> {
    let representable = match &value {
        liter_llm::types::EmbeddingFormat::Float => true,
        liter_llm::types::EmbeddingFormat::Base64 => true,
    };
    if !representable {
        return Err("EmbeddingFormat contains a variant unavailable in the Dart binding".to_string());
    }
    Ok(EmbeddingFormat::from(value))
}
...
impl From<liter_llm::types::EmbeddingInput> for EmbeddingInput {
    fn from(v: liter_llm::types::EmbeddingInput) -> Self {
        match v {
            ...
            liter_llm::types::EmbeddingInput::Multimodal(f0) => EmbeddingInput::Multimodal {
                field0: f0.into_iter().map(EmbeddingContentPart::from).collect(),
            },
        }
    }
}
fn try_convert_embedding_input_from_core(value: liter_llm::types::EmbeddingInput) -> Result<EmbeddingInput, String> {
    let representable = match &value {
        liter_llm::types::EmbeddingInput::Single(..) => true,
        liter_llm::types::EmbeddingInput::Multiple(..) => true,
        liter_llm::types::EmbeddingInput::Multimodal(..) => true,
    };
    if !representable {
        return Err("EmbeddingInput contains a variant unavailable in the Dart binding".to_string());
    }
    Ok(EmbeddingInput::from(value))
}

impl From<liter_llm::types::EmbeddingContentPart> for EmbeddingContentPart {
    fn from(v: liter_llm::types::EmbeddingContentPart) -> Self {
        match v {
            liter_llm::types::EmbeddingContentPart::Text { text } => EmbeddingContentPart::Text { text },
            liter_llm::types::EmbeddingContentPart::ImageUrl { image_url } => EmbeddingContentPart::ImageUrl {
                image_url: ImageUrl::from(image_url),
            },
            liter_llm::types::EmbeddingContentPart::ImageBase64 { image_base64 } => {
                EmbeddingContentPart::ImageBase64 { image_base64 }
            }
        }
    }
}
fn try_convert_embedding_content_part_from_core(
    value: liter_llm::types::EmbeddingContentPart,
) -> Result<EmbeddingContentPart, String> {
    let representable = match &value {
        liter_llm::types::EmbeddingContentPart::Text { .. } => true,
        liter_llm::types::EmbeddingContentPart::ImageUrl { .. } => true,
        liter_llm::types::EmbeddingContentPart::ImageBase64 { .. } => true,
    };
    if !representable {
        return Err("EmbeddingContentPart contains a variant unavailable in the Dart binding".to_string());
    }
    Ok(EmbeddingContentPart::from(value))
}
...
fn try_convert_moderation_input_from_core(value: liter_llm::types::ModerationInput) -> Result<ModerationInput, String> {
    let representable = match &value {
        liter_llm::types::ModerationInput::Single(..) => true,
        liter_llm::types::ModerationInput::Multiple(..) => true,
    };
    if !representable {
        return Err("ModerationInput contains a variant unavailable in the Dart binding".to_string());
    }
    Ok(ModerationInput::from(value))
}
...
fn try_convert_rerank_document_from_core(value: liter_llm::types::RerankDocument) -> Result<RerankDocument, String> {
    let representable = match &value {
        liter_llm::types::RerankDocument::Text(..) => true,
        liter_llm::types::RerankDocument::Object { .. } => true,
    };
    if !representable {
        return Err("RerankDocument contains a variant unavailable in the Dart binding".to_string());
    }
    Ok(RerankDocument::from(value))
}
...
fn try_convert_ocr_document_from_core(value: liter_llm::types::OcrDocument) -> Result<OcrDocument, String> {
    let representable = match &value {
        liter_llm::types::OcrDocument::Url { .. } => true,
        liter_llm::types::OcrDocument::Base64 { .. } => true,
    };
    if !representable {
        return Err("OcrDocument contains a variant unavailable in the Dart binding".to_string());
    }
    Ok(OcrDocument::from(value))
}
...
fn try_convert_file_purpose_from_core(value: liter_llm::types::FilePurpose) -> Result<FilePurpose, String> {
    let representable = match &value {
        liter_llm::types::FilePurpose::Assistants => true,
        liter_llm::types::FilePurpose::Batch => true,
        liter_llm::types::FilePurpose::FineTune => true,
        liter_llm::types::FilePurpose::Vision => true,
    };
    if !representable {
        return Err("FilePurpose contains a variant unavailable in the Dart binding".to_string());
    }
    Ok(FilePurpose::from(value))
}
...
fn try_convert_batch_status_from_core(value: liter_llm::types::BatchStatus) -> Result<BatchStatus, String> {
    let representable = match &value {
        liter_llm::types::BatchStatus::Validating => true,
        liter_llm::types::BatchStatus::Failed => true,
        liter_llm::types::BatchStatus::InProgress => true,
        liter_llm::types::BatchStatus::Finalizing => true,
        liter_llm::types::BatchStatus::Completed => true,
        liter_llm::types::BatchStatus::Expired => true,
        liter_llm::types::BatchStatus::Cancelling => true,
        liter_llm::types::BatchStatus::Cancelled => true,
    };
    if !representable {
        return Err("BatchStatus contains a variant unavailable in the Dart binding".to_string());
    }
    Ok(BatchStatus::from(value))
}
...
fn try_convert_auth_header_format_from_core(
    value: liter_llm::provider::custom::AuthHeaderFormat,
) -> Result<AuthHeaderFormat, String> {
    let representable = match &value {
        liter_llm::provider::custom::AuthHeaderFormat::Bearer => true,
        liter_llm::provider::custom::AuthHeaderFormat::ApiKey(..) => true,
        liter_llm::provider::custom::AuthHeaderFormat::None => true,
    };
    if !representable {
        return Err("AuthHeaderFormat contains a variant unavailable in the Dart binding".to_string());
    }
    Ok(AuthHeaderFormat::from(value))
}

impl From<liter_llm::provider::StreamFormat> for StreamFormat {
    fn from(v: liter_llm::provider::StreamFormat) -> Self {
        match v {
            liter_llm::provider::StreamFormat::Sse => StreamFormat::Sse,
            liter_llm::provider::StreamFormat::AwsEventStream => StreamFormat::AwsEventStream,
        }
    }
}
fn try_convert_stream_format_from_core(value: liter_llm::provider::StreamFormat) -> Result<StreamFormat, String> {
    let representable = match &value {
        liter_llm::provider::StreamFormat::Sse => true,
        liter_llm::provider::StreamFormat::AwsEventStream => true,
    };
    if !representable {
        return Err("StreamFormat contains a variant unavailable in the Dart binding".to_string());
    }
    Ok(StreamFormat::from(value))
}
...
fn try_convert_auth_type_from_core(value: liter_llm::provider::AuthType) -> Result<AuthType, String> {
    let representable = match &value {
        liter_llm::provider::AuthType::Bearer => true,
        liter_llm::provider::AuthType::ApiKey => true,
        liter_llm::provider::AuthType::None => true,
        liter_llm::provider::AuthType::Unknown => true,
    };
    if !representable {
        return Err("AuthType contains a variant unavailable in the Dart binding".to_string());
    }
    Ok(AuthType::from(value))
}
...
#[cfg(feature = "tower")]
fn try_convert_enforcement_from_core(value: liter_llm::Enforcement) -> Result<Enforcement, String> {
    let representable = match &value {
        liter_llm::Enforcement::Hard => true,
        liter_llm::Enforcement::Soft => true,
    };
    if !representable {
        return Err("Enforcement contains a variant unavailable in the Dart binding".to_string());
    }
    Ok(Enforcement::from(value))
}
...
#[cfg(feature = "tower")]
fn try_convert_cache_backend_from_core(value: liter_llm::CacheBackend) -> Result<CacheBackend, String> {
    let representable = match &value {
        liter_llm::CacheBackend::Memory => true,
        #[cfg(feature = "opendal-cache")]
        liter_llm::CacheBackend::OpenDal { .. } => true,
    };
    if !representable {
        return Err("CacheBackend contains a variant unavailable in the Dart binding".to_string());
    }
    Ok(CacheBackend::from(value))
}

#[cfg(feature = "tower")]
impl From<liter_llm::tower::CircuitState> for CircuitState {
    fn from(v: liter_llm::tower::CircuitState) -> Self {
        match v {
            liter_llm::tower::CircuitState::Closed => CircuitState::Closed,
            liter_llm::tower::CircuitState::Open => CircuitState::Open,
            liter_llm::tower::CircuitState::HalfOpen => CircuitState::HalfOpen,
        }
    }
}
...
impl From<liter_llm::tower::HealthStatus> for HealthStatus {
    fn from(v: liter_llm::tower::HealthStatus) -> Self {
        match v {
            liter_llm::tower::HealthStatus::Healthy => HealthStatus::Healthy,
            liter_llm::tower::HealthStatus::Unhealthy => HealthStatus::Unhealthy,
        }
    }
}

impl From<liter_llm::cost::refresh::RefreshOutcome> for RefreshOutcome {
    fn from(v: liter_llm::cost::refresh::RefreshOutcome) -> Self {
        match v {
            liter_llm::cost::refresh::RefreshOutcome::Disabled => RefreshOutcome::Disabled,
            liter_llm::cost::refresh::RefreshOutcome::FromCache => RefreshOutcome::FromCache,
            liter_llm::cost::refresh::RefreshOutcome::Fetched => RefreshOutcome::Fetched,
        }
    }
}
fn try_convert_refresh_outcome_from_core(
    value: liter_llm::cost::refresh::RefreshOutcome,
) -> Result<RefreshOutcome, String> {
    let representable = match &value {
        liter_llm::cost::refresh::RefreshOutcome::Disabled => true,
        liter_llm::cost::refresh::RefreshOutcome::FromCache => true,
        liter_llm::cost::refresh::RefreshOutcome::Fetched => true,
    };
    if !representable {
        return Err("RefreshOutcome contains a variant unavailable in the Dart binding".to_string());
    }
    Ok(RefreshOutcome::from(value))
}
...
impl From<AssistantMessage> for liter_llm::types::AssistantMessage {
    fn from(v: AssistantMessage) -> Self {
        liter_llm::types::AssistantMessage {
            ...
            reasoning_content: v.reasoning_content.map(Into::into),
            ..Default::default()
        }
    }
}
...
impl From<SpecificFunction> for liter_llm::types::SpecificFunction {
    fn from(v: SpecificFunction) -> Self {
        liter_llm::types::SpecificFunction {
            name: v.name.into(),
            ..Default::default()
        }
    }
...
impl From<ChatCompletionRequest> for liter_llm::types::ChatCompletionRequest {
    fn from(v: ChatCompletionRequest) -> Self {
        liter_llm::types::ChatCompletionRequest {
            ...
            modalities: v.modalities.map(|vec| vec.into_iter().map(Into::into).collect()),
            logprobs: v.logprobs.map(|x| x as _),
            top_logprobs: v.top_logprobs.map(|x| x as _),
            max_completion_tokens: v.max_completion_tokens.map(|x| x as _),
            ...
            metadata: v
                .metadata
                .map(|m| m.into_iter().map(|(k, v)| (k.into(), v.into())).collect()),
            prediction: v.prediction.as_deref().and_then(|s| serde_json::from_str(s).ok()),
            audio: v.audio.as_deref().and_then(|s| serde_json::from_str(s).ok()),
            web_search_options: v
                .web_search_options
                .as_deref()
...
impl From<OcrRequest> for liter_llm::types::OcrRequest {
    fn from(v: OcrRequest) -> Self {
        liter_llm::types::OcrRequest {
            ...
            pages: v.pages.map(|vec| vec.into_iter().map(|x| x as u32).collect::<Vec<_>>()),
            include_image_base64: v.include_image_base64.map(|x| x as _),
            ..Default::default()
        }
    }
...
impl From<WaitForBatchConfig> for liter_llm::client::WaitForBatchConfig {
    fn from(v: WaitForBatchConfig) -> Self {
        liter_llm::client::WaitForBatchConfig {
            initial_interval_secs: v.initial_interval_secs as _,
            max_interval_secs: v.max_interval_secs as _,
            backoff_multiplier: v.backoff_multiplier as _,
            timeout_secs: v.timeout_secs.map(|x| x as _),
...
impl From<CatalogRefreshConfig> for liter_llm::cost::refresh::CatalogRefreshConfig {
    fn from(v: CatalogRefreshConfig) -> Self {
        liter_llm::cost::refresh::CatalogRefreshConfig {
            enabled: v.enabled as _,
            source_url: v.source_url.into(),
            ttl_seconds: v.ttl_seconds as _,
            cache_path: v.cache_path.map(Into::into),
            ..Default::default()
        }
    }
}
...
impl From<AssistantContent> for liter_llm::types::AssistantContent {
    fn from(v: AssistantContent) -> Self {
        match v {
            AssistantContent::Text { field0 } => liter_llm::types::AssistantContent::Text(field0),
            AssistantContent::Parts { field0 } => {
                liter_llm::types::AssistantContent::Parts(field0.into_iter().map(Into::into).collect())
            }
        }
    }
}
...
impl From<AssistantPart> for liter_llm::types::AssistantPart {
    fn from(v: AssistantPart) -> Self {
        match v {
            AssistantPart::Text { text } => liter_llm::types::AssistantPart::Text { text },
            AssistantPart::Refusal { refusal } => liter_llm::types::AssistantPart::Refusal { refusal },
            AssistantPart::OutputImage { image_url } => liter_llm::types::AssistantPart::OutputImage {
                image_url: image_url.into(),
            },
            AssistantPart::OutputAudio { audio } => {
                liter_llm::types::AssistantPart::OutputAudio { audio: audio.into() }
            }
        }
    }
}
...
impl From<Modality> for liter_llm::types::Modality {
    fn from(v: Modality) -> Self {
        match v {
            Modality::Text => liter_llm::types::Modality::Text,
            Modality::Audio => liter_llm::types::Modality::Audio,
            Modality::Image => liter_llm::types::Modality::Image,
        }
    }
}
...
impl From<ReasoningEffort> for liter_llm::types::ReasoningEffort {
    fn from(v: ReasoningEffort) -> Self {
        match v {
            ...
            ReasoningEffort::Minimal => liter_llm::types::ReasoningEffort::Minimal,
            ReasoningEffort::Max => liter_llm::types::ReasoningEffort::Max,
        }
    }
}
...
impl From<EmbeddingInput> for liter_llm::types::EmbeddingInput {
    fn from(v: EmbeddingInput) -> Self {
        match v {
            ...
            EmbeddingInput::Multimodal { field0 } => {
                liter_llm::types::EmbeddingInput::Multimodal(field0.into_iter().map(Into::into).collect())
            }
        }
    }
}
...
impl From<EmbeddingContentPart> for liter_llm::types::EmbeddingContentPart {
    fn from(v: EmbeddingContentPart) -> Self {
        match v {
            EmbeddingContentPart::Text { text } => liter_llm::types::EmbeddingContentPart::Text { text },
            EmbeddingContentPart::ImageUrl { image_url } => liter_llm::types::EmbeddingContentPart::ImageUrl {
                image_url: image_url.into(),
            },
            EmbeddingContentPart::ImageBase64 { image_base64 } => {
                liter_llm::types::EmbeddingContentPart::ImageBase64 { image_base64 }
...
pub fn decode_data_url(url: String) -> Option<DecodedDataUrl> {
    liter_llm::image::decode_data_url(&url).map(DecodedDataUrl::from)
}
...
pub fn capabilities(provider_name: String) -> ProviderCapabilities {
    ProviderCapabilities::from(liter_llm::provider::capabilities(&provider_name))
}
...
pub fn model_info(model: String) -> Option<ModelInfo> {
    liter_llm::model_info(&model).map(ModelInfo::from)
}
...
pub fn check_bound(context: String, current_len: i64, incoming: i64, limit: i64) -> Result<(), String> {
...
/// call.
pub fn install_catalog_overlay_from_str(catalog_json: String) -> Result<(), String> {
...
/// availability.
pub async fn refresh_catalog(config: CatalogRefreshConfig) -> Result<RefreshOutcome, String> {
    liter_llm::cost::refresh::refresh_catalog(&liter_llm::cost::refresh::CatalogRefreshConfig::from(config))
        .await
...
pub fn create_system_message_from_json(json: String) -> Result<SystemMessage, String> {
    serde_json::from_str::<liter_llm::types::SystemMessage>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(SystemMessage::from(value)))
}
...
pub fn create_user_message_from_json(json: String) -> Result<UserMessage, String> {
    serde_json::from_str::<liter_llm::types::UserMessage>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(UserMessage::from(value)))
}
...
pub fn create_image_url_from_json(json: String) -> Result<ImageUrl, String> {
    serde_json::from_str::<liter_llm::types::ImageUrl>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(ImageUrl::from(value)))
}
...
pub fn create_document_content_from_json(json: String) -> Result<DocumentContent, String> {
    serde_json::from_str::<liter_llm::types::DocumentContent>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(DocumentContent::from(value)))
}
...
pub fn create_audio_content_from_json(json: String) -> Result<AudioContent, String> {
    serde_json::from_str::<liter_llm::types::AudioContent>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(AudioContent::from(value)))
}
...
pub fn create_assistant_message_from_json(json: String) -> Result<AssistantMessage, String> {
    serde_json::from_str::<liter_llm::types::AssistantMessage>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(AssistantMessage::from(value)))
}
...
pub fn create_tool_message_from_json(json: String) -> Result<ToolMessage, String> {
    serde_json::from_str::<liter_llm::types::ToolMessage>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(ToolMessage::from(value)))
}
...
pub fn create_developer_message_from_json(json: String) -> Result<DeveloperMessage, String> {
    serde_json::from_str::<liter_llm::types::DeveloperMessage>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(DeveloperMessage::from(value)))
}
...
pub fn create_function_message_from_json(json: String) -> Result<FunctionMessage, String> {
    serde_json::from_str::<liter_llm::types::FunctionMessage>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(FunctionMessage::from(value)))
}
...
pub fn create_chat_completion_tool_from_json(json: String) -> Result<ChatCompletionTool, String> {
    serde_json::from_str::<liter_llm::types::ChatCompletionTool>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(ChatCompletionTool::from(value)))
}
...
pub fn create_function_definition_from_json(json: String) -> Result<FunctionDefinition, String> {
    serde_json::from_str::<liter_llm::types::FunctionDefinition>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(FunctionDefinition::from(value)))
}
...
pub fn create_tool_call_from_json(json: String) -> Result<ToolCall, String> {
    serde_json::from_str::<liter_llm::types::ToolCall>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(ToolCall::from(value)))
}
...
pub fn create_function_call_from_json(json: String) -> Result<FunctionCall, String> {
    serde_json::from_str::<liter_llm::types::FunctionCall>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(FunctionCall::from(value)))
}
...
pub fn create_specific_tool_choice_from_json(json: String) -> Result<SpecificToolChoice, String> {
    serde_json::from_str::<liter_llm::types::SpecificToolChoice>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(SpecificToolChoice::from(value)))
}
...
pub fn create_specific_function_from_json(json: String) -> Result<SpecificFunction, String> {
    serde_json::from_str::<liter_llm::types::SpecificFunction>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(SpecificFunction::from(value)))
}
...
pub fn create_json_schema_format_from_json(json: String) -> Result<JsonSchemaFormat, String> {
    serde_json::from_str::<liter_llm::types::JsonSchemaFormat>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(JsonSchemaFormat::from(value)))
}
...
pub fn create_usage_from_json(json: String) -> Result<Usage, String> {
    serde_json::from_str::<liter_llm::types::Usage>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(Usage::from(value)))
}
...
pub fn create_prompt_tokens_details_from_json(json: String) -> Result<PromptTokensDetails, String> {
    serde_json::from_str::<liter_llm::types::PromptTokensDetails>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(PromptTokensDetails::from(value)))
}
...
pub fn create_chat_completion_request_from_json(json: String) -> Result<ChatCompletionRequest, String> {
    serde_json::from_str::<liter_llm::types::ChatCompletionRequest>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(ChatCompletionRequest::from(value)))
}
...
pub fn create_stream_options_from_json(json: String) -> Result<StreamOptions, String> {
    serde_json::from_str::<liter_llm::types::StreamOptions>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(StreamOptions::from(value)))
}
...
pub fn create_chat_completion_response_from_json(json: String) -> Result<ChatCompletionResponse, String> {
    serde_json::from_str::<liter_llm::types::ChatCompletionResponse>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(ChatCompletionResponse::from(value)))
}
...
pub fn create_choice_from_json(json: String) -> Result<Choice, String> {
    serde_json::from_str::<liter_llm::types::Choice>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(Choice::from(value)))
}
...
pub fn create_chat_completion_chunk_from_json(json: String) -> Result<ChatCompletionChunk, String> {
    serde_json::from_str::<liter_llm::types::ChatCompletionChunk>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(ChatCompletionChunk::from(value)))
}
...
pub fn create_stream_choice_from_json(json: String) -> Result<StreamChoice, String> {
    serde_json::from_str::<liter_llm::types::StreamChoice>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(StreamChoice::from(value)))
}
...
pub fn create_stream_delta_from_json(json: String) -> Result<StreamDelta, String> {
    serde_json::from_str::<liter_llm::types::StreamDelta>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(StreamDelta::from(value)))
}
...
pub fn create_stream_tool_call_from_json(json: String) -> Result<StreamToolCall, String> {
    serde_json::from_str::<liter_llm::types::StreamToolCall>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(StreamToolCall::from(value)))
}
...
pub fn create_stream_function_call_from_json(json: String) -> Result<StreamFunctionCall, String> {
    serde_json::from_str::<liter_llm::types::StreamFunctionCall>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(StreamFunctionCall::from(value)))
}
...
pub fn create_embedding_request_from_json(json: String) -> Result<EmbeddingRequest, String> {
    serde_json::from_str::<liter_llm::types::EmbeddingRequest>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(EmbeddingRequest::from(value)))
}
...
pub fn create_embedding_response_from_json(json: String) -> Result<EmbeddingResponse, String> {
    serde_json::from_str::<liter_llm::types::EmbeddingResponse>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(EmbeddingResponse::from(value)))
}
...
pub fn create_embedding_object_from_json(json: String) -> Result<EmbeddingObject, String> {
    serde_json::from_str::<liter_llm::types::EmbeddingObject>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(EmbeddingObject::from(value)))
}
...
pub fn create_create_image_request_from_json(json: String) -> Result<CreateImageRequest, String> {
    serde_json::from_str::<liter_llm::types::CreateImageRequest>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(CreateImageRequest::from(value)))
}
...
pub fn create_images_response_from_json(json: String) -> Result<ImagesResponse, String> {
    serde_json::from_str::<liter_llm::types::ImagesResponse>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(ImagesResponse::from(value)))
}
...
pub fn create_image_from_json(json: String) -> Result<Image, String> {
    serde_json::from_str::<liter_llm::types::Image>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(Image::from(value)))
}

#[frb]
pub fn create_decoded_data_url_from_json(json: String) -> Result<DecodedDataUrl, String> {
    serde_json::from_str::<liter_llm::image::DecodedDataUrl>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(DecodedDataUrl::from(value)))
}
...
pub fn create_create_speech_request_from_json(json: String) -> Result<CreateSpeechRequest, String> {
    serde_json::from_str::<liter_llm::types::CreateSpeechRequest>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(CreateSpeechRequest::from(value)))
}
...
pub fn create_create_transcription_request_from_json(json: String) -> Result<CreateTranscriptionRequest, String> {
    serde_json::from_str::<liter_llm::types::CreateTranscriptionRequest>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(CreateTranscriptionRequest::from(value)))
}
...
pub fn create_transcription_response_from_json(json: String) -> Result<TranscriptionResponse, String> {
    serde_json::from_str::<liter_llm::types::TranscriptionResponse>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(TranscriptionResponse::from(value)))
}
...
pub fn create_transcription_segment_from_json(json: String) -> Result<TranscriptionSegment, String> {
    serde_json::from_str::<liter_llm::types::TranscriptionSegment>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(TranscriptionSegment::from(value)))
}
...
pub fn create_moderation_request_from_json(json: String) -> Result<ModerationRequest, String> {
    serde_json::from_str::<liter_llm::types::ModerationRequest>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(ModerationRequest::from(value)))
}
...
pub fn create_moderation_response_from_json(json: String) -> Result<ModerationResponse, String> {
    serde_json::from_str::<liter_llm::types::ModerationResponse>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(ModerationResponse::from(value)))
}
...
pub fn create_moderation_result_from_json(json: String) -> Result<ModerationResult, String> {
    serde_json::from_str::<liter_llm::types::ModerationResult>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(ModerationResult::from(value)))
}
...
pub fn create_moderation_categories_from_json(json: String) -> Result<ModerationCategories, String> {
    serde_json::from_str::<liter_llm::types::ModerationCategories>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(ModerationCategories::from(value)))
}
...
pub fn create_moderation_category_scores_from_json(json: String) -> Result<ModerationCategoryScores, String> {
    serde_json::from_str::<liter_llm::types::ModerationCategoryScores>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(ModerationCategoryScores::from(value)))
}
...
pub fn create_rerank_request_from_json(json: String) -> Result<RerankRequest, String> {
    serde_json::from_str::<liter_llm::types::RerankRequest>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(RerankRequest::from(value)))
}
...
pub fn create_rerank_response_from_json(json: String) -> Result<RerankResponse, String> {
    serde_json::from_str::<liter_llm::types::RerankResponse>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(RerankResponse::from(value)))
}
...
pub fn create_rerank_result_from_json(json: String) -> Result<RerankResult, String> {
    serde_json::from_str::<liter_llm::types::RerankResult>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(RerankResult::from(value)))
}
...
pub fn create_rerank_result_document_from_json(json: String) -> Result<RerankResultDocument, String> {
    serde_json::from_str::<liter_llm::types::RerankResultDocument>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(RerankResultDocument::from(value)))
}
...
pub fn create_search_request_from_json(json: String) -> Result<SearchRequest, String> {
    serde_json::from_str::<liter_llm::types::SearchRequest>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(SearchRequest::from(value)))
}
...
pub fn create_search_response_from_json(json: String) -> Result<SearchResponse, String> {
    serde_json::from_str::<liter_llm::types::SearchResponse>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(SearchResponse::from(value)))
}
...
pub fn create_search_result_from_json(json: String) -> Result<SearchResult, String> {
    serde_json::from_str::<liter_llm::types::SearchResult>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(SearchResult::from(value)))
}
...
pub fn create_ocr_request_from_json(json: String) -> Result<OcrRequest, String> {
    serde_json::from_str::<liter_llm::types::OcrRequest>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(OcrRequest::from(value)))
}
...
pub fn create_ocr_response_from_json(json: String) -> Result<OcrResponse, String> {
    serde_json::from_str::<liter_llm::types::OcrResponse>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(OcrResponse::from(value)))
}
...
pub fn create_ocr_page_from_json(json: String) -> Result<OcrPage, String> {
    serde_json::from_str::<liter_llm::types::OcrPage>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(OcrPage::from(value)))
}
...
pub fn create_ocr_image_from_json(json: String) -> Result<OcrImage, String> {
    serde_json::from_str::<liter_llm::types::OcrImage>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(OcrImage::from(value)))
}
...
pub fn create_models_list_response_from_json(json: String) -> Result<ModelsListResponse, String> {
    serde_json::from_str::<liter_llm::types::ModelsListResponse>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(ModelsListResponse::from(value)))
}
...
pub fn create_model_object_from_json(json: String) -> Result<ModelObject, String> {
    serde_json::from_str::<liter_llm::types::ModelObject>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(ModelObject::from(value)))
}
...
pub fn create_create_file_request_from_json(json: String) -> Result<CreateFileRequest, String> {
    serde_json::from_str::<liter_llm::types::CreateFileRequest>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(CreateFileRequest::from(value)))
}
...
pub fn create_file_object_from_json(json: String) -> Result<FileObject, String> {
    serde_json::from_str::<liter_llm::types::FileObject>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(FileObject::from(value)))
}
...
pub fn create_file_list_response_from_json(json: String) -> Result<FileListResponse, String> {
    serde_json::from_str::<liter_llm::types::FileListResponse>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(FileListResponse::from(value)))
}
...
pub fn create_file_list_query_from_json(json: String) -> Result<FileListQuery, String> {
    serde_json::from_str::<liter_llm::types::FileListQuery>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(FileListQuery::from(value)))
}
...
pub fn create_delete_response_from_json(json: String) -> Result<DeleteResponse, String> {
    serde_json::from_str::<liter_llm::types::DeleteResponse>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(DeleteResponse::from(value)))
}
...
pub fn create_create_batch_request_from_json(json: String) -> Result<CreateBatchRequest, String> {
    serde_json::from_str::<liter_llm::types::CreateBatchRequest>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(CreateBatchRequest::from(value)))
}
...
pub fn create_batch_object_from_json(json: String) -> Result<BatchObject, String> {
    serde_json::from_str::<liter_llm::types::BatchObject>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(BatchObject::from(value)))
}
...
pub fn create_batch_request_counts_from_json(json: String) -> Result<BatchRequestCounts, String> {
    serde_json::from_str::<liter_llm::types::BatchRequestCounts>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(BatchRequestCounts::from(value)))
}
...
pub fn create_batch_list_response_from_json(json: String) -> Result<BatchListResponse, String> {
    serde_json::from_str::<liter_llm::types::BatchListResponse>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(BatchListResponse::from(value)))
}
...
pub fn create_batch_list_query_from_json(json: String) -> Result<BatchListQuery, String> {
    serde_json::from_str::<liter_llm::types::BatchListQuery>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(BatchListQuery::from(value)))
}
...
pub fn create_create_response_request_from_json(json: String) -> Result<CreateResponseRequest, String> {
    serde_json::from_str::<liter_llm::types::CreateResponseRequest>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(CreateResponseRequest::from(value)))
}
...
pub fn create_response_tool_from_json(json: String) -> Result<ResponseTool, String> {
    serde_json::from_str::<liter_llm::types::ResponseTool>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(ResponseTool::from(value)))
}
...
pub fn create_response_object_from_json(json: String) -> Result<ResponseObject, String> {
    serde_json::from_str::<liter_llm::types::ResponseObject>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(ResponseObject::from(value)))
}
...
pub fn create_response_output_item_from_json(json: String) -> Result<ResponseOutputItem, String> {
    serde_json::from_str::<liter_llm::types::ResponseOutputItem>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(ResponseOutputItem::from(value)))
}
...
pub fn create_response_usage_from_json(json: String) -> Result<ResponseUsage, String> {
    serde_json::from_str::<liter_llm::types::ResponseUsage>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(ResponseUsage::from(value)))
}

#[frb]
pub fn create_llm_config_from_json(json: String) -> Result<LlmConfig, String> {
    serde_json::from_str::<liter_llm::client::LlmConfig>(&json)
        .map_err(|e| e.to_string())
...
pub fn create_llm_cache_config_from_json(json: String) -> Result<LlmCacheConfig, String> {
    serde_json::from_str::<liter_llm::client::LlmCacheConfig>(&json)
        .map_err(|e| e.to_string())
...
pub fn create_llm_budget_config_from_json(json: String) -> Result<LlmBudgetConfig, String> {
    serde_json::from_str::<liter_llm::client::LlmBudgetConfig>(&json)
        .map_err(|e| e.to_string())
...
pub fn create_llm_rate_limit_config_from_json(json: String) -> Result<LlmRateLimitConfig, String> {
    serde_json::from_str::<liter_llm::client::LlmRateLimitConfig>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(LlmRateLimitConfig::from(value)))
}

#[frb]
pub fn create_llm_in_flight_limit_config_from_json(json: String) -> Result<LlmInFlightLimitConfig, String> {
    serde_json::from_str::<liter_llm::client::LlmInFlightLimitConfig>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(LlmInFlightLimitConfig::from(value)))
}

#[frb]
pub fn create_llm_provider_config_from_json(json: String) -> Result<LlmProviderConfig, String> {
    serde_json::from_str::<liter_llm::client::LlmProviderConfig>(&json)
        .map_err(|e| e.to_string())
...
pub fn create_bedrock_config_from_json(json: String) -> Result<BedrockConfig, String> {
    serde_json::from_str::<liter_llm::client::BedrockConfig>(&json)
        .map_err(|e| e.to_string())
...
#[cfg(any(feature = "native-http", feature = "wasm-http"))]
#[frb]
pub fn create_wait_for_batch_config_from_json(json: String) -> Result<WaitForBatchConfig, String> {
    serde_json::from_str::<liter_llm::client::WaitForBatchConfig>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(WaitForBatchConfig::from(value)))
}
...
pub fn create_custom_provider_config_from_json(json: String) -> Result<CustomProviderConfig, String> {
    serde_json::from_str::<liter_llm::provider::custom::CustomProviderConfig>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(CustomProviderConfig::from(value)))
}

#[frb]
pub fn create_provider_capabilities_from_json(json: String) -> Result<ProviderCapabilities, String> {
    serde_json::from_str::<liter_llm::provider::ProviderCapabilities>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(ProviderCapabilities::from(value)))
}
...
pub fn create_provider_config_from_json(json: String) -> Result<ProviderConfig, String> {
    serde_json::from_str::<liter_llm::provider::ProviderConfig>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(ProviderConfig::from(value)))
}
...
pub fn create_auth_config_from_json(json: String) -> Result<AuthConfig, String> {
    serde_json::from_str::<liter_llm::provider::AuthConfig>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(AuthConfig::from(value)))
}

#[frb]
pub fn create_model_info_from_json(json: String) -> Result<ModelInfo, String> {
    serde_json::from_str::<liter_llm::ModelInfo>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(ModelInfo::from(value)))
}

#[frb]
pub fn create_model_tier_from_json(json: String) -> Result<ModelTier, String> {
    serde_json::from_str::<liter_llm::ModelTier>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(ModelTier::from(value)))
}
...
pub fn create_budget_config_from_json(json: String) -> Result<BudgetConfig, String> {
    serde_json::from_str::<liter_llm::BudgetConfig>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(BudgetConfig::from(value)))
}
...
pub fn create_cache_config_from_json(json: String) -> Result<CacheConfig, String> {
    serde_json::from_str::<liter_llm::CacheConfig>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(CacheConfig::from(value)))
}

#[cfg(feature = "tower")]
#[frb]
pub fn create_in_flight_limit_config_from_json(json: String) -> Result<InFlightLimitConfig, String> {
    serde_json::from_str::<liter_llm::InFlightLimitConfig>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(InFlightLimitConfig::from(value)))
}
...
pub fn create_rate_limit_config_from_json(json: String) -> Result<RateLimitConfig, String> {
    serde_json::from_str::<liter_llm::RateLimitConfig>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(RateLimitConfig::from(value)))
}

#[frb]
pub fn create_catalog_refresh_config_from_json(json: String) -> Result<CatalogRefreshConfig, String> {
    serde_json::from_str::<liter_llm::cost::refresh::CatalogRefreshConfig>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(CatalogRefreshConfig::from(value)))
}

#[frb]
pub fn create_message_from_json(json: String) -> Result<Message, String> {
    serde_json::from_str::<liter_llm::types::Message>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_message_from_core)
}

#[frb]
pub fn create_user_content_from_json(json: String) -> Result<UserContent, String> {
    serde_json::from_str::<liter_llm::types::UserContent>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_user_content_from_core)
}

#[frb]
pub fn create_content_part_from_json(json: String) -> Result<ContentPart, String> {
    serde_json::from_str::<liter_llm::types::ContentPart>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_content_part_from_core)
}

#[frb]
pub fn create_image_detail_from_json(json: String) -> Result<ImageDetail, String> {
    serde_json::from_str::<liter_llm::types::ImageDetail>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_image_detail_from_core)
}

#[frb]
pub fn create_assistant_content_from_json(json: String) -> Result<AssistantContent, String> {
    serde_json::from_str::<liter_llm::types::AssistantContent>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_assistant_content_from_core)
}

#[frb]
pub fn create_assistant_part_from_json(json: String) -> Result<AssistantPart, String> {
    serde_json::from_str::<liter_llm::types::AssistantPart>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_assistant_part_from_core)
}

#[frb]
pub fn create_tool_type_from_json(json: String) -> Result<ToolType, String> {
    serde_json::from_str::<liter_llm::types::ToolType>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_tool_type_from_core)
}

#[frb]
pub fn create_tool_choice_from_json(json: String) -> Result<ToolChoice, String> {
    serde_json::from_str::<liter_llm::types::ToolChoice>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_tool_choice_from_core)
}

#[frb]
pub fn create_tool_choice_mode_from_json(json: String) -> Result<ToolChoiceMode, String> {
    serde_json::from_str::<liter_llm::types::ToolChoiceMode>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_tool_choice_mode_from_core)
}

#[frb]
pub fn create_response_format_from_json(json: String) -> Result<ResponseFormat, String> {
    serde_json::from_str::<liter_llm::types::ResponseFormat>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_response_format_from_core)
}

#[frb]
pub fn create_stop_sequence_from_json(json: String) -> Result<StopSequence, String> {
    serde_json::from_str::<liter_llm::types::StopSequence>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_stop_sequence_from_core)
}

#[frb]
pub fn create_modality_from_json(json: String) -> Result<Modality, String> {
    serde_json::from_str::<liter_llm::types::Modality>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_modality_from_core)
}

#[frb]
pub fn create_finish_reason_from_json(json: String) -> Result<FinishReason, String> {
    serde_json::from_str::<liter_llm::types::FinishReason>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_finish_reason_from_core)
}

#[frb]
pub fn create_reasoning_effort_from_json(json: String) -> Result<ReasoningEffort, String> {
    serde_json::from_str::<liter_llm::types::ReasoningEffort>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_reasoning_effort_from_core)
}

#[frb]
pub fn create_embedding_format_from_json(json: String) -> Result<EmbeddingFormat, String> {
    serde_json::from_str::<liter_llm::types::EmbeddingFormat>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_embedding_format_from_core)
}

#[frb]
pub fn create_embedding_input_from_json(json: String) -> Result<EmbeddingInput, String> {
    serde_json::from_str::<liter_llm::types::EmbeddingInput>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_embedding_input_from_core)
}

#[frb]
pub fn create_embedding_content_part_from_json(json: String) -> Result<EmbeddingContentPart, String> {
    serde_json::from_str::<liter_llm::types::EmbeddingContentPart>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_embedding_content_part_from_core)
}

#[frb]
pub fn create_moderation_input_from_json(json: String) -> Result<ModerationInput, String> {
    serde_json::from_str::<liter_llm::types::ModerationInput>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_moderation_input_from_core)
}

#[frb]
pub fn create_rerank_document_from_json(json: String) -> Result<RerankDocument, String> {
    serde_json::from_str::<liter_llm::types::RerankDocument>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_rerank_document_from_core)
}

#[frb]
pub fn create_ocr_document_from_json(json: String) -> Result<OcrDocument, String> {
    serde_json::from_str::<liter_llm::types::OcrDocument>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_ocr_document_from_core)
}

#[frb]
pub fn create_file_purpose_from_json(json: String) -> Result<FilePurpose, String> {
    serde_json::from_str::<liter_llm::types::FilePurpose>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_file_purpose_from_core)
}

#[frb]
pub fn create_batch_status_from_json(json: String) -> Result<BatchStatus, String> {
    serde_json::from_str::<liter_llm::types::BatchStatus>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_batch_status_from_core)
}

#[frb]
pub fn create_auth_header_format_from_json(json: String) -> Result<AuthHeaderFormat, String> {
    serde_json::from_str::<liter_llm::provider::custom::AuthHeaderFormat>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_auth_header_format_from_core)
}

#[frb]
pub fn create_stream_format_from_json(json: String) -> Result<StreamFormat, String> {
    serde_json::from_str::<liter_llm::provider::StreamFormat>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_stream_format_from_core)
}

#[frb]
pub fn create_auth_type_from_json(json: String) -> Result<AuthType, String> {
    serde_json::from_str::<liter_llm::provider::AuthType>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_auth_type_from_core)
}

#[cfg(feature = "tower")]
#[frb]
pub fn create_enforcement_from_json(json: String) -> Result<Enforcement, String> {
    serde_json::from_str::<liter_llm::Enforcement>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_enforcement_from_core)
}

#[cfg(feature = "tower")]
#[frb]
pub fn create_cache_backend_from_json(json: String) -> Result<CacheBackend, String> {
    serde_json::from_str::<liter_llm::CacheBackend>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_cache_backend_from_core)
}

#[frb]
pub fn create_refresh_outcome_from_json(json: String) -> Result<RefreshOutcome, String> {
    serde_json::from_str::<liter_llm::cost::refresh::RefreshOutcome>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_refresh_outcome_from_core)
}
