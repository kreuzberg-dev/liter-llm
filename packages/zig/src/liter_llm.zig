...
// alef:hash:6f869b7b1d3c6918dfa4e13b2f2f702654ef581577b9d18cec4621b43d15e12e
// alef:handle-abi:1
...
/// Must run before any deferred `_free` call: every `literllm_*` FFI entry point
...
pub const CatalogRefreshError = error{
    Disabled,
    InsecureUrl,
    Fetch,
    Parse,
...
pub const CatalogRefreshConfig = struct {
...
pub const DecodedDataUrl = struct {
...
pub const InFlightLimitConfig = struct {
...
/// Global per-client in-flight provider request limit.
pub const LlmInFlightLimitConfig = struct {
    /// Maximum simultaneously outstanding provider requests. `null` means unlimited.
    max_in_flight: ?u64,
};
...
pub const ModelInfo = struct {
    ...
    input_cost_per_audio_token: ?f64,
    /// Cost in USD per output audio token.
    output_cost_per_audio_token: ?f64,
    /// Cost in USD per reasoning (extended-thinking) output token.
    output_cost_per_reasoning_token: ?f64,
...
pub const EmbeddingInput = union(enum) {
    ...
    multimodal: []const EmbeddingContentPart,
};
...
pub fn capabilities(provider_name: []const u8) error{OutOfMemory}![]u8 {
    ...
    return blk: {
        ...
        const value = _result;
        defer c.literllm_provider_capabilities_free(value);
        const _json_ptr = c.literllm_provider_capabilities_to_json(value);
...
pub fn clear_catalog_overlay() void {
    _ = c.literllm_clear_catalog_overlay();
}
...
pub fn completion_cost(model: []const u8, prompt_tokens: u64, completion_tokens: u64) error{OutOfMemory}!?f64 {
    ...
    if (c.literllm_completion_cost_has_result(model_z, prompt_tokens, completion_tokens) != 1) {
        return null;
    }
...
pub fn completion_cost_with_cache(model: []const u8, prompt_tokens: u64, cached_tokens: u64, completion_tokens: u64) error{OutOfMemory}!?f64 {
    const model_z = try std.fmt.allocPrintSentinel(
        std.heap.c_allocator, "{s}", .{model}, 0);
    defer std.heap.c_allocator.free(model_z);
    if (c.literllm_completion_cost_with_cache_has_result(model_z, prompt_tokens, cached_tokens, completion_tokens) != 1) {
        return null;
    }
...
pub fn count_request_tokens(model: []const u8, req: []const u8) LiterLlmError!u64 {
    ...
    if (req_handle == 0) return _error_with_message(LiterLlmError);
...
pub fn create_client(api_key: []const u8, base_url: ?[]const u8, timeout_secs: ?u64, max_retries: ?u32, model_hint: ?[]const u8) LiterLlmError!DefaultClient {
    ...
    return blk: { if (_result == 0) return error.UnknownFfiError; break :blk DefaultClient{ ._handle = _result }; };
}
...
pub fn decode_data_url(url: []const u8) error{OutOfMemory}!?[]u8 {
    const url_z = try std.fmt.allocPrintSentinel(
        std.heap.c_allocator, "{s}", .{url}, 0);
    defer std.heap.c_allocator.free(url_z);
    const _result = c.literllm_decode_data_url(url_z);
    return if (_result == 0) null else blk: {
        if (_result == 0) return error.OutOfMemory;
        const value = _result;
        defer c.literllm_decoded_data_url_free(value);
        const _json_ptr = c.literllm_decoded_data_url_to_json(value);
        if (_json_ptr == null) return error.OutOfMemory;
        defer _free_string(_json_ptr);
        const slice = std.mem.sliceTo(_json_ptr, 0);
        const owned = try std.heap.c_allocator.dupe(u8, slice);
...
pub fn encode_data_url(bytes: []const u8, mime: ?[]const u8) error{OutOfMemory}![]u8 {
    const mime_z: ?[:0]u8 = if (mime) |v| try std.fmt.allocPrintSentinel(
        std.heap.c_allocator, "{s}", .{v}, 0) else null;
    defer if (mime_z) |z| std.heap.c_allocator.free(z);
    const _result = c.literllm_encode_data_url(bytes.ptr, bytes.len, if (mime_z) |z| z.ptr else null);
    const _result_len = c.literllm_encode_data_url_len(bytes.ptr, bytes.len, if (mime_z) |z| z.ptr else null);
    return blk: {
        const slice = _result[0.._result_len];
        defer _free_string(_result);
        const owned = try std.heap.c_allocator.dupe(u8, slice);
...
pub fn install_catalog_overlay_from_str(catalog_json: []const u8) CatalogRefreshError!void {
    const catalog_json_z = try std.fmt.allocPrintSentinel(
        std.heap.c_allocator, "{s}", .{catalog_json}, 0);
    defer std.heap.c_allocator.free(catalog_json_z);
    _ = c.literllm_install_catalog_overlay_from_str(catalog_json_z);
    if (c.literllm_last_error_code() != 0) {
        return _error_with_message(CatalogRefreshError);
    }
    return;
}
...
pub fn model_info(model: []const u8) error{OutOfMemory}!?[]u8 {
    const model_z = try std.fmt.allocPrintSentinel(
        std.heap.c_allocator, "{s}", .{model}, 0);
    defer std.heap.c_allocator.free(model_z);
    const _result = c.literllm_model_info(model_z);
    return if (_result == 0) null else blk: {
        if (_result == 0) return error.OutOfMemory;
        const value = _result;
        defer c.literllm_model_info_free(value);
        const _json_ptr = c.literllm_model_info_to_json(value);
        if (_json_ptr == null) return error.OutOfMemory;
        defer _free_string(_json_ptr);
        const slice = std.mem.sliceTo(_json_ptr, 0);
        const owned = try std.heap.c_allocator.dupe(u8, slice);
...
pub fn record_cost_usd(system: []const u8, model: []const u8, operation: []const u8, cost_usd: f64) error{OutOfMemory}!void {
    ...
    _ = c.literllm_record_cost_usd(system_z, model_z, operation_z, cost_usd);
}
...
pub fn refresh_catalog(config: []const u8) CatalogRefreshError!RefreshOutcome {
    ...
    const config_handle = c.literllm_catalog_refresh_config_from_json(config_z);
    if (config_handle == 0) return _error_with_message(CatalogRefreshError);
    defer c.literllm_catalog_refresh_config_free(config_handle);
    const _result = c.literllm_refresh_catalog(config_handle);
    if (c.literllm_last_error_code() != 0) {
        return _error_with_message(CatalogRefreshError);
    }
    return blk: { if (_result == 0) return error.UnknownFfiError; break :blk RefreshOutcome{ ._handle = _result }; };
}
...
pub fn register_custom_provider(config: []const u8) LiterLlmError!void {
    ...
    if (config_handle == 0) return _error_with_message(LiterLlmError);
...
pub const ChatCompletionChunkStream = struct {
    ...
    pub fn next(self: *ChatCompletionChunkStream) (LiterLlmError||error{OutOfMemory,HandleClosed})!?ChatCompletionChunk {
        const handle = self._handle;
        if (handle == 0) return error.HandleClosed;
        const _chunk = c.literllm_default_client_chat_stream_next(handle);
        if (_chunk == 0) {
            // Check errno: 0 = clean EOS, != 0 = error
            if (c.literllm_last_error_code() != 0) return error.UnknownFfiError;
            return null;
        }
        defer c.literllm_chat_completion_chunk_free(_chunk);
        const _json = c.literllm_chat_completion_chunk_to_json(_chunk);
        if (_json == null) return _error_with_message(LiterLlmError);
    ...
    pub fn deinit(self: *ChatCompletionChunkStream) void {
        ...
        if (handle == 0) return;
        self._handle = 0;
        c.literllm_default_client_chat_stream_free(handle);
    }
};
...
pub const DefaultClient = struct {
    ...
    pub fn chat(self: *DefaultClient, req: []const u8) (LiterLlmError||error{OutOfMemory,HandleClosed})![]u8 {
        const handle = self._handle;
        if (handle == 0) return error.HandleClosed;
        ...
        defer c.literllm_chat_completion_request_free(req_handle);
        const _result = c.literllm_default_client_chat(handle, req_handle);
        if (c.literllm_last_error_code() != 0) {
            return _error_with_message(LiterLlmError);
        }
        return blk: {
            if (_result == 0) return error.OutOfMemory;
            const value = _result;
            defer c.literllm_chat_completion_response_free(value);
            const _json_ptr = c.literllm_chat_completion_response_to_json(value) orelse return error.OutOfMemory;
            defer c.literllm_free_string(_json_ptr);
    ...
    pub fn chat_stream(self: *DefaultClient, req: []const u8) (LiterLlmError||error{OutOfMemory,HandleClosed})!ChatCompletionChunkStream {
        ...
        if (req_handle == 0) { return error.UnknownFfiError; }
        defer c.literllm_chat_completion_request_free(req_handle);
        const _stream_handle = c.literllm_default_client_chat_stream_start(handle, req_handle);
        if (_stream_handle == 0) { return error.UnknownFfiError; }
        return ChatCompletionChunkStream{ ._handle = _stream_handle };
    }

    pub fn embed(self: *DefaultClient, req: []const u8) (LiterLlmError||error{OutOfMemory,HandleClosed})![]u8 {
        const handle = self._handle;
        if (handle == 0) return error.HandleClosed;
        const req_z = try std.heap.c_allocator.dupeZ(u8, req);
        defer std.heap.c_allocator.free(req_z);
        ...
        const _result = c.literllm_default_client_embed(handle, req_handle);
        if (c.literllm_last_error_code() != 0) {
            return _error_with_message(LiterLlmError);
        }
        return blk: {
            if (_result == 0) return error.OutOfMemory;
            const value = _result;
            defer c.literllm_embedding_response_free(value);
            const _json_ptr = c.literllm_embedding_response_to_json(value) orelse return error.OutOfMemory;
            defer c.literllm_free_string(_json_ptr);
            const _json_slice = std.mem.span(_json_ptr);
            const owned = try std.heap.c_allocator.dupe(u8, _json_slice);
            break :blk owned;
    ...
    pub fn list_models(self: *DefaultClient) (LiterLlmError||error{OutOfMemory,HandleClosed})![]u8 {
        const handle = self._handle;
        if (handle == 0) return error.HandleClosed;
        const _result = c.literllm_default_client_list_models(handle);
        if (c.literllm_last_error_code() != 0) {
            return _error_with_message(LiterLlmError);
        }
        return blk: {
            if (_result == 0) return error.OutOfMemory;
            const value = _result;
            defer c.literllm_models_list_response_free(value);
            const _json_ptr = c.literllm_models_list_response_to_json(value) orelse return error.OutOfMemory;
            defer c.literllm_free_string(_json_ptr);
            const _json_slice = std.mem.span(_json_ptr);
            const owned = try std.heap.c_allocator.dupe(u8, _json_slice);
            break :blk owned;
    ...
    pub fn image_generate(self: *DefaultClient, req: []const u8) (LiterLlmError||error{OutOfMemory,HandleClosed})![]u8 {
        const handle = self._handle;
        if (handle == 0) return error.HandleClosed;
        const req_z = try std.heap.c_allocator.dupeZ(u8, req);
        defer std.heap.c_allocator.free(req_z);
        ...
        const _result = c.literllm_default_client_image_generate(handle, req_handle);
        if (c.literllm_last_error_code() != 0) {
            return _error_with_message(LiterLlmError);
        }
        return blk: {
            if (_result == 0) return error.OutOfMemory;
            const value = _result;
            defer c.literllm_images_response_free(value);
            const _json_ptr = c.literllm_images_response_to_json(value) orelse return error.OutOfMemory;
            defer c.literllm_free_string(_json_ptr);
            const _json_slice = std.mem.span(_json_ptr);
            const owned = try std.heap.c_allocator.dupe(u8, _json_slice);
            break :blk owned;
    ...
    pub fn speech(self: *DefaultClient, req: []const u8) (LiterLlmError||error{OutOfMemory,HandleClosed})![]u8 {
        const handle = self._handle;
        if (handle == 0) return error.HandleClosed;
        const req_z = try std.heap.c_allocator.dupeZ(u8, req);
        defer std.heap.c_allocator.free(req_z);
        ...
        _ = c.literllm_default_client_speech(handle, req_handle, &_out_ptr, &_out_len, &_out_cap);
        if (c.literllm_last_error_code() != 0) {
            return _error_with_message(LiterLlmError);
        }
        defer c.literllm_free_bytes(_out_ptr, _out_len, _out_cap);
        const _owned = try std.heap.c_allocator.dupe(u8, _out_ptr[0.._out_len]);
        return _owned;
    }

    pub fn transcribe(self: *DefaultClient, req: []const u8) (LiterLlmError||error{OutOfMemory,HandleClosed})![]u8 {
        const handle = self._handle;
        if (handle == 0) return error.HandleClosed;
        const req_z = try std.heap.c_allocator.dupeZ(u8, req);
        defer std.heap.c_allocator.free(req_z);
        ...
        const _result = c.literllm_default_client_transcribe(handle, req_handle);
        if (c.literllm_last_error_code() != 0) {
            return _error_with_message(LiterLlmError);
        }
        return blk: {
            if (_result == 0) return error.OutOfMemory;
            const value = _result;
            defer c.literllm_transcription_response_free(value);
            const _json_ptr = c.literllm_transcription_response_to_json(value) orelse return error.OutOfMemory;
            defer c.literllm_free_string(_json_ptr);
            const _json_slice = std.mem.span(_json_ptr);
            const owned = try std.heap.c_allocator.dupe(u8, _json_slice);
            break :blk owned;
    ...
    pub fn moderate(self: *DefaultClient, req: []const u8) (LiterLlmError||error{OutOfMemory,HandleClosed})![]u8 {
        const handle = self._handle;
        if (handle == 0) return error.HandleClosed;
        const req_z = try std.heap.c_allocator.dupeZ(u8, req);
        defer std.heap.c_allocator.free(req_z);
        ...
        const _result = c.literllm_default_client_moderate(handle, req_handle);
        if (c.literllm_last_error_code() != 0) {
            return _error_with_message(LiterLlmError);
        }
        return blk: {
            if (_result == 0) return error.OutOfMemory;
            const value = _result;
            defer c.literllm_moderation_response_free(value);
            const _json_ptr = c.literllm_moderation_response_to_json(value) orelse return error.OutOfMemory;
            defer c.literllm_free_string(_json_ptr);
            const _json_slice = std.mem.span(_json_ptr);
            const owned = try std.heap.c_allocator.dupe(u8, _json_slice);
            break :blk owned;
    ...
    pub fn rerank(self: *DefaultClient, req: []const u8) (LiterLlmError||error{OutOfMemory,HandleClosed})![]u8 {
        const handle = self._handle;
        if (handle == 0) return error.HandleClosed;
        const req_z = try std.heap.c_allocator.dupeZ(u8, req);
        defer std.heap.c_allocator.free(req_z);
        ...
        const _result = c.literllm_default_client_rerank(handle, req_handle);
        if (c.literllm_last_error_code() != 0) {
            return _error_with_message(LiterLlmError);
        }
        return blk: {
            if (_result == 0) return error.OutOfMemory;
            const value = _result;
            defer c.literllm_rerank_response_free(value);
            const _json_ptr = c.literllm_rerank_response_to_json(value) orelse return error.OutOfMemory;
            defer c.literllm_free_string(_json_ptr);
            const _json_slice = std.mem.span(_json_ptr);
            const owned = try std.heap.c_allocator.dupe(u8, _json_slice);
            break :blk owned;
    ...
    pub fn search(self: *DefaultClient, req: []const u8) (LiterLlmError||error{OutOfMemory,HandleClosed})![]u8 {
        const handle = self._handle;
        if (handle == 0) return error.HandleClosed;
        const req_z = try std.heap.c_allocator.dupeZ(u8, req);
        defer std.heap.c_allocator.free(req_z);
        ...
        const _result = c.literllm_default_client_search(handle, req_handle);
        if (c.literllm_last_error_code() != 0) {
            return _error_with_message(LiterLlmError);
        }
        return blk: {
            if (_result == 0) return error.OutOfMemory;
            const value = _result;
            defer c.literllm_search_response_free(value);
            const _json_ptr = c.literllm_search_response_to_json(value) orelse return error.OutOfMemory;
            defer c.literllm_free_string(_json_ptr);
            const _json_slice = std.mem.span(_json_ptr);
            const owned = try std.heap.c_allocator.dupe(u8, _json_slice);
            break :blk owned;
    ...
    pub fn ocr(self: *DefaultClient, req: []const u8) (LiterLlmError||error{OutOfMemory,HandleClosed})![]u8 {
        const handle = self._handle;
        if (handle == 0) return error.HandleClosed;
        const req_z = try std.heap.c_allocator.dupeZ(u8, req);
        defer std.heap.c_allocator.free(req_z);
        ...
        const _result = c.literllm_default_client_ocr(handle, req_handle);
        if (c.literllm_last_error_code() != 0) {
            return _error_with_message(LiterLlmError);
        }
        return blk: {
            if (_result == 0) return error.OutOfMemory;
            const value = _result;
            defer c.literllm_ocr_response_free(value);
            const _json_ptr = c.literllm_ocr_response_to_json(value) orelse return error.OutOfMemory;
            defer c.literllm_free_string(_json_ptr);
            const _json_slice = std.mem.span(_json_ptr);
            const owned = try std.heap.c_allocator.dupe(u8, _json_slice);
            break :blk owned;
    ...
    pub fn create_file(self: *DefaultClient, req: []const u8) (LiterLlmError||error{OutOfMemory,HandleClosed})![]u8 {
        const handle = self._handle;
        if (handle == 0) return error.HandleClosed;
        const req_z = try std.heap.c_allocator.dupeZ(u8, req);
        defer std.heap.c_allocator.free(req_z);
        ...
        const _result = c.literllm_default_client_create_file(handle, req_handle);
        if (c.literllm_last_error_code() != 0) {
            return _error_with_message(LiterLlmError);
        }
        return blk: {
            if (_result == 0) return error.OutOfMemory;
            const value = _result;
            defer c.literllm_file_object_free(value);
            const _json_ptr = c.literllm_file_object_to_json(value) orelse return error.OutOfMemory;
            defer c.literllm_free_string(_json_ptr);
            const _json_slice = std.mem.span(_json_ptr);
            const owned = try std.heap.c_allocator.dupe(u8, _json_slice);
            break :blk owned;
    ...
    pub fn retrieve_file(self: *DefaultClient, file_id: []const u8) (LiterLlmError||error{OutOfMemory,HandleClosed})![]u8 {
        const handle = self._handle;
        if (handle == 0) return error.HandleClosed;
        const file_id_z = try std.heap.c_allocator.dupeZ(u8, file_id);
        defer std.heap.c_allocator.free(file_id_z);
        const _result = c.literllm_default_client_retrieve_file(handle, file_id_z);
        if (c.literllm_last_error_code() != 0) {
            return _error_with_message(LiterLlmError);
        }
        return blk: {
    ...
    pub fn delete_file(self: *DefaultClient, file_id: []const u8) (LiterLlmError||error{OutOfMemory,HandleClosed})![]u8 {
        const handle = self._handle;
        if (handle == 0) return error.HandleClosed;
        const file_id_z = try std.heap.c_allocator.dupeZ(u8, file_id);
        defer std.heap.c_allocator.free(file_id_z);
        const _result = c.literllm_default_client_delete_file(handle, file_id_z);
        if (c.literllm_last_error_code() != 0) {
            return _error_with_message(LiterLlmError);
        }
        return blk: {
            if (_result == 0) return error.OutOfMemory;
            const value = _result;
            defer c.literllm_delete_response_free(value);
            const _json_ptr = c.literllm_delete_response_to_json(value) orelse return error.OutOfMemory;
            defer c.literllm_free_string(_json_ptr);
            const _json_slice = std.mem.span(_json_ptr);
            const owned = try std.heap.c_allocator.dupe(u8, _json_slice);
            break :blk owned;
    ...
    pub fn list_files(self: *DefaultClient, query: ?[]const u8) (LiterLlmError||error{OutOfMemory,HandleClosed})![]u8 {
        const handle = self._handle;
        if (handle == 0) return error.HandleClosed;
        ...
        const query_handle = if (query_z) |z| c.literllm_file_list_query_from_json(z.ptr) else 0;
        if (query_z != null and query_handle == 0) return error.UnknownFfiError;
        defer if (query_handle != 0) c.literllm_file_list_query_free(query_handle);
        const _result = c.literllm_default_client_list_files(handle, query_handle);
        if (c.literllm_last_error_code() != 0) {
            return _error_with_message(LiterLlmError);
        }
        return blk: {
            if (_result == 0) return error.OutOfMemory;
            const value = _result;
            defer c.literllm_file_list_response_free(value);
            const _json_ptr = c.literllm_file_list_response_to_json(value) orelse return error.OutOfMemory;
            defer c.literllm_free_string(_json_ptr);
            const _json_slice = std.mem.span(_json_ptr);
            const owned = try std.heap.c_allocator.dupe(u8, _json_slice);
            break :blk owned;
    ...
    pub fn file_content(self: *DefaultClient, file_id: []const u8) (LiterLlmError||error{OutOfMemory,HandleClosed})![]u8 {
        const handle = self._handle;
        if (handle == 0) return error.HandleClosed;
        const file_id_z = try std.heap.c_allocator.dupeZ(u8, file_id);
        defer std.heap.c_allocator.free(file_id_z);
        ...
        var _out_len: usize = 0;
        var _out_cap: usize = 0;
        _ = c.literllm_default_client_file_content(handle, file_id_z, &_out_ptr, &_out_len, &_out_cap);
        if (c.literllm_last_error_code() != 0) {
            return _error_with_message(LiterLlmError);
        }
        defer c.literllm_free_bytes(_out_ptr, _out_len, _out_cap);
    ...
    pub fn create_batch(self: *DefaultClient, req: []const u8) (LiterLlmError||error{OutOfMemory,HandleClosed})![]u8 {
        const handle = self._handle;
        if (handle == 0) return error.HandleClosed;
        const req_z = try std.heap.c_allocator.dupeZ(u8, req);
        defer std.heap.c_allocator.free(req_z);
        ...
        const _result = c.literllm_default_client_create_batch(handle, req_handle);
        if (c.literllm_last_error_code() != 0) {
            return _error_with_message(LiterLlmError);
        }
        return blk: {
            if (_result == 0) return error.OutOfMemory;
            const value = _result;
            defer c.literllm_batch_object_free(value);
            const _json_ptr = c.literllm_batch_object_to_json(value) orelse return error.OutOfMemory;
            defer c.literllm_free_string(_json_ptr);
            const _json_slice = std.mem.span(_json_ptr);
            const owned = try std.heap.c_allocator.dupe(u8, _json_slice);
            break :blk owned;
    ...
    pub fn retrieve_batch(self: *DefaultClient, batch_id: []const u8) (LiterLlmError||error{OutOfMemory,HandleClosed})![]u8 {
        const handle = self._handle;
        if (handle == 0) return error.HandleClosed;
        const batch_id_z = try std.heap.c_allocator.dupeZ(u8, batch_id);
        defer std.heap.c_allocator.free(batch_id_z);
        const _result = c.literllm_default_client_retrieve_batch(handle, batch_id_z);
        if (c.literllm_last_error_code() != 0) {
            return _error_with_message(LiterLlmError);
        }
        return blk: {
    ...
    pub fn list_batches(self: *DefaultClient, query: ?[]const u8) (LiterLlmError||error{OutOfMemory,HandleClosed})![]u8 {
        const handle = self._handle;
        if (handle == 0) return error.HandleClosed;
        const query_z: ?[:0]u8 = if (query) |v| try std.heap.c_allocator.dupeZ(u8, v) else null;
        defer if (query_z) |z| std.heap.c_allocator.free(z);
        const query_handle = if (query_z) |z| c.literllm_batch_list_query_from_json(z.ptr) else 0;
        if (query_z != null and query_handle == 0) return error.UnknownFfiError;
        defer if (query_handle != 0) c.literllm_batch_list_query_free(query_handle);
        const _result = c.literllm_default_client_list_batches(handle, query_handle);
        if (c.literllm_last_error_code() != 0) {
            return _error_with_message(LiterLlmError);
        }
        return blk: {
            if (_result == 0) return error.OutOfMemory;
            const value = _result;
            defer c.literllm_batch_list_response_free(value);
            const _json_ptr = c.literllm_batch_list_response_to_json(value) orelse return error.OutOfMemory;
            defer c.literllm_free_string(_json_ptr);
            const _json_slice = std.mem.span(_json_ptr);
            const owned = try std.heap.c_allocator.dupe(u8, _json_slice);
            break :blk owned;
    ...
    pub fn cancel_batch(self: *DefaultClient, batch_id: []const u8) (LiterLlmError||error{OutOfMemory,HandleClosed})![]u8 {
        const handle = self._handle;
        if (handle == 0) return error.HandleClosed;
        const batch_id_z = try std.heap.c_allocator.dupeZ(u8, batch_id);
        defer std.heap.c_allocator.free(batch_id_z);
        const _result = c.literllm_default_client_cancel_batch(handle, batch_id_z);
        if (c.literllm_last_error_code() != 0) {
            return _error_with_message(LiterLlmError);
        }
        return blk: {
    ...
    pub fn fetch_batch_for_polling(self: *DefaultClient, batch_id: []const u8) (LiterLlmError||error{OutOfMemory,HandleClosed})![]u8 {
        const handle = self._handle;
        if (handle == 0) return error.HandleClosed;
        const batch_id_z = try std.heap.c_allocator.dupeZ(u8, batch_id);
        defer std.heap.c_allocator.free(batch_id_z);
        const _result = c.literllm_default_client_fetch_batch_for_polling(handle, batch_id_z);
        if (c.literllm_last_error_code() != 0) {
            return _error_with_message(LiterLlmError);
        }
        return blk: {
    ...
    pub fn wait_for_batch(self: *DefaultClient, batch_id: []const u8, config: []const u8) (error{OutOfMemory,UnknownFfiError}||error{OutOfMemory,HandleClosed})![]u8 {
        const handle = self._handle;
        if (handle == 0) return error.HandleClosed;
        const batch_id_z = try std.heap.c_allocator.dupeZ(u8, batch_id);
        defer std.heap.c_allocator.free(batch_id_z);
        ...
        const _result = c.literllm_default_client_wait_for_batch(handle, batch_id_z, config_handle);
        if (c.literllm_last_error_code() != 0) {
    ...
    pub fn create_response(self: *DefaultClient, req: []const u8) (LiterLlmError||error{OutOfMemory,HandleClosed})![]u8 {
        const handle = self._handle;
        if (handle == 0) return error.HandleClosed;
        const req_z = try std.heap.c_allocator.dupeZ(u8, req);
        defer std.heap.c_allocator.free(req_z);
        ...
        const _result = c.literllm_default_client_create_response(handle, req_handle);
        if (c.literllm_last_error_code() != 0) {
            return _error_with_message(LiterLlmError);
        }
        return blk: {
            if (_result == 0) return error.OutOfMemory;
            const value = _result;
            defer c.literllm_response_object_free(value);
            const _json_ptr = c.literllm_response_object_to_json(value) orelse return error.OutOfMemory;
            defer c.literllm_free_string(_json_ptr);
            const _json_slice = std.mem.span(_json_ptr);
            const owned = try std.heap.c_allocator.dupe(u8, _json_slice);
            break :blk owned;
    ...
    pub fn retrieve_response(self: *DefaultClient, response_id: []const u8) (LiterLlmError||error{OutOfMemory,HandleClosed})![]u8 {
        const handle = self._handle;
        if (handle == 0) return error.HandleClosed;
        const response_id_z = try std.heap.c_allocator.dupeZ(u8, response_id);
        defer std.heap.c_allocator.free(response_id_z);
        const _result = c.literllm_default_client_retrieve_response(handle, response_id_z);
        if (c.literllm_last_error_code() != 0) {
            return _error_with_message(LiterLlmError);
        }
        return blk: {
    ...
    pub fn cancel_response(self: *DefaultClient, response_id: []const u8) (LiterLlmError||error{OutOfMemory,HandleClosed})![]u8 {
        const handle = self._handle;
        if (handle == 0) return error.HandleClosed;
        const response_id_z = try std.heap.c_allocator.dupeZ(u8, response_id);
        defer std.heap.c_allocator.free(response_id_z);
        const _result = c.literllm_default_client_cancel_response(handle, response_id_z);
        if (c.literllm_last_error_code() != 0) {
            return _error_with_message(LiterLlmError);
        }
        return blk: {
    ...
    pub fn free(self: *DefaultClient) void {
        ...
        if (handle == 0) return;
        self._handle = 0;
        c.literllm_default_client_free(handle);
    }
};
...
pub const IntentPrototype = struct {
    ...
    pub fn free(self: *IntentPrototype) void {
        ...
        if (handle == 0) return;
        self._handle = 0;
        c.literllm_intent_prototype_free(handle);
    }
};
...
pub const SingleflightResult = struct {
    ...
    pub fn free(self: *SingleflightResult) void {
        ...
        if (handle == 0) return;
        self._handle = 0;
        c.literllm_singleflight_result_free(handle);
    }
};
