...
- **liter-llm CLI** installed (see [Installation](/getting-started/installation/))
...
Configuration is identical to the proxy server — see [Proxy Configuration](/server/proxy-configuration/) for full details.
...
<TabItem label="Claude Desktop / Claude Code">
...
<TabItem label="Generic HTTP Client">
...
The MCP server reads API keys and provider configuration from environment variables, identical to the [main liter-llm client](/usage/configuration/#api-key-environment-variables):
...
See [Proxy Configuration](/server/proxy-configuration/) for all available options.
...
:::caution[Never commit API keys]
Use environment variables or a `.env` file (not committed to git). See [Secrets and API Key Handling](/getting-started/installation/#api-key-setup) for best practices.
:::
...
- [Chat & Streaming](/usage/chat/) — Learn the chat API and streaming patterns
- [Provider Registry](/providers/) — Browse all 165 runtime providers
- [Configuration](/usage/configuration/) — Timeouts, retries, and custom endpoints
