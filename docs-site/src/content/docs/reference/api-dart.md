...
<!-- alef:hash:7bb8b4de6d7f788ad044af46cdf13f5a5e05a66f5ca8d40ee378f8841faacc51 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Dart API Reference <span class="version-badge">v1.18.4</span>
...
```dart
Future<DefaultClient> createClient(String apiKey, {String? baseUrl, int? timeoutSecs, int? maxRetries, String? modelHint})
```
...
```dart
Future<DefaultClient> createClientFromJson(String json)
```
...
```dart
Future<String> encodeDataUrl(Uint8List bytes, {String? mime})
```
...
```dart
Future<DecodedDataUrl?> decodeDataUrl(String url)
```
...
```dart
Future<void> registerCustomProvider(CustomProviderConfig config)
```
...
```dart
Future<bool> unregisterCustomProvider(String name)
```
...
```dart
Future<ProviderCapabilities> capabilities(String providerName)
```
...
```dart
Future<double?> completionCost(String model, int promptTokens, int completionTokens)
```
...
```dart
Future<double?> completionCostWithCache(String model, int promptTokens, int cachedTokens, int completionTokens)
```
...
```dart
Future<ModelInfo?> modelInfo(String model)
```
...
```dart
Future<PlatformInt64> countTokens(String model, String text)
```
...
```dart
Future<PlatformInt64> countRequestTokens(String model, ChatCompletionRequest req)
```
...
```dart
Future<void> recordCostUsd(String system, String model, String operation, double costUsd)
```
...
```dart
Future<void> checkBound(String context, PlatformInt64 currentLen, PlatformInt64 incoming, PlatformInt64 limit)
```
...
| `currentLen` | `PlatformInt64` | Yes | The current len |
| `incoming` | `PlatformInt64` | Yes | The incoming |
| `limit` | `PlatformInt64` | Yes | The limit |
...
```dart
Future<void> installCatalogOverlayFromStr(String catalogJson)
```
...
```dart
Future<RefreshOutcome> refreshCatalog(CatalogRefreshConfig config)
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `maxEntries` | `PlatformInt64` | `256` | Maximum number of cached entries. |
...
```dart
Future<ChatCompletionChunk?> process(ChatCompletionChunk chunk)
```
...
```dart
Future<BatchObject> fetchBatchForPolling(String batchId)
```
...
```dart
Future<BatchObject> waitForBatch(String batchId, WaitForBatchConfig config)
```
...
| `embedding` | `List<double>` | — | The embedding vector. Providers may return this as a JSON float array or, when `encoding_format: "base64"` was requested, as a base64 string of little-endian `f32` bytes. Base64 responses are decoded on read; this field always serializes back out as a JSON float array. |
...
```dart
Future<HealthStatus> check(String upstream)
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `maxInFlight` | `PlatformInt64?` | `null` | Maximum simultaneously outstanding provider requests. `null` means unlimited. |
