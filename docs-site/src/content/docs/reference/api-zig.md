...
<!-- alef:hash:854ce62ef853622706e21ac9c543ae67b7116653c2832b634c740b191d326393 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Zig API Reference <span class="version-badge">v1.18.4</span>
...
```zig
pub fn create_client(api_key: [:0]const u8, base_url: ?[:0]const u8, timeout_secs: ?u64, max_retries: ?u32, model_hint: ?[:0]const u8) LiterLlmError!DefaultClient
```
...
|------|------|----------|-------------|
| `api_key` | `\[:0\]const u8` | Yes | The api key |
| `base_url` | `\[:0\]const u8?` | No | The base url |
| `timeout_secs` | `u64?` | No | The timeout secs |
| `max_retries` | `u32?` | No | The max retries |
| `model_hint` | `\[:0\]const u8?` | No | The model hint |
...
```zig
pub fn create_client_from_json(json: [:0]const u8) LiterLlmError!DefaultClient
```
...
```zig
pub fn encode_data_url(bytes: []const u8, mime: ?[:0]const u8) [:0]const u8
```
...
```zig
pub fn decode_data_url(url: [:0]const u8) ?DecodedDataUrl
```
...
```zig
pub fn register_custom_provider(config: CustomProviderConfig) LiterLlmError!void
```
...
```zig
pub fn unregister_custom_provider(name: [:0]const u8) LiterLlmError!bool
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `provider_name` | `\[:0\]const u8` | Yes | The provider name |
...
```zig
pub fn all_providers() LiterLlmError![]const ProviderConfig
```
...
```zig
pub fn complex_provider_names() LiterLlmError![]const [:0]const u8
```
...
```zig
pub fn model_info(model: [:0]const u8) ?ModelInfo
```
...
```zig
pub fn count_tokens(model: [:0]const u8, text: [:0]const u8) LiterLlmError!u64
```
...
```zig
pub fn count_request_tokens(model: [:0]const u8, req: ChatCompletionRequest) LiterLlmError!u64
```
...
```zig
pub fn record_cost_usd(system: [:0]const u8, model: [:0]const u8, operation: [:0]const u8, cost_usd: f64) void
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `system` | `\[:0\]const u8` | Yes | The system |
| `model` | `\[:0\]const u8` | Yes | The model |
| `operation` | `\[:0\]const u8` | Yes | The operation |
...
```zig
pub fn check_bound(context: [:0]const u8, current_len: u64, incoming: u64, limit: u64) LiterLlmError!void
```
...
```zig
pub fn install_catalog_overlay_from_str(catalog_json: [:0]const u8) CatalogRefreshError!void
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `catalog_json` | `\[:0\]const u8` | Yes | The catalog json |
...
```zig
pub fn refresh_catalog(config: CatalogRefreshConfig) CatalogRefreshError!RefreshOutcome
```
...
| `refusal` | `\[:0\]const u8?` | `null` | Refusal reason, if the model declined to respond per safety policies. OpenAI's response schema requires this key to be present even when null, so it is deliberately not `skip_serializing_if`. |
| `function_call` | `FunctionCall?` | `null` | Deprecated legacy function_call field; retained for API compatibility. |
| `reasoning_content` | `\[:0\]const u8?` | `null` | Reasoning/thinking tokens returned by the provider, if any (e.g. DeepSeek R1, Qwen `reasoning_content`, or Anthropic extended thinking). |
...
| `env_var` | `\[:0\]const u8?` | `null` | Name of the environment variable that holds the API key (e.g. `"OPENAI_API_KEY"`). Holds the variable name, never the secret value. |
...
| `first_id` | `\[:0\]const u8?` | `null` | First batch ID in the result set (for pagination). |
| `last_id` | `\[:0\]const u8?` | `null` | Last batch ID in the result set (for pagination). |
...
| `input_file_id` | `\[:0\]const u8` | — | ID of the input file. |
| `completion_window` | `\[:0\]const u8` | — | Completion window (e.g., `"24h"`). |
| `status` | `BatchStatus` | `BatchStatus.validating` | Current job status. |
| `output_file_id` | `\[:0\]const u8?` | `null` | ID of the output file (present when completed). |
| `error_file_id` | `\[:0\]const u8?` | `null` | ID of the error file (present if some requests failed). |
...
| `cross_region_prefix` | `\[:0\]const u8?` | `null` | Cross-region inference profile prefix (e.g. `"us"`). |
| `access_key_id` | `\[:0\]const u8?` | `null` | Explicit AWS access key ID. |
| `secret_access_key` | `\[:0\]const u8?` | `null` | Explicit AWS secret access key. |
| `session_token` | `\[:0\]const u8?` | `null` | Explicit AWS session token (temporary credentials). |
...
| `source_url` | `\[:0\]const u8` | `"<https://github.com/xberg-io/liter-llm/releases/download/model-catalog/catalog.json"`> | Source URL to fetch `catalog.json` from. Must be `https`. Defaults to `DEFAULT_CATALOG_URL`; configurable so self-hosted mirrors work. |
| `ttl_seconds` | `u64` | `86400` | How long a cached `catalog.json` remains valid before a network refetch is attempted, in seconds. |
| `cache_path` | `\[:0\]const u8?` | `null` | Filesystem path for the on-disk cache. `null` uses a default path under `std.env.temp_dir()`. |
...
| `system_fingerprint` | `\[:0\]const u8?` | `null` | Fingerprint of the system configuration (OpenAI-specific). |
| `service_tier` | `\[:0\]const u8?` | `null` | Service tier used (OpenAI-specific). |
...
| `response_format` | `ResponseFormat?` | `ResponseFormat.text` | Output format constraint (text, JSON, JSON schema). |
...
| `reasoning_effort` | `ReasoningEffort?` | `ReasoningEffort.medium` | Reasoning effort level (minimal, low, medium, high, max) for extended-thinking models. |
...
| `service_tier` | `\[:0\]const u8?` | `null` | Latency tier to process the request under (e.g. `"auto"`, `"default"`, `"flex"`). |
| `store` | `bool?` | `null` | Whether to store the completion for later retrieval by the provider. |
| `metadata` | `std.StringHashMap(\[:0\]const u8)?` | `{}` | Developer-defined tags attached to the completion. |
| `prediction` | `\[:0\]const u8?` | `null` | Predicted output, for latency reduction when much of the response is known ahead of time. Untyped: the shape is provider-defined and still evolving, and the value is forwarded verbatim. |
| `audio` | `\[:0\]const u8?` | `null` | Audio output parameters, required when `modalities` includes `audio`. Untyped for the same reason as `prediction`. |
| `web_search_options` | `\[:0\]const u8?` | `null` | Web-search tool configuration for search-enabled models. Untyped for the same reason as `prediction`. |
| `extra_body` | `\[:0\]const u8?` | `null` | Provider-specific extra parameters merged into the request body. Use for guardrails, safety settings, grounding config, etc. |
...
| `finish_reason` | `FinishReason?` | `FinishReason.stop` | Why the model stopped generating (stop, length, tool_calls, content_filter, etc.). |
| `logprobs` | `\[:0\]const u8?` | `null` | Per-token log probabilities, when the request asked for them. Required by OpenAI's response schema as an always-present, nullable key, so this is deliberately not `skip_serializing_if`. |
...
```zig
pub fn process(self: *const ChunkMiddleware, chunk: ChatCompletionChunk) LiterLlmError!?ChatCompletionChunk
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `input_file_id` | `\[:0\]const u8` | — | ID of the uploaded input file (JSONL format). |
| `endpoint` | `\[:0\]const u8` | — | API endpoint (e.g., `"/v1/chat/completions"`). |
| `completion_window` | `\[:0\]const u8` | — | Completion window (e.g., `"24h"`). |
...
| `response_format` | `\[:0\]const u8?` | `null` | Response format: `"url"` or `"b64_json"`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `model` | `\[:0\]const u8` | — | Model ID, as named by the OpenAI Responses API (e.g. `"gpt-5"`). Sent to the wire exactly as given. The Responses path performs none of the chat path's model handling: a `provider/model` routing prefix is **not** stripped and does **not** re-route the request, which stays pinned to the provider the client was constructed with. |
...
| `response_format` | `\[:0\]const u8?` | `null` | Audio format (e.g., `"mp3"`, `"opus"`, `"aac"`, `"flac"`, `"wav"`, `"pcm"`). |
...
| `response_format` | `\[:0\]const u8?` | `null` | Output format (e.g., `"json"`, `"text"`, `"vtt"`, `"srt"`, `"verbose_json"`). |
...
| `base_url` | `\[:0\]const u8` | — | Base URL for the provider's API (e.g., `<https://api.my-provider.com/v1>`). |
| `auth_header` | `AuthHeaderFormat` | — | Authentication header format. |
| `model_prefixes` | `\[\]const \[:0\]const u8` | — | Model name prefixes that route to this provider (e.g., `["my-"]`). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `mime` | `\[:0\]const u8` | — | MIME type extracted from the URL prefix (verbatim, not normalised). |
...
```zig
pub fn fetch_batch_for_polling(self: *const DefaultClient, batch_id: [:0]const u8) LiterLlmError!BatchObject
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `batch_id` | `\[:0\]const u8` | Yes | The batch id |
...
```zig
pub fn wait_for_batch(self: *const DefaultClient, batch_id: [:0]const u8, config: WaitForBatchConfig) BatchWaitError!BatchObject
```
...
| `media_type` | `\[:0\]const u8` | — | MIME type (e.g., "application/pdf", "text/csv"). |
...
| `b64_json` | `\[:0\]const u8?` | `null` | Base64-encoded image data (if response_format was "b64_json"). |
| `revised_prompt` | `\[:0\]const u8?` | `null` | The final prompt used to generate the image (DALL-E 3). |
...
| `backend_config` | `std.StringHashMap(\[:0\]const u8)?` | `{}` | Backend-specific configuration key/value pairs. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `model` | `\[:0\]const u8` | — | Model identifier (e.g. `"gpt-4o"`, `"bedrock/anthropic.claude-3-sonnet-20240229-v1:0"`). |
| `api_key` | `\[:0\]const u8?` | `null` | API key for authentication. |
| `base_url` | `\[:0\]const u8?` | `null` | Override base URL. When set, all requests go here and provider auto-detection is skipped. |
...
| `headers` | `std.StringHashMap(\[:0\]const u8)?` | `{}` | Extra headers sent on every request. |
...
| `base_url` | `\[:0\]const u8` | — | Base URL for the provider's OpenAI-compatible API. |
| `auth_header` | `\[:0\]const u8?` | `null` | Header name used to carry the API key (defaults to `Authorization` when unset). |
| `model_prefixes` | `\[\]const \[:0\]const u8` | `[]` | Model name prefixes routed to this provider (e.g. `["my-provider/"]`). |
...
| `mode` | `\[:0\]const u8?` | `null` | Best-effort operating mode, e.g. `"chat"`, `"embedding"`. |
...
| `object` | `\[:0\]const u8` | — | Always `"model"` from OpenAI-compatible APIs.  Stored as a plain `String` so non-standard provider values do not break deserialization. Defaults to empty when a provider omits the field. |
...
| `owned_by` | `\[:0\]const u8` | — | Organization or entity that owns the model. Defaults to empty when a provider omits the field. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `object` | `\[:0\]const u8` | — | Always `"list"` from OpenAI-compatible APIs.  Stored as a plain `String` so non-standard provider values do not break deserialization. Defaults to empty when a provider omits the field. |
...
| `image_base64` | `\[:0\]const u8?` | `/* serde(default) */` | Base64-encoded image data (if `include_image_base64` was true). |
...
| `display_name` | `\[:0\]const u8?` | `null` | Human-readable provider name shown in UIs. |
| `base_url` | `\[:0\]const u8?` | `null` | Base URL used as the default for this provider's HTTP client. |
...
| `model_prefixes` | `\[\]const \[:0\]const u8?` | `null` | Model-name prefixes claimed by this provider (e.g. `["gpt-", "o1-"]`). |
| `param_mappings` | `std.StringHashMap(\[:0\]const u8)?` | `null` | Parameter key renaming for this provider. Each entry maps an OpenAI-spec field name (e.g. `"max_completion_tokens"`) to the name this provider expects (e.g. `"max_tokens"`).  Applied automatically by `ConfigDrivenProvider.transform_request`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `item_type` | `\[:0\]const u8` | — | Output type (e.g., `"text"`, `"object"`, `"error"`). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `tool_type` | `\[:0\]const u8` | — | Tool type (e.g., "extractor", "search"). |
...
| `search_domain_filter` | `\[\]const \[:0\]const u8?` | `[]` | Domain filter — restrict results to specific domains. |
...
| `finish_reason` | `FinishReason?` | `FinishReason.stop` | Why the stream ended (present only in final chunk). |
...
| `reasoning_content` | `\[:0\]const u8?` | `null` | Partial reasoning/thinking tokens (OpenAI-compatible extension used by DeepSeek R1, Qwen, etc.). |
...
| `call_type` | `ToolType?` | `ToolType.function` | Tool type (typically "function"). |
...
| `tool_call_id` | `\[:0\]const u8` | — | ID of the tool call this result responds to. |
...
| Value | Description |
|-------|-------------|
| `text` | Plain text response (the common case for text-only models). — Fields: `0`: `[:0]const u8` |
...
| Value | Description |
|-------|-------------|
| `text` | A text segment of the response. — Fields: `text`: `[:0]const u8` |
| `refusal` | A refusal — the model declined to respond. — Fields: `refusal`: `[:0]const u8` |
...
| `image_base64` | Image encoded as a complete data URL. — Fields: `image_base64`: `[:0]const u8` |
...
| `base64` | Inline base64-encoded document data. — Fields: `data`: `[:0]const u8`, `media_type`: `[:0]const u8` |
...
| `api_key` | Custom header: e.g., `X-Api-Key: <key>` — Fields: `0`: `[:0]const u8` |
...
| `open_dal` | OpenDAL-backed storage. Supports 40+ backends (S3, Redis, GCS, local FS, etc.). — Fields: `scheme`: `[:0]const u8`, `config`: `std.StringHashMap([:0]const u8)` |
