...
<!-- alef:hash:b2aa7bb43c8e7eecfafb454e7caf44fa7d5b66d0cc4d185203f60d5a25f3cb39 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Swift API Reference <span class="version-badge">v1.18.4</span>
...
```swift
public static func modelInfo(model: String) -> ModelInfo?
```

**Example:**

```swift
let result = modelInfo("value")
```
...
```swift
public static func countTokens(model: String, text: String) throws -> UInt
```
...
```swift
public static func countRequestTokens(model: String, req: ChatCompletionRequest) throws -> UInt
```
...
```swift
public static func recordCostUsd(system: String, model: String, operation: String, costUsd: Double)
```
...
```swift
public static func checkBound(context: String, currentLen: UInt, incoming: UInt, limit: UInt) throws
```
...
```swift
public static func installCatalogOverlayFromStr(catalogJson: String) throws
```

**Example:**

```swift
try installCatalogOverlayFromStr("value")
```
...
```swift
public static func clearCatalogOverlay()
```
...
```swift
public static func refreshCatalog(config: CatalogRefreshConfig) throws -> RefreshOutcome
```

**Example:**

```swift
let result = try refreshCatalog(CatalogRefreshConfig())
```
...
| `topLogprobs` | `UInt32?` | `null` | Number of most-likely tokens to return log probabilities for, `0..=20`. Requires `logprobs` to be `true`. |
| `maxCompletionTokens` | `UInt64?` | `null` | Upper bound on generated tokens, including reasoning tokens. Supersedes `max_tokens` on OpenAI reasoning models, which reject `max_tokens` outright. |
...
| `metadata` | `\[String: String\]?` | `{}` | Developer-defined tags attached to the completion. |
| `prediction` | `String?` | `null` | Predicted output, for latency reduction when much of the response is known ahead of time. Untyped: the shape is provider-defined and still evolving, and the value is forwarded verbatim. |
| `audio` | `String?` | `null` | Audio output parameters, required when `modalities` includes `audio`. Untyped for the same reason as `prediction`. |
| `webSearchOptions` | `String?` | `null` | Web-search tool configuration for search-enabled models. Untyped for the same reason as `prediction`. |
...
| `logprobs` | `String?` | `null` | Per-token log probabilities, when the request asked for them. Required by OpenAI's response schema as an always-present, nullable key, so this is deliberately not `skip_serializing_if`. |
...
| `embedding` | `\[Float\]` | — | The embedding vector. Providers may return this as a JSON float array or, when `encoding_format: "base64"` was requested, as a base64 string of little-endian `f32` bytes. Base64 responses are decoded on read; this field always serializes back out as a JSON float array. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `maxInFlight` | `UInt?` | `null` | Maximum simultaneously outstanding provider requests. `null` means unlimited. |
...
| `timeoutSecs` | `UInt64?` | `null` | Request timeout, in seconds. |
| `maxRetries` | `UInt32?` | `null` | Maximum number of retries on 429 / 5xx responses. |
| `temperature` | `Double?` | `null` | Sampling temperature for requests built from this config. |
| `maxTokens` | `UInt64?` | `null` | Maximum number of tokens to generate for requests built from this config. |
...
| `headers` | `\[String: String\]?` | `{}` | Extra headers sent on every request. |
| `providers` | `\[LlmProviderConfig\]?` | `[]` | Custom provider configurations, in addition to the built-in providers. |
...
| `cooldownSecs` | `UInt64?` | `null` | Cooldown duration after transient errors, in seconds. |
| `healthCheckSecs` | `UInt64?` | `null` | Background health check interval, in seconds. |
...
| `maxTokens` | `UInt64?` | `null` | Total context window size in tokens (input + output). |
| `maxInputTokens` | `UInt64?` | `null` | Maximum input (prompt) tokens accepted. |
| `maxOutputTokens` | `UInt64?` | `null` | Maximum output (completion) tokens the model can generate. |
...
| `tiers` | `\[ModelTier\]` | `[]` | Context-tiered pricing overrides, sorted by ascending `min_context_tokens`. Empty when the model has flat pricing. |
