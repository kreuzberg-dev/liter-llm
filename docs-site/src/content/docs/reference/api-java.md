...
<!-- alef:hash:fa68e2b723625c9044960d927e29980119e39d5137a8632f888875ad955886b5 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Java API Reference <span class="version-badge">v1.18.4</span>
...
| `baseUrl` | `@Nullable String` | No | The base url |
| `timeoutSecs` | `@Nullable Long` | No | The timeout secs |
| `maxRetries` | `@Nullable Integer` | No | The max retries |
| `modelHint` | `@Nullable String` | No | The model hint |

**Returns:** `DefaultClient`

**Errors:** Throws `LiterLlmRsException`.
...
```java
public static String encodeDataUrl(byte[] bytes, String mime) throws LiterLlmRsException
```
...
| `mime` | `@Nullable String` | No | The mime |
...
```java
public static @Nullable DecodedDataUrl decodeDataUrl(String url) throws LiterLlmRsException
```
...
**Returns:** `@Nullable DecodedDataUrl`
...
```java
public static ProviderCapabilities capabilities(String providerName) throws LiterLlmRsException
```
...
```java
public static @Nullable Double completionCost(String model, long promptTokens, long completionTokens) throws LiterLlmRsException
```
...
```java
public static @Nullable Double completionCostWithCache(String model, long promptTokens, long cachedTokens, long completionTokens) throws LiterLlmRsException
```
...
```java
public static @Nullable ModelInfo modelInfo(String model) throws LiterLlmRsException
```
...
```java
public static long countTokens(String model, String text) throws LiterLlmRsException
```
...
```java
public static long countRequestTokens(String model, ChatCompletionRequest req) throws LiterLlmRsException
```
...
```java
public static void recordCostUsd(String system, String model, String operation, double costUsd) throws LiterLlmRsException
```

**Example:**

```java
recordCostUsd("value", "value", "value", 0.5);
```
...
```java
public static void checkBound(String context, long currentLen, long incoming, long limit) throws LiterLlmRsException
```
...
```java
public static void installCatalogOverlayFromStr(String catalogJson) throws LiterLlmRsException
```
...
```java
public static RefreshOutcome refreshCatalog(CatalogRefreshConfig config) throws LiterLlmRsException
```
...
|-------|------|---------|-------------|
| `content` | `@Nullable AssistantContent` | `null` | The assistant's response: plain text, structured parts, or absent. `null` is valid when the model replies with tool calls only. |
| `name` | `@Nullable String` | `null` | Optional name for the assistant. |
| `toolCalls` | `@Nullable List<ToolCall>` | `Collections.emptyList()` | Tool calls the model wants to execute, if any. |
| `refusal` | `@Nullable String` | `null` | Refusal reason, if the model declined to respond per safety policies. OpenAI's response schema requires this key to be present even when null, so it is deliberately not `skip_serializing_if`. |
| `functionCall` | `@Nullable FunctionCall` | `null` | Deprecated legacy function_call field; retained for API compatibility. |
| `reasoningContent` | `@Nullable String` | `null` | Reasoning/thinking tokens returned by the provider, if any (e.g. DeepSeek R1, Qwen `reasoning_content`, or Anthropic extended thinking). |
...
| `envVar` | `@Nullable String` | `null` | Name of the environment variable that holds the API key (e.g. `"OPENAI_API_KEY"`). Holds the variable name, never the secret value. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `limit` | `@Nullable Integer` | `null` | Maximum number of results to return. Defaults to 20. |
| `after` | `@Nullable String` | `null` | Pagination cursor: return results after this batch ID. |
...
| `hasMore` | `@Nullable Boolean` | `null` | Whether more results are available. |
| `firstId` | `@Nullable String` | `null` | First batch ID in the result set (for pagination). |
| `lastId` | `@Nullable String` | `null` | Last batch ID in the result set (for pagination). |
...
| `outputFileId` | `@Nullable String` | `null` | ID of the output file (present when completed). |
| `errorFileId` | `@Nullable String` | `null` | ID of the error file (present if some requests failed). |
| `createdAt` | `long` | — | Unix timestamp of batch creation. |
| `completedAt` | `@Nullable Long` | `null` | Unix timestamp of completion (if completed). |
| `failedAt` | `@Nullable Long` | `null` | Unix timestamp of failure (if failed). |
| `expiredAt` | `@Nullable Long` | `null` | Unix timestamp of expiration (if expired). |
| `requestCounts` | `@Nullable BatchRequestCounts` | `null` | Request processing counts. |
| `metadata` | `@Nullable Object` | `null` | Metadata attached to the batch. |
...
| `accessKeyId` | `@Nullable String` | `null` | Explicit AWS access key ID. |
| `secretAccessKey` | `@Nullable String` | `null` | Explicit AWS secret access key. |
| `sessionToken` | `@Nullable String` | `null` | Explicit AWS session token (temporary credentials). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `globalLimit` | `@Nullable Double` | `null` | Maximum total spend across all models, in USD.  `null` means unlimited. |
...
| `sourceUrl` | `String` | `"<https://github.com/xberg-io/liter-llm/releases/download/model-catalog/catalog.json"`> | Source URL to fetch `catalog.json` from. Must be `https`. Defaults to `DEFAULT_CATALOG_URL`; configurable so self-hosted mirrors work. |
| `ttlSeconds` | `long` | `86400` | How long a cached `catalog.json` remains valid before a network refetch is attempted, in seconds. |
| `cachePath` | `@Nullable String` | `null` | Filesystem path for the on-disk cache. `null` uses a default path under `std.env.temp_dir()`. |
...
| `usage` | `@Nullable Usage` | `null` | Token usage (typically only in the final chunk). |
| `systemFingerprint` | `@Nullable String` | `null` | Fingerprint of the system configuration (OpenAI-specific). |
| `serviceTier` | `@Nullable String` | `null` | Service tier used (OpenAI-specific). |
...
| `temperature` | `@Nullable Double` | `null` | Sampling temperature. Higher increases randomness, lower is more deterministic. Defaults to 1.0. The accepted range depends on the provider the request is routed to. OpenAI-compatible providers accept `[0.0, 2.0]`; Anthropic and Amazon Bedrock both cap it at `1.0`, and for those two a value above the cap is rejected with a `BadRequest` error before the request is sent, rather than being silently clamped or left for the provider to reject. No range is enforced for providers whose own documentation does not state one — the value is forwarded and the provider decides. Consult the target provider's reference rather than assuming `[0.0, 2.0]` is portable. |
| `topP` | `@Nullable Double` | `null` | Nucleus sampling parameter. Lower is more focused. Accepted ranges vary by provider (most document `[0.0, 1.0]`, but this is not universal — check the target provider's own documentation for its exact bounds). |
| `n` | `@Nullable Integer` | `null` | Number of chat completions to generate. Defaults to 1. |
| `stream` | `@Nullable Boolean` | `null` | Whether to stream the response. Managed by the client layer — do not set directly. |
| `stop` | `@Nullable StopSequence` | `null` | Stop sequence(s) that halt token generation. |
| `maxTokens` | `@Nullable Long` | `null` | Max output tokens. Different from max_completion_tokens in some providers. |
| `presencePenalty` | `@Nullable Double` | `null` | Presence penalty in `[-2.0, 2.0]`. Positive discourages repeated topics. |
| `frequencyPenalty` | `@Nullable Double` | `null` | Frequency penalty in `[-2.0, 2.0]`. Positive discourages repeated tokens. |
| `logitBias` | `@Nullable Map<String, Double>` | `Collections.emptyMap()` | Token bias map.  Uses `BTreeMap` (sorted keys) for deterministic serialization order — important when hashing or signing requests. |
| `user` | `@Nullable String` | `null` | User identifier for request tracking and abuse detection. |
| `tools` | `@Nullable List<ChatCompletionTool>` | `Collections.emptyList()` | Tools the model can invoke. |
| `toolChoice` | `@Nullable ToolChoice` | `null` | Tool usage mode (auto, required, none, or specific tool). |
| `parallelToolCalls` | `@Nullable Boolean` | `null` | Whether the model can call multiple tools in parallel. Defaults to true. |
| `responseFormat` | `@Nullable ResponseFormat` | `ResponseFormat.TEXT` | Output format constraint (text, JSON, JSON schema). |
| `streamOptions` | `@Nullable StreamOptions` | `null` | Streaming options (e.g., include_usage). |
| `seed` | `@Nullable Long` | `null` | Random seed for reproducible outputs. Provider support varies. |
| `reasoningEffort` | `@Nullable ReasoningEffort` | `ReasoningEffort.MEDIUM` | Reasoning effort level (minimal, low, medium, high, max) for extended-thinking models. |
| `modalities` | `@Nullable List<Modality>` | `Collections.emptyList()` | Output modalities to request from the model. For OpenAI audio models, pass `["text", "audio"]`. Vertex AI / Gemini translates these to `generationConfig.responseModalities` (uppercase). |
| `logprobs` | `@Nullable Boolean` | `null` | Whether to return log probabilities of the output tokens. |
| `topLogprobs` | `@Nullable Integer` | `null` | Number of most-likely tokens to return log probabilities for, `0..=20`. Requires `logprobs` to be `true`. |
| `maxCompletionTokens` | `@Nullable Long` | `null` | Upper bound on generated tokens, including reasoning tokens. Supersedes `max_tokens` on OpenAI reasoning models, which reject `max_tokens` outright. |
| `serviceTier` | `@Nullable String` | `null` | Latency tier to process the request under (e.g. `"auto"`, `"default"`, `"flex"`). |
| `store` | `@Nullable Boolean` | `null` | Whether to store the completion for later retrieval by the provider. |
| `metadata` | `@Nullable Map<String, String>` | `Collections.emptyMap()` | Developer-defined tags attached to the completion. |
| `prediction` | `@Nullable Object` | `null` | Predicted output, for latency reduction when much of the response is known ahead of time. Untyped: the shape is provider-defined and still evolving, and the value is forwarded verbatim. |
| `audio` | `@Nullable Object` | `null` | Audio output parameters, required when `modalities` includes `audio`. Untyped for the same reason as `prediction`. |
| `webSearchOptions` | `@Nullable Object` | `null` | Web-search tool configuration for search-enabled models. Untyped for the same reason as `prediction`. |
| `extraBody` | `@Nullable Object` | `null` | Provider-specific extra parameters merged into the request body. Use for guardrails, safety settings, grounding config, etc. |
...
| `usage` | `@Nullable Usage` | `null` | Token usage statistics. |
| `systemFingerprint` | `@Nullable String` | `null` | Fingerprint of the system configuration (OpenAI-specific). |
| `serviceTier` | `@Nullable String` | `null` | Service tier used (OpenAI-specific). |
...
| `finishReason` | `@Nullable FinishReason` | `FinishReason.STOP` | Why the model stopped generating (stop, length, tool_calls, content_filter, etc.). |
| `logprobs` | `@Nullable Object` | `null` | Per-token log probabilities, when the request asked for them. Required by OpenAI's response schema as an always-present, nullable key, so this is deliberately not `skip_serializing_if`. |
...
```java
public @Nullable ChatCompletionChunk process(ChatCompletionChunk chunk) throws LiterLlmRsException
```
...
**Returns:** `@Nullable ChatCompletionChunk`
...
| `endpoint` | `String` | — | API endpoint (e.g., `"/v1/chat/completions"`). |
| `completionWindow` | `String` | — | Completion window (e.g., `"24h"`). |
| `metadata` | `@Nullable Object` | `null` | Optional metadata to attach to the batch. |
...
| `filename` | `@Nullable String` | `null` | Optional filename to associate with the upload. |
...
| `model` | `@Nullable String` | `null` | Model ID (e.g., `"dall-e-3"`). Optional; API may use default if unset. |
| `n` | `@Nullable Integer` | `null` | Number of images to generate. Defaults to 1. |
| `size` | `@Nullable String` | `null` | Image size (e.g., `"1024x1024"`, `"1792x1024"`). |
| `quality` | `@Nullable String` | `null` | Image quality: `"standard"` or `"hd"`. |
| `style` | `@Nullable String` | `null` | Style: `"natural"` or `"vivid"` (DALL-E 3 only). |
| `responseFormat` | `@Nullable String` | `null` | Response format: `"url"` or `"b64_json"`. |
| `user` | `@Nullable String` | `null` | User identifier for request tracking. |
...
| `responseFormat` | `@Nullable String` | `null` | Audio format (e.g., `"mp3"`, `"opus"`, `"aac"`, `"flac"`, `"wav"`, `"pcm"`). |
| `speed` | `@Nullable Double` | `null` | Playback speed in `[0.25, 4.0]`. Defaults to 1.0. |
...
| `language` | `@Nullable String` | `null` | Language ISO-639-1 code (e.g., `"en"`, `"fr"`, `"de"`). Optional; model auto-detects. |
| `prompt` | `@Nullable String` | `null` | Optional text to guide the model (improves accuracy for domain-specific terms). |
| `responseFormat` | `@Nullable String` | `null` | Output format (e.g., `"json"`, `"text"`, `"vtt"`, `"srt"`, `"verbose_json"`). |
| `temperature` | `@Nullable Double` | `null` | Sampling temperature in `[0.0, 1.0]`. Higher increases variability. Defaults to 0. |
...
```java
public BatchObject fetchBatchForPolling(String batchId) throws LiterLlmRsException
```
...
```java
public BatchObject waitForBatch(String batchId, WaitForBatchConfig config) throws LiterLlmRsException
```
...
```java
public void close() throws LiterLlmRsException
```
...
| `name` | `@Nullable String` | `null` | Optional name for the developer message source. |
...
| `encodingFormat` | `@Nullable EmbeddingFormat` | `null` | Output format: float (native) or base64. |
| `dimensions` | `@Nullable Integer` | `null` | Requested embedding dimensions (if supported by the model). |
| `user` | `@Nullable String` | `null` | User identifier for request tracking. |
...
| `usage` | `@Nullable Usage` | `/* serde(default) */` | Token usage (input tokens only; embeddings have zero output tokens). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `purpose` | `@Nullable String` | `null` | Filter by file purpose (e.g., `"batch"`, `"fine-tune"`). |
| `limit` | `@Nullable Integer` | `null` | Maximum number of results to return. Defaults to 20. |
| `after` | `@Nullable String` | `null` | Pagination cursor: return results after this file ID. |
...
| `status` | `@Nullable String` | `null` | Processing status (e.g., `"uploaded"`, `"processed"`). |
...
| `description` | `@Nullable String` | `/* serde(default) */` | Human-readable description explaining what the function does. |
| `parameters` | `@Nullable Object` | `/* serde(default) */` | JSON Schema defining the function's parameters. |
| `strict` | `@Nullable Boolean` | `/* serde(default) */` | If true, enforce strict JSON schema validation for arguments. |
...
```java
public HealthStatus check(String upstream) throws LiterLlmRsException
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `url` | `@Nullable String` | `null` | Image URL (if response_format was "url"). |
| `b64Json` | `@Nullable String` | `null` | Base64-encoded image data (if response_format was "b64_json"). |
| `revisedPrompt` | `@Nullable String` | `null` | The final prompt used to generate the image (DALL-E 3). |
...
| `detail` | `@Nullable ImageDetail` | `null` | Detail level: low (512x512), high (2x2 tiles), or auto (model-selected). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `maxInFlight` | `@Nullable Long` | `null` | Maximum simultaneously outstanding provider requests. `null` means unlimited. |
...
| `description` | `@Nullable String` | `null` | Description of what the schema represents. |
| `schema` | `Object` | — | JSON Schema object defining the output structure. |
| `strict` | `@Nullable Boolean` | `null` | If true, enforce strict schema validation. |
...
| `apiKey` | `@Nullable String` | `null` | API key for authentication. |
| `baseUrl` | `@Nullable String` | `null` | Override base URL. When set, all requests go here and provider auto-detection is skipped. |
| `timeoutSecs` | `@Nullable Long` | `null` | Request timeout, in seconds. |
| `maxRetries` | `@Nullable Integer` | `null` | Maximum number of retries on 429 / 5xx responses. |
| `temperature` | `@Nullable Double` | `null` | Sampling temperature for requests built from this config. |
| `maxTokens` | `@Nullable Long` | `null` | Maximum number of tokens to generate for requests built from this config. |
| `loadEnv` | `@Nullable Boolean` | `null` | Automatically load the API key from the provider's environment variable when no explicit key is provided (default: `true`). |
| `headers` | `@Nullable Map<String, String>` | `Collections.emptyMap()` | Extra headers sent on every request. |
| `providers` | `@Nullable List<LlmProviderConfig>` | `Collections.emptyList()` | Custom provider configurations, in addition to the built-in providers. |
| `cache` | `@Nullable LlmCacheConfig` | `null` | Response cache configuration. |
| `budget` | `@Nullable LlmBudgetConfig` | `null` | Budget enforcement configuration. |
| `rateLimit` | `@Nullable LlmRateLimitConfig` | `null` | Per-model rate limiting configuration. |
| `inFlightLimit` | `@Nullable LlmInFlightLimitConfig` | `null` | Global per-client in-flight provider request limit. |
| `costTracking` | `@Nullable Boolean` | `null` | Enable per-request cost tracking. |
| `tracing` | `@Nullable Boolean` | `null` | Enable OpenTelemetry-compatible tracing spans. |
| `cooldownSecs` | `@Nullable Long` | `null` | Cooldown duration after transient errors, in seconds. |
| `healthCheckSecs` | `@Nullable Long` | `null` | Background health check interval, in seconds. |
| `bedrock` | `@Nullable BedrockConfig` | `null` | AWS Bedrock configuration (region, credentials, cross-region routing). |
...
| `cacheReadInputTokenCost` | `@Nullable Double` | `null` | Cost in USD per cached input token (cache hit / read). |
| `cacheCreationInputTokenCost` | `@Nullable Double` | `null` | Cost in USD per token written to the prompt cache. |
| `inputCostPerAudioToken` | `@Nullable Double` | `null` | Cost in USD per input audio token. |
| `outputCostPerAudioToken` | `@Nullable Double` | `null` | Cost in USD per output audio token. |
| `outputCostPerReasoningToken` | `@Nullable Double` | `null` | Cost in USD per reasoning (extended-thinking) output token. |
| `maxTokens` | `@Nullable Long` | `null` | Total context window size in tokens (input + output). |
| `maxInputTokens` | `@Nullable Long` | `null` | Maximum input (prompt) tokens accepted. |
| `maxOutputTokens` | `@Nullable Long` | `null` | Maximum output (completion) tokens the model can generate. |
| `mode` | `@Nullable String` | `null` | Best-effort operating mode, e.g. `"chat"`, `"embedding"`. |
| `supportsVision` | `@Nullable Boolean` | `null` | The model accepts image input. |
| `supportsFunctionCalling` | `@Nullable Boolean` | `null` | The model supports tool / function calling. |
| `supportsReasoning` | `@Nullable Boolean` | `null` | The model supports extended-thinking / reasoning tokens. |
| `supportsStructuredOutput` | `@Nullable Boolean` | `null` | The model supports JSON-mode or `response_format` structured output. |
| `supportsAudioInput` | `@Nullable Boolean` | `null` | The model accepts audio input. |
| `supportsAudioOutput` | `@Nullable Boolean` | `null` | The model can generate audio output. |
| `supportsPromptCaching` | `@Nullable Boolean` | `null` | The model supports prompt caching. |
...
| `cacheReadInputTokenCost` | `@Nullable Double` | `null` | Cost in USD per cached input token within this tier. |
| `cacheCreationInputTokenCost` | `@Nullable Double` | `null` | Cost in USD per cache-write token within this tier. |
| `inputCostPerAudioToken` | `@Nullable Double` | `null` | Cost in USD per input audio token within this tier. |
| `outputCostPerAudioToken` | `@Nullable Double` | `null` | Cost in USD per output audio token within this tier. |
| `outputCostPerReasoningToken` | `@Nullable Double` | `null` | Cost in USD per reasoning output token within this tier. |
...
| `model` | `@Nullable String` | `null` | Model ID (e.g., `"text-moderation-latest"`). Optional; API uses default if unset. |
...
| `imageBase64` | `@Nullable String` | `/* serde(default) */` | Base64-encoded image data (if `include_image_base64` was true). |
...
| `images` | `@Nullable List<OcrImage>` | `/* serde(default) */` | Embedded images extracted from the page (if `include_image_base64` was true). |
| `dimensions` | `@Nullable PageDimensions` | `/* serde(default) */` | Page dimensions in pixels, if available. |
...
| `pages` | `@Nullable List<Integer>` | `Collections.emptyList()` | Specific pages to process (1-indexed). `null` means all pages. |
| `includeImageBase64` | `@Nullable Boolean` | `null` | Whether to include base64-encoded images of each processed page. |
...
| `usage` | `@Nullable Usage` | `/* serde(default) */` | Token usage, if reported by the provider. |
...
| `displayName` | `@Nullable String` | `null` | Human-readable provider name shown in UIs. |
| `baseUrl` | `@Nullable String` | `null` | Base URL used as the default for this provider's HTTP client. |
| `auth` | `@Nullable AuthConfig` | `null` | Authentication scheme metadata (auth type + env var holding the key). |
| `endpoints` | `@Nullable List<String>` | `null` | Supported endpoint kinds (e.g. `chat`, `embeddings`). |
| `modelPrefixes` | `@Nullable List<String>` | `null` | Model-name prefixes claimed by this provider (e.g. `["gpt-", "o1-"]`). |
| `paramMappings` | `@Nullable Map<String, String>` | `null` | Parameter key renaming for this provider. Each entry maps an OpenAI-spec field name (e.g. `"max_completion_tokens"`) to the name this provider expects (e.g. `"max_tokens"`).  Applied automatically by `ConfigDrivenProvider.transform_request`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `rpm` | `@Nullable Integer` | `null` | Maximum requests per window.  `null` means unlimited. |
| `tpm` | `@Nullable Long` | `null` | Maximum tokens per window.  `null` means unlimited. |
...
| `topN` | `@Nullable Integer` | `null` | Return only the top N results. Optional. |
| `returnDocuments` | `@Nullable Boolean` | `null` | Include the document content in results. Defaults to false. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `id` | `@Nullable String` | `null` | Unique identifier for this rerank request. |
| `results` | `List<RerankResult>` | — | Reranked documents in order of relevance. |
| `meta` | `@Nullable Object` | `/* serde(default) */` | Optional metadata about the reranking operation. |
...
| `document` | `@Nullable RerankResultDocument` | `/* serde(default) */` | Original document content (if `return_documents` was true). |
...
| `usage` | `@Nullable ResponseUsage` | `null` | Token usage. |
| `error` | `@Nullable Object` | `null` | Error details (if status is "failed"). |
...
| `maxResults` | `@Nullable Integer` | `null` | Maximum number of results to return. |
| `searchDomainFilter` | `@Nullable List<String>` | `Collections.emptyList()` | Domain filter — restrict results to specific domains. |
| `country` | `@Nullable String` | `null` | Country code for localized results (ISO 3166-1 alpha-2, e.g., `"US"`, `"FR"`). |
...
| `date` | `@Nullable String` | `/* serde(default) */` | Publication or last-updated date, if available. |
...
| `finishReason` | `@Nullable FinishReason` | `FinishReason.STOP` | Why the stream ended (present only in final chunk). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `role` | `@Nullable String` | `null` | Role (typically present only in the first chunk). |
| `content` | `@Nullable String` | `null` | Partial content chunk (e.g., a few words of the response). |
| `toolCalls` | `@Nullable List<StreamToolCall>` | `Collections.emptyList()` | Partial tool calls being streamed. |
| `functionCall` | `@Nullable StreamFunctionCall` | `null` | Deprecated legacy function_call delta; retained for API compatibility. |
| `refusal` | `@Nullable String` | `null` | Partial refusal message. |
| `reasoningContent` | `@Nullable String` | `null` | Partial reasoning/thinking tokens (OpenAI-compatible extension used by DeepSeek R1, Qwen, etc.). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `@Nullable String` | `null` | Function name (typically in the first chunk). |
| `arguments` | `@Nullable String` | `null` | Partial JSON arguments chunk. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `includeUsage` | `@Nullable Boolean` | `null` | If true, include token usage in the final stream chunk. |
...
| `id` | `@Nullable String` | `null` | Tool call ID (typically in the first chunk for this call). |
| `callType` | `@Nullable ToolType` | `ToolType.FUNCTION` | Tool type (typically "function"). |
| `function` | `@Nullable StreamFunctionCall` | `null` | Partial function name and arguments. |
...
| `name` | `@Nullable String` | `null` | Optional name for the system message source. |
...
| `name` | `@Nullable String` | `null` | Optional tool/function name. |
...
| `language` | `@Nullable String` | `null` | Detected language (ISO-639-1 code). |
| `duration` | `@Nullable Double` | `null` | Total audio duration in seconds. |
| `segments` | `@Nullable List<TranscriptionSegment>` | `Collections.emptyList()` | Detailed segment-level transcription (if response_format is "verbose_json"). |
...
| `promptTokensDetails` | `@Nullable PromptTokensDetails` | `null` | Breakdown of tokens used in the prompt, including cached tokens served at the provider's discounted cache-read rate. Absent when the provider does not return prompt-token details. |
...
| `name` | `@Nullable String` | `null` | Optional name for the user. |
...
| `timeoutSecs` | `@Nullable Double` | `null` | Optional timeout in seconds — polling fails if this duration is exceeded. |
...
| `MULTIMODAL` | Text and image parts for a single multimodal embedding. — Fields: `0`: `List<EmbeddingContentPart>` |
...
|-------|-------------|
| `TEXT` | Plain text. — Fields: `text`: `String` |
| `IMAGE_URL` | Image identified by a data URL or HTTP/HTTPS URL. — Fields: `imageUrl`: `ImageUrl` |
| `IMAGE_BASE64` | Image encoded as a complete data URL. — Fields: `imageBase64`: `String` |
...
| Variant | Description |
|---------|-------------|
| `AuthenticationException` | `status` preserves the exact HTTP status code received (401 or 403). |
| `RateLimitedException` | rate limited: {message} |
| `BadRequestException` | `status` preserves the exact HTTP status code received (400, 405, 413, 422, …). |
| `ContextWindowExceededException` | context window exceeded: {message} |
| `ContentPolicyException` | content policy violation: {message} |
| `NotFoundException` | not found: {message} |
| `ServerErrorException` | `status` preserves the exact HTTP status code received (500, or other 5xx not covered by `ServiceUnavailable`). |
| `ServiceUnavailableException` | `status` preserves the exact HTTP status code received (502, 503, or 504). |
| `TimeoutException` | request timeout |
| `StreamingException` | A catch-all for errors that occur during streaming response processing. This variant covers multiple sub-conditions including UTF-8 decoding failures, CRC/checksum mismatches (AWS EventStream), JSON parse errors in individual SSE chunks, and buffer overflow conditions.  The `message` field contains a human-readable description of the specific failure. |
| `EndpointNotSupportedException` | provider {provider} does not support {endpoint} |
| `InvalidHeaderException` | invalid header {name:?}: {reason} |
| `SerializationException` | serialization error: {0} |
| `BudgetExceededException` | budget exceeded: {message} |
| `HookRejectedException` | hook rejected: {message} |
| `InternalErrorException` | An internal logic error (e.g. unexpected Tower response variant). This should never surface in normal operation — if it does, it indicates a bug in the library. |
| `OutboundForbiddenException` | An outbound request was blocked by the active `OutboundPolicy`. Returned when `register_custom_provider` is called with a `base_url` that violates the policy (e.g. a private-range IP under `DenyPrivate`), or when the per-connection DNS resolver detects a forbidden address at connect time. |
| `IdempotencyConflictException` | A different request body was submitted for an existing `Idempotency-Key`. Per the OpenAI `Idempotency-Key` convention, once a key is used with a particular request body, subsequent requests using the same key must carry an identical body.  A body mismatch is a hard error (not retryable). HTTP equivalent: 409 Conflict. |
| `IdempotencyInFlightException` | The same `Idempotency-Key` is already in-flight (another request with the same key is currently being processed). The caller should wait briefly and retry.  The response is not yet available, and this request has been short-circuited to avoid running the operation twice. HTTP equivalent: 409 Conflict (retryable after a brief delay). |
...
| Variant | Description |
|---------|-------------|
| `DisabledException` | Runtime catalog refresh was not enabled. `refresh_catalog` itself never returns this — it returns `Ok(RefreshOutcome.Disabled)` instead — but the variant is part of the public error surface for callers that want to treat "disabled" as a hard error. |
| `InsecureUrlException` | `source_url` did not use the `https` scheme, or failed to parse as a URL at all. There is no host allowlist: the URL is user-configurable for self-hosted catalog mirrors, so only the scheme is enforced. |
| `FetchException` | The network fetch failed, timed out, or returned a non-success status. |
| `ParseException` | The fetched or cached catalog JSON failed to parse. |
| `CacheException` | A cache file **read** failed on an otherwise-fresh cache file. Cache **write** failures are best-effort and never surface as this error (see `refresh_catalog`). |
