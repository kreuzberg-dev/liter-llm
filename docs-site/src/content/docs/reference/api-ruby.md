...
<!-- alef:hash:61ccec53d599b5210dbb4402cd36870c2ca5df8b9488ef3de259171217ad50c0 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Ruby API Reference <span class="version-badge">v1.18.4</span>
...
```ruby
def self.record_cost_usd(system, model, operation, cost_usd)
```
...
```ruby
def self.install_catalog_overlay_from_str(catalog_json)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `catalog_json` | `String` | Yes | The catalog json |
...
```ruby
def self.clear_catalog_overlay()
```
...
```ruby
def self.refresh_catalog(config)
```

**Example:**

```ruby
result = refresh_catalog(CatalogRefreshConfig.new)
```
...
| `reasoning_content` | `String?` | `nil` | Reasoning/thinking tokens returned by the provider, if any (e.g. DeepSeek R1, Qwen `reasoning_content`, or Anthropic extended thinking). |
...
| `access_key_id` | `String?` | `nil` | Explicit AWS access key ID. |
| `secret_access_key` | `String?` | `nil` | Explicit AWS secret access key. |
| `session_token` | `String?` | `nil` | Explicit AWS session token (temporary credentials). |
...
| `cache_path` | `String?` | `nil` | Filesystem path for the on-disk cache. `nil` uses a default path under `std.env.temp_dir()`. |
...
| `top_p` | `Float?` | `nil` | Nucleus sampling parameter. Lower is more focused. Accepted ranges vary by provider (most document `[0.0, 1.0]`, but this is not universal — check the target provider's own documentation for its exact bounds). |
...
| `response_format` | `ResponseFormat?` | `:text` | Output format constraint (text, JSON, JSON schema). |
...
| `reasoning_effort` | `ReasoningEffort?` | `:medium` | Reasoning effort level (minimal, low, medium, high, max) for extended-thinking models. |
...
| `logprobs` | `Boolean?` | `nil` | Whether to return log probabilities of the output tokens. |
| `top_logprobs` | `Integer?` | `nil` | Number of most-likely tokens to return log probabilities for, `0..=20`. Requires `logprobs` to be `true`. |
| `max_completion_tokens` | `Integer?` | `nil` | Upper bound on generated tokens, including reasoning tokens. Supersedes `max_tokens` on OpenAI reasoning models, which reject `max_tokens` outright. |
| `service_tier` | `String?` | `nil` | Latency tier to process the request under (e.g. `"auto"`, `"default"`, `"flex"`). |
| `store` | `Boolean?` | `nil` | Whether to store the completion for later retrieval by the provider. |
| `metadata` | `Hash{String=>String}?` | `{}` | Developer-defined tags attached to the completion. |
| `prediction` | `Object?` | `nil` | Predicted output, for latency reduction when much of the response is known ahead of time. Untyped: the shape is provider-defined and still evolving, and the value is forwarded verbatim. |
| `audio` | `Object?` | `nil` | Audio output parameters, required when `modalities` includes `audio`. Untyped for the same reason as `prediction`. |
| `web_search_options` | `Object?` | `nil` | Web-search tool configuration for search-enabled models. Untyped for the same reason as `prediction`. |
...
| `finish_reason` | `FinishReason?` | `:stop` | Why the model stopped generating (stop, length, tool_calls, content_filter, etc.). |
| `logprobs` | `Object?` | `nil` | Per-token log probabilities, when the request asked for them. Required by OpenAI's response schema as an always-present, nullable key, so this is deliberately not `skip_serializing_if`. |
...
| `embedding` | `Array<Float>` | — | The embedding vector. Providers may return this as a JSON float array or, when `encoding_format: "base64"` was requested, as a base64 string of little-endian `f32` bytes. Base64 responses are decoded on read; this field always serializes back out as a JSON float array. |
...
| `input` | `EmbeddingInput` | `:single` | Text, texts, or multimodal content to embed. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `max_in_flight` | `Integer?` | `nil` | Maximum simultaneously outstanding provider requests. `nil` means unlimited. |
...
| `base_url` | `String?` | `nil` | Override base URL. When set, all requests go here and provider auto-detection is skipped. |
| `timeout_secs` | `Integer?` | `nil` | Request timeout, in seconds. |
| `max_retries` | `Integer?` | `nil` | Maximum number of retries on 429 / 5xx responses. |
| `temperature` | `Float?` | `nil` | Sampling temperature for requests built from this config. |
| `max_tokens` | `Integer?` | `nil` | Maximum number of tokens to generate for requests built from this config. |
| `load_env` | `Boolean?` | `nil` | Automatically load the API key from the provider's environment variable when no explicit key is provided (default: `true`). |
| `headers` | `Hash{String=>String}?` | `{}` | Extra headers sent on every request. |
| `providers` | `Array<LlmProviderConfig>?` | `[]` | Custom provider configurations, in addition to the built-in providers. |
| `cache` | `LlmCacheConfig?` | `nil` | Response cache configuration. |
| `budget` | `LlmBudgetConfig?` | `nil` | Budget enforcement configuration. |
| `rate_limit` | `LlmRateLimitConfig?` | `nil` | Per-model rate limiting configuration. |
| `in_flight_limit` | `LlmInFlightLimitConfig?` | `nil` | Global per-client in-flight provider request limit. |
| `cost_tracking` | `Boolean?` | `nil` | Enable per-request cost tracking. |
| `tracing` | `Boolean?` | `nil` | Enable OpenTelemetry-compatible tracing spans. |
| `cooldown_secs` | `Integer?` | `nil` | Cooldown duration after transient errors, in seconds. |
| `health_check_secs` | `Integer?` | `nil` | Background health check interval, in seconds. |
...
| `cache_read_input_token_cost` | `Float?` | `nil` | Cost in USD per cached input token (cache hit / read). |
| `cache_creation_input_token_cost` | `Float?` | `nil` | Cost in USD per token written to the prompt cache. |
| `input_cost_per_audio_token` | `Float?` | `nil` | Cost in USD per input audio token. |
| `output_cost_per_audio_token` | `Float?` | `nil` | Cost in USD per output audio token. |
| `output_cost_per_reasoning_token` | `Float?` | `nil` | Cost in USD per reasoning (extended-thinking) output token. |
| `max_tokens` | `Integer?` | `nil` | Total context window size in tokens (input + output). |
| `max_input_tokens` | `Integer?` | `nil` | Maximum input (prompt) tokens accepted. |
| `max_output_tokens` | `Integer?` | `nil` | Maximum output (completion) tokens the model can generate. |
| `mode` | `String?` | `nil` | Best-effort operating mode, e.g. `"chat"`, `"embedding"`. |
| `supports_vision` | `Boolean?` | `nil` | The model accepts image input. |
| `supports_function_calling` | `Boolean?` | `nil` | The model supports tool / function calling. |
| `supports_reasoning` | `Boolean?` | `nil` | The model supports extended-thinking / reasoning tokens. |
| `supports_structured_output` | `Boolean?` | `nil` | The model supports JSON-mode or `response_format` structured output. |
| `supports_audio_input` | `Boolean?` | `nil` | The model accepts audio input. |
| `supports_audio_output` | `Boolean?` | `nil` | The model can generate audio output. |
| `supports_prompt_caching` | `Boolean?` | `nil` | The model supports prompt caching. |
...
| `cache_read_input_token_cost` | `Float?` | `nil` | Cost in USD per cached input token within this tier. |
| `cache_creation_input_token_cost` | `Float?` | `nil` | Cost in USD per cache-write token within this tier. |
| `input_cost_per_audio_token` | `Float?` | `nil` | Cost in USD per input audio token within this tier. |
| `output_cost_per_audio_token` | `Float?` | `nil` | Cost in USD per output audio token within this tier. |
| `output_cost_per_reasoning_token` | `Float?` | `nil` | Cost in USD per reasoning output token within this tier. |
...
| `finish_reason` | `FinishReason?` | `:stop` | Why the stream ended (present only in final chunk). |
...
| `reasoning_content` | `String?` | `nil` | Partial reasoning/thinking tokens (OpenAI-compatible extension used by DeepSeek R1, Qwen, etc.). |
...
| `call_type` | `ToolType?` | `:function` | Tool type (typically "function"). |
...
| `image_base64` | Image encoded as a complete data URL. — Fields: `image_base64`: `String` |
...
| `open_dal` | OpenDAL-backed storage. Supports 40+ backends (S3, Redis, GCS, local FS, etc.). — Fields: `scheme`: `String`, `config`: `Hash{String=>String}` |
