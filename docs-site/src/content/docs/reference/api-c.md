...
<!-- alef:hash:469dc3bd0901d224398100375f589e3980b50874e231545fe54c8ef8467b256d -->
...
| `status` | `LITERLLMAlefHandle` | `LITERLLM_LITERLLM_VALIDATING` | Current job status. |
...
| `enforcement` | `LITERLLMAlefHandle` | `LITERLLM_LITERLLM_HARD` | Whether to reject requests or merely warn when a limit is exceeded. |
...
| `backend` | `LITERLLMAlefHandle` | `LITERLLM_LITERLLM_MEMORY` | Storage backend to use. |
...
| `response_format` | `LITERLLMAlefHandle` | `LITERLLM_LITERLLM_TEXT` | Output format constraint (text, JSON, JSON schema). |
...
| `reasoning_effort` | `LITERLLMAlefHandle` | `LITERLLM_LITERLLM_MEDIUM` | Reasoning effort level (minimal, low, medium, high, max) for extended-thinking models. |
...
| `finish_reason` | `LITERLLMAlefHandle` | `LITERLLM_LITERLLM_STOP` | Why the model stopped generating (stop, length, tool_calls, content_filter, etc.). |
...
| `purpose` | `LITERLLMAlefHandle` | `LITERLLM_LITERLLM_ASSISTANTS` | Purpose for the file. |
...
| `input` | `LITERLLMAlefHandle` | `LITERLLM_LITERLLM_SINGLE` | Text, texts, or multimodal content to embed. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `input` | `LITERLLMAlefHandle` | `LITERLLM_LITERLLM_SINGLE` | Text or texts to check. |
...
| `document` | `LITERLLMAlefHandle` | `LITERLLM_LITERLLM_URL` | The document to process (URL or base64). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `choice_type` | `LITERLLMAlefHandle` | `LITERLLM_LITERLLM_FUNCTION` | Tool type (always "function"). |
...
| `finish_reason` | `LITERLLMAlefHandle` | `LITERLLM_LITERLLM_STOP` | Why the stream ended (present only in final chunk). |
...
| `call_type` | `LITERLLMAlefHandle` | `LITERLLM_LITERLLM_FUNCTION` | Tool type (typically "function"). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `content` | `LITERLLMAlefHandle` | `LITERLLM_LITERLLM_TEXT` | Instructions or context that apply throughout the conversation. Accepts either a plain text string or an array of content parts, mirroring `UserContent` so that `Message.system_with_parts` works. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `content` | `LITERLLMAlefHandle` | `LITERLLM_LITERLLM_TEXT` | Message content as plain text or array of content parts (text, images, documents, audio). |
