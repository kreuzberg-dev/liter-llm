...
<!-- alef:hash:16cda86bdcde61c9612a0bc36b12a8f2a81efb62768352be4c1938b0660fe1c5 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## C# API Reference <span class="version-badge">v1.18.3</span>
...
```csharp
var result = LiterLlmConverter.CreateClient("value", "value", 42, 42, "value");
```
...
```csharp
var result = LiterLlmConverter.CreateClientFromJson("value");
```
...
```csharp
var result = LiterLlmConverter.EncodeDataUrl(System.Text.Encoding.UTF8.GetBytes("data"), "value");
```
...
```csharp
var result = LiterLlmConverter.DecodeDataUrl("value");
```
...
```csharp
LiterLlmConverter.RegisterCustomProvider(new CustomProviderConfig());
```
...
```csharp
var result = LiterLlmConverter.UnregisterCustomProvider("value");
```
...
```csharp
var result = LiterLlmConverter.Capabilities("value");
```
...
```csharp
var result = LiterLlmConverter.AllProviders();
```
...
```csharp
var result = LiterLlmConverter.ComplexProviderNames();
```
...
```csharp
var result = LiterLlmConverter.CompletionCost("value", 42, 42);
```
...
```csharp
var result = LiterLlmConverter.CompletionCostWithCache("value", 42, 42, 42);
```
...
```csharp
var result = LiterLlmConverter.ModelInfo("value");
```
...
```csharp
var result = LiterLlmConverter.CountTokens("value", "value");
```
...
```csharp
var result = LiterLlmConverter.CountRequestTokens("value", new ChatCompletionRequest());
```
...
```csharp
public static void RecordCostUsd(string system, string model, string operation, double costUsd)
```

**Example:**

```csharp
LiterLlmConverter.RecordCostUsd("value", "value", "value", 0.5);
```
...
```csharp
LiterLlmConverter.CheckBound("value", 42, 42, 42);
```
...
| `CurrentLen` | `ulong` | Yes | The current len |
| `Incoming` | `ulong` | Yes | The incoming |
| `Limit` | `ulong` | Yes | The limit |
...
```csharp
LiterLlmConverter.EnsureCryptoProvider();
```
...
```csharp
LiterLlmConverter.InstallCatalogOverlayFromStr("value");
```
...
```csharp
LiterLlmConverter.ClearCatalogOverlay();
```
...
```csharp
var result = await LiterLlmConverter.RefreshCatalogAsync(new CatalogRefreshConfig());
```
...
| `Refusal` | `string?` | `null` | Refusal reason, if the model declined to respond per safety policies. OpenAI's response schema requires this key to be present even when null, so it is deliberately not `skip_serializing_if`. |
| `FunctionCall` | `FunctionCall?` | `null` | Deprecated legacy function_call field; retained for API compatibility. |
| `ReasoningContent` | `string?` | `null` | Reasoning/thinking tokens returned by the provider, if any (e.g. DeepSeek R1, Qwen `reasoning_content`, or Anthropic extended thinking). |
...
| `AccessKeyId` | `string?` | `null` | Explicit AWS access key ID. |
| `SecretAccessKey` | `string?` | `null` | Explicit AWS secret access key. |
| `SessionToken` | `string?` | `null` | Explicit AWS session token (temporary credentials). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `MaxEntries` | `ulong` | `256` | Maximum number of cached entries. |
...
| `TtlSeconds` | `ulong` | `86400` | How long a cached `catalog.json` remains valid before a network refetch is attempted, in seconds. |
...
| `TopP` | `double?` | `null` | Nucleus sampling parameter. Lower is more focused. Accepted ranges vary by provider (most document `[0.0, 1.0]`, but this is not universal — check the target provider's own documentation for its exact bounds). |
...
| `TopLogprobs` | `uint?` | `null` | Number of most-likely tokens to return log probabilities for, `0..=20`. Requires `logprobs` to be `true`. |
| `MaxCompletionTokens` | `ulong?` | `null` | Upper bound on generated tokens, including reasoning tokens. Supersedes `max_tokens` on OpenAI reasoning models, which reject `max_tokens` outright. |
| `ServiceTier` | `string?` | `null` | Latency tier to process the request under (e.g. `"auto"`, `"default"`, `"flex"`). |
| `Store` | `bool?` | `null` | Whether to store the completion for later retrieval by the provider. |
| `Metadata` | `Dictionary<string, string>?` | `new Dictionary<string, string>()` | Developer-defined tags attached to the completion. |
| `Prediction` | `object?` | `null` | Predicted output, for latency reduction when much of the response is known ahead of time. Untyped: the shape is provider-defined and still evolving, and the value is forwarded verbatim. |
| `Audio` | `object?` | `null` | Audio output parameters, required when `modalities` includes `audio`. Untyped for the same reason as `prediction`. |
| `WebSearchOptions` | `object?` | `null` | Web-search tool configuration for search-enabled models. Untyped for the same reason as `prediction`. |
...
| `Logprobs` | `object?` | `null` | Per-token log probabilities, when the request asked for them. Required by OpenAI's response schema as an always-present, nullable key, so this is deliberately not `skip_serializing_if`. |
...
```csharp
var result = await instance.FetchBatchForPollingAsync("value");
```
...
```csharp
var result = await instance.WaitForBatchAsync("value", new WaitForBatchConfig());
```
...
```csharp
var result = await instance.CheckAsync("value");
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `MaxInFlight` | `ulong?` | `null` | Maximum simultaneously outstanding provider requests. `null` means unlimited. |
...
| `BaseUrl` | `string?` | `null` | Override base URL. When set, all requests go here and provider auto-detection is skipped. |
| `TimeoutSecs` | `ulong?` | `null` | Request timeout, in seconds. |
| `MaxRetries` | `uint?` | `null` | Maximum number of retries on 429 / 5xx responses. |
| `Temperature` | `double?` | `null` | Sampling temperature for requests built from this config. |
| `MaxTokens` | `ulong?` | `null` | Maximum number of tokens to generate for requests built from this config. |
| `LoadEnv` | `bool?` | `null` | Automatically load the API key from the provider's environment variable when no explicit key is provided (default: `true`). |
| `Headers` | `Dictionary<string, string>?` | `new Dictionary<string, string>()` | Extra headers sent on every request. |
| `Providers` | `List<LlmProviderConfig>?` | `new List<LlmProviderConfig>()` | Custom provider configurations, in addition to the built-in providers. |
| `Cache` | `LlmCacheConfig?` | `null` | Response cache configuration. |
| `Budget` | `LlmBudgetConfig?` | `null` | Budget enforcement configuration. |
| `RateLimit` | `LlmRateLimitConfig?` | `null` | Per-model rate limiting configuration. |
| `InFlightLimit` | `LlmInFlightLimitConfig?` | `null` | Global per-client in-flight provider request limit. |
| `CostTracking` | `bool?` | `null` | Enable per-request cost tracking. |
| `Tracing` | `bool?` | `null` | Enable OpenTelemetry-compatible tracing spans. |
| `CooldownSecs` | `ulong?` | `null` | Cooldown duration after transient errors, in seconds. |
| `HealthCheckSecs` | `ulong?` | `null` | Background health check interval, in seconds. |
...
| `MaxTokens` | `ulong?` | `null` | Total context window size in tokens (input + output). |
| `MaxInputTokens` | `ulong?` | `null` | Maximum input (prompt) tokens accepted. |
| `MaxOutputTokens` | `ulong?` | `null` | Maximum output (completion) tokens the model can generate. |
...
| `Tiers` | `List<ModelTier>` | `new List<ModelTier>()` | Context-tiered pricing overrides, sorted by ascending `min_context_tokens`. Empty when the model has flat pricing. |
...
| `Created` | `ulong` | — | Unix timestamp of model creation (or release date). Defaults to `0` when a provider omits it — DeepSeek and some other OpenAI-compatible providers do not return `created` from `/v1/models`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `MinContextTokens` | `ulong` | — | The tier applies when the prompt/context token count is at least this value. |
...
| `ReasoningContent` | `string?` | `null` | Partial reasoning/thinking tokens (OpenAI-compatible extension used by DeepSeek R1, Qwen, etc.). |
