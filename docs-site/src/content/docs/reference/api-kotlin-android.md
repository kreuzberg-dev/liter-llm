...
<!-- alef:hash:691a1d6ac1d6f2da492cce87ec5d6a341079b1b33444f14687a55434b7fe01e4 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Kotlin (Android) API Reference <span class="version-badge">v1.18.3</span>
...
```kotlin
fun encodeDataUrl(bytes: ByteArray, mime: String? = null): String
```

**Example:**

```kotlin
val result = encodeDataUrl("data".toByteArray(), "value")
```
...
```kotlin
fun decodeDataUrl(url: String): DecodedDataUrl?
```

**Example:**

```kotlin
val result = decodeDataUrl("value")
```
...
```kotlin
fun modelInfo(model: String): ModelInfo?
```

**Example:**

```kotlin
val result = modelInfo("value")
```
...
```kotlin
fun recordCostUsd(system: String, model: String, operation: String, costUsd: Double)
```
...
```kotlin
@Throws(CatalogRefreshError::class)
fun installCatalogOverlayFromStr(catalogJson: String)
```
...
```kotlin
val result = refreshCatalog(CatalogRefreshConfig())
```
...
| `hasMore` | `Boolean?` | `null` | Whether more results are available. |
...
| `completedAt` | `Long?` | `null` | Unix timestamp of completion (if completed). |
| `failedAt` | `Long?` | `null` | Unix timestamp of failure (if failed). |
| `expiredAt` | `Long?` | `null` | Unix timestamp of expiration (if expired). |
| `requestCounts` | `BatchRequestCounts?` | `null` | Request processing counts. |
| `metadata` | `Any?` | `null` | Metadata attached to the batch. |
...
| `maxTokens` | `Long?` | `null` | Max output tokens. Different from max_completion_tokens in some providers. |
...
| `parallelToolCalls` | `Boolean?` | `null` | Whether the model can call multiple tools in parallel. Defaults to true. |
...
| `topLogprobs` | `Int?` | `null` | Number of most-likely tokens to return log probabilities for, `0..=20`. Requires `logprobs` to be `true`. |
| `maxCompletionTokens` | `Long?` | `null` | Upper bound on generated tokens, including reasoning tokens. Supersedes `max_tokens` on OpenAI reasoning models, which reject `max_tokens` outright. |
| `serviceTier` | `String?` | `null` | Latency tier to process the request under (e.g. `"auto"`, `"default"`, `"flex"`). |
| `store` | `Boolean?` | `null` | Whether to store the completion for later retrieval by the provider. |
| `metadata` | `Map<String, String>?` | `{}` | Developer-defined tags attached to the completion. |
| `prediction` | `Any?` | `null` | Predicted output, for latency reduction when much of the response is known ahead of time. Untyped: the shape is provider-defined and still evolving, and the value is forwarded verbatim. |
| `audio` | `Any?` | `null` | Audio output parameters, required when `modalities` includes `audio`. Untyped for the same reason as `prediction`. |
| `webSearchOptions` | `Any?` | `null` | Web-search tool configuration for search-enabled models. Untyped for the same reason as `prediction`. |
...
| `logprobs` | `Any?` | `null` | Per-token log probabilities, when the request asked for them. Required by OpenAI's response schema as an always-present, nullable key, so this is deliberately not `skip_serializing_if`. |
...
| `endpoint` | `String` | — | API endpoint (e.g., `"/v1/chat/completions"`). |
| `completionWindow` | `String` | — | Completion window (e.g., `"24h"`). |
| `metadata` | `Any?` | `null` | Optional metadata to attach to the batch. |
...
| `input` | `Any` | — | Input data to process (e.g., a document to extract from). |
...
| `data` | `ByteArray` | — | Decoded base64 payload. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `maxInFlight` | `Long?` | `null` | Maximum simultaneously outstanding provider requests. `null` means unlimited. |
...
| `schema` | `Any` | — | JSON Schema object defining the output structure. |
| `strict` | `Boolean?` | `null` | If true, enforce strict schema validation. |
...
| `timeoutSecs` | `Long?` | `null` | Request timeout, in seconds. |
| `maxRetries` | `Int?` | `null` | Maximum number of retries on 429 / 5xx responses. |
| `temperature` | `Double?` | `null` | Sampling temperature for requests built from this config. |
| `maxTokens` | `Long?` | `null` | Maximum number of tokens to generate for requests built from this config. |
| `loadEnv` | `Boolean?` | `null` | Automatically load the API key from the provider's environment variable when no explicit key is provided (default: `true`). |
| `headers` | `Map<String, String>?` | `{}` | Extra headers sent on every request. |
| `providers` | `List<LlmProviderConfig>?` | `[]` | Custom provider configurations, in addition to the built-in providers. |
...
| `costTracking` | `Boolean?` | `null` | Enable per-request cost tracking. |
| `tracing` | `Boolean?` | `null` | Enable OpenTelemetry-compatible tracing spans. |
| `cooldownSecs` | `Long?` | `null` | Cooldown duration after transient errors, in seconds. |
| `healthCheckSecs` | `Long?` | `null` | Background health check interval, in seconds. |
...
| `maxTokens` | `Long?` | `null` | Total context window size in tokens (input + output). |
| `maxInputTokens` | `Long?` | `null` | Maximum input (prompt) tokens accepted. |
| `maxOutputTokens` | `Long?` | `null` | Maximum output (completion) tokens the model can generate. |
| `mode` | `String?` | `null` | Best-effort operating mode, e.g. `"chat"`, `"embedding"`. |
| `supportsVision` | `Boolean?` | `null` | The model accepts image input. |
| `supportsFunctionCalling` | `Boolean?` | `null` | The model supports tool / function calling. |
| `supportsReasoning` | `Boolean?` | `null` | The model supports extended-thinking / reasoning tokens. |
| `supportsStructuredOutput` | `Boolean?` | `null` | The model supports JSON-mode or `response_format` structured output. |
| `supportsAudioInput` | `Boolean?` | `null` | The model accepts audio input. |
| `supportsAudioOutput` | `Boolean?` | `null` | The model can generate audio output. |
| `supportsPromptCaching` | `Boolean?` | `null` | The model supports prompt caching. |
...
| `includeImageBase64` | `Boolean?` | `null` | Whether to include base64-encoded images of each processed page. |
...
| `returnDocuments` | `Boolean?` | `null` | Include the document content in results. Defaults to false. |
...
| `error` | `Any?` | `null` | Error details (if status is "failed"). |
...
| `content` | `Any` | — | Output content (flattened into the object). |
...
| `config` | `Any` | — | Tool configuration (flattened into the object). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `includeUsage` | `Boolean?` | `null` | If true, include token usage in the final stream chunk. |
