...
<!-- alef:hash:b7686d76147792581b04ed5106874ba165a70cf685923b02c5426dca1b500014 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Elixir API Reference <span class="version-badge">v1.18.3</span>
...
```elixir
@spec record_cost_usd(system, model, operation, cost_usd) :: {:ok, term()} | {:error, term()}
def record_cost_usd(system, model, operation, cost_usd)
```

**Example:**

```elixir
:ok = record_cost_usd("value", "value", "value", 0.5)
```
...
| `refusal` | `String.t() \| nil` | `nil` | Refusal reason, if the model declined to respond per safety policies. OpenAI's response schema requires this key to be present even when null, so it is deliberately not `skip_serializing_if`. |
...
| `source_url` | `String.t()` | `"<https://github.com/xberg-io/liter-llm/releases/download/model-catalog/catalog.json"`> | Source URL to fetch `catalog.json` from. Must be `https`. Defaults to `DEFAULT_CATALOG_URL`; configurable so self-hosted mirrors work. |
...
| `response_format` | `ResponseFormat \| nil` | `:text` | Output format constraint (text, JSON, JSON schema). |
...
| `reasoning_effort` | `ReasoningEffort \| nil` | `:medium` | Reasoning effort level (minimal, low, medium, high, max) for extended-thinking models. |
...
| `logprobs` | `boolean() \| nil` | `nil` | Whether to return log probabilities of the output tokens. |
| `top_logprobs` | `integer() \| nil` | `nil` | Number of most-likely tokens to return log probabilities for, `0..=20`. Requires `logprobs` to be `true`. |
| `max_completion_tokens` | `integer() \| nil` | `nil` | Upper bound on generated tokens, including reasoning tokens. Supersedes `max_tokens` on OpenAI reasoning models, which reject `max_tokens` outright. |
| `service_tier` | `String.t() \| nil` | `nil` | Latency tier to process the request under (e.g. `"auto"`, `"default"`, `"flex"`). |
| `store` | `boolean() \| nil` | `nil` | Whether to store the completion for later retrieval by the provider. |
| `metadata` | `map() \| nil` | `%{}` | Developer-defined tags attached to the completion. |
| `prediction` | `term() \| nil` | `nil` | Predicted output, for latency reduction when much of the response is known ahead of time. Untyped: the shape is provider-defined and still evolving, and the value is forwarded verbatim. |
| `audio` | `term() \| nil` | `nil` | Audio output parameters, required when `modalities` includes `audio`. Untyped for the same reason as `prediction`. |
| `web_search_options` | `term() \| nil` | `nil` | Web-search tool configuration for search-enabled models. Untyped for the same reason as `prediction`. |
...
| `finish_reason` | `FinishReason \| nil` | `:stop` | Why the model stopped generating (stop, length, tool_calls, content_filter, etc.). |
| `logprobs` | `term() \| nil` | `nil` | Per-token log probabilities, when the request asked for them. Required by OpenAI's response schema as an always-present, nullable key, so this is deliberately not `skip_serializing_if`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `model` | `String.t()` | — | Model ID, as named by the OpenAI Responses API (e.g. `"gpt-5"`). Sent to the wire exactly as given. The Responses path performs none of the chat path's model handling: a `provider/model` routing prefix is **not** stripped and does **not** re-route the request, which stays pinned to the provider the client was constructed with. |
...
```elixir
def fetch_batch_for_polling(obj, batch_id)
```
...
```elixir
def wait_for_batch(obj, batch_id, config)
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `max_in_flight` | `integer() \| nil` | `nil` | Maximum simultaneously outstanding provider requests. `nil` means unlimited. |
...
| `in_flight_limit` | `LlmInFlightLimitConfig \| nil` | `nil` | Global per-client in-flight provider request limit. |
...
| `finish_reason` | `FinishReason \| nil` | `:stop` | Why the stream ended (present only in final chunk). |
...
| `call_type` | `ToolType \| nil` | `:function` | Tool type (typically "function"). |
...
| `image_base64` | Image encoded as a complete data URL. — Fields: `image_base64`: `String.t()` |
