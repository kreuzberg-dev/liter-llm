...
<!-- alef:hash:7bf6e599d45d9d2918eda40799db0d5887376921a9d39d5f55a076ef04803c0f -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## WebAssembly API Reference <span class="version-badge">v1.18.4</span>
...
```typescript
const result = encodeDataUrl(new Uint8Array([100, 97, 116, 97]), "value");
```
...
```typescript
function modelInfo(model: string): ModelInfo | null
```

**Example:**

```typescript
const result = modelInfo("value");
```
...
```typescript
function installCatalogOverlayFromStr(catalogJson: string): void
```
...
```typescript
function clearCatalogOverlay(): void
```
...
```typescript
function refreshCatalog(config: CatalogRefreshConfig): Promise<RefreshOutcome>
```

**Example:**

```typescript
const result = await refreshCatalog(new CatalogRefreshConfig());
```
...
| `refusal` | `string \| null` | `null` | Refusal reason, if the model declined to respond per safety policies. OpenAI's response schema requires this key to be present even when null, so it is deliberately not `skip_serializing_if`. |
| `functionCall` | `FunctionCall \| null` | `null` | Deprecated legacy function_call field; retained for API compatibility. |
| `reasoningContent` | `string \| null` | `null` | Reasoning/thinking tokens returned by the provider, if any (e.g. DeepSeek R1, Qwen `reasoning_content`, or Anthropic extended thinking). |
...
| `accessKeyId` | `string \| null` | `null` | Explicit AWS access key ID. |
| `secretAccessKey` | `string \| null` | `null` | Explicit AWS secret access key. |
| `sessionToken` | `string \| null` | `null` | Explicit AWS session token (temporary credentials). |
...
| `ttlSeconds` | `number` | `86400` | How long a cached `catalog.json` remains valid before a network refetch is attempted, in seconds. |
| `cachePath` | `string \| null` | `null` | Filesystem path for the on-disk cache. `null` uses a default path under `std.env.temp_dir()`. |
...
| `responseFormat` | `ResponseFormat \| null` | `ResponseFormat.Text` | Output format constraint (text, JSON, JSON schema). |
...
| `reasoningEffort` | `ReasoningEffort \| null` | `ReasoningEffort.Medium` | Reasoning effort level (minimal, low, medium, high, max) for extended-thinking models. |
...
| `logprobs` | `boolean \| null` | `null` | Whether to return log probabilities of the output tokens. |
| `topLogprobs` | `number \| null` | `null` | Number of most-likely tokens to return log probabilities for, `0..=20`. Requires `logprobs` to be `true`. |
| `maxCompletionTokens` | `number \| null` | `null` | Upper bound on generated tokens, including reasoning tokens. Supersedes `max_tokens` on OpenAI reasoning models, which reject `max_tokens` outright. |
| `serviceTier` | `string \| null` | `null` | Latency tier to process the request under (e.g. `"auto"`, `"default"`, `"flex"`). |
| `store` | `boolean \| null` | `null` | Whether to store the completion for later retrieval by the provider. |
| `metadata` | `Record<string, string> \| null` | `{}` | Developer-defined tags attached to the completion. |
| `prediction` | `unknown \| null` | `null` | Predicted output, for latency reduction when much of the response is known ahead of time. Untyped: the shape is provider-defined and still evolving, and the value is forwarded verbatim. |
| `audio` | `unknown \| null` | `null` | Audio output parameters, required when `modalities` includes `audio`. Untyped for the same reason as `prediction`. |
| `webSearchOptions` | `unknown \| null` | `null` | Web-search tool configuration for search-enabled models. Untyped for the same reason as `prediction`. |
...
| `finishReason` | `FinishReason \| null` | `FinishReason.Stop` | Why the model stopped generating (stop, length, tool_calls, content_filter, etc.). |
| `logprobs` | `unknown \| null` | `null` | Per-token log probabilities, when the request asked for them. Required by OpenAI's response schema as an always-present, nullable key, so this is deliberately not `skip_serializing_if`. |
...
| `embedding` | `Array<number>` | — | The embedding vector. Providers may return this as a JSON float array or, when `encoding_format: "base64"` was requested, as a base64 string of little-endian `f32` bytes. Base64 responses are decoded on read; this field always serializes back out as a JSON float array. |
...
| `apiKey` | `string \| null` | `null` | API key for authentication. |
| `baseUrl` | `string \| null` | `null` | Override base URL. When set, all requests go here and provider auto-detection is skipped. |
| `timeoutSecs` | `number \| null` | `null` | Request timeout, in seconds. |
| `maxRetries` | `number \| null` | `null` | Maximum number of retries on 429 / 5xx responses. |
| `temperature` | `number \| null` | `null` | Sampling temperature for requests built from this config. |
| `maxTokens` | `number \| null` | `null` | Maximum number of tokens to generate for requests built from this config. |
| `loadEnv` | `boolean \| null` | `null` | Automatically load the API key from the provider's environment variable when no explicit key is provided (default: `true`). |
| `headers` | `Record<string, string> \| null` | `{}` | Extra headers sent on every request. |
| `providers` | `Array<LlmProviderConfig> \| null` | `[]` | Custom provider configurations, in addition to the built-in providers. |
| `cache` | `LlmCacheConfig \| null` | `null` | Response cache configuration. |
| `budget` | `LlmBudgetConfig \| null` | `null` | Budget enforcement configuration. |
| `rateLimit` | `LlmRateLimitConfig \| null` | `null` | Per-model rate limiting configuration. |
| `inFlightLimit` | `LlmInFlightLimitConfig \| null` | `null` | Global per-client in-flight provider request limit. |
| `costTracking` | `boolean \| null` | `null` | Enable per-request cost tracking. |
| `tracing` | `boolean \| null` | `null` | Enable OpenTelemetry-compatible tracing spans. |
| `cooldownSecs` | `number \| null` | `null` | Cooldown duration after transient errors, in seconds. |
| `healthCheckSecs` | `number \| null` | `null` | Background health check interval, in seconds. |
| `bedrock` | `BedrockConfig \| null` | `null` | AWS Bedrock configuration (region, credentials, cross-region routing). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `maxInFlight` | `number \| null` | `null` | Maximum simultaneously outstanding provider requests. `null` means unlimited. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `inputCostPerToken` | `number` | — | Cost in USD per input (prompt) token. |
| `outputCostPerToken` | `number` | — | Cost in USD per output (completion) token. |
| `cacheReadInputTokenCost` | `number \| null` | `null` | Cost in USD per cached input token (cache hit / read). |
| `cacheCreationInputTokenCost` | `number \| null` | `null` | Cost in USD per token written to the prompt cache. |
| `inputCostPerAudioToken` | `number \| null` | `null` | Cost in USD per input audio token. |
| `outputCostPerAudioToken` | `number \| null` | `null` | Cost in USD per output audio token. |
| `outputCostPerReasoningToken` | `number \| null` | `null` | Cost in USD per reasoning (extended-thinking) output token. |
| `maxTokens` | `number \| null` | `null` | Total context window size in tokens (input + output). |
| `maxInputTokens` | `number \| null` | `null` | Maximum input (prompt) tokens accepted. |
| `maxOutputTokens` | `number \| null` | `null` | Maximum output (completion) tokens the model can generate. |
| `mode` | `string \| null` | `null` | Best-effort operating mode, e.g. `"chat"`, `"embedding"`. |
| `supportsVision` | `boolean \| null` | `null` | The model accepts image input. |
| `supportsFunctionCalling` | `boolean \| null` | `null` | The model supports tool / function calling. |
| `supportsReasoning` | `boolean \| null` | `null` | The model supports extended-thinking / reasoning tokens. |
| `supportsStructuredOutput` | `boolean \| null` | `null` | The model supports JSON-mode or `response_format` structured output. |
| `supportsAudioInput` | `boolean \| null` | `null` | The model accepts audio input. |
| `supportsAudioOutput` | `boolean \| null` | `null` | The model can generate audio output. |
| `supportsPromptCaching` | `boolean \| null` | `null` | The model supports prompt caching. |
| `tiers` | `Array<ModelTier>` | `[]` | Context-tiered pricing overrides, sorted by ascending `min_context_tokens`. Empty when the model has flat pricing. |
...
| `created` | `number` | — | Unix timestamp of model creation (or release date). Defaults to `0` when a provider omits it — DeepSeek and some other OpenAI-compatible providers do not return `created` from `/v1/models`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `minContextTokens` | `number` | — | The tier applies when the prompt/context token count is at least this value. |
| `inputCostPerToken` | `number` | — | Cost in USD per input (prompt) token within this tier. |
| `outputCostPerToken` | `number` | — | Cost in USD per output (completion) token within this tier. |
| `cacheReadInputTokenCost` | `number \| null` | `null` | Cost in USD per cached input token within this tier. |
| `cacheCreationInputTokenCost` | `number \| null` | `null` | Cost in USD per cache-write token within this tier. |
| `inputCostPerAudioToken` | `number \| null` | `null` | Cost in USD per input audio token within this tier. |
| `outputCostPerAudioToken` | `number \| null` | `null` | Cost in USD per output audio token within this tier. |
| `outputCostPerReasoningToken` | `number \| null` | `null` | Cost in USD per reasoning output token within this tier. |
...
| `finishReason` | `FinishReason \| null` | `FinishReason.Stop` | Why the stream ended (present only in final chunk). |
...
| `reasoningContent` | `string \| null` | `null` | Partial reasoning/thinking tokens (OpenAI-compatible extension used by DeepSeek R1, Qwen, etc.). |
...
| `callType` | `ToolType \| null` | `ToolType.Function` | Tool type (typically "function"). |
...
This type has no dedicated class in the WASM binding. It is expressed as the following structural TypeScript type:
