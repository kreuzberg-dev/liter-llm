...
<!-- alef:hash:6e9f003ddcfc197c57f96c895e95823837c17ba42942c5f1fe7cab43a5e3f79d -->
...
| `ResponseFormat` | `*ResponseFormat` | `ResponseFormat.Text` | Output format constraint (text, JSON, JSON schema). |
...
| `ReasoningEffort` | `*ReasoningEffort` | `ReasoningEffort.Medium` | Reasoning effort level (minimal, low, medium, high, max) for extended-thinking models. |
...
| `FinishReason` | `*FinishReason` | `FinishReason.Stop` | Why the model stopped generating (stop, length, tool_calls, content_filter, etc.). |
...
| `FinishReason` | `*FinishReason` | `FinishReason.Stop` | Why the stream ended (present only in final chunk). |
...
| `CallType` | `*ToolType` | `ToolType.Function` | Tool type (typically "function"). |
