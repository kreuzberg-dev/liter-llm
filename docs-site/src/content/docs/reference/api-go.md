...
<!-- alef:hash:91ab63c0a1db00cc503bbe4acd2eea96da986dd11b14a3b3bb1fde911c4b6296 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Go API Reference <span class="version-badge">v1.19.2</span>
...
Returns `nil` if the model is not present in the active pricing
...
```go
func ModelInfo(model string) *ModelInfo
```

**Example:**

```go
result := ModelInfo("value")
```
...
```go
func RecordCostUsd(system string, model string, operation string, costUsd float64)
```
...
| `CurrentLen` | `uint` | Yes | The current len |
| `Incoming` | `uint` | Yes | The incoming |
| `Limit` | `uint` | Yes | The limit |
...
```go
if err := InstallCatalogOverlayFromStr("value"); err != nil {
    return err
}
```
...
```go
result, err := RefreshCatalog(CatalogRefreshConfig{})
if err != nil {
    return err
}
```
...
| `Refusal` | `*string` | `nil` | Refusal reason, if the model declined to respond per safety policies. OpenAI's response schema requires this key to be present even when null, so it is deliberately not `skip_serializing_if`. |
| `FunctionCall` | `*FunctionCall` | `nil` | Deprecated legacy function_call field; retained for API compatibility. |
| `ReasoningContent` | `*string` | `nil` | Reasoning/thinking tokens returned by the provider, if any (e.g. DeepSeek R1, Qwen `reasoning_content`, or Anthropic extended thinking). |
...
```go
func (o *AssistantMessage) ReasoningText() *string
```

**Example:**

```go
result := instance.ReasoningText()
```
...
| `Status` | `BatchStatus` | `BatchStatusValidating` | Current job status. |
...
| `AccessKeyId` | `*string` | `nil` | Explicit AWS access key ID. |
| `SecretAccessKey` | `*string` | `nil` | Explicit AWS secret access key. |
| `SessionToken` | `*string` | `nil` | Explicit AWS session token (temporary credentials). |
...
| `Enforcement` | `Enforcement` | `EnforcementHard` | Whether to reject requests or merely warn when a limit is exceeded. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `MaxEntries` | `uint` | `256` | Maximum number of cached entries. |
| `Ttl` | `time.Duration` | `300000ms` | Time-to-live for each cached entry. |
| `Backend` | `CacheBackend` | `CacheBackendMemory` | Storage backend to use. |
...
| `TtlSeconds` | `uint64` | `86400` | How long a cached `catalog.json` remains valid before a network refetch is attempted, in seconds. |
| `CachePath` | `*string` | `nil` | Filesystem path for the on-disk cache. `nil` uses a default path under `std.env.temp_dir()`. |
...
| `ReasoningEffort` | `*ReasoningEffort` | `nil` | Reasoning effort level (minimal, low, medium, high, max) for extended-thinking models. |
...
| `Logprobs` | `*bool` | `nil` | Whether to return log probabilities of the output tokens. |
| `TopLogprobs` | `*uint32` | `nil` | Number of most-likely tokens to return log probabilities for, `0..=20`. Requires `logprobs` to be `true`. |
| `MaxCompletionTokens` | `*uint64` | `nil` | Upper bound on generated tokens, including reasoning tokens. Supersedes `max_tokens` on OpenAI reasoning models, which reject `max_tokens` outright. |
| `ServiceTier` | `*string` | `nil` | Latency tier to process the request under (e.g. `"auto"`, `"default"`, `"flex"`). |
| `Store` | `*bool` | `nil` | Whether to store the completion for later retrieval by the provider. |
| `Metadata` | `*map\[string\]string` | `nil` | Developer-defined tags attached to the completion. |
| `Prediction` | `*interface{}` | `nil` | Predicted output, for latency reduction when much of the response is known ahead of time. Untyped: the shape is provider-defined and still evolving, and the value is forwarded verbatim. |
| `Audio` | `*interface{}` | `nil` | Audio output parameters, required when `modalities` includes `audio`. Untyped for the same reason as `prediction`. |
| `WebSearchOptions` | `*interface{}` | `nil` | Web-search tool configuration for search-enabled models. Untyped for the same reason as `prediction`. |
...
| `Logprobs` | `*interface{}` | `nil` | Per-token log probabilities, when the request asked for them. Required by OpenAI's response schema as an always-present, nullable key, so this is deliberately not `skip_serializing_if`. |
...
| `Purpose` | `FilePurpose` | `FilePurposeAssistants` | Purpose for the file. |
...
| `Embedding` | `\[\]float32` | — | The embedding vector. Providers may return this as a JSON float array or, when `encoding_format: "base64"` was requested, as a base64 string of little-endian `f32` bytes. Base64 responses are decoded on read; this field always serializes back out as a JSON float array. |
...
| `Input` | `EmbeddingInput` | `EmbeddingInputSingle` | Text, texts, or multimodal content to embed. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `MaxInFlight` | `*uint` | `nil` | Maximum simultaneously outstanding provider requests. `nil` means unlimited. |
...
| `ApiKey` | `*string` | `nil` | API key for authentication. |
| `BaseUrl` | `*string` | `nil` | Override base URL. When set, all requests go here and provider auto-detection is skipped. |
| `TimeoutSecs` | `*uint64` | `nil` | Request timeout, in seconds. |
| `MaxRetries` | `*uint32` | `nil` | Maximum number of retries on 429 / 5xx responses. |
| `Temperature` | `*float64` | `nil` | Sampling temperature for requests built from this config. |
| `MaxTokens` | `*uint64` | `nil` | Maximum number of tokens to generate for requests built from this config. |
| `LoadEnv` | `*bool` | `nil` | Automatically load the API key from the provider's environment variable when no explicit key is provided (default: `true`). |
| `Headers` | `*map\[string\]string` | `nil` | Extra headers sent on every request. |
| `Providers` | `*\[\]LlmProviderConfig` | `nil` | Custom provider configurations, in addition to the built-in providers. |
| `Cache` | `*LlmCacheConfig` | `nil` | Response cache configuration. |
| `Budget` | `*LlmBudgetConfig` | `nil` | Budget enforcement configuration. |
| `RateLimit` | `*LlmRateLimitConfig` | `nil` | Per-model rate limiting configuration. |
| `InFlightLimit` | `*LlmInFlightLimitConfig` | `nil` | Global per-client in-flight provider request limit. |
| `CostTracking` | `*bool` | `nil` | Enable per-request cost tracking. |
| `Tracing` | `*bool` | `nil` | Enable OpenTelemetry-compatible tracing spans. |
| `CooldownSecs` | `*uint64` | `nil` | Cooldown duration after transient errors, in seconds. |
| `HealthCheckSecs` | `*uint64` | `nil` | Background health check interval, in seconds. |
| `Bedrock` | `*BedrockConfig` | `nil` | AWS Bedrock configuration (region, credentials, cross-region routing). |
...
```go
func (o *LlmConfig) Providers() []LlmProviderConfig
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `InputCostPerToken` | `float64` | — | Cost in USD per input (prompt) token. |
| `OutputCostPerToken` | `float64` | — | Cost in USD per output (completion) token. |
| `CacheReadInputTokenCost` | `*float64` | `nil` | Cost in USD per cached input token (cache hit / read). |
| `CacheCreationInputTokenCost` | `*float64` | `nil` | Cost in USD per token written to the prompt cache. |
| `InputCostPerAudioToken` | `*float64` | `nil` | Cost in USD per input audio token. |
| `OutputCostPerAudioToken` | `*float64` | `nil` | Cost in USD per output audio token. |
| `OutputCostPerReasoningToken` | `*float64` | `nil` | Cost in USD per reasoning (extended-thinking) output token. |
| `MaxTokens` | `*uint64` | `nil` | Total context window size in tokens (input + output). |
| `MaxInputTokens` | `*uint64` | `nil` | Maximum input (prompt) tokens accepted. |
| `MaxOutputTokens` | `*uint64` | `nil` | Maximum output (completion) tokens the model can generate. |
| `Mode` | `*string` | `nil` | Best-effort operating mode, e.g. `"chat"`, `"embedding"`. |
| `SupportsVision` | `*bool` | `nil` | The model accepts image input. |
| `SupportsFunctionCalling` | `*bool` | `nil` | The model supports tool / function calling. |
| `SupportsReasoning` | `*bool` | `nil` | The model supports extended-thinking / reasoning tokens. |
| `SupportsStructuredOutput` | `*bool` | `nil` | The model supports JSON-mode or `response_format` structured output. |
| `SupportsAudioInput` | `*bool` | `nil` | The model accepts audio input. |
| `SupportsAudioOutput` | `*bool` | `nil` | The model can generate audio output. |
| `SupportsPromptCaching` | `*bool` | `nil` | The model supports prompt caching. |
| `Tiers` | `\[\]ModelTier` | `nil` | Context-tiered pricing overrides, sorted by ascending `min_context_tokens`. Empty when the model has flat pricing. |
...
| `Created` | `uint64` | — | Unix timestamp of model creation (or release date). Defaults to `0` when a provider omits it — DeepSeek and some other OpenAI-compatible providers do not return `created` from `/v1/models`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `MinContextTokens` | `uint64` | — | The tier applies when the prompt/context token count is at least this value. |
| `InputCostPerToken` | `float64` | — | Cost in USD per input (prompt) token within this tier. |
| `OutputCostPerToken` | `float64` | — | Cost in USD per output (completion) token within this tier. |
| `CacheReadInputTokenCost` | `*float64` | `nil` | Cost in USD per cached input token within this tier. |
| `CacheCreationInputTokenCost` | `*float64` | `nil` | Cost in USD per cache-write token within this tier. |
| `InputCostPerAudioToken` | `*float64` | `nil` | Cost in USD per input audio token within this tier. |
| `OutputCostPerAudioToken` | `*float64` | `nil` | Cost in USD per output audio token within this tier. |
| `OutputCostPerReasoningToken` | `*float64` | `nil` | Cost in USD per reasoning output token within this tier. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `Input` | `ModerationInput` | `ModerationInputSingle` | Text or texts to check. |
...
| `Document` | `OcrDocument` | `OcrDocumentURL` | The document to process (URL or base64). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `ChoiceType` | `ToolType` | `ToolTypeFunction` | Tool type (always "function"). |
...
| `ReasoningContent` | `*string` | `nil` | Partial reasoning/thinking tokens (OpenAI-compatible extension used by DeepSeek R1, Qwen, etc.). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `Content` | `UserContent` | `UserContentText` | Instructions or context that apply throughout the conversation. Accepts either a plain text string or an array of content parts, mirroring `UserContent` so that `Message.system_with_parts` works. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `Content` | `UserContent` | `UserContentText` | Message content as plain text or array of content parts (text, images, documents, audio). |
...
| `Multimodal` | Text and image parts for a single multimodal embedding. — Fields: `0`: `[]EmbeddingContentPart` |
...
|-------|-------------|
| `Text` | Plain text. — Fields: `Text`: `string` |
| `ImageUrl` | Image identified by a data URL or HTTP/HTTPS URL. — Fields: `ImageUrl`: `ImageUrl` |
| `ImageBase64` | Image encoded as a complete data URL. — Fields: `ImageBase64`: `string` |
...
| Variant | Description |
|---------|-------------|
| `ErrAuthentication` | `status` preserves the exact HTTP status code received (401 or 403). |
| `ErrRateLimited` | rate limited: {message} |
| `ErrBadRequest` | `status` preserves the exact HTTP status code received (400, 405, 413, 422, …). |
| `ErrContextWindowExceeded` | context window exceeded: {message} |
| `ErrContentPolicy` | content policy violation: {message} |
| `ErrNotFound` | not found: {message} |
| `ErrServerError` | `status` preserves the exact HTTP status code received (500, or other 5xx not covered by `ServiceUnavailable`). |
| `ErrServiceUnavailable` | `status` preserves the exact HTTP status code received (502, 503, or 504). |
| `ErrTimeout` | request timeout |
| `ErrStreaming` | A catch-all for errors that occur during streaming response processing. This variant covers multiple sub-conditions including UTF-8 decoding failures, CRC/checksum mismatches (AWS EventStream), JSON parse errors in individual SSE chunks, and buffer overflow conditions.  The `message` field contains a human-readable description of the specific failure. |
| `ErrEndpointNotSupported` | provider {provider} does not support {endpoint} |
| `ErrInvalidHeader` | invalid header {name:?}: {reason} |
| `ErrSerialization` | serialization error: {0} |
| `ErrBudgetExceeded` | budget exceeded: {message} |
| `ErrHookRejected` | hook rejected: {message} |
| `ErrInternalError` | An internal logic error (e.g. unexpected Tower response variant). This should never surface in normal operation — if it does, it indicates a bug in the library. |
| `ErrOutboundForbidden` | An outbound request was blocked by the active `OutboundPolicy`. Returned when `register_custom_provider` is called with a `base_url` that violates the policy (e.g. a private-range IP under `DenyPrivate`), or when the per-connection DNS resolver detects a forbidden address at connect time. |
| `ErrIdempotencyConflict` | A different request body was submitted for an existing `Idempotency-Key`. Per the OpenAI `Idempotency-Key` convention, once a key is used with a particular request body, subsequent requests using the same key must carry an identical body.  A body mismatch is a hard error (not retryable). HTTP equivalent: 409 Conflict. |
| `ErrIdempotencyInFlight` | The same `Idempotency-Key` is already in-flight (another request with the same key is currently being processed). The caller should wait briefly and retry.  The response is not yet available, and this request has been short-circuited to avoid running the operation twice. HTTP equivalent: 409 Conflict (retryable after a brief delay). |
...
| Variant | Description |
|---------|-------------|
| `ErrDisabled` | Runtime catalog refresh was not enabled. `refresh_catalog` itself never returns this — it returns `Ok(RefreshOutcome.Disabled)` instead — but the variant is part of the public error surface for callers that want to treat "disabled" as a hard error. |
| `ErrInsecureUrl` | `source_url` did not use the `https` scheme, or failed to parse as a URL at all. There is no host allowlist: the URL is user-configurable for self-hosted catalog mirrors, so only the scheme is enforced. |
| `ErrFetch` | The network fetch failed, timed out, or returned a non-success status. |
| `ErrParse` | The fetched or cached catalog JSON failed to parse. |
| `ErrCache` | A cache file **read** failed on an otherwise-fresh cache file. Cache **write** failures are best-effort and never surface as this error (see `refresh_catalog`). |
