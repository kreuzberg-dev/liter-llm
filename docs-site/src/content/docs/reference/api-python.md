...
<!-- alef:hash:8b02e5ec1b03ee0283117be607095591ce29387cef117ee51b398c7979e8f3fc -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Python API Reference <span class="version-badge">v1.18.4</span>
...
| `refusal` | `str \| None` | `None` | Refusal reason, if the model declined to respond per safety policies. OpenAI's response schema requires this key to be present even when null, so it is deliberately not `skip_serializing_if`. |
...
| `source_url` | `str` | `"<https://github.com/xberg-io/liter-llm/releases/download/model-catalog/catalog.json"`> | Source URL to fetch `catalog.json` from. Must be `https`. Defaults to `DEFAULT_CATALOG_URL`; configurable so self-hosted mirrors work. |
...
| `response_format` | `ResponseFormat \| None` | `ResponseFormat.TEXT` | Output format constraint (text, JSON, JSON schema). |
...
| `reasoning_effort` | `ReasoningEffort \| None` | `ReasoningEffort.MEDIUM` | Reasoning effort level (minimal, low, medium, high, max) for extended-thinking models. |
...
| `logprobs` | `bool \| None` | `None` | Whether to return log probabilities of the output tokens. |
| `top_logprobs` | `int \| None` | `None` | Number of most-likely tokens to return log probabilities for, `0..=20`. Requires `logprobs` to be `True`. |
| `max_completion_tokens` | `int \| None` | `None` | Upper bound on generated tokens, including reasoning tokens. Supersedes `max_tokens` on OpenAI reasoning models, which reject `max_tokens` outright. |
| `service_tier` | `str \| None` | `None` | Latency tier to process the request under (e.g. `"auto"`, `"default"`, `"flex"`). |
| `store` | `bool \| None` | `None` | Whether to store the completion for later retrieval by the provider. |
| `metadata` | `dict\[str, str\] \| None` | `{}` | Developer-defined tags attached to the completion. |
| `prediction` | `dict\[str, Any\] \| None` | `None` | Predicted output, for latency reduction when much of the response is known ahead of time. Untyped: the shape is provider-defined and still evolving, and the value is forwarded verbatim. |
| `audio` | `dict\[str, Any\] \| None` | `None` | Audio output parameters, required when `modalities` includes `audio`. Untyped for the same reason as `prediction`. |
| `web_search_options` | `dict\[str, Any\] \| None` | `None` | Web-search tool configuration for search-enabled models. Untyped for the same reason as `prediction`. |
...
| `finish_reason` | `FinishReason \| None` | `FinishReason.STOP` | Why the model stopped generating (stop, length, tool_calls, content_filter, etc.). |
| `logprobs` | `dict\[str, Any\] \| None` | `None` | Per-token log probabilities, when the request asked for them. Required by OpenAI's response schema as an always-present, nullable key, so this is deliberately not `skip_serializing_if`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `max_in_flight` | `int \| None` | `None` | Maximum simultaneously outstanding provider requests. `None` means unlimited. |
...
| `in_flight_limit` | `LlmInFlightLimitConfig \| None` | `None` | Global per-client in-flight provider request limit. |
...
| `finish_reason` | `FinishReason \| None` | `FinishReason.STOP` | Why the stream ended (present only in final chunk). |
...
| `call_type` | `ToolType \| None` | `ToolType.FUNCTION` | Tool type (typically "function"). |
...
| `IMAGE_BASE64` | Image encoded as a complete data URL. — Fields: `image_base64`: `str` |
...
|-----------|-------------|
| `AuthenticationError(LiterLlmError)` | `status` preserves the exact HTTP status code received (401 or 403). |
| `RateLimitedError(LiterLlmError)` | rate limited: {message} |
| `BadRequestError(LiterLlmError)` | `status` preserves the exact HTTP status code received (400, 405, 413, 422, …). |
| `ContextWindowExceededError(LiterLlmError)` | context window exceeded: {message} |
| `ContentPolicyError(LiterLlmError)` | content policy violation: {message} |
| `NotFoundError(LiterLlmError)` | not found: {message} |
...
| `ServiceUnavailableError(LiterLlmError)` | `status` preserves the exact HTTP status code received (502, 503, or 504). |
| `LiterLlmTimeoutError(LiterLlmError)` | request timeout |
| `StreamingError(LiterLlmError)` | A catch-all for errors that occur during streaming response processing. This variant covers multiple sub-conditions including UTF-8 decoding failures, CRC/checksum mismatches (AWS EventStream), JSON parse errors in individual SSE chunks, and buffer overflow conditions.  The `message` field contains a human-readable description of the specific failure. |
| `EndpointNotSupportedError(LiterLlmError)` | provider {provider} does not support {endpoint} |
| `InvalidHeaderError(LiterLlmError)` | invalid header {name:?}: {reason} |
| `SerializationError(LiterLlmError)` | serialization error: {0} |
| `BudgetExceededError(LiterLlmError)` | budget exceeded: {message} |
| `HookRejectedError(LiterLlmError)` | hook rejected: {message} |
...
| `OutboundForbiddenError(LiterLlmError)` | An outbound request was blocked by the active `OutboundPolicy`. Returned when `register_custom_provider` is called with a `base_url` that violates the policy (e.g. a private-range IP under `DenyPrivate`), or when the per-connection DNS resolver detects a forbidden address at connect time. |
| `IdempotencyConflictError(LiterLlmError)` | A different request body was submitted for an existing `Idempotency-Key`. Per the OpenAI `Idempotency-Key` convention, once a key is used with a particular request body, subsequent requests using the same key must carry an identical body.  A body mismatch is a hard error (not retryable). HTTP equivalent: 409 Conflict. |
| `IdempotencyInFlightError(LiterLlmError)` | The same `Idempotency-Key` is already in-flight (another request with the same key is currently being processed). The caller should wait briefly and retry.  The response is not yet available, and this request has been short-circuited to avoid running the operation twice. HTTP equivalent: 409 Conflict (retryable after a brief delay). |
...
| Exception | Description |
|-----------|-------------|
| `DisabledError(CatalogRefreshError)` | Runtime catalog refresh was not enabled. `refresh_catalog` itself never returns this — it returns `Ok(RefreshOutcome.Disabled)` instead — but the variant is part of the public error surface for callers that want to treat "disabled" as a hard error. |
| `InsecureUrlError(CatalogRefreshError)` | `source_url` did not use the `https` scheme, or failed to parse as a URL at all. There is no host allowlist: the URL is user-configurable for self-hosted catalog mirrors, so only the scheme is enforced. |
| `FetchError(CatalogRefreshError)` | The network fetch failed, timed out, or returned a non-success status. |
| `ParseError(CatalogRefreshError)` | The fetched or cached catalog JSON failed to parse. |
| `CacheError(CatalogRefreshError)` | A cache file **read** failed on an otherwise-fresh cache file. Cache **write** failures are best-effort and never surface as this error (see `refresh_catalog`). |
