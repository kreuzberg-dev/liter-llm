...
<!-- alef:hash:dd5e199482fc1696aa165f3f95aee154ce0e9627943967a5b24d0b7d3b3b89a5 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Rust API Reference <span class="version-badge">v1.18.3</span>
...
```rust
pub fn create_client(api_key: String, base_url: Option<String>, timeout_secs: Option<u64>, max_retries: Option<u32>, model_hint: Option<String>) -> Result<DefaultClient, LiterLlmError>
```
...
**Errors:** Returns `Err(LiterLlmError)`.
...
```rust
pub fn create_client_from_json(json: &str) -> Result<DefaultClient, LiterLlmError>
```
...
```rust
pub fn register_custom_provider(config: CustomProviderConfig) -> Result<(), LiterLlmError>
```
...
```rust
pub fn unregister_custom_provider(name: &str) -> Result<bool, LiterLlmError>
```
...
```rust
pub fn all_providers() -> Result<Vec<ProviderConfig>, LiterLlmError>
```
...
```rust
pub fn complex_provider_names() -> Result<Vec<String>, LiterLlmError>
```
...
```rust
pub fn count_tokens(model: &str, text: &str) -> Result<usize, LiterLlmError>
```
...
```rust
pub fn count_request_tokens(model: &str, req: &ChatCompletionRequest) -> Result<usize, LiterLlmError>
```
...
```rust
pub fn check_bound(context: &str, current_len: usize, incoming: usize, limit: usize) -> Result<(), LiterLlmError>
```
...
**Errors:** Returns `Err(CatalogRefreshError)`.
...
```rust
let result = refresh_catalog(&CatalogRefreshConfig::default()).await?;
```
...
| `refusal` | `Option<String>` | `Default::default()` | Refusal reason, if the model declined to respond per safety policies. OpenAI's response schema requires this key to be present even when null, so it is deliberately not `skip_serializing_if`. |
| `function_call` | `Option<FunctionCall>` | `Default::default()` | Deprecated legacy function_call field; retained for API compatibility. |
| `reasoning_content` | `Option<String>` | `Default::default()` | Reasoning/thinking tokens returned by the provider, if any (e.g. DeepSeek R1, Qwen `reasoning_content`, or Anthropic extended thinking). |
...
```rust
let result = instance.reasoning_text();
```
...
| `access_key_id` | `Option<String>` | `Default::default()` | Explicit AWS access key ID. |
| `secret_access_key` | `Option<String>` | `Default::default()` | Explicit AWS secret access key. |
| `session_token` | `Option<String>` | `Default::default()` | Explicit AWS session token (temporary credentials). |
...
| `source_url` | `String` | `"<https://github.com/xberg-io/liter-llm/releases/download/model-catalog/catalog.json"`> | Source URL to fetch `catalog.json` from. Must be `https`. Defaults to `DEFAULT_CATALOG_URL`; configurable so self-hosted mirrors work. |
...
```rust
let result = CatalogRefreshConfig::default();
```
...
| `response_format` | `Option<ResponseFormat>` | `ResponseFormat::Text` | Output format constraint (text, JSON, JSON schema). |
...
| `reasoning_effort` | `Option<ReasoningEffort>` | `ReasoningEffort::Medium` | Reasoning effort level (minimal, low, medium, high, max) for extended-thinking models. |
...
| `logprobs` | `Option<bool>` | `Default::default()` | Whether to return log probabilities of the output tokens. |
| `top_logprobs` | `Option<u32>` | `Default::default()` | Number of most-likely tokens to return log probabilities for, `0..=20`. Requires `logprobs` to be `true`. |
| `max_completion_tokens` | `Option<u64>` | `Default::default()` | Upper bound on generated tokens, including reasoning tokens. Supersedes `max_tokens` on OpenAI reasoning models, which reject `max_tokens` outright. |
| `service_tier` | `Option<String>` | `Default::default()` | Latency tier to process the request under (e.g. `"auto"`, `"default"`, `"flex"`). |
| `store` | `Option<bool>` | `Default::default()` | Whether to store the completion for later retrieval by the provider. |
| `metadata` | `Option<HashMap<String, String>>` | `HashMap::new()` | Developer-defined tags attached to the completion. |
| `prediction` | `Option<serde_json::Value>` | `Default::default()` | Predicted output, for latency reduction when much of the response is known ahead of time. Untyped: the shape is provider-defined and still evolving, and the value is forwarded verbatim. |
| `audio` | `Option<serde_json::Value>` | `Default::default()` | Audio output parameters, required when `modalities` includes `audio`. Untyped for the same reason as `prediction`. |
| `web_search_options` | `Option<serde_json::Value>` | `Default::default()` | Web-search tool configuration for search-enabled models. Untyped for the same reason as `prediction`. |
...
| `finish_reason` | `Option<FinishReason>` | `FinishReason::Stop` | Why the model stopped generating (stop, length, tool_calls, content_filter, etc.). |
| `logprobs` | `Option<serde_json::Value>` | `Default::default()` | Per-token log probabilities, when the request asked for them. Required by OpenAI's response schema as an always-present, nullable key, so this is deliberately not `skip_serializing_if`. |
...
```rust
pub fn process(&self, chunk: ChatCompletionChunk) -> Result<Option<ChatCompletionChunk>, LiterLlmError>
```
...
```rust
pub async fn chat(&self, req: ChatCompletionRequest) -> Result<ChatCompletionResponse, LiterLlmError>
```
...
```rust
pub async fn embed(&self, req: EmbeddingRequest) -> Result<EmbeddingResponse, LiterLlmError>
```
...
```rust
pub async fn list_models(&self) -> Result<ModelsListResponse, LiterLlmError>
```
...
```rust
pub async fn image_generate(&self, req: CreateImageRequest) -> Result<ImagesResponse, LiterLlmError>
```
...
```rust
pub async fn speech(&self, req: CreateSpeechRequest) -> Result<Vec<u8>, LiterLlmError>
```
...
```rust
pub async fn transcribe(&self, req: CreateTranscriptionRequest) -> Result<TranscriptionResponse, LiterLlmError>
```
...
```rust
pub async fn moderate(&self, req: ModerationRequest) -> Result<ModerationResponse, LiterLlmError>
```
...
```rust
pub async fn rerank(&self, req: RerankRequest) -> Result<RerankResponse, LiterLlmError>
```
...
```rust
pub async fn search(&self, req: SearchRequest) -> Result<SearchResponse, LiterLlmError>
```
...
```rust
pub async fn ocr(&self, req: OcrRequest) -> Result<OcrResponse, LiterLlmError>
```
...
```rust
pub async fn create_file(&self, req: CreateFileRequest) -> Result<FileObject, LiterLlmError>
```
...
```rust
pub async fn retrieve_file(&self, file_id: &str) -> Result<FileObject, LiterLlmError>
```
...
```rust
pub async fn delete_file(&self, file_id: &str) -> Result<DeleteResponse, LiterLlmError>
```
...
```rust
pub async fn list_files(&self, query: Option<FileListQuery>) -> Result<FileListResponse, LiterLlmError>
```
...
```rust
pub async fn file_content(&self, file_id: &str) -> Result<Vec<u8>, LiterLlmError>
```
...
```rust
pub async fn create_batch(&self, req: CreateBatchRequest) -> Result<BatchObject, LiterLlmError>
```
...
```rust
pub async fn retrieve_batch(&self, batch_id: &str) -> Result<BatchObject, LiterLlmError>
```
...
```rust
pub async fn list_batches(&self, query: Option<BatchListQuery>) -> Result<BatchListResponse, LiterLlmError>
```
...
```rust
pub async fn cancel_batch(&self, batch_id: &str) -> Result<BatchObject, LiterLlmError>
```
...
```rust
pub async fn fetch_batch_for_polling(&self, batch_id: &str) -> Result<BatchObject, LiterLlmError>
```
...
```rust
pub async fn create_response(&self, req: CreateResponseRequest) -> Result<ResponseObject, LiterLlmError>
```
...
```rust
pub async fn retrieve_response(&self, response_id: &str) -> Result<ResponseObject, LiterLlmError>
```
...
```rust
pub async fn cancel_response(&self, response_id: &str) -> Result<ResponseObject, LiterLlmError>
```
...
| `embedding` | `Vec<f32>` | — | The embedding vector. Providers may return this as a JSON float array or, when `encoding_format: "base64"` was requested, as a base64 string of little-endian `f32` bytes. Base64 responses are decoded on read; this field always serializes back out as a JSON float array. |
...
```rust
pub async fn check(&self, upstream: String) -> HealthStatus
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `max_in_flight` | `Option<usize>` | `Default::default()` | Maximum simultaneously outstanding provider requests. `None` means unlimited. |
...
| `api_key` | `Option<String>` | `Default::default()` | API key for authentication. |
| `base_url` | `Option<String>` | `Default::default()` | Override base URL. When set, all requests go here and provider auto-detection is skipped. |
| `timeout_secs` | `Option<u64>` | `Default::default()` | Request timeout, in seconds. |
| `max_retries` | `Option<u32>` | `Default::default()` | Maximum number of retries on 429 / 5xx responses. |
| `temperature` | `Option<f64>` | `Default::default()` | Sampling temperature for requests built from this config. |
| `max_tokens` | `Option<u64>` | `Default::default()` | Maximum number of tokens to generate for requests built from this config. |
| `load_env` | `Option<bool>` | `Default::default()` | Automatically load the API key from the provider's environment variable when no explicit key is provided (default: `true`). |
| `headers` | `Option<HashMap<String, String>>` | `HashMap::new()` | Extra headers sent on every request. |
| `providers` | `Option<Vec<LlmProviderConfig>>` | `vec![]` | Custom provider configurations, in addition to the built-in providers. |
| `cache` | `Option<LlmCacheConfig>` | `Default::default()` | Response cache configuration. |
| `budget` | `Option<LlmBudgetConfig>` | `Default::default()` | Budget enforcement configuration. |
| `rate_limit` | `Option<LlmRateLimitConfig>` | `Default::default()` | Per-model rate limiting configuration. |
| `in_flight_limit` | `Option<LlmInFlightLimitConfig>` | `Default::default()` | Global per-client in-flight provider request limit. |
| `cost_tracking` | `Option<bool>` | `Default::default()` | Enable per-request cost tracking. |
| `tracing` | `Option<bool>` | `Default::default()` | Enable OpenTelemetry-compatible tracing spans. |
| `cooldown_secs` | `Option<u64>` | `Default::default()` | Cooldown duration after transient errors, in seconds. |
| `health_check_secs` | `Option<u64>` | `Default::default()` | Background health check interval, in seconds. |
| `bedrock` | `Option<BedrockConfig>` | `Default::default()` | AWS Bedrock configuration (region, credentials, cross-region routing). |
...
```rust
let result = instance.providers();
```

**Returns:** `Vec<LlmProviderConfig>`
...
| `finish_reason` | `Option<FinishReason>` | `FinishReason::Stop` | Why the stream ended (present only in final chunk). |
...
| `reasoning_content` | `Option<String>` | `Default::default()` | Partial reasoning/thinking tokens (OpenAI-compatible extension used by DeepSeek R1, Qwen, etc.). |
...
| `call_type` | `Option<ToolType>` | `ToolType::Function` | Tool type (typically "function"). |
...
| `Multimodal` | Text and image parts for a single multimodal embedding. — Fields: `0`: `Vec<EmbeddingContentPart>` |
...
|-------|-------------|
| `Text` | Plain text. — Fields: `text`: `String` |
| `ImageUrl` | Image identified by a data URL or HTTP/HTTPS URL. — Fields: `image_url`: `ImageUrl` |
| `ImageBase64` | Image encoded as a complete data URL. — Fields: `image_base64`: `String` |
