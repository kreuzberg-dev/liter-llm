...
fn write_composer_json(dir: &Utf8Path) -> Result<()> {
    ...
    //
    // We autoload `LiterLlm\*` classes directly from the PHP wrapper package
    // instead of requiring it via Composer, because the package is typed as
    // `php-ext` which causes resolution failures when the extension binary
    // is not loaded.  The native extension is loaded via php.ini in CI.
    let content = r#"{
    "require-dev": {
    ...
    match category {
        ...
        "custom_provider" => {
            if let Some(provider_cfg) = &fixture.client_config.custom_provider {
                ...
                let name_php = php_string_escape(name);

                let auth_header = provider_cfg
                    .get("auth_header")
                    .and_then(|v| v.as_str())
                    .unwrap_or("Bearer");
                let auth_php = php_string_escape(auth_header);

                // Collect model prefixes as a PHP array literal.
                let prefixes: Vec<String> = provider_cfg
                    .get("model_prefixes")
                    .and_then(|v| v.as_array())
                    .map(|arr| {
                        arr.iter()
                            .filter_map(|v| v.as_str())
                            .map(|s| format!("'{}'", php_string_escape(s)))
                            .collect()
                    })
                    .unwrap_or_default();
                let prefixes_php = prefixes.join(", ");
