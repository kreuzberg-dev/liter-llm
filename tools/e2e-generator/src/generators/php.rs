...
fn write_composer_json(dir: &Utf8Path) -> Result<()> {
    ...
    //
    // The path repository points to the PHP wrapper package so that the
    // `LiterLlm\*` classes (LlmClient, LlmHook, CacheConfig, BudgetConfig,
    // ProviderConfig, etc.) are autoloaded without requiring the native
