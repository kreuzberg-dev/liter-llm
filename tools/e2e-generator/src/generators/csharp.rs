...
impl Generator for CSharpGenerator {
    fn generate(&self, fixtures: &[Fixture], output_root: &Utf8Path) -> Result<()> {
        ...
        let _ = std::process::Command::new("dotnet")
            .args(["format", "LiterLlmE2E.csproj"])
            .current_dir(&cs_dir)
            .status();
...
"#;
    std::fs::write(dir.join("LiterLlmE2E.csproj"), content)
        .map_err(|e| anyhow::anyhow!("Failed to write LiterLlmE2E.csproj: {e}"))
}
...
fn write_new_category_test_method(out: &mut String, fixture: &Fixture, category: &str) {
    ...
    writeln!(out).unwrap();
    writeln!(out, "        using var server = new MockServer(routes);").unwrap();
    writeln!(out, "        var mockUrl = server.Url;").unwrap();
    writeln!(out).unwrap();
    ...
    match category {
        "cache" => {
            ...
            writeln!(out, "            .WithApiKey(\"test-key\")").unwrap();
            writeln!(out, "            .WithBaseUrl(mockUrl)").unwrap();
            writeln!(
                out,
            ...
            if fixture.assertions.cache_hit == Some(true) {
                writeln!(out, "        var request = \"{escaped_req}\";").unwrap();
                writeln!(out, "        var resp1 = await client.ChatAsync(request);").unwrap();
                writeln!(out, "        var resp2 = await client.ChatAsync(request);").unwrap();
                writeln!(
                    out,
                ...
                writeln!(out, "        var request = \"{escaped_req}\";").unwrap();
                writeln!(out, "        var resp = await client.ChatStreamAsync(request);").unwrap();
                writeln!(
                    out,
                ...
                writeln!(out, "        var request = \"{escaped_req}\";").unwrap();
                writeln!(out, "        var resp1 = await client.ChatAsync(request);").unwrap();
                writeln!(out, "        await Task.Delay(100);").unwrap();
                writeln!(out, "        var resp2 = await client.ChatAsync(request);").unwrap();
                writeln!(
                    out,
        ...
        "budget" => {
            ...
            if is_error {
                ...
                writeln!(out).unwrap();
                writeln!(out, "        var request = \"{escaped_req}\";").unwrap();
                writeln!(out, "        await Assert.ThrowsAsync<BudgetExceededException>(").unwrap();
                writeln!(out, "            () => client.ChatAsync(request));").unwrap();
            } else {
                writeln!(out, "        var config = new LlmClientConfig.Builder()").unwrap();
                writeln!(out, "            .WithApiKey(\"test-key\")").unwrap();
                writeln!(out, "            .WithBaseUrl(mockUrl)").unwrap();
                ...
                writeln!(out).unwrap();
                writeln!(out, "        var request = \"{escaped_req}\";").unwrap();
                writeln!(out, "        var resp = await client.ChatAsync(request);").unwrap();
                writeln!(out, "        Assert.NotNull(resp);").unwrap();
        ...
        "hooks" => {
            ...
            if fixture.assertions.hook_on_request_called == Some(true) {
                writeln!(out, "        var hookCalled = false;").unwrap();
                writeln!(
                    out,
                ...
                writeln!(out).unwrap();
                writeln!(out, "        var request = \"{escaped_req}\";").unwrap();
                writeln!(out, "        await client.ChatAsync(request);").unwrap();
                writeln!(
                    out,
                ...
                writeln!(out).unwrap();
                writeln!(out, "        var request = \"{escaped_req}\";").unwrap();
                writeln!(out, "        try {{ await client.ChatAsync(request); }} catch {{ }}").unwrap();
                writeln!(
                    out,
                ...
                writeln!(out, "        client.AddHook(\"on_request\", (req) => {{ throw new HookRejectedException(\"Blocked\"); }});").unwrap();
                writeln!(out).unwrap();
                writeln!(out, "        var request = \"{escaped_req}\";").unwrap();
                writeln!(out, "        await Assert.ThrowsAsync<HookRejectedException>(").unwrap();
                writeln!(out, "            () => client.ChatAsync(request));").unwrap();
            }
        }
        "custom_provider" => {
            ...
            if let Some(provider_cfg) = &fixture.client_config.custom_provider {
                ...
                if let Some(prefixes) = provider_cfg.get("model_prefixes").and_then(|v| v.as_array()) {
                    let prefix_strs: Vec<String> = prefixes
                        ...
                        .collect();
                    writeln!(out, "            ModelPrefixes: new[] {{ {} }}", prefix_strs.join(", ")).unwrap();
                } else {
                    writeln!(out, "            ModelPrefixes: Array.Empty<string>()").unwrap();
                }
